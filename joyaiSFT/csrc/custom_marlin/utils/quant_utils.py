...
def quantize_weights(w: torch.Tensor, num_bits: int, group_size: int, act_order: bool):
    ...
    return (w_ref.to(device=orig_device), q_w.to(device=orig_device), s.to(device=orig_device), g_idx.to(device=orig_device), rand_perm.to(device=orig_device))
...
def gptq_unpack(q_res: torch.Tensor, num_bits: int, size_k: int, size_n: int):
