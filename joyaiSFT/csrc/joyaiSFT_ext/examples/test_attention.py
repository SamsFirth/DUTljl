...
with torch.inference_mode(mode=True):
    config = cpuinfer_ext.kvcache.KVCacheConfig(layer_num, kv_head_num, q_head_num, head_dim, block_len, anchor_num, anchor_type, kv_type, retrieval_type, layer_step, token_step, layer_offset, max_block_num, max_batch_size, max_thread_num)
    ...
    for layer_idx in range(layer_num):
        ...
        CPUInfer.submit(local_kvcache.update_kvcache_fp16(k_cache.data_ptr(), v_cache.data_ptr(), layer_idx, block_table.data_ptr(), 1, max_block_num, seqlens_zero.data_ptr(), cache_seqlen))
        CPUInfer.sync()
    ...
    for i in range(validation_iter):
        ...
        input = input / 100
        CPUInfer.submit(local_kvcache.attn(input.data_ptr(), output.data_ptr(), attn_lse.data_ptr(), i % layer_num, 0, 1, 1, max_block_num, block_table.data_ptr(), cache_seqlens.data_ptr(), -1, -1, -1))
        CPUInfer.sync()
        t_output = flash_attn_with_kvcache(q=input.to('cuda'), k_cache=k_cache, v_cache=v_cache, cache_seqlens=cache_seqlens.to('cuda'))
