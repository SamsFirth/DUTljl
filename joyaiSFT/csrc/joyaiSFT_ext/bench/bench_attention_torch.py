...
def bench_linear(cache_seqlen: int, device):
    with torch.inference_mode(mode=True):
        ...
        for layer_idx in range(layer_num):
            k_cache = torch.randn((1, 32, cache_seqlen, head_dim), dtype=torch.float16, device=device).contiguous()
            v_cache = torch.randn((1, 32, cache_seqlen, head_dim), dtype=torch.float16, device=device).contiguous()
        ...
        print('Bandwidth: ', cache_seqlen * q_head_num * head_dim * 2 * 2 * test_iter / total_time / 1000 / 1000 / 1000, 'GB/s')
        print('')
