...
JOYAISFT_BUILD_XPU = torch.xpu.is_available()
dev_backend = os.environ.get('DEV_BACKEND', '').lower()
if dev_backend == 'xpu':
    triton_dep = ['pytorch-triton-xpu==3.3.0']
else:
    triton_dep = []
...
class VersionInfo:
    ...
    PACKAGE_NAME = 'joyaiSFT'
    BASE_WHEEL_URL: str = 'https://github.com/joyaiSFT/releases/download/{tag_name}/{wheel_filename}'
    FORCE_BUILD = os.getenv('JOYAISFT_FORCE_BUILD', 'FALSE') == 'TRUE'

    def get_musa_bare_metal_version(self, musa_dir):
        raw_output = subprocess.run([musa_dir + '/bin/mcc', '-v'], check=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT).stdout.decode('utf-8')
    ...
    def get_rocm_bare_metal_version(self, rocm_dir):
        ...
        """
        try:
            raw_output = subprocess.check_output([rocm_dir + '/bin/rocminfo', '--version'], universal_newlines=True, stderr=subprocess.STDOUT)
            match = re.search('(\\d+\\.\\d+)', raw_output)
            if match:
        ...
        match = re.search('rocm-(\\d+\\.\\d+)', dir_name)
        if match:
            version_str = match.group(1)
            version = parse(version_str)
            rocm_version = f'{version.major}{version.minor}'
            return rocm_version
        try:
            raw_output = subprocess.check_output([rocm_dir + '/bin/hipcc', '--version'], universal_newlines=True, stderr=subprocess.STDOUT)
            match = re.search('HIP version: (\\d+\\.\\d+)', raw_output)
            if match:
                version_str = match.group(1)
                version = parse(version_str)
                rocm_version = f'{version.major}{version.minor}'
    ...
    def get_flash_version(self):
        ...
        with open(version_file, 'r', encoding='utf-8') as f:
            version_match = re.search('^__version__\\s*=\\s*(.*)$', f.read(), re.MULTILINE)
...
class BuildWheelsCommand(_bdist_wheel):
    ...
    def run(self):
        ...
        try:
            ...
            super().run()
ANSI_ESCAPE = re.compile('\\033[@-Z\\\\-_\\[\\]P]|\\033\\[[0-?]*[ -/]*[@-~]|\\033][^\\007\\033]*\\007|[\\000-\\037]')
...
def colored(text, color=None, bold=False):
    ...
    if bold:
        fmt.append('1')
    return f"\x1b[{';'.join(fmt)}m{text}\x1b[0m"
...
def run_command_with_live_tail(ext: str, command: List[str], output_lines: int=20, refresh_rate: float=0.1, cwd: Optional[str]=None):
    """
    ...
    SAVE_CURSOR = '\x1b7'
    RESTORE_CURSOR = '\x1b8'
    CLEAR_REMAINING = '\x1b[J'

    def write_progress(status: Literal['RUNNING', 'SUCCEED', 'FAILED']='RUNNING', new_line: Optional[str]=None):
    ...
    sys.stdout.flush()
    process = subprocess.Popen(command, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, cwd=cwd, text=True, bufsize=1)
    try:
        write_progress()
...
class CMakeBuild(BuildExtension):
    ...
    def build_extension(self, ext) -> None:
        ...
        cmake_args = [f'-DCMAKE_LIBRARY_OUTPUT_DIRECTORY={extdir}{os.sep}', f'-DPYTHON_EXECUTABLE={sys.executable}', f'-DCMAKE_BUILD_TYPE={cfg}']
        if CUDA_HOME is not None:
            cmake_args += ['-DJOYAISFT_USE_CUDA=ON']
        elif MUSA_HOME is not None:
            cmake_args += ['-DJOYAISFT_USE_MUSA=ON']
        elif ROCM_HOME is not None:
            cmake_args += ['-DJOYAISFT_USE_ROCM=ON']
        elif JOYAISFT_BUILD_XPU:
            cmake_args += ['-DJOYAISFT_USE_XPU=ON', '-DJOYAISFT_USE_CUDA=OFF']
        else:
        ...
        if sys.platform.startswith('darwin'):
            archs = re.findall('-arch (\\S+)', os.environ.get('ARCHFLAGS', ''))
            if archs:
        ...
        run_command_with_live_tail(ext.name, ['cmake', ext.sourcedir, *cmake_args], cwd=build_temp)
        run_command_with_live_tail(ext.name, ['cmake', '--build', build_temp, '--verbose', *build_args], cwd=build_temp)
if CUDA_HOME is not None or ROCM_HOME is not None:
    ops_module = CUDAExtension('JOYSFTOps', ['csrc/joyaiSFT_ext/cuda/custom_gguf/dequant.cu', 'csrc/joyaiSFT_ext/cuda/binding.cpp', 'csrc/joyaiSFT_ext/cuda/gptq_marlin/gptq_marlin.cu'], extra_compile_args={'cxx': ['-O3', '-DJOYAISFT_USE_CUDA'], 'nvcc': ['-O3', '-Xcompiler', '-fPIC', '-DJOYAISFT_USE_CUDA']})
elif MUSA_HOME is not None:
    SimplePorting(cuda_dir_path='csrc/joyaiSFT_ext/cuda', mapping_rule={'at::cuda': 'at::musa', '#include <ATen/cuda/CUDAContext.h>': '#include "torch_musa/csrc/aten/musa/MUSAContext.h"', '#include <c10/cuda/CUDAGuard.h>': '#include "torch_musa/csrc/core/MUSAGuard.h"', 'nv_bfloat16': 'mt_bfloat16'}).run()
    ops_module = MUSAExtension('JOYSFTOps', ['csrc/joyaiSFT_ext/cuda_musa/custom_gguf/dequant.mu', 'csrc/joyaiSFT_ext/cuda_musa/binding.cpp'], extra_compile_args={'cxx': ['force_mcc'], 'mcc': ['-O3', '-DJOYAISFT_USE_MUSA', '-DTHRUST_IGNORE_CUB_VERSION_CHECK']})
elif torch.xpu.is_available():
    ops_module = None
else:
    print('Warning: No GPU backend (CUDA/ROCM/MUSA/XPU) detected. Building CPU version only.')
    ops_module = None
if not torch.xpu.is_available():
    ext_modules = [CMakeExtension('cpuinfer_ext', os.fspath(Path('').resolve() / 'csrc' / 'joyaiSFT_ext')), ops_module, CUDAExtension('vLLMMarlin', ['csrc/custom_marlin/binding.cpp', 'csrc/custom_marlin/gptq_marlin/gptq_marlin.cu', 'csrc/custom_marlin/gptq_marlin/gptq_marlin_repack.cu'], extra_compile_args={'cxx': ['-O3'], 'nvcc': ['-O3', '-Xcompiler', '-fPIC']})]
    if with_balance:
        print('using balance_serve')
        ext_modules.append(CMakeExtension('balance_serve', os.fspath(Path('').resolve() / 'csrc' / 'balance_serve')))
else:
    ext_modules = [CMakeExtension('cpuinfer_ext', os.fspath(Path('').resolve() / 'csrc' / 'joyaiSFT_ext'))]
setup(name=VersionInfo.PACKAGE_NAME, version=VersionInfo().get_package_version(), packages=find_packages(include=['joyaiSFT', 'joyaiSFT.*']), include_package_data=True, install_requires=['accelerate==1.11.0', 'aiofiles==24.1.0', 'aiohappyeyeballs==2.6.1', 'aiohttp==3.13.2', 'aiosignal==1.4.0', 'annotated-doc==0.0.4', 'annotated-types==0.7.0', 'antlr4-python3-runtime==4.9.3', 'anyio==4.12.0', 'attrs==25.4.0', 'audioread==3.1.0', 'av==16.0.1', 'blessed==1.25.0', 'brotli==1.2.0', 'build==1.3.0', 'certifi==2025.11.12', 'cffi==2.0.0', 'charset-normalizer==3.4.4', 'click==8.3.1', 'colorlog==6.10.1', 'contourpy==1.3.3', 'cuda-bindings==13.1.1', 'cuda-pathfinder==1.3.3', 'cuda-python==13.1.1', 'cycler==0.12.1', 'datasets==4.0.0', 'decorator==5.2.1', 'dill==0.3.8', 'docstring_parser==0.17.0', 'einops==0.8.1', 'fastapi==0.124.4', 'ffmpy==1.0.0', 'filelock==3.20.0', 'fire==0.7.1', 'fonttools==4.61.1', 'frozenlist==1.8.0', 'fsspec==2025.3.0', 'gradio==5.45.0', 'gradio_client==1.13.0', 'graphviz==0.21', 'groovy==0.1.2', 'h11==0.16.0', 'hf-xet==1.2.0', 'hf_transfer==0.1.9', 'httpcore==1.0.9', 'httpx==0.28.1', 'huggingface-hub==0.36.0', 'idna==3.11', 'jieba==0.42.1', 'Jinja2==3.1.6', 'joblib==1.5.3', 'jsonpatch==1.33', 'jsonpointer==3.0.0', 'kiwisolver==1.4.9', 'langchain==1.2.0', 'langchain-core==1.2.1', 'langgraph==1.0.5', 'langgraph-checkpoint==3.0.1', 'langgraph-prebuilt==1.0.5', 'langgraph-sdk==0.3.0', 'langsmith==0.4.59', 'lazy_loader==0.4', 'librosa==0.11.0', 'llvmlite==0.46.0', 'markdown-it-py==4.0.0', 'MarkupSafe==3.0.3', 'matplotlib==3.10.8', 'mdurl==0.1.2', 'modelscope==1.33.0', 'mpmath==1.3.0', 'msgpack==1.1.2', 'multidict==6.7.0', 'multiprocess==0.70.16', 'networkx==3.6.1', 'ninja==1.13.0', 'nltk==3.9.2', 'numba==0.63.1', 'numpy==1.26.4', 'nvidia-cublas-cu12==12.8.4.1', 'nvidia-cuda-cupti-cu12==12.8.90', 'nvidia-cuda-nvrtc-cu12==12.8.93', 'nvidia-cuda-runtime-cu12==12.8.90', 'nvidia-cudnn-cu12==9.10.2.21', 'nvidia-cudnn-frontend==1.16.0', 'nvidia-cufft-cu12==11.3.3.83', 'nvidia-cufile-cu12==1.13.1.3', 'nvidia-curand-cu12==10.3.9.90', 'nvidia-cusolver-cu12==11.7.3.90', 'nvidia-cusparse-cu12==12.5.8.93', 'nvidia-cusparselt-cu12==0.7.1', 'nvidia-ml-py==13.590.44', 'nvidia-nccl-cu12==2.27.3', 'nvidia-nvjitlink-cu12==12.8.93', 'nvidia-nvshmem-cu12==3.3.20', 'nvidia-nvtx-cu12==12.8.90', 'omegaconf==2.3.0', 'orjson==3.11.5', 'ormsgpack==1.12.1', 'packaging==25.0', 'pandas==2.3.3', 'peft==0.14.0', 'pillow==11.3.0', 'platformdirs==4.5.1', 'pooch==1.8.2', 'propcache==0.4.1', 'protobuf==6.33.2', 'psutil==7.1.3', 'pyarrow==22.0.0', 'pycparser==2.23', 'pydantic==2.10.6', 'pydantic_core==2.27.2', 'pydub==0.25.1', 'Pygments==2.19.2', 'pynvml==13.0.1', 'pyparsing==3.2.5', 'pyproject_hooks==1.2.0', 'python-dateutil==2.9.0.post0', 'python-multipart==0.0.20', 'pytz==2025.2', 'PyYAML==6.0.3', 'regex==2025.11.3', 'requests==2.32.5', 'requests-toolbelt==1.0.0', 'rich==14.2.0', 'rouge-chinese==1.0.3', 'ruff==0.14.9', 'safehttpx==0.1.7', 'safetensors==0.5.3', 'scikit-learn==1.8.0', 'scipy==1.16.3', 'semantic-version==2.10.0', 'sentencepiece==0.2.1', 'shellingham==1.5.4', 'shtab==1.8.0', 'six==1.17.0', 'soundfile==0.13.1', 'soxr==1.0.0', 'sse-starlette==3.0.4', 'starlette==0.50.0', 'sympy==1.14.0', 'tenacity==9.1.2', 'termcolor==3.2.0', 'threadpoolctl==3.6.0', 'tiktoken==0.12.0', 'tokenizers==0.21.4', 'tomlkit==0.13.3', 'torchviz==0.0.3', 'tqdm==4.67.1', 'transformers==4.51.3', 'triton==3.4.0', 'trl==0.9.6', 'typer==0.20.0', 'typing_extensions==4.15.0', 'tyro==0.8.14', 'tzdata==2025.3', 'urllib3==2.6.2', 'uuid_utils==0.12.0', 'uvicorn==0.38.0', 'wcwidth==0.2.14', 'websockets==15.0.1', 'xxhash==3.6.0', 'yarl==1.22.0', 'zstandard==0.25.0'], cmdclass={'bdist_wheel': BuildWheelsCommand, 'build_ext': CMakeBuild}, ext_modules=ext_modules)