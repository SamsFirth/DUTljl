...
class FlashInferRotaryEmbedding(nn.Module):
    ...
    def __init__(self, head_size: int, rotary_dim: int, max_position_embeddings: int, base: int, is_neox_style: bool, dtype: torch.dtype) -> None:
        super().__init__()
    ...
    def _apply_rotary_emb(self, x: torch.Tensor, cos: torch.Tensor, sin: torch.Tensor, is_neox_style: bool) -> torch.Tensor:
        """
        Args:
    ...
    def forward_cuda(self, positions: torch.Tensor, query: torch.Tensor, key: torch.Tensor, offsets: Optional[torch.Tensor]=None) -> Tuple[torch.Tensor, torch.Tensor]:
        apply_rope_with_cos_sin_cache_inplace(positions=positions, query=query, key=key, head_size=self.head_size, cos_sin_cache=self.cos_sin_cache, is_neox=self.is_neox_style)
        return (query, key)

@triton.testing.perf_report(triton.testing.Benchmark(x_names=['seq_len'], x_vals=[2, 4, 8, 16, 32, 64, 128, 256, 512, 1024, 2048, 4096, 8192, 16384, 32768, 65536], line_arg='provider', line_vals=['flashinfer', 'native', 'vllm'], line_names=['FlashInfer', 'Native', 'vLLM'], styles=[('blue', '-'), ('red', '-'), ('green', '-')], ylabel='Latency (ms)', plot_name='rope-latency', args={'head_size': 4096 // 32, 'rotary_dim': 4096 // 32, 'max_position_embeddings': 65536, 'base': 500000, 'is_neox_style': True, 'dtype': torch.bfloat16, 'device': 'cuda', 'batch_size': 2, 'num_q_heads': 32, 'num_kv_heads': 8}))
