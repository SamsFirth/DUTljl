    ...
    for batch_size in args.batch_sizes:
        for hidden_size in args.hidden_sizes:
            for dtype_str in args.dtypes:
                ...
                throughput = (x.numel() * x.element_size() * 2 + residual.numel() * residual.element_size() * 2 + weight.numel() * weight.element_size()) / (latency_ms * 0.001)
                print(f'batch_size: {batch_size:3},', f'hidden_size: {hidden_size:5},', f'dtype: {dtype_str:8},', f'latency: {latency_ms * 1000.0:2.0f}us,', f'throughput: {throughput * 1e-09:7.3f}GB/s')
        print('---')
