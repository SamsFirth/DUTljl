...
MODELS = {'l1b': ModelConfig(num_kv_heads=8, num_layers=16, head_dim=64), 'l3b': ModelConfig(num_kv_heads=8, num_layers=28, head_dim=128), 'l8b': ModelConfig(num_kv_heads=8, num_layers=32, head_dim=128), 'l70b-tp8': _make_70b(8)}
...
def main():
    ...
    for model_name, model in MODELS.items():
        ...
        for seqlens in seqlens_:
            k = torch.rand((sum(seqlens), model.num_kv_heads, model.head_dim), dtype=dtype, device=device)
            v = torch.rand((sum(seqlens), model.num_kv_heads, model.head_dim), dtype=dtype, device=device)
            ...
            def fn_convert() -> Tuple[torch.Tensor, torch.Tensor]:
                return flashinfer.get_batch_indices_positions(x_indptr, flashinfer.get_seq_lens(kv_indptr, kv_last_page_len, page_len), k.shape[0])
            ...
            def fn() -> None:
                flashinfer.append_paged_kv_cache(k, v, batch_indices, positions, layer_buf, kv_indices, kv_indptr, kv_last_page_len, 'NHD')
            latency_ms = cast(float, do_bench(fn))
            all_layers_latency_ms = convert_latency_ms + latency_ms * model.num_layers
            throughput = k.numel() * k.element_size() * sum((1 for _ in ['k', 'v'])) * sum((1 for _ in ['read', 'write'])) / (latency_ms * 0.001)
