...
from .utils import _get_cache_buf, determine_gemm_backend, get_indptr, register_custom_op, register_fake_op
_gemm_module = None
...
def get_gemm_module():
    ...
    if _gemm_module is None:
        if has_prebuilt_ops:
            ...
            module = load_cuda_ops('gemm', [FLASHINFER_CSRC_DIR / 'bmm_fp8.cu', FLASHINFER_CSRC_DIR / 'group_gemm.cu', FLASHINFER_CSRC_DIR / 'flashinfer_gemm_ops.cu'], extra_ldflags=['-lcublas', '-lcublasLt'])

        @register_custom_op('flashinfer::bmm_fp8', mutates_args=('workspace_buffer', 'D'))
        def bmm_fp8(workspace_buffer: torch.Tensor, A: torch.Tensor, B: torch.Tensor, D: torch.Tensor, A_scale: torch.Tensor, B_scale: torch.Tensor) -> None:
            cublas_handle = torch.cuda.current_blas_handle()
            module.bmm_fp8.default(A, B, D, A_scale, B_scale, workspace_buffer, cublas_handle)

        @register_fake_op('flashinfer::bmm_fp8')
        def _fake_bmm_fp8(workspace_buffer: torch.Tensor, A: torch.Tensor, B: torch.Tensor, D: torch.Tensor, A_scale: torch.Tensor, B_scale: torch.Tensor) -> None:
            pass

        @register_custom_op('flashinfer::cutlass_segment_gemm', mutates_args='y')
        def cutlass_segment_gemm(workspace_buffer: torch.Tensor, all_problems: torch.Tensor, x_data: torch.Tensor, w_data: torch.Tensor, y_data: torch.Tensor, x_ld: torch.Tensor, w_ld: torch.Tensor, y_ld: torch.Tensor, y: torch.Tensor, empty_x_data: torch.Tensor, weight_column_major: bool) -> None:
            module.cutlass_segment_gemm.default(workspace_buffer, all_problems, x_data, w_data, y_data, x_ld, w_ld, y_ld, empty_x_data, weight_column_major)

        @register_fake_op('flashinfer::cutlass_segment_gemm')
        def _fake_cutlass_segment_gemm(workspace_buffer: torch.Tensor, all_problems: torch.Tensor, x_data: torch.Tensor, w_data: torch.Tensor, y_data: torch.Tensor, x_ld: torch.Tensor, w_ld: torch.Tensor, y_ld: torch.Tensor, y: torch.Tensor, empty_x_data: torch.Tensor, weight_column_major: bool) -> None:
            pass
        _gemm_module = SimpleNamespace(bmm_fp8=bmm_fp8, cutlass_segment_gemm=cutlass_segment_gemm)
    return _gemm_module
...
def get_gemm_sm90_module():
    ...
    if _gemm_module_sm90 is None:
        if has_prebuilt_ops:
            ...
            module = load_cuda_ops('gemm_sm90', [FLASHINFER_CSRC_DIR / 'group_gemm_sm90.cu', FLASHINFER_CSRC_DIR / 'flashinfer_gemm_sm90_ops.cu'], extra_cuda_cflags=['-gencode', 'arch=compute_90a,code=sm_90a'])

        @register_custom_op('flashinfer::cutlass_segment_gemm_sm90', mutates_args=('workspace_buffer', 'y'))
        def cutlass_segment_gemm_sm90(workspace_buffer: torch.Tensor, int_workspace_buffer: torch.Tensor, all_problems: torch.Tensor, x_data: torch.Tensor, w_data: torch.Tensor, y_data: torch.Tensor, x_stride: torch.Tensor, w_stride: torch.Tensor, y_stride: torch.Tensor, y: torch.Tensor, empty_x_data: torch.Tensor, weight_column_major: bool) -> None:
            module.cutlass_segment_gemm_sm90.default(workspace_buffer, int_workspace_buffer, all_problems, x_data, w_data, y_data, x_stride, w_stride, y_stride, empty_x_data, weight_column_major)

        @register_fake_op('flashinfer::cutlass_segment_gemm_sm90')
        def _fake_cutlass_segment_gemm_sm90(workspace_buffer: torch.Tensor, int_workspace_buffer: torch.Tensor, all_problems: torch.Tensor, x_data: torch.Tensor, w_data: torch.Tensor, y_data: torch.Tensor, x_stride: torch.Tensor, w_stride: torch.Tensor, y_stride: torch.Tensor, y: torch.Tensor, empty_x_data: torch.Tensor, weight_column_major: bool) -> None:
            pass
        _gemm_module_sm90 = SimpleNamespace(cutlass_segment_gemm_sm90=cutlass_segment_gemm_sm90)
    return _gemm_module_sm90

def launch_compute_sm80_group_gemm_args(x: torch.Tensor, weights: torch.Tensor, y: torch.Tensor, w_column_major: bool, batch_size: int, seg_indptr: torch.Tensor, weight_indices: Optional[torch.Tensor]=None):
    device = x.device
    prob_type = torch.int32
    ptr_type = torch.int64
    ld_type = torch.int64
    ...
    compute_sm80_group_gemm_args[batch_size,](all_problems, x_data, w_data, y_data, x_stride_data, w_stride_data, y_stride_data, x, weights, y, seg_indptr, weight_indices, d_in, d_out, w_column_major)
    return (all_problems, x_data, w_data, y_data, x_stride_data, w_stride_data, y_stride_data)

def launch_compute_sm90_group_gemm_args(x: torch.Tensor, weights: torch.Tensor, y: torch.Tensor, w_column_major: bool, batch_size: int, seg_indptr: torch.Tensor, weight_indices: Optional[torch.Tensor]=None):
    device = x.device
    prob_type = torch.int32
    ptr_type = torch.int64
    stride_type = torch.int64
    seg_indptr = seg_indptr.to(ptr_type)
    if weight_indices is not None:
        weight_indices = weight_indices.to(ptr_type)
    d_out = weights.size(1) if w_column_major else weights.size(2)
    ...
    compute_sm90_group_gemm_args[batch_size,](all_problems, x_data, w_data, y_data, x_stride_data, w_stride_data, y_stride_data, x, weights, y, seg_indptr, weight_indices, d_in, d_out, w_column_major)
    return (all_problems, x_data, w_data, y_data, x_stride_data, w_stride_data, y_stride_data)
...
class SegmentGEMMWrapper:
    ...
    def __init__(self, float_workspace_buffer: torch.Tensor, backend: str='auto') -> None:
        ...
        """
        self._int_workspace_buffer = torch.empty((1024 * 1024,), dtype=torch.int8, device=float_workspace_buffer.device)
    ...
    def run(self, x: torch.Tensor, weights: torch.Tensor, batch_size: int, weight_column_major: bool, seg_lens: Optional[torch.Tensor]=None, seg_indptr: Optional[torch.Tensor]=None, weight_indices: Optional[torch.Tensor]=None) -> torch.Tensor:
        ...
        if backend == 'sm90':
            all_problems, x_data, w_data, y_data, x_stride_data, w_stride_data, y_stride_data = launch_compute_sm90_group_gemm_args(x, weights, y, weight_column_major, batch_size, seg_indptr, weight_indices)
            get_gemm_sm90_module().cutlass_segment_gemm_sm90(self._float_workspace_buffer, self._int_workspace_buffer, all_problems, x_data, w_data, y_data, x_stride_data, w_stride_data, y_stride_data, y, empty_x_data, weight_column_major)
        elif backend == 'sm80':
            all_problems, x_data, w_data, y_data, x_ld_data, w_ld_data, y_ld_data = launch_compute_sm80_group_gemm_args(x, weights, y, weight_column_major, batch_size, seg_indptr, weight_indices)
            get_gemm_module().cutlass_segment_gemm(self._int_workspace_buffer, all_problems, x_data, w_data, y_data, x_ld_data, w_ld_data, y_ld_data, y, empty_x_data, weight_column_major)
        else:
            raise ValueError(f'Unsupported gemm backend: {backend}')
        return y
    forward = run

def bmm_fp8(A: torch.Tensor, B: torch.Tensor, A_scale: torch.Tensor, B_scale: torch.Tensor, dtype: torch.dtype, out: Optional[torch.Tensor]=None) -> torch.Tensor:
    """BMM FP8
