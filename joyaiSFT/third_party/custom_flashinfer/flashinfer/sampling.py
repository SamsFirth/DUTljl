...
def get_sampling_module():
    ...
    if _sampling_module is None:
        if has_prebuilt_ops:
            ...
            module = load_cuda_ops('sampling', [FLASHINFER_CSRC_DIR / 'sampling.cu', FLASHINFER_CSRC_DIR / 'renorm.cu', FLASHINFER_CSRC_DIR / 'flashinfer_sampling_ops.cu'])

        @register_custom_op('flashinfer::sampling_from_probs', mutates_args=())
        def sampling_from_probs(probs: torch.Tensor, indices: Optional[torch.Tensor], deterministic: bool, generator: Optional[torch.Generator]) -> torch.Tensor:
            device = probs.device
            probs = probs.float()
            ...
            module.sampling_from_probs.default(probs, samples, indices, deterministic, generator)
            return samples

        @register_fake_op('flashinfer::sampling_from_probs')
        def _fake_sampling_from_probs(probs: torch.Tensor, indices: Optional[torch.Tensor], deterministic: bool, generator: Optional[torch.Generator]) -> torch.Tensor:
            batch_size = indices.size(0) if indices is not None else probs.size(0)
        ...
        def top_p_sampling_from_probs(probs: torch.Tensor, indices: Optional[torch.Tensor], maybe_top_p_arr: Optional[torch.Tensor], top_p_val: float, deterministic: bool, generator: Optional[torch.Generator]) -> torch.Tensor:
            device = probs.device
            probs = probs.float()
            maybe_top_p_arr = maybe_top_p_arr.float() if maybe_top_p_arr is not None else None
            batch_size = indices.size(0) if indices is not None else probs.size(0)
            samples = torch.empty(batch_size, dtype=torch.int32, device=device)
            module.top_p_sampling_from_probs.default(probs, samples, indices, maybe_top_p_arr, top_p_val, deterministic, generator)
            return samples

        @register_fake_op('flashinfer::top_p_sampling_from_probs')
        def _fake_top_p_sampling_from_probs(probs: torch.Tensor, indices: Optional[torch.Tensor], maybe_top_p_arr: Optional[torch.Tensor], top_p_val: float, deterministic: bool, generator: Optional[torch.Generator]) -> torch.Tensor:
        ...
        def top_k_sampling_from_probs(probs: torch.Tensor, indices: Optional[torch.Tensor], maybe_top_k_arr: Optional[torch.Tensor], top_k_val: int, deterministic: bool, generator: Optional[torch.Generator]) -> torch.Tensor:
            device = probs.device
            probs = probs.float()
            batch_size = indices.size(0) if indices is not None else probs.size(0)
            maybe_top_k_arr = maybe_top_k_arr.int() if maybe_top_k_arr is not None else None
            samples = torch.empty(batch_size, dtype=torch.int32, device=device)
            module.top_k_sampling_from_probs.default(probs, samples, indices, maybe_top_k_arr, top_k_val, deterministic, generator)
            return samples

        @register_fake_op('flashinfer::top_k_sampling_from_probs')
        def _fake_top_k_sampling_from_probs(probs: torch.Tensor, indices: Optional[torch.Tensor], maybe_top_k_arr: Optional[torch.Tensor], top_k_val: int, deterministic: bool, generator: Optional[torch.Generator]) -> torch.Tensor:
            batch_size = indices.size(0) if indices is not None else probs.size(0)
        ...
        def min_p_sampling_from_probs(probs: torch.Tensor, indices: Optional[torch.Tensor], maybe_min_p_arr: Optional[torch.Tensor], min_p_val: float, deterministic: bool, generator: Optional[torch.Generator]) -> torch.Tensor:
            device = probs.device
            probs = probs.float()
            maybe_min_p_arr = maybe_min_p_arr.float() if maybe_min_p_arr is not None else None
            batch_size = indices.size(0) if indices is not None else probs.size(0)
            samples = torch.empty(batch_size, dtype=torch.int32, device=device)
            module.min_p_sampling_from_probs.default(probs, samples, indices, maybe_min_p_arr, min_p_val, deterministic, generator)
            return samples

        @register_custom_op('flashinfer::top_k_top_p_sampling_from_probs', mutates_args=())
        def top_k_top_p_sampling_from_probs(probs: torch.Tensor, indices: Optional[torch.Tensor], maybe_top_k_arr: Optional[torch.Tensor], top_k_val: int, maybe_top_p_arr: Optional[torch.Tensor], top_p_val: float, deterministic: bool, generator: Optional[torch.Generator]) -> torch.Tensor:
            device = probs.device
            probs = probs.float()
            maybe_top_k_arr = maybe_top_k_arr.int() if maybe_top_k_arr is not None else None
            maybe_top_p_arr = maybe_top_p_arr.float() if maybe_top_p_arr is not None else None
            batch_size = indices.size(0) if indices is not None else probs.size(0)
            samples = torch.empty(batch_size, dtype=torch.int32, device=device)
            module.top_k_top_p_sampling_from_probs.default(probs, samples, indices, maybe_top_k_arr, top_k_val, maybe_top_p_arr, top_p_val, deterministic, generator)
            return samples

        @register_fake_op('flashinfer::top_k_top_p_sampling_from_probs')
        def _fake_top_k_top_p_sampling_from_probs(probs: torch.Tensor, indices: Optional[torch.Tensor], maybe_top_k_arr: Optional[torch.Tensor], top_k_val: int, maybe_top_p_arr: Optional[torch.Tensor], top_p_val: float, deterministic: bool, generator: Optional[torch.Generator]) -> torch.Tensor:
            batch_size = indices.size(0) if indices is not None else probs.size(0)
            sample = torch.empty(batch_size, dtype=torch.int32, device=probs.device)
            return sample

        @register_custom_op('flashinfer::top_p_renorm_probs', mutates_args=())
        def top_p_renorm_probs(probs: torch.Tensor, maybe_top_p_arr: Optional[torch.Tensor], top_p_val: float) -> torch.Tensor:
            device = probs.device
            probs = probs.float()
            maybe_top_p_arr = maybe_top_p_arr.float() if maybe_top_p_arr is not None else None
            renorm_probs = torch.empty_like(probs)
            module.top_p_renorm_probs.default(probs, renorm_probs, maybe_top_p_arr, top_p_val)
            return renorm_probs

        @register_fake_op('flashinfer::top_p_renorm_probs')
        def _fake_top_p_renorm_probs(probs: torch.Tensor, maybe_top_p_arr: Optional[torch.Tensor], top_p_val: float) -> torch.Tensor:
            return torch.empty_like(probs)

        @register_custom_op('flashinfer::top_k_renorm_probs', mutates_args=())
        def top_k_renorm_probs(probs: torch.Tensor, maybe_top_k_arr: Optional[torch.Tensor], top_k_val: int) -> torch.Tensor:
            device = probs.device
            probs = probs.float()
            maybe_top_k_arr = maybe_top_k_arr.int() if maybe_top_k_arr is not None else None
            renorm_probs = torch.empty_like(probs)
            module.top_k_renorm_probs.default(probs, renorm_probs, maybe_top_k_arr, top_k_val)
            return renorm_probs

        @register_fake_op('flashinfer::top_k_renorm_probs')
        def _fake_top_k_renorm_probs(probs: torch.Tensor, maybe_top_k_arr: Optional[torch.Tensor], top_k_val: int) -> torch.Tensor:
            return torch.empty_like(probs)

        @register_custom_op('flashinfer::top_k_mask_logits', mutates_args=())
        def top_k_mask_logits(logits: torch.Tensor, maybe_top_k_arr: Optional[torch.Tensor], top_k_val: int) -> torch.Tensor:
            device = logits.device
            logits = logits.float()
            maybe_top_k_arr = maybe_top_k_arr.int() if maybe_top_k_arr is not None else None
            mask_logits = torch.empty_like(logits)
            module.top_k_mask_logits.default(logits, mask_logits, maybe_top_k_arr, top_k_val)
            return mask_logits

        @register_fake_op('flashinfer::top_k_mask_logits')
        def _fake_top_k_mask_logits(logits: torch.Tensor, maybe_top_k_arr: Optional[torch.Tensor], top_k_val: int) -> torch.Tensor:
            return torch.empty_like(logits)

        @register_custom_op('flashinfer::chain_speculative_sampling', mutates_args=('output_accepted_token_num', 'output_emitted_token_num'))
        def chain_speculative_sampling(draft_probs: torch.Tensor, draft_token_ids: torch.Tensor, target_probs: torch.Tensor, output_accepted_token_num: torch.Tensor, output_emitted_token_num: torch.Tensor, deterministic: bool, generator: Optional[torch.Generator]) -> torch.Tensor:
            ...
            module.chain_speculative_sampling.default(draft_probs, draft_token_ids, target_probs, output_token_ids, output_accepted_token_num, output_emitted_token_num, deterministic, generator)
            return output_token_ids

        @register_fake_op('flashinfer::chain_speculative_sampling')
        def _fake_chain_speculative_sampling(draft_probs: torch.Tensor, draft_token_ids: torch.Tensor, target_probs: torch.Tensor, output_accepted_token_num: torch.Tensor, output_emitted_token_num: torch.Tensor, deterministic: bool, generator: Optional[torch.Generator]) -> torch.Tensor:
            b, n = draft_token_ids.shape
            device = draft_token_ids.device
            return torch.empty((b, n + 1), dtype=torch.int32, device=device)
        _sampling_module = SimpleNamespace(sampling_from_probs=sampling_from_probs, top_p_sampling_from_probs=top_p_sampling_from_probs, top_k_sampling_from_probs=top_k_sampling_from_probs, min_p_sampling_from_probs=min_p_sampling_from_probs, top_k_top_p_sampling_from_probs=top_k_top_p_sampling_from_probs, top_p_renorm_probs=top_p_renorm_probs, top_k_renorm_probs=top_k_renorm_probs, top_k_mask_logits=top_k_mask_logits, chain_speculative_sampling=chain_speculative_sampling)
...
def sampling_from_probs(probs: torch.Tensor, indices: Optional[torch.Tensor]=None, deterministic: bool=True, generator: Optional[torch.Generator]=None, check_nan: bool=False) -> torch.Tensor:
    ...
    return get_sampling_module().sampling_from_probs(probs, indices, deterministic, generator)

def top_p_sampling_from_probs(probs: torch.Tensor, top_p: Union[torch.Tensor, float], indices: Optional[torch.Tensor]=None, deterministic: bool=True, generator: Optional[torch.Generator]=None, check_nan: bool=False) -> torch.Tensor:
    ...
    if check_nan:
        if torch.any(torch.isnan(probs)):
            raise ValueError('Input probs contains NaN.')
    return get_sampling_module().top_p_sampling_from_probs(probs, indices, *_to_tensor_scalar_tuple(top_p), deterministic, generator)

def top_k_sampling_from_probs(probs: torch.Tensor, top_k: Union[torch.Tensor, int], indices: Optional[torch.Tensor]=None, deterministic: bool=True, generator: Optional[torch.Generator]=None, check_nan: bool=False) -> torch.Tensor:
    ...
    if check_nan:
        if torch.any(torch.isnan(probs)):
            raise ValueError('Input probs contains NaN.')
    return get_sampling_module().top_k_sampling_from_probs(probs, indices, *_to_tensor_scalar_tuple(top_k), deterministic, generator)

def min_p_sampling_from_probs(probs: torch.Tensor, min_p: Union[torch.Tensor, float], indices: Optional[torch.Tensor]=None, deterministic: bool=True, generator: Optional[torch.Generator]=None, check_nan: bool=False) -> torch.Tensor:
    ...
    if check_nan:
        if torch.any(torch.isnan(probs)):
            raise ValueError('Input probs contains NaN.')
    return get_sampling_module().min_p_sampling_from_probs(probs, indices, *_to_tensor_scalar_tuple(min_p), deterministic, generator)

def top_k_top_p_sampling_from_logits(logits: torch.Tensor, top_k: Union[torch.Tensor, int], top_p: Union[torch.Tensor, float], indices: Optional[torch.Tensor]=None, filter_apply_order: str='top_k_first', deterministic: bool=True, generator: Optional[torch.Generator]=None, check_nan: bool=False) -> torch.Tensor:
    ...
    if filter_apply_order == 'top_k_first':
        ...
        return top_p_sampling_from_probs(probs, top_p, indices, deterministic, check_nan=check_nan, generator=generator)
    elif filter_apply_order == 'joint':
        probs = torch.softmax(logits, dim=-1)
        if check_nan:
            if torch.any(torch.isnan(probs)):
                raise ValueError('Input probs contains NaN.')
        return get_sampling_module().top_k_top_p_sampling_from_probs(probs, indices, *_to_tensor_scalar_tuple(top_k), *_to_tensor_scalar_tuple(top_p), deterministic, generator)
    else:
        raise ValueError(f'Invalid filter_apply_order: {filter_apply_order}')

def top_k_top_p_sampling_from_probs(probs: torch.Tensor, top_k: Union[torch.Tensor, int], top_p: Union[torch.Tensor, float], indices: Optional[torch.Tensor]=None, filter_apply_order: str='top_k_first', deterministic: bool=True, generator: Optional[torch.Generator]=None, check_nan: bool=False) -> torch.Tensor:
    ...
    if filter_apply_order == 'top_k_first':
        ...
        return top_p_sampling_from_probs(renorm_probs, top_p, indices, deterministic, check_nan=check_nan, generator=generator)
    elif filter_apply_order == 'joint':
        if check_nan:
            if torch.any(torch.isnan(probs)):
                raise ValueError('Input probs contains NaN.')
...
def top_p_renorm_probs(probs: torch.Tensor, top_p: Union[torch.Tensor, float]) -> torch.Tensor:
    ...
    top_k_renorm_probs
    """
    return get_sampling_module().top_p_renorm_probs(probs, *_to_tensor_scalar_tuple(top_p))
top_p_renorm_prob = top_p_renorm_probs

def top_k_renorm_probs(probs: torch.Tensor, top_k: Union[torch.Tensor, int]) -> torch.Tensor:
    ...
    top_p_renorm_probs
    """
    return get_sampling_module().top_k_renorm_probs(probs, *_to_tensor_scalar_tuple(top_k))
...
def top_k_mask_logits(logits: torch.Tensor, top_k: Union[torch.Tensor, int]) -> torch.Tensor:
    ...
    top_k_renorm_probs
    """
    return get_sampling_module().top_k_mask_logits(logits, *_to_tensor_scalar_tuple(top_k))

def chain_speculative_sampling(draft_probs, draft_token_ids, target_probs, maybe_output_accepted_token_num: Optional[torch.Tensor]=None, maybe_output_emitted_token_num: Optional[torch.Tensor]=None, deterministic: bool=True, generator: Optional[torch.Generator]=None) -> torch.Tensor:
    ...
    output_token_ids = get_sampling_module().chain_speculative_sampling(draft_probs, draft_token_ids, target_probs, output_accepted_token_num, output_emitted_token_num, deterministic, generator)
