...
def get_quantization_module():
    ...
    if _quantization_module is None:
        if has_prebuilt_ops:
            ...
            _quantization_module = load_cuda_ops('quantization', [FLASHINFER_CSRC_DIR / 'quantization.cu', FLASHINFER_CSRC_DIR / 'flashinfer_quantization_ops.cu'])
...
def segment_packbits(x: torch.Tensor, indptr: torch.Tensor, bitorder: str='big') -> Tuple[torch.Tensor, torch.Tensor]:
