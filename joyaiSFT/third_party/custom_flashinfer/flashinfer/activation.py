...
act_func_def_str = {'silu': silu_def_cu_str, 'gelu': gelu_def_cu_str, 'gelu_tanh': gelu_def_tanh_cu_str}
_jit_modules = {}
...
def get_act_and_mul_module(act_func_name: str):
    ...
    if act_func_name not in _jit_modules:
        if has_prebuilt_ops:
            ...
            module = gen_act_and_mul_module(act_func_name, act_func_def_str[act_func_name])
        fname = f'{act_func_name}_and_mul'
        fn = getattr(module, fname).default

        @register_custom_op(f'flashinfer::{fname}', mutates_args=('out',))
        def _act_and_mul(out: torch.Tensor, input: torch.Tensor, enable_pdl: bool=False) -> None:
            fn(out, input, enable_pdl)

        @register_fake_op(f'flashinfer::{fname}')
        def _fake_act_and_mul(out: torch.Tensor, input: torch.Tensor, enable_pdl: bool=False) -> None:
            pass
...
def _check_shape(input: torch.Tensor, output: torch.Tensor) -> None:
    ...
    assert input.shape[:-1] == output.shape[:-1], f'{input.shape[:-1]} != {output.shape[:-1]}'
    assert input.shape[-1] == 2 * output.shape[-1], f'{input.shape[-1]} != {2 * output.shape[-1]}'

def silu_and_mul(input: torch.Tensor, out: torch.Tensor=None, enable_pdl: bool=False) -> torch.Tensor:
    ...
    if out is not None:
        ...
        out = torch.empty(input.shape[:-1] + (input.shape[-1] // 2,), device=input.device, dtype=input.dtype)
    get_act_and_mul_module('silu').silu_and_mul(out, input, enable_pdl)
    return out

def gelu_tanh_and_mul(input: torch.Tensor, out: torch.Tensor=None, enable_pdl: bool=False) -> torch.Tensor:
...
def gelu_and_mul(input: torch.Tensor, out: torch.Tensor=None, enable_pdl: bool=False) -> torch.Tensor:
