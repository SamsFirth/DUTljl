...
class FlashInferJITLogger(logging.Logger):
    ...
    def __init__(self, name):
        ...
        self.handlers[0].setFormatter(logging.Formatter('%(asctime)s - %(levelname)s - %(message)s'))
        self.handlers[1].setFormatter(logging.Formatter('%(asctime)s - %(levelname)s - %(message)s'))
...
def load_cuda_ops(name: str, sources: List[Union[str, Path]], extra_cflags: Optional[List[str]]=None, extra_cuda_cflags: Optional[List[str]]=None, extra_ldflags=None, extra_include_paths=None, verbose=False):
    if extra_cflags is None:
        extra_cflags = []
    if extra_cuda_cflags is None:
        extra_cuda_cflags = []
    cflags = ['-O3', '-Wno-switch-bool']
    cuda_cflags = ['-O3', '-std=c++17', '--threads', '4', '-use_fast_math', '-DFLASHINFER_ENABLE_F16', '-DFLASHINFER_ENABLE_BF16', '-DFLASHINFER_ENABLE_FP8_E4M3', '-DFLASHINFER_ENABLE_FP8_E5M2']
    cflags += extra_cflags
    ...
    if extra_include_paths is None:
        extra_include_paths = []
    extra_include_paths += [FLASHINFER_INCLUDE_DIR, FLASHINFER_CSRC_DIR] + CUTLASS_INCLUDE_DIRS
    lock = FileLock(FLASHINFER_JIT_DIR / f'{name}.lock', thread_local=False)
    with lock:
        torch_cpp_ext.load(name, list(map(lambda _: str(_), sources)), extra_cflags=cflags, extra_cuda_cflags=cuda_cflags, extra_ldflags=extra_ldflags, extra_include_paths=list(map(lambda _: str(_), extra_include_paths)), build_directory=build_directory, verbose=verbose, with_cuda=True, is_python_module=False)
