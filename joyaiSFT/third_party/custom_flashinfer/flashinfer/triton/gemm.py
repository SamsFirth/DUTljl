...
@triton.jit
def compute_sm80_group_gemm_args(all_problems_ptr, x_ptr, w_ptr, y_ptr, x_ld_ptr, w_ld_ptr, y_ld_ptr, x, w, y, xy_indptr, w_indices, d_in, d_out, w_column_major):
    pid = tl.program_id(0)
...
@triton.jit
def compute_sm90_group_gemm_args(all_problems_ptr, x_ptr, w_ptr, y_ptr, x_stride_ptr, w_stride_ptr, y_stride_ptr, x, w, y, xy_indptr, w_indices, d_in, d_out, w_column_major):
    pid = tl.program_id(0)
    m = tl.load(xy_indptr + pid + 1) - tl.load(xy_indptr + pid)
    k, n = (d_in, d_out)
    tl.store(all_problems_ptr + pid * 3, m)
