...
def _unpack_paged_kv_cache(paged_kv_cache: Union[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]], kv_layout: str) -> Tuple[torch.Tensor, torch.Tensor]:
    if isinstance(paged_kv_cache, tuple):
        paged_k_cache, paged_v_cache = paged_kv_cache
        return (_expand_4d(paged_k_cache, kv_layout), _expand_4d(paged_v_cache, kv_layout))
        ...
        raise KeyError('Unrecognized paged_kv_cache type {}, expect a single tensor or a tuple of tensor.'.format(type(paged_kv_cache)))
...
def _get_cache_alibi_slopes_buf(num_qo_heads: int, device: torch.device) -> torch.Tensor:
...
def canonicalize_torch_dtype(dtype: Union[torch.dtype, str]) -> torch.dtype:
    if isinstance(dtype, str):
        ...
        raise TypeError('dtype must be a string or torch.dtype, got {}'.format(type(dtype)))
...
def _check_cached_qkv_data_type(q: torch.Tensor, k: torch.Tensor, dtype_q: torch.dtype, dtype_kv: torch.dtype) -> None:
    if q.dtype != dtype_q:
        raise ValueError(f'The dtype of q {q.dtype} does not match the q_data_type {dtype_q} specified in plan function.')
    if k.dtype != dtype_kv:
        raise ValueError(f'The dtype of k {k.dtype} does not match the kv_data_type {dtype_kv} specified in plan function.')
if IS_BUILDING_DOCS or TorchVersion(torch_version) < TorchVersion('2.4'):

    def register_custom_op(name: str, fn: Optional[Callable]=None, /, *, mutates_args: Union[str, Iterable[str]], device_types: Optional[Union[str, Sequence[str]]]=None, schema: Optional[str]=None) -> Callable:
        return lambda x: x
...
def is_fa3_backend_supported(pos_encoding_mode: int, use_fp16_qk_reductions: bool, use_custom_mask: bool, dtype_q: torch.dtype, dtype_kv: torch.dtype) -> bool:
    """
...
def determine_attention_backend(device: torch.device, pos_encoding_mode: int, use_fp16_qk_reductions: bool, use_custom_mask: bool, dtype_q: torch.dtype, dtype_kv: torch.dtype) -> str:
    """
    ...
    """
    if is_sm90a_supported(device) and is_fa3_backend_supported(pos_encoding_mode, use_fp16_qk_reductions, use_custom_mask, dtype_q, dtype_kv):
        return 'fa3'
    else:
        return 'fa2'
...
def _check_shape_dtype_device(x: torch.Tensor, expected_shape: Sequence[int], expected_dtype: torch.dtype, expected_device: torch.device, name: str) -> None:
    if x.shape != torch.Size(expected_shape):
        raise ValueError(f'Invalid shape of {name}: expected {expected_shape}, got {x.shape}')
    if x.dtype != expected_dtype:
        raise ValueError(f'Invalid dtype of {name}: expected {expected_dtype}, got {x.dtype}')
    if x.device != expected_device:
        raise ValueError(f'Invalid device of {name}: expected {expected_device}, got {x.device}')