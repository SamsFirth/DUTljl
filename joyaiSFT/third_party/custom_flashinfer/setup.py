...
head_dims_sm90.extend([(k, v) for k, v in SM90_ALLOWED_HEAD_DIMS if k != v])
mask_modes = [0, 1, 2]
...
def generate_cuda() -> None:
    try:
        ...
        from aot_build_utils.generate_aot_default_additional_params_header import get_aot_default_additional_params_header_str
        from aot_build_utils.generate_sm90 import get_sm90_instantiation_cu
    except ImportError:
        return
    write_if_different(gen_dir / 'dispatch.inc', generate_dispatch_inc.get_dispatch_inc_str(argparse.Namespace(head_dims=head_dims, head_dims_sm90=head_dims_sm90, pos_encoding_modes=[0], use_fp16_qk_reductions=[0], mask_modes=mask_modes)))
    aot_kernel_uris = get_instantiation_cu(argparse.Namespace(path=gen_dir, head_dims=head_dims, pos_encoding_modes=[0], use_fp16_qk_reductions=[0], mask_modes=mask_modes, enable_f16=enable_f16, enable_bf16=enable_bf16, enable_fp8_e4m3=enable_fp8_e4m3, enable_fp8_e5m2=enable_fp8_e5m2))
    if enable_sm90:
        aot_kernel_uris += get_sm90_instantiation_cu(argparse.Namespace(path=gen_dir, head_dims=head_dims_sm90, pos_encoding_modes=[0], use_fp16_qk_reductions=[0], mask_modes=mask_modes, enable_f16=enable_f16, enable_bf16=enable_bf16))
    aot_config_str = f'prebuilt_ops_uri = set({aot_kernel_uris})'
    write_if_different(root / 'flashinfer' / 'jit' / 'aot_config.py', aot_config_str)
    write_if_different(root / 'csrc' / 'aot_default_additional_params.h', get_aot_default_additional_params_header_str())
ext_modules = []
cmdclass = {}
...
if enable_aot:
    ...
    include_dirs = [root.resolve() / 'include', cutlass.resolve() / 'include', cutlass.resolve() / 'tools' / 'util' / 'include']
    cxx_flags = ['-O3', '-Wno-switch-bool', '-DPy_LIMITED_API=0x03080000']
    nvcc_flags = ['-O3', '-std=c++17', '--threads=1', '-Xfatbin', '-compress-all', '-use_fast_math', '-DPy_LIMITED_API=0x03080000']
    libraries = ['cublas', 'cublasLt']
    sm90a_flags = '-gencode arch=compute_90a,code=sm_90a'.split()
    kernel_sources = ['csrc/bmm_fp8.cu', 'csrc/cascade.cu', 'csrc/group_gemm.cu', 'csrc/norm.cu', 'csrc/page.cu', 'csrc/quantization.cu', 'csrc/rope.cu', 'csrc/sampling.cu', 'csrc/renorm.cu', 'csrc/activation.cu', 'csrc/batch_decode.cu', 'csrc/batch_prefill.cu', 'csrc/single_decode.cu', 'csrc/single_prefill.cu', 'csrc/flashinfer_ops.cu']
    kernel_sm90_sources = ['csrc/group_gemm_sm90.cu', 'csrc/single_prefill_sm90.cu', 'csrc/batch_prefill_sm90.cu', 'csrc/flashinfer_ops_sm90.cu']
    decode_sources = list(gen_dir.glob('*decode_head*.cu'))
    prefill_sources = [f for f in gen_dir.glob('*prefill_head*.cu') if '_sm90' not in f.name]
    prefill_sm90_sources = list(gen_dir.glob('*prefill_head*_sm90.cu'))
    ext_modules = [torch_cpp_ext.CUDAExtension(name='flashinfer.flashinfer_kernels', sources=kernel_sources + decode_sources + prefill_sources, include_dirs=include_dirs, libraries=libraries, extra_compile_args={'cxx': cxx_flags, 'nvcc': nvcc_flags}, py_limited_api=True)]
    if enable_sm90:
        ext_modules += [torch_cpp_ext.CUDAExtension(name='flashinfer.flashinfer_kernels_sm90', sources=kernel_sm90_sources + prefill_sm90_sources, include_dirs=include_dirs, libraries=libraries, extra_compile_args={'cxx': cxx_flags, 'nvcc': nvcc_flags + sm90a_flags}, py_limited_api=True)]
setuptools.setup(version=get_version(), ext_modules=ext_modules, cmdclass=cmdclass, install_requires=install_requires, options={'bdist_wheel': {'py_limited_api': 'cp38'}})