...
from .literal_map import dtype_literal, idtype_literal, mask_mode_literal, pos_encoding_mode_literal
...
def get_cu_file_str(head_dim_qk, head_dim_vo, pos_encoding_mode, use_fp16_qk_reduction, mask_mode, dtype_q, dtype_kv, dtype_out, idtype):
    ...
    def get_insts(attention_variant, dtype_out):
        return '\n'.join(['template cudaError_t BatchPrefillWithRaggedKVCacheDispatched<{cta_tile_q}, {head_dim_qk}, {head_dim_vo}, {pos_encoding_mode}, {use_fp16_qk_reduction}, {mask_mode}, {attention_variant}, Params>(\n    Params params,\n    {dtype_out}* tmp_v,\n    float* tmp_s, cudaStream_t stream);\n        '.format(cta_tile_q=cta_tile_q, head_dim_qk=head_dim_qk, head_dim_vo=head_dim_vo, pos_encoding_mode=pos_encoding_mode_literal[int(pos_encoding_mode)], use_fp16_qk_reduction=use_fp16_qk_reduction, mask_mode=mask_mode_literal[int(mask_mode)], attention_variant=attention_variant, dtype_out=dtype_out) for cta_tile_q in cta_tile_q_choice])
...
if __name__ == '__main__':
    pattern = 'batch_ragged_prefill_head_qk_([0-9]+)_head_vo_([0-9]+)_posenc_([0-9]+)_fp16qkred_([a-z]+)_mask_([0-9]+)_dtypeq_([a-z0-9]+)_dtypekv_([a-z0-9]+)_dtypeout_([a-z0-9]+)_idtype_([a-z0-9]+)\\.cu'
