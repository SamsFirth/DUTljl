...
def warmup_jit():
    if flashinfer.jit.has_prebuilt_ops:
        ...
        try:
            flashinfer.jit.parallel_load_modules(jit_decode_attention_func_args([torch.float16], [torch.float16], [128, 256], [0], [False], [False, True]) + jit_prefill_attention_func_args([torch.float16], [torch.float16], [128, 256], [0], [False], [False, True], [False]))
