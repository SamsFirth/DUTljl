...
from flashinfer.jit.attention import gen_customize_single_decode_module, gen_customize_single_prefill_module
...
def test_single_decode_mask():
    ...
    jit_module = gen_customize_single_decode_module('single_decode_custom_mask', torch.float16, torch.float16, torch.float16, 128, 128, ['custom_mask'], ['uint8_t'], ['sm_scale'], ['double'], 'SingleDecodeWithCustomMask', variant_decl)
...
def test_flash_sigmoid():
    ...
    jit_module = gen_customize_single_prefill_module('fa2', 'single_prefill_flash_sigmoid', torch.float16, torch.float16, torch.float16, 128, 128, [], [], ['logits_scale', 'sigmoid_bias'], ['double', 'double'], 'FlashSigmoid', variant_decl)
    ...
    p = torch.sigmoid(torch.einsum('mhd,nhd->hmn', q.float(), k.float()) * logits_scale + sigmoid_bias)
    o_ref = torch.einsum('hmn,nhd->mhd', p, v.float()).half()
    torch.testing.assert_close(o, o_ref, rtol=0.02, atol=0.02)
...
def test_dump_logits():
    ...
    jit_module = gen_customize_single_prefill_module('fa2', 'single_prefill_dump_logits', torch.float16, torch.float16, torch.float16, 128, 128, ['output_logits'], ['float'], ['sm_scale'], ['double'], 'DumpLogits', variant_decl)
    f = functools.partial(single_prefill_with_kv_cache_with_jit_module, jit_module)
    ...
    torch.testing.assert_close(o, o_ref, rtol=0.001, atol=0.001)
    torch.testing.assert_close(logits, p, rtol=0.02, atol=0.02)

@pytest.mark.parametrize('use_tensor_cores', [False, True])
def test_batch_decode_flash_sigmoid(use_tensor_cores):
    torch.manual_seed(42)
    variant_decl = flash_sigmoid_sm80_decl
    jit_args = (f'batch_decode_flash_sigmoid_sm80_{use_tensor_cores}', torch.float16, torch.float16, torch.float16, torch.int32, 128, 128, [], [], ['logits_scale', 'sigmoid_bias'], ['double', 'double'], 'FlashSigmoid', variant_decl)
    float_workspace_buffer = torch.empty(128 * 1024 * 1024, dtype=torch.uint8, device='cuda')
    wrapper = flashinfer.BatchDecodeWithPagedKVCacheWrapper(float_workspace_buffer, kv_layout='NHD', use_tensor_cores=use_tensor_cores, jit_args=jit_args)
    batch_size = 128
    seq_len_per_request = 1024
    kv_indptr_host = torch.arange(0, batch_size * seq_len_per_request + 1, seq_len_per_request, dtype=torch.int32)
    page_size = 1
    kv_indices_host = torch.arange(0, batch_size * seq_len_per_request, dtype=torch.int32)
    last_page_len_host = torch.full((batch_size,), 1, dtype=torch.int32)
    num_qo_heads = 32
    num_kv_heads = 32
    head_dim = 128
    wrapper.plan(kv_indptr_host, kv_indices_host, last_page_len_host, num_qo_heads, num_kv_heads, head_dim, page_size, q_data_type=torch.float16, kv_data_type=torch.float16)
    q = torch.randn(batch_size, num_qo_heads, head_dim, dtype=torch.float16, device='cuda')
    k_cache = torch.randn(batch_size * seq_len_per_request, num_kv_heads, head_dim, dtype=torch.float16, device='cuda')
    v_cache = torch.randn(batch_size * seq_len_per_request, num_kv_heads, head_dim, dtype=torch.float16, device='cuda')
    logits_scale = 1.0 / math.sqrt(128)
    sigmoid_bias = 0.25
    o = wrapper.run(q, (k_cache, v_cache), logits_scale, sigmoid_bias)
    p = torch.sigmoid(torch.einsum('bhd,bnhd->bhn', q.view(batch_size, num_qo_heads, head_dim).float(), k_cache.view(batch_size, seq_len_per_request, num_kv_heads, head_dim).float()) * logits_scale + sigmoid_bias)
    o_ref = torch.einsum('bhn,bnhd->bhd', p, v_cache.view(batch_size, seq_len_per_request, num_kv_heads, head_dim).float()).half().reshape(batch_size, num_qo_heads, head_dim)
    torch.testing.assert_close(o, o_ref, rtol=0.02, atol=0.02)

def test_batch_prefill_flash_sigmoid():
    torch.manual_seed(42)
    variant_decl = flash_sigmoid_sm80_decl
    jit_args = ('batch_prefill_flash_sigmoid_sm80', torch.float16, torch.float16, torch.float16, torch.int32, 128, 128, [], [], ['logits_scale', 'sigmoid_bias'], ['double', 'double'], 'FlashSigmoid', variant_decl)
    float_workspace_buffer = torch.empty(128 * 1024 * 1024, dtype=torch.uint8, device='cuda')
    wrapper = flashinfer.BatchPrefillWithRaggedKVCacheWrapper(float_workspace_buffer, kv_layout='NHD', backend='fa2', jit_args=jit_args)
    batch_size = 128
    seq_len_per_request = 1024
    qo_indptr_host = torch.arange(0, batch_size * seq_len_per_request + 1, seq_len_per_request, dtype=torch.int32)
    kv_indptr_host = torch.arange(0, batch_size * seq_len_per_request + 1, seq_len_per_request, dtype=torch.int32)
    num_qo_heads = 32
    num_kv_heads = 32
    head_dim = 128
    wrapper.plan(qo_indptr_host, kv_indptr_host, num_qo_heads, num_kv_heads, head_dim, causal=False, q_data_type=torch.float16, kv_data_type=torch.float16)
    q = torch.randn(batch_size * seq_len_per_request, num_qo_heads, head_dim, dtype=torch.float16, device='cuda')
    k = torch.randn(batch_size * seq_len_per_request, num_kv_heads, head_dim, dtype=torch.float16, device='cuda')
    v = torch.randn(batch_size * seq_len_per_request, num_kv_heads, head_dim, dtype=torch.float16, device='cuda')
    logits_scale = 1.0 / math.sqrt(128)
    sigmoid_bias = 0.25
    o = wrapper.run(q, k, v, logits_scale, sigmoid_bias)
    wrapper_paged = flashinfer.BatchPrefillWithPagedKVCacheWrapper(float_workspace_buffer, kv_layout='NHD', backend='fa2', jit_args=jit_args)
    kv_indices_host = torch.arange(0, batch_size * seq_len_per_request, dtype=torch.int32)
    paged_kv_last_page_len_host = torch.full((batch_size,), 1, dtype=torch.int32)
    wrapper_paged.plan(qo_indptr_host, kv_indptr_host, kv_indices_host, paged_kv_last_page_len_host, num_qo_heads, num_kv_heads, head_dim, 1)
    o_paged = wrapper_paged.run(q, (k, v), logits_scale, sigmoid_bias)
    p = torch.sigmoid(torch.einsum('bmhd,bnhd->bhmn', q.view(batch_size, seq_len_per_request, num_qo_heads, head_dim).float(), k.view(batch_size, seq_len_per_request, num_kv_heads, head_dim).float()) * logits_scale + sigmoid_bias)
    o_ref = torch.einsum('bhmn,bnhd->bmhd', p, v.view(batch_size, seq_len_per_request, num_kv_heads, head_dim).float()).half().reshape(batch_size * seq_len_per_request, num_qo_heads, head_dim)
    torch.testing.assert_close(o, o_ref, rtol=0.02, atol=0.02)
    torch.testing.assert_close(o_paged, o_ref, rtol=0.02, atol=0.02)
...
def test_batch_prefill_sm90_flash_sigmoid():
    ...
    jit_args = ('batch_prefill_flash_sigmoid', torch.float16, torch.float16, torch.float16, torch.int32, 128, 128, [], [], ['logits_scale', 'sigmoid_bias'], ['double', 'double'], 'FlashSigmoid', variant_decl)
    float_workspace_buffer = torch.empty(128 * 1024 * 1024, dtype=torch.uint8, device='cuda')
    wrapper = flashinfer.BatchPrefillWithRaggedKVCacheWrapper(float_workspace_buffer, kv_layout='NHD', backend='fa3', jit_args=jit_args)
    batch_size = 128
    seq_len_per_request = 1024
    qo_indptr_host = torch.arange(0, batch_size * seq_len_per_request + 1, seq_len_per_request, dtype=torch.int32)
    kv_indptr_host = torch.arange(0, batch_size * seq_len_per_request + 1, seq_len_per_request, dtype=torch.int32)
    ...
    sigmoid_bias = 0.25
    o = wrapper.run(q, k, v, logits_scale, sigmoid_bias)
    wrapper_paged = flashinfer.BatchPrefillWithPagedKVCacheWrapper(float_workspace_buffer, kv_layout='NHD', backend='fa3', jit_args=jit_args)
    kv_indices_host = torch.arange(0, batch_size * seq_len_per_request, dtype=torch.int32)
    paged_kv_last_page_len_host = torch.full((batch_size,), 1, dtype=torch.int32)
    wrapper_paged.plan(qo_indptr_host, kv_indptr_host, kv_indices_host, paged_kv_last_page_len_host, num_qo_heads, num_kv_heads, head_dim, 1)
    o_paged = wrapper_paged.run(q, (k, v), logits_scale, sigmoid_bias)
...
def test_debug_print_logits():
    ...
    jit_module = gen_customize_single_prefill_module('fa2', 'batch_prefill_debug_print_logits', torch.float16, torch.float16, torch.float16, 128, 128, [], [], ['sm_scale'], ['double'], 'DebugPrintLogits', variant_decl)
    f = functools.partial(single_prefill_with_kv_cache_with_jit_module, jit_module)
    q = torch.randn(128, 32, 128, dtype=torch.float16, device='cuda')
    k = torch.randn(1023, 32, 128, dtype=torch.float16, device='cuda')
    v = torch.randn(1023, 32, 128, dtype=torch.float16, device='cuda')
...
def test_sm90_debug_print_logits():
    ...
    jit_module = gen_customize_single_prefill_module('fa3', 'debug_print_logits', torch.float16, torch.float16, torch.float16, 128, 128, [], [], ['sm_scale'], ['double'], 'DebugPrintLogits', variant_decl)
    f = functools.partial(single_prefill_with_kv_cache_with_jit_module, jit_module)
