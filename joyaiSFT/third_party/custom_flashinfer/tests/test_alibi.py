...
def warmup_jit():
    if flashinfer.jit.has_prebuilt_ops:
        ...
        try:
            flashinfer.jit.parallel_load_modules(jit_decode_attention_func_args([torch.float16], [torch.float16], [128, 256], [0, 2], [False], [False]) + jit_prefill_attention_func_args([torch.float16], [torch.float16], [128, 256], [0, 2], [False], [False], [False]))
...
def test_single_prefill_alibi(q_len, kv_len, num_heads, head_dim, causal):
    ...
    o = flashinfer.single_prefill_with_kv_cache(q, k, v, causal=causal, pos_encoding_mode='ALIBI')
