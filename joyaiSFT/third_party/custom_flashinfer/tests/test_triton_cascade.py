...
def test_merge_state(seq_len, num_heads, head_dim):
    ...
    assert torch.allclose(v_merged, v_merged_std, atol=0.01)
    assert torch.allclose(s_merged, s_merged_std, atol=0.01)
...
def test_merge_state_in_place(seq_len, num_heads, head_dim):
    ...
    assert torch.allclose(v, v_std, atol=0.01)
    assert torch.allclose(s, s_std, atol=0.01)
...
def test_variable_length_merge_states(seq_len, num_heads, head_dim):
    ...
    v_merged, s_merged = flashinfer.triton.cascade.variable_length_merge_states(v, s, indptr)
    for i in range(seq_len):
        ...
        assert torch.allclose(v_merged[i], v_merged_std, atol=0.01)
        assert torch.allclose(s_merged[i], s_merged_std, atol=0.01)