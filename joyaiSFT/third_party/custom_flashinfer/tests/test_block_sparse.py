...
def warmup_jit():
    if flashinfer.jit.has_prebuilt_ops:
        ...
        try:
            flashinfer.jit.parallel_load_modules(jit_decode_attention_func_args([torch.float16], [torch.float16], [128, 256], [0], [False], [False]) + jit_prefill_attention_func_args([torch.float16], [torch.float16], [128, 256], [0], [False], [False], [False]))
...
def test_block_sparse_attention(R, C, M, N, num_qo_heads, num_kv_heads, head_dim, mask_inside_block):
    ...
    sparse_attention_wrapper = flashinfer.sparse.BlockSparseAttentionWrapper(workspace_buffer)
    sparse_attention_wrapper.plan(indptr, indices, M, N, R, C, num_qo_heads, num_kv_heads, head_dim, mask=data_mask if mask_inside_block else None)
    o = sparse_attention_wrapper.run(q, k, v)
    torch.testing.assert_close(o_ref, o, atol=0.01, rtol=0.001)
    o_buffer = torch.empty_like(o)
    sparse_attention_wrapper.run(q, k, v, out=o_buffer)
    torch.testing.assert_close(o_ref, o_buffer, atol=0.01, rtol=0.001)
