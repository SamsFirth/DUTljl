...
def test_norm(batch_size, hidden_size, dtype, specify_out, enable_pdl, contiguous):
    ...
    torch.testing.assert_close(y_ref, y, rtol=0.001, atol=0.001)
...
def test_fused_add_rmsnorm(batch_size, hidden_size, dtype, enable_pdl, contiguous):
    ...
    torch.testing.assert_close(x_fused, x_native, rtol=0.001, atol=0.001)
    torch.testing.assert_close(residual_fused, residual_native, rtol=0.001, atol=0.001)
...
def test_gemma_fused_add_rmsnorm(batch_size, hidden_size, dtype, enable_pdl, contiguous):
    ...
    residual = torch.randn_like(x)
    weight = torch.randn(hidden_size, dtype=dtype, device='cuda')
    x_native, residual_native = gemma_fused_add_rms_norm(x.clone(), residual.clone(), weight, eps)
    x_fused = x.clone()
    residual_fused = residual.clone()
    flashinfer.gemma_fused_add_rmsnorm(x_fused, residual_fused, weight, eps, enable_pdl=enable_pdl)
    torch.testing.assert_close(x_fused, x_native, rtol=0.001, atol=0.001)
    torch.testing.assert_close(residual_fused, residual_native, rtol=0.001, atol=0.001)
