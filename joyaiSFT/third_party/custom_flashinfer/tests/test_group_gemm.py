...
def warmup_jit():
    if flashinfer.jit.has_prebuilt_ops:
        ...
        modules = [(flashinfer.gemm.get_gemm_module, [])]
...
def test_segment_gemm(batch_size, num_rows_per_batch, d_in, d_out, use_weight_indices, column_major, dtype, device, backend):
    ...
    y = segment_gemm.run(x, weight, batch_size, weight_column_major=column_major, seg_lens=torch.full((batch_size,), num_rows_per_batch, dtype=torch.int64), weight_indices=torch.arange(0, batch_size, device=device) % num_weights if use_weight_indices else None)
    if use_weight_indices:
        for i in range(batch_size):
            torch.testing.assert_close(y[i * num_rows_per_batch:(i + 1) * num_rows_per_batch], torch.matmul(x[i * num_rows_per_batch:(i + 1) * num_rows_per_batch].float(), weight[i % num_weights].float().T if column_major else weight[i % num_weights].float()).to(dtype), rtol=0.001, atol=0.001)
    else:
        torch.testing.assert_close(y, torch.matmul(x.view(batch_size, num_rows_per_batch, d_in).float(), weight.float().transpose(-1, -2) if column_major else weight.float()).view(batch_size * num_rows_per_batch, d_out).to(dtype), rtol=0.001, atol=0.002)
