...
class ChatrhinoV0AttentionVanilla(nn.Module):
    ...
    def __init__(self):
        ...
        self.q_a_layernorm = ChatrhinoV0RMSNorm(self.q_lora_rank)
    ...
    def run_decode(self, hidden_states: torch.Tensor, compressed_kv_normed_cache: torch.Tensor, k_pe_cache: torch.Tensor) -> Tuple[torch.Tensor, Optional[torch.Tensor], Optional[Tuple[torch.Tensor]]]:
        ...
        if ckv_bsz != bsz or ckv_dim != self.kv_lora_rank:
            raise ValueError(f'Unexpected shape: compressed_kv_normed_cache[{compressed_kv_normed_cache.size()}]')
        ...
        kv = self.kv_b_proj(compressed_kv_normed_cache).view(bsz, kv_len, self.num_heads, self.qk_nope_head_dim + self.v_head_dim).transpose(1, 2)
        ...
        freqs_cis = precompute_freqs_cis(self.qk_rope_head_dim, kv_len, self.rope_theta, use_scaled=False).to(q_pe.device)
        q_pe, k_pe = apply_rotary_emb(q_pe.transpose(1, 2).repeat(1, kv_len, 1, 1), k_pe.transpose(1, 2), freqs_cis)
...
class ChatrhinoV0AttentionMatAbsorbDecode(nn.Module):

    def __init__(self, mla_vanilla: ChatrhinoV0AttentionVanilla):
        super().__init__()
        ...
        self.q_a_layernorm = ChatrhinoV0RMSNorm(self.q_lora_rank)
        W_UQ, W_QR = torch.split(mla_vanilla.q_b_proj.weight.t().view(self.q_lora_rank, self.num_heads, self.q_head_dim), [self.qk_nope_head_dim, self.qk_rope_head_dim], -1)
        self.W_QR = W_QR.reshape(self.q_lora_rank, self.num_heads * self.qk_rope_head_dim)
        W_UK, W_UV = torch.split(mla_vanilla.kv_b_proj.weight.t().view(self.kv_lora_rank, self.num_heads, self.qk_nope_head_dim + self.v_head_dim), [self.qk_nope_head_dim, self.v_head_dim], -1)
        self.W_UQ_UK = torch.einsum('q n d, l n d -> q n l', W_UQ, W_UK).flatten(start_dim=1)
        W_O = mla_vanilla.o_proj.weight.view(self.hidden_size, self.num_heads, self.v_head_dim)
        self.W_UV_O = torch.einsum('l n d, h n d -> n l h', W_UV, W_O).flatten(start_dim=0, end_dim=1)

    def run_proof_of_concept(self, hidden_states: torch.Tensor, compressed_kv_normed_cache: torch.Tensor, k_pe_cache: torch.Tensor, use_flashinfer_kernel: bool, convert_float16: bool) -> Tuple[torch.Tensor, Optional[torch.Tensor], Optional[Tuple[torch.Tensor]]]:
        c_Q = torch.matmul(hidden_states, self.W_DQ)
        c_Q = self.q_a_layernorm(c_Q)
        q_pe = torch.matmul(c_Q, self.W_QR)
        q_pe = q_pe.reshape(bsz, self.num_heads, self.qk_rope_head_dim)
        q_nope = torch.matmul(c_Q, self.W_UQ_UK)
        ...
        if not use_flashinfer_kernel:
            freqs_cis = precompute_freqs_cis(self.qk_rope_head_dim, kv_len, self.rope_theta, use_scaled=False).to(k_pe_cache.device)
            q_pe, k_pe_cache = apply_rotary_emb(q_pe.unsqueeze(1).repeat(1, kv_len, 1, 1), k_pe_cache.unsqueeze(2), freqs_cis)
            q_pe = q_pe[:, -1:, :, :].squeeze(1)
            k_pe_cache = k_pe_cache.squeeze(2)
            attn_weights_pe = torch.matmul(q_pe, k_pe_cache.transpose(1, 2))
            attn_weights_nope = torch.matmul(q_nope, compressed_kv_normed_cache.transpose(1, 2))
            ...
            attn_output = torch.matmul(attn_weights, compressed_kv_normed_cache)
        else:
            ...
            kv_last_page_len = torch.full((bsz,), page_size, dtype=torch.int32).to(dev_id)
            ...
            wrapper = flashinfer.BatchDecodeMlaWithPagedKVCacheWrapper(workspace_buffer, use_cuda_graph=True, use_tensor_cores=True, paged_kv_indptr_buffer=kv_indptr, paged_kv_indices_buffer=kv_indices, paged_kv_last_page_len_buffer=kv_last_page_len)
            wrapper.plan(kv_indptr, kv_indices, kv_last_page_len, num_qo_heads=self.num_heads, head_dim_compressed_kv=self.kv_lora_rank, page_size=page_size, sm_scale=self.softmax_scale, rope_theta=self.rope_theta, data_type=q_kv_dtype, q_data_type=q_kv_dtype)
            g = torch.cuda.CUDAGraph()
            with torch.cuda.graph(g):
                attn_output = wrapper.run(q_nope, q_pe, paged_ckv_cache, paged_kpe_cache)
            g.replay()
        output = torch.matmul(attn_output.to(self.W_UV_O.dtype).reshape(bsz, self.num_heads * self.kv_lora_rank), self.W_UV_O)
        return output
if __name__ == '__main__':
    dev_id = 0
    torch.manual_seed(666)
    torch.set_grad_enabled(False)
    mla_vanilla = ChatrhinoV0AttentionVanilla().cuda(device=dev_id)
    bsz = 6
    kv_len = 640
    page_size = 16
    ...
    mla_mat_absorb = ChatrhinoV0AttentionMatAbsorbDecode(mla_vanilla).cuda(device=dev_id)
    output_mat_absorbed_use_torch_f32 = mla_mat_absorb.run_proof_of_concept(hidden_states.squeeze(1), compressed_kv_normed_cache, k_pe_cache, use_flashinfer_kernel=False, convert_float16=False)
    output_mat_absorbed_use_torch_f16 = mla_mat_absorb.run_proof_of_concept(hidden_states.squeeze(1), compressed_kv_normed_cache, k_pe_cache, use_flashinfer_kernel=False, convert_float16=True)
    output_mat_absorbed_use_flashinfer = mla_mat_absorb.run_proof_of_concept(hidden_states.squeeze(1), compressed_kv_normed_cache, k_pe_cache, use_flashinfer_kernel=True, convert_float16=True)
