...
@pytest.mark.parametrize('distribution', [normal_distribution(1), normal_distribution(5), gumbel_distribution(0.1)])
...
def test_sampling_freq(vocab_size, distribution, zero_ratio):
    ...
    samples = flashinfer.sampling.sampling_from_probs(probs, indices=torch.zeros(num_trials, dtype=torch.int32, device=logits.device))
...
def test_top_p_sampling_freq(vocab_size, distribution, p):
    ...
    counter = torch.zeros(vocab_size, dtype=torch.int32, device=logits.device)
    num_trials = 5000000
    samples = flashinfer.sampling.top_p_sampling_from_probs(probs, p, indices=torch.zeros(num_trials, dtype=torch.int32, device=logits.device))
    counter.scatter_add_(0, samples.long(), torch.ones_like(samples))
    freq = counter.float() / num_trials
...
def test_top_k_sampling_freq(vocab_size, distribution, k):
    ...
    counter = torch.zeros(vocab_size, dtype=torch.int32, device=logits.device)
    num_trials = 5000000
    samples = flashinfer.sampling.top_k_sampling_from_probs(probs, k, indices=torch.zeros(num_trials, dtype=torch.int32, device=logits.device))
    counter.scatter_add_(0, samples.long(), torch.ones_like(samples))
    freq = counter.float() / num_trials
    assert torch.all(mask[torch.arange(1), samples] == 1)
    similarity = torch.cosine_similarity(freq, renorm_probs)
...
def test_top_k_sampling(batch_size, vocab_size, k):
    ...
    for _ in range(num_trails):
        ...
        assert torch.all(samples < vocab_size) and torch.all(samples >= 0)
        assert torch.all(mask[torch.arange(batch_size), samples] == 1), normalized_prob[torch.arange(batch_size), samples]
...
def test_min_p_sampling(batch_size, vocab_size, p):
    ...
    num_trails = 1000
    for _ in range(num_trails):
        samples = flashinfer.sampling.min_p_sampling_from_probs(normalized_prob, min_p_tensor)
        assert torch.all(mask[torch.arange(batch_size), samples] == 1), samples[torch.nonzero(mask[torch.arange(batch_size), samples] == 0)]
...
def test_top_k_top_p_joint_sampling_from_probs(batch_size, vocab_size, p):
    ...
    num_trails = 1000
    for _ in range(num_trails):
        samples = flashinfer.sampling.top_k_top_p_sampling_from_probs(normalized_prob, top_k_tensor, top_p_tensor, filter_apply_order='joint')
        assert torch.all(samples < vocab_size) and torch.all(samples >= 0)
        assert torch.all(mask[torch.arange(batch_size), samples] == 1), normalized_prob[torch.arange(batch_size), samples]
...
def test_top_k_top_p_sampling_from_probs_logits_alignment(batch_size, vocab_size, k, p):
    ...
    samples = flashinfer.sampling.top_k_top_p_sampling_from_logits(logits, k, p, filter_apply_order='top_k_first', generator=generator_logits)
    samples_ref = flashinfer.sampling.top_k_top_p_sampling_from_probs(torch.softmax(logits, dim=-1), k, p, filter_apply_order='top_k_first', generator=generator_probs)
...
def test_top_k_top_p_joint_sampling_from_logits(batch_size, vocab_size, p):
    ...
    if p == 0.1:
        ...
        raise ValueError('p not recognized')
    samples = flashinfer.sampling.top_k_top_p_sampling_from_logits(logits, k, p, filter_apply_order='joint', generator=generator_logits)
    samples_ref = flashinfer.sampling.top_k_top_p_sampling_from_probs(torch.softmax(logits, dim=-1), k, p, filter_apply_order='joint', generator=generator_probs)
    assert torch.all(samples == samples_ref)
...
def test_top_p_renorm_probs(batch_size, vocab_size, p):
    ...
    renorm_prob_ground_truth = renorm_prob_ground_truth / renorm_prob_ground_truth.sum(dim=-1, keepdim=True)
    renorm_prob = flashinfer.sampling.top_p_renorm_probs(normalized_prob, p)
    torch.testing.assert_close(renorm_prob_ground_truth, renorm_prob, rtol=0.001, atol=0.001)
...
def test_top_k_renorm_probs(batch_size, vocab_size, k):
    ...
    for i in range(batch_size):
        torch.testing.assert_close(renorm_prob_ground_truth[i], renorm_prob[i], rtol=0.001, atol=0.001)
...
def test_top_k_mask_logits(batch_size, vocab_size, k):
    ...
    torch.testing.assert_close(renormed_probs, renormed_probs_ref, rtol=0.001, atol=0.001)

@pytest.mark.parametrize('batch_size', [1, 99, 989])
@pytest.mark.parametrize('vocab_size', [111, 32000, 128256])
@pytest.mark.parametrize('num_speculate_tokens', [1, 3, 5, 7])
@pytest.mark.parametrize('onehot_target', [False, True])
def test_chain_speculative_sampling(batch_size, vocab_size, num_speculate_tokens, onehot_target):
    pre_norm_draft_prob = torch.rand(batch_size, num_speculate_tokens, vocab_size, device='cuda:0')
    normalized_draft_prob = pre_norm_draft_prob / pre_norm_draft_prob.sum(dim=-1, keepdim=True)
    draft_token_ids = torch.randint(vocab_size, (batch_size, num_speculate_tokens), device='cuda:0')
    if not onehot_target:
        pre_norm_target_prob = torch.rand(batch_size, num_speculate_tokens + 1, vocab_size, device='cuda:0')
        target_onehot_prob = pre_norm_target_prob / pre_norm_target_prob.sum(dim=-1, keepdim=True)
    else:
        target_token_ids = torch.randint(vocab_size, (batch_size, num_speculate_tokens + 1), device='cuda:0')
        target_token_ids[..., :num_speculate_tokens] = draft_token_ids
        target_onehot_prob = torch.zeros((batch_size, num_speculate_tokens + 1, vocab_size), device='cuda:0')
    ...
    for trials in range(10):
        ...
        output_token_ids, accepted_num, emitted_num = flashinfer.sampling.chain_speculative_sampling(normalized_draft_prob, draft_token_ids, target_onehot_prob, accepted_num, emitted_num)
        if onehot_target:
        ...
        batch_indices = torch.arange(batch_size, device=normalized_draft_prob.device)[:, None]
        probs_indicies = torch.arange(num_speculate_tokens, device=normalized_draft_prob.device)
        selected_draft_probs = normalized_draft_prob[batch_indices, probs_indicies, draft_token_ids]
        selected_target_probs = target_onehot_prob[batch_indices, probs_indicies, draft_token_ids]
