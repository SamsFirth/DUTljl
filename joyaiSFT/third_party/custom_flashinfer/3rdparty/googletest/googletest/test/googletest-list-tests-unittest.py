...
class GTestListTestsUnitTest(gtest_test_utils.TestCase):
    ...
    """
        ...
        if expected_output_re:
            self.assertTrue(expected_output_re.match(output), 'when %s is %s, the output of "%s" is "%s",\nwhich does not match regex "%s"' % (LIST_TESTS_FLAG, flag_expression, ' '.join(args), output, expected_output_re.pattern))
        else:
            self.assertTrue(not EXPECTED_OUTPUT_NO_FILTER_RE.match(output), 'when %s is %s, the output of "%s" is "%s"' % (LIST_TESTS_FLAG, flag_expression, ' '.join(args), output))
