...
class GTestShuffleUnitTest(gtest_test_utils.TestCase):
    ...
    def testShuffleChangesTestOrder(self):
        ...
        self.assertTrue(SHUFFLED_FILTERED_TESTS != FILTERED_TESTS, SHUFFLED_FILTERED_TESTS)
        self.assertTrue(SHUFFLED_SHARDED_TESTS != SHARDED_TESTS, SHUFFLED_SHARDED_TESTS)

    def testShuffleChangesTestCaseOrder(self):
        self.assertTrue(GetTestCases(SHUFFLED_ALL_TESTS) != GetTestCases(ALL_TESTS), GetTestCases(SHUFFLED_ALL_TESTS))
        self.assertTrue(GetTestCases(SHUFFLED_ACTIVE_TESTS) != GetTestCases(ACTIVE_TESTS), GetTestCases(SHUFFLED_ACTIVE_TESTS))
        self.assertTrue(GetTestCases(SHUFFLED_FILTERED_TESTS) != GetTestCases(FILTERED_TESTS), GetTestCases(SHUFFLED_FILTERED_TESTS))
        self.assertTrue(GetTestCases(SHUFFLED_SHARDED_TESTS) != GetTestCases(SHARDED_TESTS), GetTestCases(SHUFFLED_SHARDED_TESTS))
    ...
    def testShuffleLeavesDeathTestsAtFront(self):
        ...
        for test in SHUFFLED_ACTIVE_TESTS:
            if 'DeathTest.' in test:
                self.assertTrue(not non_death_test_found, '%s appears after a non-death test' % (test,))
            else:
    ...
    def testShuffleRestoresOrderAfterEachIteration(self):
        ...
        tests_with_seed1 = GetTestsForAllIterations({}, [ShuffleFlag(), RandomSeedFlag(1)])[0]
        self.assertEqual(tests_in_iteration1, tests_with_seed1)
        tests_with_seed2 = GetTestsForAllIterations({}, [ShuffleFlag(), RandomSeedFlag(2)])[0]
        self.assertEqual(tests_in_iteration2, tests_with_seed2)
        tests_with_seed3 = GetTestsForAllIterations({}, [ShuffleFlag(), RandomSeedFlag(3)])[0]
    ...
    def testShuffleShardedTestsPreservesPartition(self):
        tests1 = GetTestsForAllIterations({TOTAL_SHARDS_ENV_VAR: '3', SHARD_INDEX_ENV_VAR: '0'}, [ShuffleFlag(), RandomSeedFlag(1)])[0]
        tests2 = GetTestsForAllIterations({TOTAL_SHARDS_ENV_VAR: '3', SHARD_INDEX_ENV_VAR: '1'}, [ShuffleFlag(), RandomSeedFlag(20)])[0]
        tests3 = GetTestsForAllIterations({TOTAL_SHARDS_ENV_VAR: '3', SHARD_INDEX_ENV_VAR: '2'}, [ShuffleFlag(), RandomSeedFlag(25)])[0]
