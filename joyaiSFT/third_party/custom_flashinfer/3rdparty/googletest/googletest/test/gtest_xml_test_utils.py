...
class GTestXMLTestCase(gtest_test_utils.TestCase):
    ...
    *  It has an equivalent set of child nodes (including elements and
    ...
    """
        ...
        self.assertEqual(expected_attributes.length, actual_attributes.length, 'attribute numbers differ in element %s:\nExpected: %r\nActual: %r' % (actual_node.tagName, expected_attributes.keys(), actual_attributes.keys()))
        for i in range(expected_attributes.length):
            expected_attr = expected_attributes.item(i)
            actual_attr = actual_attributes.get(expected_attr.name)
            self.assertTrue(actual_attr is not None, 'expected attribute %s not found in element %s' % (expected_attr.name, actual_node.tagName))
            self.assertEqual(expected_attr.value, actual_attr.value, ' values of attribute %s in element %s differ: %s vs %s' % (expected_attr.name, actual_node.tagName, expected_attr.value, actual_attr.value))
        expected_children = self._GetChildren(expected_node)
        actual_children = self._GetChildren(actual_node)
        self.assertEqual(len(expected_children), len(actual_children), 'number of child elements differ in element ' + actual_node.tagName)
        for child_id, child in expected_children.items():
            self.assertTrue(child_id in actual_children, '<%s> is not in <%s> (in element %s)' % (child_id, actual_children, actual_node.tagName))
            self.AssertEquivalentNodes(child, actual_children[child_id])
    identifying_attribute = {'testsuites': 'name', 'testsuite': 'name', 'testcase': 'name', 'failure': 'message', 'skipped': 'message', 'property': 'name'}
        ...
        for child in element.childNodes:
            if child.nodeType == Node.ELEMENT_NODE:
                if child.tagName == 'properties':
                    self.assertTrue(child.parentNode is not None, 'Encountered <properties> element without a parent')
                    child_id = child.parentNode.getAttribute('name') + '-properties'
                else:
                    self.assertTrue(child.tagName in self.identifying_attribute, 'Encountered unknown element <%s>' % child.tagName)
    ...
    *  The stack traces are removed.
    ...
    """
        if element.tagName == 'testcase':
            ...
            if source_file:
                source_file.value = re.sub('^.*[/\\\\](.*)', '\\1', source_file.value)
        if element.tagName in ('testsuites', 'testsuite', 'testcase'):
            timestamp = element.getAttributeNode('timestamp')
            timestamp.value = re.sub('^\\d{4}-\\d\\d-\\d\\dT\\d\\d:\\d\\d:\\d\\d\\.\\d\\d\\d$', '*', timestamp.value)
        if element.tagName in ('testsuites', 'testsuite', 'testcase'):
