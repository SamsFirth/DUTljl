...
def format_walltime(seconds_in):
    ...
    return '{}{}{}{}'.format('{:0>2d}:'.format(h) if h > 1e-09 else '', '{:0>2d}:'.format(m) if h > 1e-09 or m > 1e-09 else '', '{:0>2d}.'.format(s) if h > 1e-09 or m > 1e-09 else '{:d}.'.format(s), '{:0>3d}'.format(ms))
...
def get_measures(state):
    ...
    for name in measure_names:
        ...
        summary = next(filter(lambda s: s['tag'] == measure_walltime_tag, summaries), None)
        if not summary:
            continue
