    ...
    for cmp_bench in cmp_benches:
        ...
        for device_id in device_ids:
            ...
            for cmp_state in cmp_states:
                ...
                ref_state = next(filter(lambda st: st['name'] == cmp_state_name, ref_states), None)
                if not ref_state:
                    continue
                ...
                if not all([cmp_time_summary, ref_time_summary, cmp_noise_summary, ref_noise_summary]):
                    continue
            ...
            if tabulate_version >= (0, 8, 3):
                print(tabulate.tabulate(rows, headers=headers, colalign=colalign, tablefmt='github'))
            else:
...
def main():
    ...
    parser.add_argument('--threshold-diff', type=float, dest='threshold', default=0.0, help='only show benchmarks where percentage diff is >= THRESHOLD')
    ...
    if os.path.isdir(files_or_dirs[0]) and os.path.isdir(files_or_dirs[1]):
        for f in os.listdir(files_or_dirs[1]):
            ...
            if os.path.isfile(r) and os.path.isfile(c) and (os.path.getsize(r) > 0) and (os.path.getsize(c) > 0):
