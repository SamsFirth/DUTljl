...
def SummarizeEntries(entries, extra_step_types, elapsed_time_sorting):
    ...
    for target in entries:
        ...
        weighted_time_by_ext[extension] = weighted_time_by_ext.get(extension, 0) + target.WeightedDuration()
    ...
    if elapsed_time_sorting:
        ...
        weighted_time_by_ext_sorted = sorted(((y, x) for x, y in weighted_time_by_ext.items()))
    for time, extension in weighted_time_by_ext_sorted[-long_ext_count:]:
        print('      %8.1f s weighted time to generate %d %s files (%1.1f s elapsed time sum)' % (time, count_by_ext[extension], extension, time_by_ext[extension]))
...
def main():
    ...
    parser.add_argument('-e', '--elapsed_time_sorting', default=False, action='store_true', help='Sort output by elapsed time instead of weighted time')
