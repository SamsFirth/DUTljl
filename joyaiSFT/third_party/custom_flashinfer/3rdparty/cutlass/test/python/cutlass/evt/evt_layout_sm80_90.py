...
class TestEVTLayout(EVTTestCaseBase):
    ...
    def test_permute_1(self):
        ...
        for m, n, k, l in self.get_problem_sizes(8):
            example_inputs = {'accum': self.fake_tensor(self.element, (l, m, n)), 'alpha': 0.5, 'C': self.fake_tensor(self.element, (l, m, n)), 'F': self.fake_tensor(self.element, (l, m, n)), 'D': self.fake_tensor(self.element, (l, m, n))}
    ...
    def test_permute_2(self):
        ...
        def evt_permute(accum, alpha, C):
            ...
            return (D, F)
        for m, n, k, l in self.get_problem_sizes(8):
            example_inputs = {'accum': self.fake_tensor(self.element, (l, m, n)), 'alpha': 0.5, 'C': self.fake_tensor(self.element, (l, n, m)), 'F': self.fake_tensor(self.element, (l, m, n)), 'D': self.fake_tensor(self.element, (l, n, m))}
            launcher = EVTTestBed(self.element, evt_permute, example_inputs)
            input_keys = ['C', 'alpha']
            result_keys = ['D', 'F']
            launcher.verify((m, n, k), input_keys, result_keys, l)
    ...
    def test_permute_3(self):
        ...
        def evt_permute(accum, alpha, C):
            ...
            return (D, F)
        for m, n, k, l in self.get_problem_sizes(8):
            example_inputs = {'accum': self.fake_tensor(self.element, (l, m, n)), 'alpha': 0.5, 'C': self.fake_tensor(self.element, (m, l, n)), 'F': self.fake_tensor(self.element, (l, m, n)), 'D': self.fake_tensor(self.element, (m, l, n))}
            launcher = EVTTestBed(self.element, evt_permute, example_inputs)
            input_keys = ['C', 'alpha']
            result_keys = ['D', 'F']
            launcher.verify((m, n, k), input_keys, result_keys, l)
    ...
    def test_reshape(self):
        ...
        def evt_reshape(accum, alpha, TensorE):
            ...
            D = F + E_reshape
            return D
        example_inputs = {'accum': self.fake_tensor(self.element, (self.l, self.m, self.n)), 'alpha': 0.5, 'TensorE': self.fake_tensor(self.element, (16, 32)), 'D': self.fake_tensor(self.element, (self.l, self.m, self.n))}
    ...
    def test_reshape2(self):
        ...
        def evt_reshape(accum, alpha, TensorE):
            ...
            D = F_reshape + TensorE
            return D
        example_inputs = {'accum': self.fake_tensor(self.element, (self.l, self.m, self.n)), 'alpha': 0.5, 'TensorE': self.fake_tensor(self.element, (2, 3, 1, self.n)), 'D': self.fake_tensor(self.element, (2, 3, self.m, self.n))}
        launcher = EVTTestBed(self.element, evt_reshape, example_inputs)
        input_keys = ['alpha', 'TensorE']
        result_keys = ['D']
        launcher.verify(self.problem_size, input_keys, result_keys, self.l)
