...
import subprocess
import torch
from cutlass_library import DataType, DataTypeSize, GemmUniversalMode, LayoutType, OpcodeClass, ShortDataTypeNames, SwizzlingFunctor
...
class GemmUniversalLauncher:
    ...
    def __init__(self, operation, seed=2080, verification=True, iterations=500, compiler_mode='nvcc', **kwargs) -> None:
    ...
    def uniform_init(self, shape, dtype, layout):
        ...
        if dtype.is_floating_point:
            data = torch.ceil(torch.empty(size=(size,), dtype=torch.float32, device='cuda').uniform_(self.rand_min - 0.5, self.rand_max - 0.5)).to(dtype)
        else:
    ...
    def run(self, mode, problem_size, batch_count=1, split_k_slices=1, alpha=1.0, beta=0.0):
        ...
        tensor_A, tensor_A_ref = self.uniform_init((true_batch_count, problem_size.m, problem_size.k), self.dtype_A, self.operation.A.layout if not self.operation.switched else transpose(self.operation.B.layout))
        tensor_B, tensor_B_ref = self.uniform_init((true_batch_count, problem_size.k, problem_size.n), self.dtype_B, self.operation.B.layout if not self.operation.switched else transpose(self.operation.A.layout))
        if self.dtype_C is not None:
            tensor_C, tensor_C_ref = self.uniform_init((true_batch_count, problem_size.m, problem_size.n), self.dtype_C, self.operation.C.layout if not self.operation.switched else transpose(self.operation.C.layout))
        else:
            tensor_C = None
            tensor_C_ref = None
        tensor_D, _ = self.uniform_init((true_batch_count, problem_size.m, problem_size.n), self.dtype_D, self.operation.C.layout if not self.operation.switched else transpose(self.operation.C.layout))
        ...
        if self.verification:
            if mode == GemmUniversalMode.GemmSplitKParallel:
                ...
                arguments.sync()
            tensor_D_ref = self.reference(problem_size, tensor_A_ref, tensor_B_ref, tensor_C_ref, alpha, beta)
