...
class EVTErrorTests(unittest.TestCase):
    ...
    def test_root_not_d(self):
        ...
        def evt_root_not_d(accum, alpha):
            F = accum * alpha
            return F
        example_tensors = {'accum': self.fake_tensor(np.float16, (6, 512, 512)), 'alpha': 1.2, 'F': self.fake_tensor(np.float16, (6, 512, 512))}
        with ExpectException(device_cc() == 90, "SyntaxError: Sm90 EVT requires the epilogue to have a returned tensor D, but the variable 'D' is not found in the return values.", True):
    ...
    def test_no_accum(self):
        ...
        def evt_no_accum(alpha, C):
            D = alpha * C
            return D
        example_tensors = {'C': self.fake_tensor(np.float16, (6, 512, 512)), 'alpha': 1.2, 'D': self.fake_tensor(np.float16, (6, 512, 512))}
    ...
    def test_too_much_shared_memory(self):
        ...
        with ExpectException(True, 'RuntimeError: The epilogue consumes too much shared memory. No valid tile description is found in the generator.', True):
    ...
    def test_not_ssa(self):
        ...
        def evt_redefine(accum, C, alpha):
            ...
            D = F
            return (D, F)
        example_tensors = {'accum': self.fake_tensor(np.float16, (6, 512, 512)), 'C': self.fake_tensor(np.float16, (6, 512, 512)), 'alpha': 1.5, 'D': self.fake_tensor(np.float16, (6, 512, 512)), 'F': self.fake_tensor(np.float16, (6, 512, 512))}
        ...
        def evt_undefine(accum, alpha):
            ...
            D = F * alpha
            return (D, F)
        example_tensors = {'accum': self.fake_tensor(np.float16, (6, 512, 512)), 'alpha': 1.5, 'D': self.fake_tensor(np.float16, (6, 512, 512)), 'F': self.fake_tensor(np.float16, (6, 512, 512))}
    ...
    def test_missing_example_tensor(self):
        ...
        def evt_missing_example_tensor(accum, C):
            D = accum + C
            return D
        example_tensors = {'accum': self.fake_tensor(np.float16, (6, 512, 512)), 'C': self.fake_tensor(np.float16, (6, 512, 512))}
        ...
        example_tensors = {'accum': self.fake_tensor(np.float16, (6, 512, 512)), 'D': self.fake_tensor(np.float16, (6, 512, 512))}
    ...
    def test_incompatible_shape(self):
        ...
        def evt_incompatible_shape(accum, C):
            D = accum + C
            return D
        example_tensors = {'accum': self.fake_tensor(np.float16, (6, 256, 512)), 'C': self.fake_tensor(np.float16, (6, 512, 512)), 'D': self.fake_tensor(np.float16, (6, 512, 512))}
        with ExpectException(True, 'RuntimeError: Dimension mismatch between accum(6, 256, 512), C(6, 512, 512).', True):
    ...
    def test_no_matching_impl(self):
        ...
        def evt_no_matching_impl(accum, bias):
            ...
            return D
        example_tensors = {'accum': self.fake_tensor(np.float16, (6, 512, 256)), 'bias': self.fake_tensor(np.float16, (16, 32)), 'D': self.fake_tensor(np.float16, (6, 512, 256))}
