...
import torch
import cutlass
from cutlass_library import ConvKind, ConvMode, DataType, DataTypeNames, EpilogueScheduleSuffixes, KernelScheduleSuffixes, LayoutType, OpcodeClassNames, ShortDataTypeNames, ShortLayoutTypeNames, SplitKMode
...
def get_name_conv2d(arch, conv_kind, element, element_accumulator, element_output, opclass, threadblock_shape, warp_count, instruction_shape, stages, iterator_algorithm, swizzle, split_k_mode, split_k_slices, activation):
    """
    ...
    return SubstituteTemplate(name_format, {'arch': str(arch), 'conv_kind': conv_kind, 'iter_alg': iterator_algorithm, 'eA': DataTypeNames[element], 'eB': DataTypeNames[element], 'eC': DataTypeNames[element_output], 'opclass': opclass, 'acc': DataTypeNames[element_accumulator], 'tbM': str(threadblock_shape[0]), 'tbN': str(threadblock_shape[1]), 'tbK': str(threadblock_shape[2]), 'wM': str(threadblock_shape[0] // warp_count[0]), 'wN': str(threadblock_shape[1] // warp_count[1]), 'wK': str(threadblock_shape[2] // warp_count[2]), 'IM': str(instruction_shape[0]), 'IN': str(instruction_shape[1]), 'IK': str(instruction_shape[2]), 'stages': str(stages), 'swizzle': str(swizzle), 'split_k_mode': split_k_mode, 'split_k_slices': str(split_k_slices), 'activation': activation})

def conv2d_few_channel_problemsizes(channels):
    problem_sizes = [Conv2DProblemSize(1, 8, 8, channels, 16, 3, 3, channels, 1, 1, 2, 2, 1, 1, ConvMode.CrossCorrelation, 1, 1), Conv2DProblemSize(1, 16, 16, channels, 16, 3, 3, channels, 1, 1, 2, 2, 1, 1, ConvMode.CrossCorrelation, 1, 1), Conv2DProblemSize(1, 16, 16, channels, 16, 7, 7, channels, 1, 1, 1, 1, 1, 1, ConvMode.CrossCorrelation, 1, 1), Conv2DProblemSize(1, 224, 224, channels, 32, 7, 7, channels, 1, 1, 1, 1, 1, 1, ConvMode.CrossCorrelation, 1, 1), Conv2DProblemSize(1, 224, 224, channels, 64, 7, 7, channels, 1, 1, 2, 2, 1, 1, ConvMode.CrossCorrelation, 1, 1), Conv2DProblemSize(1, 224, 224, channels, 64, 5, 5, channels, 1, 1, 1, 1, 1, 1, ConvMode.CrossCorrelation, 1, 1), Conv2DProblemSize(1, 224, 224, channels, 64, 5, 5, channels, 1, 1, 2, 2, 1, 1, ConvMode.CrossCorrelation, 1, 1)]
...
class Conv2dLauncherFrontend:
    ...
    def uniform_init(self, size, dtype):
        tensor = torch.ceil(torch.empty(size=size, dtype=torch_type(dtype), device='cuda').uniform_(-self.rand_max - 0.5, self.rand_max - 0.5)).to(memory_format=torch.channels_last)
        return tensor

    def reference(self, ps, A, B, C, alpha, beta, activation):
        if self.conv_kind == ConvKind.Fprop:
            torch_result = alpha * torch.ops.aten.conv2d(A, B, stride=(ps.stride_h, ps.stride_w), padding=(ps.pad_h, ps.pad_w), dilation=(ps.dilation_h, ps.dilation_w)) + beta * C
        elif self.conv_kind == ConvKind.Dgrad:
            torch_result = alpha * torch.nn.grad.conv2d_input((ps.N, ps.C, ps.H, ps.W), B, A, padding=(ps.pad_h, ps.pad_w), stride=(ps.stride_h, ps.stride_w)) + beta * C
        elif self.conv_kind == ConvKind.Wgrad:
            torch_result = alpha * torch.nn.grad.conv2d_weight(B, (ps.K, ps.C, ps.R, ps.S), A, padding=(ps.pad_h, ps.pad_w), stride=(ps.stride_h, ps.stride_w)) + beta * C
        else:
    ...
    def run(self, ps, split_k_mode=SplitKMode.Serial, split_k_slices=1, alpha=1.0, beta=0.0):
        ...
        args = self.operation.run(tensor_A, tensor_B, tensor_C, tensor_D, stride=(ps.stride_h, ps.stride_w), padding=(ps.pad_h, ps.pad_w), dilation=(ps.dilation_h, ps.dilation_w), alpha=alpha, beta=beta, split_k=(split_k_mode, split_k_slices))
        args.sync()
...
def add_test(cls, cc, conv_kind, problem_sizes, element, element_accumulator, element_output, opclass, threadblock_shape, warp_count, instruction_shape, stages, iterator_algorithm=None, swizzle=None, split_k_mode='serial', split_k_slices=1, activation='identity'):
    """Create a test-running function with the given specification"""
    test_name = get_name_conv2d(cc, conv_kind, element, element_accumulator, element_output, opclass, threadblock_shape, warp_count, instruction_shape, stages, iterator_algorithm, swizzle, split_k_mode, split_k_slices, activation)

    def run(self):
        plan = cutlass.Conv2d(kind=conv_kind, element=element, element_accumulator=element_accumulator, element_C=element_output, element_D=element_output)
        plan.opclass = opclass
        td = {'threadblock_shape': threadblock_shape, 'warp_count': warp_count, 'stages': stages, 'instruction_shape': instruction_shape}
...
def get_conv_problems():
    ...
    conv_problems += [Conv2DProblemSize(1, 4, 4, 12, 8, 3, 3, 12, 0, 0, 3, 3, 1, 1, ConvMode.CrossCorrelation, 1, 1), Conv2DProblemSize(1, 4, 4, 14, 8, 3, 3, 14, 0, 0, 3, 3, 1, 1, ConvMode.CrossCorrelation, 1, 1), Conv2DProblemSize(1, 23, 56, 98, 128, 3, 3, 98, 4, 5, 3, 3, 1, 1, ConvMode.CrossCorrelation, 1, 1)]
