...
class gen_default_b2b_mma:
    ...
    def gen_using_MmaCore(self, stage):
        ...
        for i in range(self.b2b_num):
            ...
            gen_code += code_using + ' = ' + gen_ir.gen_declare_template_struct(Mma_typename, helper.var_idx(threadBlockShape, i), helper.var_idx(warpShape, i), instrunctionShape, 'ElementA', 'LayoutA', helper.var_idx('ElementB', i), helper.var_idx('LayoutB', i), helper.var_idx('ElementAccumulator', i), 'layout::RowMajor', 'OperatorClass', str(stage), 'Operator')
        return gen_code
    ...
    def gen_using_Iterator(self):
        ...
        gen_code = code_using + ' = ' + gen_ir.gen_declare_template_struct(iterator_typename, matrix_shape, 'ElementA', 'LayoutA', '1', iterator_map, 'AlignmentA_')
        for i in range(self.b2b_num):
            ...
            gen_code += code_using + ' = ' + gen_ir.gen_declare_template_struct(iterator_typename, matrix_shape, helper.var_idx('ElementB', i), helper.var_idx('LayoutB', i), '0', iterator_map, 'AlignmentB_')
        return gen_code
    ...
    def gen_fragment_iterator(self):
        ...
        for i in range(1, self.b2b_num):
            ...
            gen_code += code_using + ' = ' + gen_ir.gen_declare_template_struct(iterator_typename, Matrix_shape_curr, Matrix_shape_prev, Curr_shape_kK, helper.var_idx('ElementAccumulator', i - 1), 'ElementA', 'AccumulatorLayout', 'InstructionShape_', 'true')
        return gen_code
...
class gen_b2b_mma_base:
    ...
    def gen_using_and_misc(self, b2b_num):
        ...
        for i in range(b2b_num):
            code_using += 'using WarpCount' + str(i) + ' = GemmShape<' + helper.var_idx('Shape', i) + '::kM / ' + helper.var_idx('WarpGemm', i) + '::kM, ' + helper.var_idx('Shape', i) + '::kN / ' + helper.var_idx('WarpGemm', i) + '::kN, ' + helper.var_idx('Shape', i) + '::kK / ' + helper.var_idx('WarpGemm', i) + '::kK>;\n'
        code_misc = ''
        for i in range(b2b_num):
