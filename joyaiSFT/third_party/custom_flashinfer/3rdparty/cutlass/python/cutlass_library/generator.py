...
def CreateSparseGemmUniversal3xOperator(manifest, layouts, tile_descriptions, data_types, schedules=[[KernelScheduleType.ScheduleAuto, EpilogueScheduleType.ScheduleAuto]], complex_transforms=None, epilogue_functor=EpilogueFunctor.LinearCombination, swizzling_functor=SwizzlingFunctor.Identity1, tile_schedulers=[TileSchedulerType.Persistent]):
    ...
    for layout, tile_description, data_type, complex_transform, schedules, tile_scheduler in combinations:
        ...
        D = TensorDescription(data_type['d_type'], layout[2][0], layout[2][1])
        element_compute = data_type.get('epi_type', data_type['acc_type'])
        operation = GemmOperation(GemmKind.SparseUniversal3x, tile_description.minimum_compute_capability, tile_description, A, B, C, element_compute, epilogue_functor, swizzling_functor, D, kernel_schedule, epilogue_schedule, tile_scheduler)
        manifest.append(operation)
        operations.append(operation)
    return operations
        ...
        for alignment in alignment_constraints:
            ...
            if ConvKind.Fprop in conv_kinds:
                for iterator_algorithm in iterator_algorithms:
                    new_operations = [Conv2dOperation(ConvKind.Fprop, iterator_algorithm, tile.minimum_compute_capability, tile, A, B, C, element_epilogue, StrideSupport.Unity, epilogue_functor, swizzling_functor_)]
                    if tile.math_instruction.opcode_class == OpcodeClass.TensorOp and A.layout == LayoutType.TensorNHWC and (tile.minimum_compute_capability >= 80):
                        new_operations.append(Conv2dOperation(ConvKind.Fprop, iterator_algorithm, tile.minimum_compute_capability, tile, A, B, C, element_epilogue, StrideSupport.Unity, epilogue_functor, swizzling_functor_, group_mode=GroupMode.SingleGroup))
                        if iterator_algorithm == IteratorAlgorithm.Analytic:
                            new_operations.append(Conv2dOperation(ConvKind.Fprop, iterator_algorithm, tile.minimum_compute_capability, tile, A, B, C, element_epilogue, StrideSupport.Unity, epilogue_functor, swizzling_functor_, group_mode=GroupMode.MultipleGroup))
...
def GenerateSM80_TensorOp_16832_TN_mixed_input_upcast_a(manifest, cuda_version):
    if not CudaToolkitVersionSatisfies(cuda_version, 11, 0):
        return
    layouts = [(LayoutType.RowMajor, LayoutType.ColumnMajor, LayoutType.ColumnMajor)]
    math_instructions = [MathInstruction([16, 8, 32], DataType.s4, DataType.s8, DataType.s32, OpcodeClass.TensorOp, MathOperation.multiply_add_mixed_input_upcast)]
    min_cc = 80
    max_cc = 1024
    alignment_constraints = [[32, 16, 4]]
    for math_inst in math_instructions:
        tile_descriptions = [TileDescription([256, 128, 64], 3, [4, 2, 1], math_inst, min_cc, max_cc), TileDescription([128, 256, 64], 3, [2, 4, 1], math_inst, min_cc, max_cc), TileDescription([256, 64, 64], 4, [4, 1, 1], math_inst, min_cc, max_cc), TileDescription([64, 256, 64], 4, [1, 4, 1], math_inst, min_cc, max_cc), TileDescription([32, 256, 64], 4, [1, 4, 1], math_inst, min_cc, max_cc), TileDescription([128, 128, 64], 5, [2, 2, 1], math_inst, min_cc, max_cc), TileDescription([64, 128, 64], 6, [2, 2, 1], math_inst, min_cc, max_cc), TileDescription([256, 128, 128], 3, [4, 2, 1], math_inst, min_cc, max_cc), TileDescription([128, 256, 128], 3, [2, 4, 1], math_inst, min_cc, max_cc), TileDescription([256, 64, 128], 4, [4, 1, 1], math_inst, min_cc, max_cc), TileDescription([64, 256, 128], 4, [1, 4, 1], math_inst, min_cc, max_cc), TileDescription([256, 32, 128], 4, [4, 1, 1], math_inst, min_cc, max_cc), TileDescription([32, 256, 128], 4, [1, 4, 1], math_inst, min_cc, max_cc), TileDescription([128, 128, 128], 4, [2, 2, 1], math_inst, min_cc, max_cc), TileDescription([64, 128, 128], 3, [2, 2, 1], math_inst, min_cc, max_cc), TileDescription([128, 32, 128], 4, [4, 1, 1], math_inst, min_cc, max_cc)]
        data_type = [math_inst.element_a, math_inst.element_b, math_inst.element_accumulator, math_inst.element_accumulator]
        operations = CreateGemmOperator(manifest, layouts, tile_descriptions, data_type, alignment_constraints, None, EpilogueFunctor.LinearCombination, SwizzlingFunctor.Identity8)
        if math_inst.element_a != math_inst.element_accumulator:
            alignment_constraints = [[32, 16, 16]]
            data_type_mixed = [math_inst.element_a, math_inst.element_b, math_inst.element_b, DataType.f32]
            operations += CreateGemmOperator(manifest, layouts, tile_descriptions, data_type_mixed, alignment_constraints, None, EpilogueFunctor.LinearCombinationClamp, SwizzlingFunctor.Identity8)
        for op in operations:
            if op.tile_description.threadblock_shape[1] >= 128:
                if op.tile_description.threadblock_shape[0] == 32:
                    op.C.alignment = 8
...
def GenerateSM80_TensorOp_16832_TN_mixed_input_upcast_b(manifest, cuda_version):
    if not CudaToolkitVersionSatisfies(cuda_version, 11, 0):
        return
    layouts = [(LayoutType.RowMajor, LayoutType.ColumnMajor, LayoutType.ColumnMajor)]
    math_instructions = [MathInstruction([16, 8, 32], DataType.s8, DataType.s4, DataType.s32, OpcodeClass.TensorOp, MathOperation.multiply_add_mixed_input_upcast)]
    min_cc = 80
    max_cc = 1024
    alignment_constraints = [[16, 32, 4]]
    for math_inst in math_instructions:
        tile_descriptions = [TileDescription([256, 128, 64], 3, [4, 2, 1], math_inst, min_cc, max_cc), TileDescription([128, 256, 64], 3, [2, 4, 1], math_inst, min_cc, max_cc), TileDescription([256, 64, 64], 4, [4, 1, 1], math_inst, min_cc, max_cc), TileDescription([64, 256, 64], 4, [1, 4, 1], math_inst, min_cc, max_cc), TileDescription([256, 32, 64], 4, [4, 1, 1], math_inst, min_cc, max_cc), TileDescription([128, 128, 64], 5, [2, 2, 1], math_inst, min_cc, max_cc), TileDescription([64, 128, 64], 6, [2, 2, 1], math_inst, min_cc, max_cc), TileDescription([128, 32, 64], 6, [4, 1, 1], math_inst, min_cc, max_cc), TileDescription([256, 128, 128], 3, [4, 2, 1], math_inst, min_cc, max_cc), TileDescription([128, 256, 128], 3, [2, 4, 1], math_inst, min_cc, max_cc), TileDescription([256, 64, 128], 4, [4, 1, 1], math_inst, min_cc, max_cc), TileDescription([64, 256, 128], 4, [1, 4, 1], math_inst, min_cc, max_cc), TileDescription([256, 32, 128], 4, [4, 1, 1], math_inst, min_cc, max_cc), TileDescription([32, 256, 128], 4, [1, 4, 1], math_inst, min_cc, max_cc), TileDescription([128, 128, 128], 4, [2, 2, 1], math_inst, min_cc, max_cc), TileDescription([64, 128, 128], 3, [2, 2, 1], math_inst, min_cc, max_cc), TileDescription([128, 32, 128], 4, [4, 1, 1], math_inst, min_cc, max_cc)]
        data_type = [math_inst.element_a, math_inst.element_b, math_inst.element_accumulator, math_inst.element_accumulator]
        operations = CreateGemmOperator(manifest, layouts, tile_descriptions, data_type, alignment_constraints, None, EpilogueFunctor.LinearCombination, SwizzlingFunctor.Identity8)
        if math_inst.element_a != math_inst.element_accumulator:
...
def GenerateSM89_TensorOp_16832_fp8(manifest, cuda_version):
    if not CudaToolkitVersionSatisfies(cuda_version, 12, 4):
        return
    layouts = [(LayoutType.RowMajor, LayoutType.ColumnMajor, LayoutType.ColumnMajor), (LayoutType.RowMajor, LayoutType.ColumnMajor, LayoutType.RowMajor)]
    ...
    alignment_constraints_small_channels = [16, 8, 4]
    for math_inst in math_instructions:
        tile_descriptions = [TileDescription([256, 128, 128], 3, [4, 2, 1], math_inst, min_cc, max_cc), TileDescription([256, 128, 64], 3, [4, 2, 1], math_inst, min_cc, max_cc), TileDescription([256, 128, 64], 6, [4, 2, 1], math_inst, min_cc, max_cc), TileDescription([128, 256, 128], 3, [2, 4, 1], math_inst, min_cc, max_cc), TileDescription([128, 256, 64], 3, [2, 4, 1], math_inst, min_cc, max_cc), TileDescription([128, 256, 64], 6, [2, 4, 1], math_inst, min_cc, max_cc), TileDescription([256, 64, 128], 4, [4, 1, 1], math_inst, min_cc, max_cc), TileDescription([256, 64, 64], 3, [4, 1, 1], math_inst, min_cc, max_cc), TileDescription([256, 64, 64], 4, [4, 1, 1], math_inst, min_cc, max_cc), TileDescription([64, 256, 128], 4, [1, 4, 1], math_inst, min_cc, max_cc), TileDescription([64, 256, 64], 3, [1, 4, 1], math_inst, min_cc, max_cc), TileDescription([64, 256, 64], 4, [1, 4, 1], math_inst, min_cc, max_cc), TileDescription([256, 32, 128], 4, [4, 1, 1], math_inst, min_cc, max_cc), TileDescription([256, 32, 64], 4, [4, 1, 1], math_inst, min_cc, max_cc), TileDescription([32, 256, 128], 4, [1, 4, 1], math_inst, min_cc, max_cc), TileDescription([32, 256, 64], 4, [1, 4, 1], math_inst, min_cc, max_cc), TileDescription([128, 128, 128], 3, [2, 2, 1], math_inst, min_cc, max_cc), TileDescription([128, 128, 128], 4, [2, 2, 1], math_inst, min_cc, max_cc), TileDescription([128, 128, 128], 5, [2, 2, 1], math_inst, min_cc, max_cc), TileDescription([128, 128, 64], 3, [2, 2, 1], math_inst, min_cc, max_cc), TileDescription([128, 128, 64], 4, [2, 2, 1], math_inst, min_cc, max_cc), TileDescription([128, 128, 64], 5, [2, 2, 1], math_inst, min_cc, max_cc), TileDescription([128, 128, 64], 6, [2, 2, 1], math_inst, min_cc, max_cc), TileDescription([128, 64, 128], 3, [2, 2, 1], math_inst, min_cc, max_cc), TileDescription([128, 64, 128], 4, [2, 2, 1], math_inst, min_cc, max_cc), TileDescription([64, 128, 128], 3, [2, 2, 1], math_inst, min_cc, max_cc), TileDescription([64, 128, 128], 4, [2, 2, 1], math_inst, min_cc, max_cc), TileDescription([128, 64, 64], 3, [2, 2, 1], math_inst, min_cc, max_cc), TileDescription([128, 64, 64], 4, [2, 2, 1], math_inst, min_cc, max_cc), TileDescription([128, 64, 64], 5, [2, 2, 1], math_inst, min_cc, max_cc), TileDescription([128, 64, 64], 6, [2, 2, 1], math_inst, min_cc, max_cc), TileDescription([64, 128, 64], 3, [2, 2, 1], math_inst, min_cc, max_cc), TileDescription([64, 128, 64], 4, [2, 2, 1], math_inst, min_cc, max_cc), TileDescription([64, 128, 64], 5, [2, 2, 1], math_inst, min_cc, max_cc), TileDescription([64, 128, 64], 6, [2, 2, 1], math_inst, min_cc, max_cc), TileDescription([128, 32, 128], 4, [4, 1, 1], math_inst, min_cc, max_cc), TileDescription([128, 32, 64], 6, [4, 1, 1], math_inst, min_cc, max_cc), TileDescription([32, 128, 128], 4, [1, 4, 1], math_inst, min_cc, max_cc), TileDescription([32, 128, 64], 6, [1, 4, 1], math_inst, min_cc, max_cc), TileDescription([64, 64, 128], 5, [2, 2, 1], math_inst, min_cc, max_cc), TileDescription([64, 64, 128], 6, [2, 2, 1], math_inst, min_cc, max_cc), TileDescription([64, 64, 64], 6, [2, 2, 1], math_inst, min_cc, max_cc), TileDescription([64, 64, 64], 10, [2, 2, 1], math_inst, min_cc, max_cc)]
        data_types = [[math_inst.element_a, math_inst.element_b, DataType.f32, math_inst.element_accumulator], [math_inst.element_a, math_inst.element_b, DataType.bf16, math_inst.element_accumulator]]
        operations = []
...
try:
    from .sm90_utils import generate_fp16_bf16_math_instructions_sm90, generate_tf32_math_instructions_sm90, generate_int8_math_instructions_sm90, generate_fp8_math_instructions_sm90, make_sparse_math_instructions, generate_tile_descriptions_sm90, get_valid_schedules, generate_data_types_from_math_instruction, fix_alignments
except ImportError:
    from sm90_utils import generate_fp16_bf16_math_instructions_sm90, generate_tf32_math_instructions_sm90, generate_int8_math_instructions_sm90, generate_fp8_math_instructions_sm90, make_sparse_math_instructions, generate_tile_descriptions_sm90, get_valid_schedules, generate_data_types_from_math_instruction, fix_alignments
...
def GenerateSM90_TensorOp_16b_WGMMA_gemm(manifest, cuda_version):
    ...
    tile_descriptions = generate_tile_descriptions_sm90(math_instructions=math_instructions, is_aligned=is_aligned, level=instantiation_level)
    for tile_desc in tile_descriptions:
        ...
        if math_inst.element_a != math_inst.element_accumulator:
            data_type_mixed_w_source = generate_data_types_from_math_instruction(math_inst, element_source=math_inst.element_a, element_dest=math_inst.element_a)
            data_type_mixed_wo_source = generate_data_types_from_math_instruction(math_inst, element_source=DataType.void, element_dest=math_inst.element_a)
        ...
        for layout in layouts:
            for data_type in data_types:
                ...
                schedules, stream_k_schedules = get_valid_schedules(tile_description=tile_desc, cuda_version=cuda_version, is_aligned=is_aligned, data_types=data_type, instantiation_level=instantiation_level, layout=layout)
                if len(schedules):
                    ...
                    if len(stream_k_schedules):
                        ...
                        CreateGemmUniversal3xOperator(manifest, [layout], [tile_desc], data_type, stream_k_schedules, tile_schedulers=[TileSchedulerType.StreamK])
...
def GenerateSM90_SparseTensorOp_16b_WGMMA_gemm(manifest, cuda_version):
    ...
    instantiation_level = manifest.get_sm90_instantiation_level(pruned_level=100, default_level=131, exhaustive_level=9999)
    is_aligned = True
    layouts = [[[LayoutType.ColumnMajor, 8], [LayoutType.ColumnMajor, 8], [LayoutType.ColumnMajor, 1]], [[LayoutType.ColumnMajor, 8], [LayoutType.RowMajor, 8], [LayoutType.ColumnMajor, 1]], [[LayoutType.RowMajor, 16], [LayoutType.ColumnMajor, 8], [LayoutType.ColumnMajor, 1]], [[LayoutType.RowMajor, 16], [LayoutType.RowMajor, 8], [LayoutType.ColumnMajor, 1]]]
        ...
        for layout in layouts:
            for data_type in data_types:
                ...
                if len(schedules):
                    ...
                    if len(stream_k_schedules):
                        assert CudaToolkitVersionSatisfies(cuda_version, 12, 1)
                        CreateSparseGemmUniversal3xOperator(manifest, [layout], [tile_desc], data_type, stream_k_schedules, tile_schedulers=[TileSchedulerType.StreamK])
...
def GenerateSM90_TensorOp_tf32_WGMMA_gemm(manifest, cuda_version):
    ...
    is_aligned = True
    layouts = [[[LayoutType.RowMajor, 4], [LayoutType.ColumnMajor, 4], [LayoutType.ColumnMajor, 4]], [[LayoutType.RowMajor, 4], [LayoutType.RowMajor, 4], [LayoutType.ColumnMajor, 4]], [[LayoutType.ColumnMajor, 4], [LayoutType.ColumnMajor, 4], [LayoutType.ColumnMajor, 4]], [[LayoutType.ColumnMajor, 4], [LayoutType.RowMajor, 4], [LayoutType.ColumnMajor, 4]]]
...
def GenerateSM90_SparseTensorOp_tf32_WGMMA_gemm(manifest, cuda_version):
    ...
    instantiation_level = manifest.get_sm90_instantiation_level(pruned_level=120, default_level=121, exhaustive_level=9999)
    is_aligned = True
    layouts = [[[LayoutType.RowMajor, 8], [LayoutType.ColumnMajor, 4], [LayoutType.ColumnMajor, 4]]]
...
def GenerateSM90_TensorOp_int8_WGMMA_gemm(manifest, cuda_version):
    ...
    for tile_desc in tile_descriptions:
        ...
        data_type_w_source = generate_data_types_from_math_instruction(math_inst)
        data_type_wo_source = generate_data_types_from_math_instruction(math_inst, element_source=DataType.void)
        data_type_int8_output = generate_data_types_from_math_instruction(math_inst, element_source=DataType.s8, element_dest=math_inst.element_a, element_epilogue=DataType.f32)
...
def GenerateSM90_SparseTensorOp_int8_WGMMA_gemm(manifest, cuda_version):
    ...
    instantiation_level = manifest.get_sm90_instantiation_level(pruned_level=100, default_level=111, exhaustive_level=9999)
    is_aligned = True
    layouts = [[[LayoutType.RowMajor, 32], [LayoutType.ColumnMajor, 16], [LayoutType.ColumnMajor, 16]]]
...
def GenerateSM90_TensorOp_fp8_WGMMA_gemm(manifest, cuda_version):
    ...
    for tile_desc in tile_descriptions:
        ...
        for c_type, d_type in product(valid_types_for_c, valid_types_for_d):
            data_types.append(generate_data_types_from_math_instruction(math_inst, element_source=c_type, element_dest=d_type))
        else:
            for d_type in valid_types_for_d:
                data_types.append(generate_data_types_from_math_instruction(math_inst, element_source=DataType.void, element_dest=d_type))
        for layout in layouts:
            for data_type in data_types:
                schedules, stream_k_schedules = get_valid_schedules(tile_description=tile_desc, cuda_version=cuda_version, is_aligned=is_aligned, data_types=data_type, instantiation_level=instantiation_level, layout=layout)
                if len(schedules):
...
def GenerateSM90_SparseTensorOp_fp8_WGMMA_gemm(manifest, cuda_version):
    ...
    instantiation_level = manifest.get_sm90_instantiation_level(pruned_level=20, default_level=121, exhaustive_level=9999)
    is_aligned = True
    layouts = [[[LayoutType.RowMajor, 32], [LayoutType.ColumnMajor, 16], [LayoutType.ColumnMajor, 1]]]
...
def GenerateSM90_Conv3x(manifest, cuda_version, log_indent_level: int=0):
  ...
  """
    ...
    tma_byte_alignments = {'A': byte_alignment_required_by_tma, 'B': byte_alignment_required_by_tma, 'C': byte_alignment_required_by_tma}
    all_byte_alignments = (tma_byte_alignments,)
    mma_64x64x16 = (64, 64, 16)
    mma_64x64x8 = (64, 64, 8)
    num_mma_per_tile = 4
    cluster_shapes = ((2, 1, 1), (1, 2, 1))
    fp16 = DataType.f16
    bf16 = DataType.bf16
    fp32 = DataType.f32
    s8 = DataType.s8
    s32 = DataType.s32
    fp16_fp32_fp16_fp32 = {'a_type': fp16, 'b_type': fp16, 'c_type': fp32, 'd_type': fp32, 'acc_type': fp16, 'epi_type': fp32}
    fp16_fp32_fp32_fp32 = {'a_type': fp16, 'b_type': fp16, 'c_type': fp32, 'd_type': fp32, 'acc_type': fp32, 'epi_type': fp32}
    fp32_fp32_fp32_fp32 = {'a_type': fp32, 'b_type': fp32, 'c_type': fp32, 'd_type': fp32, 'acc_type': fp32, 'epi_type': fp32}
    s8_s32_s32_s32 = {'a_type': s8, 'b_type': s8, 'c_type': s32, 'd_type': s32, 'acc_type': s32, 'epi_type': s32}
    bf16bf16_bf16f32_f32 = {'a_type': bf16, 'b_type': bf16, 'c_type': fp32, 'd_type': fp32, 'acc_type': fp32, 'epi_type': fp32}
    f16f16_f16f16_f16 = {'a_type': fp16, 'b_type': fp16, 'c_type': fp16, 'd_type': fp16, 'acc_type': fp16, 'epi_type': fp16}
    f16f16_f16f32_f32 = {'a_type': fp16, 'b_type': fp16, 'c_type': fp16, 'd_type': fp16, 'acc_type': fp32, 'epi_type': fp32}
    f32f32_tf32f32_f32 = fp32_fp32_fp32_fp32
    i8i8_i8i32_f32 = {'a_type': s8, 'b_type': s8, 'c_type': s32, 'd_type': s32, 'acc_type': s32, 'epi_type': s32}
    combinations_of_parameters = chain(product((ConvKind.Fprop,), spatial_dims, (fp16_fp32_fp16_fp32, fp16_fp32_fp32_fp32, s8_s32_s32_s32), all_byte_alignments, (mma_64x64x16,), cluster_shapes), product((ConvKind.Fprop,), spatial_dims, (fp32_fp32_fp32_fp32,), all_byte_alignments, (mma_64x64x8,), cluster_shapes), product((ConvKind.Dgrad,), spatial_dims, (fp16_fp32_fp16_fp32, fp16_fp32_fp32_fp32), all_byte_alignments, (mma_64x64x16,), cluster_shapes), ((ConvKind.Fprop, 2, bf16bf16_bf16f32_f32, tma_byte_alignments, (128, 256, 8), (2, 1, 1)), (ConvKind.Fprop, 2, bf16bf16_bf16f32_f32, tma_byte_alignments, (128, 256, 16), (2, 1, 1)), (ConvKind.Fprop, 2, bf16bf16_bf16f32_f32, tma_byte_alignments, (256, 128, 8), (2, 1, 1)), (ConvKind.Fprop, 2, bf16bf16_bf16f32_f32, tma_byte_alignments, (256, 128, 16), (2, 1, 1)), (ConvKind.Fprop, 2, f16f16_f16f16_f16, tma_byte_alignments, (64, 64, 8), (1, 1, 1)), (ConvKind.Fprop, 2, f16f16_f16f16_f16, tma_byte_alignments, (64, 64, 16), (1, 1, 1)), (ConvKind.Fprop, 2, f16f16_f16f16_f16, tma_byte_alignments, (64, 128, 8), (1, 1, 1)), (ConvKind.Fprop, 2, f16f16_f16f16_f16, tma_byte_alignments, (64, 128, 16), (1, 1, 1)), (ConvKind.Fprop, 2, f16f16_f16f16_f16, tma_byte_alignments, (64, 256, 8), (1, 1, 1)), (ConvKind.Fprop, 2, f16f16_f16f16_f16, tma_byte_alignments, (64, 256, 16), (1, 1, 1)), (ConvKind.Fprop, 2, f16f16_f16f16_f16, tma_byte_alignments, (128, 128, 8), (1, 1, 1)), (ConvKind.Fprop, 2, f16f16_f16f16_f16, tma_byte_alignments, (128, 128, 16), (1, 1, 1)), (ConvKind.Fprop, 2, f16f16_f16f16_f16, tma_byte_alignments, (128, 256, 8), (1, 1, 1)), (ConvKind.Fprop, 2, f16f16_f16f16_f16, tma_byte_alignments, (128, 256, 16), (1, 1, 1)), (ConvKind.Fprop, 2, f16f16_f16f16_f16, tma_byte_alignments, (256, 64, 8), (1, 1, 1)), (ConvKind.Fprop, 2, f16f16_f16f16_f16, tma_byte_alignments, (256, 64, 16), (1, 1, 1)), (ConvKind.Fprop, 2, f16f16_f16f16_f16, tma_byte_alignments, (256, 128, 8), (1, 1, 1)), (ConvKind.Fprop, 2, f16f16_f16f16_f16, tma_byte_alignments, (256, 128, 16), (1, 1, 1)), (ConvKind.Fprop, 2, f16f16_f16f32_f32, tma_byte_alignments, (128, 192, 8), (2, 1, 1)), (ConvKind.Fprop, 2, f16f16_f16f32_f32, tma_byte_alignments, (128, 192, 16), (2, 1, 1)), (ConvKind.Fprop, 2, f16f16_f16f32_f32, tma_byte_alignments, (128, 256, 8), (2, 1, 1)), (ConvKind.Fprop, 2, f16f16_f16f32_f32, tma_byte_alignments, (128, 256, 16), (2, 1, 1)), (ConvKind.Fprop, 2, f16f16_f16f32_f32, tma_byte_alignments, (256, 96, 8), (2, 1, 1)), (ConvKind.Fprop, 2, f16f16_f16f32_f32, tma_byte_alignments, (256, 96, 16), (2, 1, 1)), (ConvKind.Fprop, 2, f16f16_f16f32_f32, tma_byte_alignments, (256, 128, 8), (2, 1, 1)), (ConvKind.Fprop, 2, f16f16_f16f32_f32, tma_byte_alignments, (256, 128, 16), (2, 1, 1)), (ConvKind.Fprop, 2, f32f32_tf32f32_f32, tma_byte_alignments, (128, 192, 8), (2, 1, 1)), (ConvKind.Fprop, 2, f32f32_tf32f32_f32, tma_byte_alignments, (128, 256, 8), (2, 1, 1)), (ConvKind.Fprop, 2, f32f32_tf32f32_f32, tma_byte_alignments, (256, 128, 8), (2, 1, 1)), (ConvKind.Fprop, 2, f32f32_tf32f32_f32, tma_byte_alignments, (256, 96, 8), (2, 1, 1)), (ConvKind.Fprop, 2, i8i8_i8i32_f32, tma_byte_alignments, (128, 256, 16), (2, 1, 1)), (ConvKind.Fprop, 2, i8i8_i8i32_f32, tma_byte_alignments, (128, 256, 32), (2, 1, 1)), (ConvKind.Fprop, 2, i8i8_i8i32_f32, tma_byte_alignments, (256, 128, 16), (2, 1, 1)), (ConvKind.Fprop, 2, i8i8_i8i32_f32, tma_byte_alignments, (256, 128, 32), (2, 1, 1)), (ConvKind.Dgrad, 2, bf16bf16_bf16f32_f32, tma_byte_alignments, (128, 256, 8), (2, 1, 1)), (ConvKind.Dgrad, 2, bf16bf16_bf16f32_f32, tma_byte_alignments, (128, 256, 16), (2, 1, 1)), (ConvKind.Dgrad, 2, bf16bf16_bf16f32_f32, tma_byte_alignments, (256, 128, 8), (2, 1, 1)), (ConvKind.Dgrad, 2, bf16bf16_bf16f32_f32, tma_byte_alignments, (256, 128, 16), (2, 1, 1)), (ConvKind.Dgrad, 2, f16f16_f16f16_f16, tma_byte_alignments, (64, 64, 8), (1, 1, 1)), (ConvKind.Dgrad, 2, f16f16_f16f16_f16, tma_byte_alignments, (64, 64, 16), (1, 1, 1)), (ConvKind.Dgrad, 2, f16f16_f16f16_f16, tma_byte_alignments, (64, 128, 8), (1, 1, 1)), (ConvKind.Dgrad, 2, f16f16_f16f16_f16, tma_byte_alignments, (64, 128, 16), (1, 1, 1)), (ConvKind.Dgrad, 2, f16f16_f16f16_f16, tma_byte_alignments, (64, 256, 8), (1, 1, 1)), (ConvKind.Dgrad, 2, f16f16_f16f16_f16, tma_byte_alignments, (64, 256, 16), (1, 1, 1)), (ConvKind.Dgrad, 2, f16f16_f16f16_f16, tma_byte_alignments, (128, 128, 8), (1, 1, 1)), (ConvKind.Dgrad, 2, f16f16_f16f16_f16, tma_byte_alignments, (128, 128, 16), (1, 1, 1)), (ConvKind.Dgrad, 2, f16f16_f16f16_f16, tma_byte_alignments, (128, 256, 8), (1, 1, 1)), (ConvKind.Dgrad, 2, f16f16_f16f16_f16, tma_byte_alignments, (128, 256, 16), (1, 1, 1)), (ConvKind.Dgrad, 2, f16f16_f16f16_f16, tma_byte_alignments, (256, 64, 8), (1, 1, 1)), (ConvKind.Dgrad, 2, f16f16_f16f16_f16, tma_byte_alignments, (256, 64, 16), (1, 1, 1)), (ConvKind.Dgrad, 2, f16f16_f16f16_f16, tma_byte_alignments, (256, 128, 8), (1, 1, 1)), (ConvKind.Dgrad, 2, f16f16_f16f16_f16, tma_byte_alignments, (256, 128, 16), (1, 1, 1)), (ConvKind.Dgrad, 2, f16f16_f16f32_f32, tma_byte_alignments, (128, 256, 8), (2, 1, 1)), (ConvKind.Dgrad, 2, f16f16_f16f32_f32, tma_byte_alignments, (128, 256, 16), (2, 1, 1)), (ConvKind.Dgrad, 2, f16f16_f16f32_f32, tma_byte_alignments, (256, 128, 8), (2, 1, 1)), (ConvKind.Dgrad, 2, f16f16_f16f32_f32, tma_byte_alignments, (256, 128, 16), (2, 1, 1))))
    warp_count = [4, 1, 1]
    stages = 0
    ...
    def make_math_instruction(data_types: Dict[str, DataType], mma_shape: Tuple[int, int, int]) -> MathInstruction:
        ...
        return MathInstruction(mma_shape, data_types['a_type'], data_types['b_type'], data_types['c_type'], default_opcode, default_math_op)
    for conv_kind, spatial_dim, data_types, byte_alignments, mma_shape, cluster_shape in combinations_of_parameters:
        ...
        dims_and_alignments = (((spatial_dim, byte_alignments['A']), (spatial_dim, byte_alignments['B']), (spatial_dim, byte_alignments['C'])),)
        CreateConvOperator3x(manifest, dims_and_alignments=dims_and_alignments, tile_descriptions=[tile_description], data_types=data_types, schedule_pairs=schedule_pairs, tile_schedulers=tile_schedulers, conv_kind=conv_kind, log_indent_level=log_indent_level)
...
def define_parser():
    ...
    parser.add_argument('--kernels', default='', help='Comma-delimited list to filter kernels by name.  ' + 'Specifying this as "all" includes ALL the kernels, ' + 'while not specifying this includes only the default set of kernels.')
    parser.add_argument('--ignore-kernels', default='', help='Comma-delimited list of kernels ' + 'to exclude from build.  For backwards compatibility reasons, ' + 'this option only takes effect if --kernels is set to a nonempty value.')
    parser.add_argument('--exclude-kernels', default='', help='Comma-delimited list of kernels ' + 'to exclude from build.  In contrast to --ignore-kernels, ' + 'this option always takes effect, ' + 'whether or not --kernels is set to a nonempty value.  ' + 'It also can exclude kernels from the filter file ' + '(see --kernel-filter-file option below).')
