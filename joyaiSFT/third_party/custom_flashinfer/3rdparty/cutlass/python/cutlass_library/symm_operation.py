...
class SymmOperation:
    ...
    def __init__(self, symm_kind, arch, tile_description, A, B, C, element_epilogue, epilogue_functor=EpilogueFunctor.LinearCombination, swizzling_functor=SwizzlingFunctor.Identity8, blas_mode=BlasMode.symmetric):
...
class EmitSymmConfigurationLibrary:
    ...
    def __init__(self, operation_path, configuration_name):
        ...
        self.instance_emitter = {SymmKind.Universal: EmitSymmUniversalInstance}
        self.symm_kind_wrappers = {SymmKind.Universal: 'SymmOperation'}
        self.instance_template = {SymmKind.Universal: '\n${compile_guard_start}\n  manifest.append(new ${symm_kind}<\n    Operation_${operation_name}\n  >("${operation_name}"));\n${compile_guard_end}\n'}
    ...
    def emit(self, operation):
        ...
        self.instance_wrappers.append(SubstituteTemplate(self.instance_template[operation.symm_kind], {'configuration_name': self.configuration_name, 'operation_name': operation.procedural_name(), 'symm_kind': self.symm_kind_wrappers[operation.symm_kind], 'compile_guard_start': SubstituteTemplate(self.wmma_guard_start, {'sm_number': str(operation.arch)}) if operation.tile_description.math_instruction.opcode_class == OpcodeClass.WmmaTensorOp else '', 'compile_guard_end': '#endif' if operation.tile_description.math_instruction.opcode_class == OpcodeClass.WmmaTensorOp else ''}))
