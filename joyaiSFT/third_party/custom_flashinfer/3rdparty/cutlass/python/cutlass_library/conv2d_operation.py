...
class Conv2dOperation:
    ...
    def __init__(self, conv_kind, iterator_algorithm, arch, tile_description, A, B, C, element_epilogue, stride_support, epilogue_functor=EpilogueFunctor.LinearCombination, swizzling_functor=SwizzlingFunctor.Identity1, group_mode=GroupMode.NoneGroup):
    ...
    def configuration_name(self):
        ...
        return SubstituteTemplate(configuration_name, {'opcode_class': opcode_class_name, 'extended_name': self.extended_name(), 'threadblock': threadblock, 'layout': self.layout_name(), 'alignment': '%d' % self.A.alignment, 'group_conv_name': group_conv_name})
    ...
    for tile in tile_descriptions:
        for conv_kind in [ConvKind.Fprop, ConvKind.Dgrad, ConvKind.Wgrad]:
            if conv_kind == ConvKind.Fprop or tile.math_instruction.element_accumulator in [DataType.f16, DataType.f32]:
                output_types = [tile.math_instruction.element_a, tile.math_instruction.element_accumulator] if DataTypeSize[tile.math_instruction.element_accumulator] == 32 else [tile.math_instruction.element_accumulator]
...
class EmitConv2dConfigurationLibrary:
    ...
    """
        ...
        if is_stub:
            ...
            stub_end = '#endif // 0'
        self.configuration_file.write(Template(self.instance_template).substitute({'configuration_name': self.configuration_name, 'operation_name': operation.procedural_name(), 'operation_instance': self.instance_emitter.emit(operation), 'stub_begin': stub_begin, 'stub_end': stub_end}))
    ...
    """
        ...
        for operation in self.operations:
            ...
            self.configuration_file.write(SubstituteTemplate(self.configuration_instance, {'configuration_name': self.configuration_name, 'operation_name': operation.procedural_name(), 'kernel_name': kernel_name, 'operation_wrapper': operation_wrapper, 'stub_begin': stub_begin, 'stub_end': stub_end}))
