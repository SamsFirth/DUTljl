...
def calculate_smem_usage_per_stage(td: TileDescription, operation_kind: OperationKind) -> int:
    ...
    if operation_kind == OperationKind.Gemm:
        ...
        return DataTypeSize[td.math_instruction.element_a] * m * k // 8 + DataTypeSize[td.math_instruction.element_b] * k * n // 8 + stage_barrier_bytes
    else:
...
def valid_stage_count(cc: int, kernel_cc: int, td: TileDescription, element_C: cutlass.DataType=None, element_D: cutlass.DataType=None, verbose: bool=True) -> tuple:
    """
    ...
    if kernel_cc == 90:
        if td.stages is None or td.stages == 0:
            ...
            cutlass.logger.warning('Setting an explicit stage count for SM90 kernels currently may result in compilation errors if the combination of tile shape, stage count, and shared memory requirement of the epilogue exceeds the available shared memory per SM.')
    if td.stages <= 0:
    ...
    if cc < 80 and td.stages != 2:
        return (False, f'Tile description has stage count of {td.stages}, but only 2 stages are supported on SM{cc}.')
    ...
    if smem_usage_mainloop > smem_arch:
        return (False, f'Configuration uses too much shared memory. Consider reducing stage count or tile shape.\nDetails:\nMainloop uses {smem_per_stage} bytes of shared memory per stage, and {td.stages} stages for a total of {smem_usage_mainloop} bytes.\nThe maxmium amount of shared memory that can be used per block on CC {cc} is {smem_arch}.')
    return (True, '')
...
def valid_cluster_shape(cc: int, cluster_shape: list) -> tuple:
    ...
    if cc < 90:
        if cluster_shape != [1, 1, 1]:
            return (False, f'Cluster shape for pre-SM90 architectures must be [1, 1, 1]. Received cluster shape of {cluster_shape} for SM{cc}.')
        else:
            return (True, '')
    ...
    if cluster_shape[2] != 1:
        return (False, f'CUTLASS kernels currently require the third dimension of cluster shape to be 1. Received cluster shape of {cluster_shape}.')
    blocks_in_2d = cluster_shape[0] * cluster_shape[1]
    if blocks_in_2d > 8:
        return (False, f'Thread block clusters with more than 8 thread blocks are currently unsupported on SM{cc}. Received cluster shape {cluster_shape}, which has {blocks_in_2d} thread blocks.')
    return (True, '')

def valid_schedule(cc: int, kernel_schedule: cutlass.KernelScheduleType, epilogue_schedule: cutlass.EpilogueScheduleType, tile_scheduler: cutlass.TileSchedulerType) -> tuple:
    """
    ...
    if not tile_scheduler_default:
        cooperative_kernels = [cutlass.KernelScheduleType.TmaWarpSpecializedCooperative, cutlass.KernelScheduleType.CpAsyncWarpSpecializedCooperative]
