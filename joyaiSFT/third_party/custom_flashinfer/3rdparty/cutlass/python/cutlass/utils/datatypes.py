...
def is_numpy_available():
    ...
    if numpy_available is None:
        try:
            ...
            _library_to_numpy_dict = {cutlass.DataType.f16: np.float16, cutlass.DataType.f32: np.float32, cutlass.DataType.f64: np.float64, cutlass.DataType.s8: np.int8, cutlass.DataType.s32: np.int32}
        except ImportError:
...
def is_cupy_available():
    ...
    if cupy_available is None:
        try:
            import cupy as cp
            cupy_available = True
            _library_to_cupy_dict = {cutlass.DataType.f16: cp.float16, cutlass.DataType.f32: cp.float32, cutlass.DataType.f64: cp.float64, cutlass.DataType.s8: cp.int8, cutlass.DataType.s32: cp.int32}
        except ImportError:
...
def library_type(inp):
    if inp in DataTypeSize:
        return inp
    for cvt_fn in [bfloat16_library_type, cupy_library_type, numpy_library_type, torch_library_type]:
        out = cvt_fn(inp)
        if out is not None:
            return out
...
def construct_backend_td(td: cutlass.TileDescription, kernel_schedule: cutlass.KernelScheduleType, epilogue_schedule: cutlass.EpilogueScheduleType, tile_scheduler: cutlass.TileSchedulerType) -> TileDescription:
    mi = td.math_instruction
    backend_mi = MathInstruction(mi.instruction_shape, mi.element_a, mi.element_b, mi.element_accumulator, mi.opcode_class, backend_math_operation(mi.math_operation))
    cluster_shape = td.cluster_shape if hasattr(td, 'cluster_shape') else [1, 1, 1]
    return TileDescription(td.threadblock_shape, td.stages, td.warp_count, backend_mi, cluster_shape, kernel_schedule, epilogue_schedule, tile_scheduler)
