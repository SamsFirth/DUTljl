...
class ReductionArguments:
    ...
    def __init__(self, operation: ReductionOperation, problem_size: 'list[int]', partitions: int, workspace: cuda.CUdeviceptr, destination: 'Union[cuda.CUdeviceptr, np.ndarray, torch.Tensor]', source: 'Union[cuda.CUdeviceptr, np.ndarray, torch.Tensor]', **kwargs) -> None:
    ...
    @staticmethod
    def get_tensor_ref(extent: 'tuple[int]', device_ptr: cuda.CUdeviceptr, layout: LayoutType):
    ...
    def get_arguments(self):
        ref_workspace = ReductionArguments.get_tensor_ref(extent=[self.problem_size.row, self.problem_size.column], device_ptr=self.ptr_workspace, layout=LayoutType.RowMajor)
        if self.bias:
            ref_source = ReductionArguments.get_tensor_ref(extent=[0, 0], device_ptr=self.ptr_source, layout=LayoutType.RowMajor)
        else:
            ref_source = ReductionArguments.get_tensor_ref(extent=[self.problem_size.row, self.problem_size.column], device_ptr=self.ptr_source, layout=LayoutType.RowMajor)
        ref_destination = ReductionArguments.get_tensor_ref(extent=[self.problem_size.row, self.problem_size.column], device_ptr=self.ptr_destination, layout=LayoutType.RowMajor)
        self.c_arguments = self.operation.argument_type(self.problem_size, self.partitions, self.partition_stride, ref_workspace, ref_destination, ref_source, self.output_op)
    ...
    def sync(self):
        ...
        if hasattr(self, 'host_D'):
            err, = cuda.cuMemcpyDtoH(self.host_D, self.ptr_destination, self.host_D.size * self.host_D.itemsize)
            if err != cuda.CUresult.CUDA_SUCCESS:
...
class ReductionRT(ExecutableOperation):
    ...
    def plan(self, arguments: ReductionArguments):
        block_shape = [self.operation.shape.column // self.elements_per_access, self.operation.shape.row, 1]
        grid_shape = [(arguments.problem_size.row + self.operation.shape.row - 1) // self.operation.shape.row, (arguments.problem_size.column + self.operation.shape.column - 1) // self.operation.shape.column, 1]
...
class ReductionOperation:
    ...
    def __init__(self, shape: MatrixCoord, C: TensorDescription, element_accumulator, element_workspace=None, element_compute=None, epilogue_functor=None, count: int=1, partitions_per_stage: int=4) -> None:
        self.shape = shape
    ...
    def extended_name(self):
        ...
        return SubstituteTemplate(extend_name, {'element_workspace': DataTypeNames[self.element_workspace], 'element_accumulator': DataTypeNames[self.element_accumulator], 'element_compute': DataTypeNames[self.element_compute], 'element_output': DataTypeNames[self.element_output]})
    ...
    def configuration_name(self):
        ...
        return SubstituteTemplate(configuration_name, {'extended_name': self.extended_name(), 'threadblock': threadblock})
...
class EmitReductionInstance:
    ...
    def __init__(self, operation_suffix='') -> None:
        ...
        self.includes = ['cutlass/cutlass.h', 'cutlass/numeric_types.h', 'cutlass/arch/arch.h', 'cutlass/arch/mma.h', 'cutlass/layout/matrix.h', 'cutlass/gemm/device/gemm.h', 'cutlass/gemm/device/gemm_universal_adapter.h', 'cutlass/gemm/kernel/default_gemm_universal.h', 'cutlass/reduction/kernel/reduce_split_k.h', 'cutlass/reduction/thread/reduction_operators.h']
    ...
    def emit(self, operation: ReductionOperation):
        ...
        values = {'operation_name': operation.configuration_name(), 'operation_suffix': self.operation_suffix, 'shape_row': str(operation.shape.row), 'shape_column': str(operation.shape.column), 'epilogue_functor': operation.epilogue_functor.emit(), 'element_output': DataTypeTag[operation.element_output], 'epilogue_vector_length': str(epilogue_vector_length), 'element_accumulator': DataTypeTag[operation.element_accumulator], 'element_compute': DataTypeTag[operation.element_compute], 'element_workspace': DataTypeTag[operation.element_workspace], 'count': str(operation.count), 'partition_per_stage': str(operation.partitions_per_stage)}
