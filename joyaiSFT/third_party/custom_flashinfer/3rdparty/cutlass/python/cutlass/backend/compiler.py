...
class ArtifactManager:
    ...
    def __init__(self) -> None:
        ...
        self._nvcc_compile_options = ['-std=c++17', '--expt-relaxed-constexpr', '-Xcudafe --diag_suppress=esa_on_defaulted_function_ignored']
        self.nvcc()
    ...
    def emit_compile_(self, operation_list, compilation_options, host_compilation_options):
        ...
        for operation in operation_list:
            ...
            values = {'operation_name': operation.name(), 'operation_suffix': operation.emitter.operation_suffix}
        ...
        if self.backend == 'nvrtc':
            err, program = nvrtc.nvrtcCreateProgram(str.encode(source_buffer_device), bytes(str.encode('module.cu')), 0, [], [])
            ...
            values = {'cuda_install_path': cuda_install_path(), 'options': compilation_options.get_str(), 'srcfile': temp_cu.name, 'tarfile': temp_cubin.name}
    ...
    def add_module(self, operations, compile_options=None, bypass_cache=False):
        ...
        """
        include_paths = [cuda_install_path() + '/include', CUTLASS_PATH + '/include', CUTLASS_PATH + '/tools/util/include', CUTLASS_PATH + '/python/cutlass/cpp/include']
