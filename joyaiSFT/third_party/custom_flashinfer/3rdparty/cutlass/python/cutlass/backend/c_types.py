...
class GenericMainloopArguments3x_(ctypes.Structure):
    ...
    """
    _fields_ = [('ptr_A', ctypes.c_void_p), ('stride_A', StrideBatched_), ('ptr_B', ctypes.c_void_p), ('stride_B', StrideBatched_), ('mma_promotion_interval', ctypes.c_int)]

class _PersistentTileSchedulerArguments(ctypes.Structure):
    _fields_ = [('max_swizzle_size', ctypes.c_int), ('raster_order_option', ctypes.c_int)]

class _PersistentTileSchedulerStreamKArguments(ctypes.Structure):
    _fields_ = [('splits', ctypes.c_int), ('max_swizzle_size', ctypes.c_int), ('raster_order_option', ctypes.c_int), ('reduction_mode', ctypes.c_int), ('decomposition_mode', ctypes.c_int)]

def get_tile_scheduler_arguments_3x(tile_scheduler: TileSchedulerType, splits: int=1):
    max_swizzle_size = 1
    raster_order_option = 0
    if tile_scheduler == TileSchedulerType.Persistent:
        return _PersistentTileSchedulerArguments(max_swizzle_size, raster_order_option)
    elif tile_scheduler == TileSchedulerType.StreamK:
        reduction_mode = 0
        decomposition_mode = 0
        return _PersistentTileSchedulerStreamKArguments(splits, max_swizzle_size, raster_order_option, reduction_mode, decomposition_mode)

def get_mainloop_arguments_3x(kernel_schedule: KernelScheduleType, element_A, element_B, alignment_A: int, alignment_B: int) -> ctypes.Structure:
    """
    ...
    class _MainloopArgumentsTma(ctypes.Structure):
        ...
        def from_generic_mainloop_args(args: GenericMainloopArguments3x_):
            return _MainloopArgumentsTma(args.ptr_A, args.stride_A, args.ptr_B, args.stride_B, args.mma_promotion_interval)

    class _MainloopArgumentsMultistage(ctypes.Structure):
        _fields_ = [('ptr_A', ctypes.c_void_p), ('stride_A', StrideBatched_), ('ptr_B', ctypes.c_void_p), ('stride_B', StrideBatched_)]

        @staticmethod
        def from_generic_mainloop_args(args: GenericMainloopArguments3x_):
            return _MainloopArgumentsMultistage(args.ptr_A, args.stride_A, args.ptr_B, args.stride_B)
...
def get_gemm_arguments_3x(mainloop_arguments, epilogue_functor, scheduler_args, default_epilogue):
    ...
    if hasattr(epilogue_functor, 'visitor'):
        ...
        class _EpilogueArguments(ctypes.Structure):
            _fields_ = [('epilogue', _EpilogueOutputOpParams), ('arg_C', epilogue_functor.arg_c_type), ('arg_D', epilogue_functor.arg_d_type)]
        ...
        class _EpilogueArguments(ctypes.Structure):
            _fields_ = [('epilogue', _EpilogueOutputOpParams), ('ptr_C', ctypes.c_void_p), ('stride_C', StrideBatched_), ('ptr_D', ctypes.c_void_p), ('stride_D', StrideBatched_)]

    class _HardwareInfo(ctypes.Structure):
        _fields_ = [('device_id', ctypes.c_int), ('sm_count', ctypes.c_int)]

    class _GemmArguments(ctypes.Structure):
        _fields_ = [('mode', ctypes.c_int), ('problem_size', GemmCoordBatched_), ('mainloop', mainloop_arguments), ('epilogue', _EpilogueArguments), ('hw_info', _HardwareInfo), ('scheduler', type(scheduler_args))]
...
def get_conv2d_arguments(epilogue_functor):
    ...
    class _Conv2dArguments(ctypes.Structure):
        _fields_ = [('conv_kind', ctypes.c_int), ('problem_size', Conv2DProblemSize_), ('ptr_A', ctypes.c_void_p), ('ptr_B', ctypes.c_void_p), ('ptr_C', ctypes.c_void_p), ('ptr_D', ctypes.c_void_p), ('tensor_C_numel', ctypes.c_int), ('output_op', _EpilogueOutputOpParams), ('split_k_mode', ctypes.c_int)]
...
def get_reduction_params(epilogue_functor):
    ...
    class _ReductionParams(ctypes.Structure):
        _fields_ = [('problem_size', MatrixCoord_), ('partitions', ctypes.c_int), ('partition_stride', ctypes.c_longlong), ('workspace', TensorRef2D_), ('destination', TensorRef2D_), ('source', TensorRef2D_), ('output_op', _EpilogueOutputParams)]
