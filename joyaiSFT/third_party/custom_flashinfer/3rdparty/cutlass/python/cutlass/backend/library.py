...
"""
import enum
from cutlass_library import ComplexTransform, DataType, DataTypeSize, EpilogueScheduleType, KernelScheduleType, MathOperation, OpcodeClass, TileSchedulerType
try:
...
class SchedulerMode(enum.Enum):
    Device = enum_auto()
    Host = enum_auto()
SchedulerModeTag = {SchedulerMode.Device: 'cutlass::gemm::kernel::GroupScheduleMode::kDeviceOnly', SchedulerMode.Host: 'cutlass::gemm::kernel::GroupScheduleMode::kHostPrecompute'}
...
class FunctionalOp(enum.Enum):
    ...
    Plus = enum_auto()
FunctionalOpTag = {FunctionalOp.AtomicAdd: 'cutlass::atomic_add', FunctionalOp.AtomicMaximum: 'cutlass::atomic_maximum', FunctionalOp.Divides: 'cutlass::divides', FunctionalOp.Maximum: 'cutlass::maximum', FunctionalOp.Minimum: 'cutlass::minimum', FunctionalOp.Minus: 'cutlass::minus', FunctionalOp.Multiplies: 'cutlass::multiplies', FunctionalOp.MultiplyAdd: 'cutlass::multiply_add', FunctionalOp.Plus: 'cutlass::plus'}
...
class ActivationOp(enum.Enum):
    ...
    SiLU = enum_auto()
    Tanh = enum_auto()
ActivationOpTag = {ActivationOp.DGelu: 'cutlass::epilogue::thread::dGELU', ActivationOp.Gelu: 'cutlass::epilogue::thread::GELU', ActivationOp.GeluTaylor: 'cutlass::epilogue::thread::GELU_taylor', ActivationOp.HardSwish: 'cutlass::epilogue::thread::HardSwish', ActivationOp.Identity: 'cutlass::epilogue::thread::Identity', ActivationOp.LeakyReLU: 'cutlass::epilogue::thread::LeakyReLU', ActivationOp.ReLU: 'cutlass::epilogue::thread::ReLu', ActivationOp.Sigmoid: 'cutlass::epilogue::thread::Sigmoid', ActivationOp.SiLU: 'cutlass::epilogue::thread::SiLu', ActivationOp.Tanh: 'cutlass::epilogue::thread::Tanh'}
...
FloatRoundStyleTag = {FloatRoundStyle.ToNearest: 'cutlass::FloatRoundStyle::round_to_nearest', FloatRoundStyle.ToNearestSatfinite: 'cutlass::FloatRoundStyle::round_to_nearest_satfinite', FloatRoundStyle.Indeterminate: 'cutlass::FloatRoundStyle::round_indeterminate', FloatRoundStyle.TowardZero: 'cutlass::FloatRoundStyle::round_toward_zero', FloatRoundStyle.TowardInfinity: 'cutlass::FloatRoundStyle::round_toward_infinity', FloatRoundStyle.TowardNegInfinity: 'cutlass::FloatRoundStyle::round_toward_neg_infinity', FloatRoundStyle.HalfUlpTruncDntz: 'cutlass::FloatRoundStyle::round_half_ulp_trunc_dntz', FloatRoundStyle.HalfUlpTruncate: 'cutlass::FloatRoundStyle::round_half_ulp_truncate'}
...
class MathInstruction:
    ...
    def __init__(self, instruction_shape, element_a, element_b, element_accumulator, opcode_class=OpcodeClass.Simt, math_operation=MathOperation.multiply_add):
        """
...
class TileDescription:
    ...
    def __init__(self, threadblock_shape, stages, warp_count, math_instruction, cluster_shape=[1, 1, 1], kernel_schedule: KernelScheduleType=None, epilogue_schedule: EpilogueScheduleType=None, tile_scheduler: TileSchedulerType=None):
        """
        ...
        """
        if kernel_schedule is None and epilogue_schedule is not None or (kernel_schedule is not None and epilogue_schedule is None):
    ...
    def clone_and_update(self, td: dict):
        attrs = {'cluster_shape': None, 'threadblock_shape': None, 'warp_count': None, 'stages': None, 'instruction_shape': None, 'kernel_schedule': None, 'epilogue_schedule': None, 'tile_scheduler': None}
        for key in attrs.keys():
            if key in td.keys():
                attrs[key] = td[key]
            else:
                attrs[key] = getattr(self, key)
        attrs['math_instruction'] = MathInstruction(attrs['instruction_shape'], self.math_instruction.element_a, self.math_instruction.element_b, self.math_instruction.element_accumulator, self.math_instruction.opcode_class, self.math_instruction.math_operation)
    ...
    def procedural_name(self):
        ...
        name = '%dx%dx%d_%dx%d_%dx%d' % (self.cluster_shape[0], self.cluster_shape[1], self.cluster_shape[2], self.threadblock_shape[0], self.threadblock_shape[1], self.threadblock_shape[2], emit_stages)
        return name
...
def CalculateSmemUsagePerStage(operation):
    ...
    if operation.operation_kind == OperationKind.Gemm:
        ...
        return DataTypeSize[operation.A.element] * m * k // 8 + DataTypeSize[operation.B.element] * k * n // 8 + stage_barrier_bytes
    else:
