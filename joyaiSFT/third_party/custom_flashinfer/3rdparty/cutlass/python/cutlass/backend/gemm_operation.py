...
import numpy as np
from cutlass_library import ComplexTransformTag, DataType, DataTypeNames, DataTypeSize, DataTypeTag, EpilogueScheduleSuffixes, EpilogueScheduleTag, EpilogueScheduleType, GemmKind, GemmKindNames, GemmUniversalMode, KernelScheduleSuffixes, KernelScheduleTag, KernelScheduleType, LayoutTag, LayoutType, MathOperation, MathOperationTag, OpcodeClass, OpcodeClassNames, OpcodeClassTag, OperationKind, ShortComplexLayoutNames, ShortDataTypeNames, ShortLayoutTypeNames, SwizzlingFunctor, SwizzlingFunctorTag, TileSchedulerSuffixes, TileSchedulerTag, TileSchedulerType, get_complex_from_real
from cutlass.backend.arguments import ArgumentBase
from cutlass.backend.c_types import GemmCoord_, GemmCoordBatched_, GenericMainloopArguments3x_, StrideBatched_, dim3_, get_gemm_arguments, get_gemm_arguments_3x, get_gemm_arguments_streamk, get_gemm_grouped_arguments, get_mainloop_arguments_3x, get_tile_scheduler_arguments_3x
from cutlass.backend.library import ApiVersion, EmissionType, SchedulerMode, SchedulerModeTag, TensorDescription, TileDescription, api_version
...
class GemmArguments2x(ArgumentBase):
    ...
    def get_arguments(self):
        ...
        if self.gemm_mode == GemmUniversalMode.Array:
            arguments = self.operation.argument_type(self.gemm_mode, problem_size_, self.batch_count, 0, self.output_op, int(self.ptr_A_array_buffer.ptr), int(self.ptr_B_array_buffer.ptr), int(self.ptr_C_array_buffer.ptr), int(self.ptr_D_array_buffer.ptr), 0, 0, 0, self.lda, self.ldb, self.ldc, self.ldd, self.lda, self.ldb, self.ldc, self.ldd, 0, 0, 0)
        else:
            arguments = self.operation.argument_type(self.gemm_mode, problem_size_, self.batch_count, self.batched_stride_D, self.output_op, int(self.ptr_A), int(self.ptr_B), int(self.ptr_C), int(self.ptr_D), self.batched_stride_A, self.batched_stride_B, self.batched_stride_C, self.lda, self.ldb, self.ldc, self.ldd, self.lda, self.ldb, self.ldc, self.ldd, 0, 0, 0)
...
class GemmArguments2xStreamK(GemmArguments2x):
    ...
    def get_arguments(self):
        ...
        arguments = self.operation.argument_type(self.gemm_mode, GemmCoord_(self.problem_size.m, self.problem_size.n, self.problem_size.k), self.batch_count, self.output_op, int(self.ptr_A), int(self.ptr_B), int(self.ptr_C), int(self.ptr_D), batch_stride_A, batch_stride_B, batch_stride_C, batch_stride_D, self.lda, self.ldb, self.ldc, self.ldd, self.lda, self.ldb, self.ldc, self.ldd, -1)
        return arguments

    def initialize(self):
        device_workspace_size = self.operation.rt_module.get_device_workspace_size(self, device_sm_count(), self.operation.rt_module.occupancy)
        if device_workspace_size > 0:
            self.workspace_buffer = device_mem_alloc(device_workspace_size)
            workspace_ptr = self.workspace_buffer.ptr
            err, = cuda.cuMemsetD32(workspace_ptr, 0, device_workspace_size // 4)
        ...
        res_arg = self.operation.rt_module.get_args(ctypes.byref(arguments), ctypes.c_void_p(int(device_workspace)), device_sm_count(), self.operation.rt_module.occupancy)
        host_workspace = bytearray(res_arg.contents)
        grid = self.operation.rt_module.get_grid_shape(ctypes.byref(arguments), device_sm_count(), self.operation.rt_module.occupancy)
        device_workspace = None
        self.host_workspace = host_workspace
        self.device_workspace = device_workspace
        self.launch_config = LaunchConfiguration([grid.m, grid.n, grid.k], [self.operation.rt_module.threads, 1, 1], self.operation.rt_module.shared_memory_capacity)
...
class GemmArguments3x(GemmArguments2x):
    ...
    def get_arguments(self):
        mainloop_args = get_mainloop_arguments_3x(self.operation.tile_description.kernel_schedule, self.operation.A.element, self.operation.B.element, self.operation.A.alignment, self.operation.B.alignment)
        ...
        argument_type, epilogue_args, epilogue_type, hw_info = get_gemm_arguments_3x(mainloop_args, self.operation.epilogue_functor, scheduler_args, uses_default_epilogue)
        ...
        generic_args = GenericMainloopArguments3x_(int(self.ptr_A), stride_A, int(self.ptr_B), stride_B, 4)
        ...
        epilogue = epilogue_args(self.output_op, int(self.ptr_C), stride_C, int(self.ptr_D), stride_D)
    ...
    def initialize(self):
        ...
        if workspace_ptr is not None and self.gemm_mode == GemmUniversalMode.GemmSplitKParallel:
            ...
            device_workspace = workspace_ptr
        self.get_arguments()
        res_arg = self.operation.rt_module.get_args(ctypes.byref(self.arguments), ctypes.c_void_p(int(device_workspace)))
        host_workspace = bytearray(res_arg.contents)
        grid = self.operation.rt_module.get_grid_shape(ctypes.byref(self.arguments), ctypes.c_void_p(int(device_workspace)))
        block = self.operation.rt_module.get_block_shape()
        device_workspace = None
        self.host_workspace = host_workspace
        self.device_workspace = device_workspace
        self.launch_config = LaunchConfiguration([grid.x, grid.y, grid.z], [block.x, block.y, block.z], self.operation.rt_module.shared_memory_capacity)
...
class GemmGroupedArguments:
    ...
    def __init__(self, operation, problem_sizes, A, B, C, D, **kwargs):
        ...
        for idx, problem_size in enumerate(problem_sizes):
            ...
            temp_argument = GemmArguments2x(operation=operation, problem_size=GemmCoord(M, N, K), A=A[idx], B=B[idx], C=C[idx], D=D[idx])
            self.gemm_arguments.append(temp_argument)
            problem_size_host.append([temp_argument.problem_size.m, temp_argument.problem_size.n, temp_argument.problem_size.k])
            ...
            grid = self.operation.rt_module.get_grid_shape(self.operation.rt_module.get_tiled_shape(temp_argument.problem_size.ctype, self.threadblock_shape.ctype, temp_argument.batch_count))
    ...
    def initialize(self):
        ...
        if self.operation.precompute_mode == SchedulerMode.Host:
            device_workspace_ptr = self.operation.rt_module.host_precompute(self, self.operation.rt_module.get_workspace_size(self))
        else:
            device_workspace_ptr = 0
        result = self.operation.rt_module.get_args(ctypes.byref(self.arguments), self.total_tiles, ctypes.c_void_p(int(device_workspace_ptr)))
...
class GemmRTbase(ExecutableOperation):
    ...
    def initialize(self):
        err, = cuda.cuFuncSetAttribute(self.kernel, attrib=cuda.CUfunction_attribute.CU_FUNC_ATTRIBUTE_MAX_DYNAMIC_SHARED_SIZE_BYTES, value=self.shared_memory_capacity)
        if err != cuda.CUresult.CUDA_SUCCESS:
...
class GemmRTUniversal(GemmRTbase):
    ...
    def __init__(self, operation):
        ...
        self.extra_funcs = {'get_tiled_shape': GemmCoord_, 'get_grid_shape': dim3_}
    ...
    def plan(self, arguments):
        ...
        if arguments.gemm_mode in [GemmUniversalMode.Gemm, GemmUniversalMode.GemmSplitKParallel]:
            alignk = max(max(128 // DataTypeSize[self.operation.A.element], 128 // DataTypeSize[self.operation.B.element]), 1)
            gemm_k_size = ((arguments.problem_size.k + arguments.batch_count - 1) // arguments.batch_count + alignk - 1) // alignk * alignk
            if gemm_k_size:
    ...
    def get_device_workspace_size(self, arguments: GemmArguments):
        ...
        if arguments.gemm_mode == GemmUniversalMode.GemmSplitKParallel:
            workspace_bytes = DataTypeSize[arguments.operation.C.element] * arguments.batched_stride_D * arguments.grid_tiled_shape.z // 8
...
class GemmRTUniversalStreamK(GemmRTUniversal):
    ...
    def __init__(self, operation: 'GemmOperation'):
        ...
        self.extra_funcs = {'get_grid_shape': GemmCoord_, 'get_kernel_workspace_size': ctypes.c_uint64}
    ...
    def occupancy(self):
        if self._occupancy is None:
            err, self._occupancy = cuda.cuOccupancyMaxActiveBlocksPerMultiprocessorWithFlags(self.kernel, self.threads, self.shared_memory_capacity, cuda.CUoccupancy_flags.CU_OCCUPANCY_DISABLE_CACHING_OVERRIDE)
            if err != cuda.CUresult.CUDA_SUCCESS:
                raise RuntimeError(f'CUDA error on call to cuOccupancyMaxActiveBlocksPerMultiprocessorWithFlags: {cuda.cuGetErrorString(err)[1]}')
...
class GemmRTUniversal3x(GemmRTUniversal):
    ...
    def __init__(self, operation):
        ...
        self.extra_funcs = {'get_grid_shape': dim3_, 'get_block_shape': dim3_, 'get_persistent_tiled_blk_shape_mnl': ctypes.c_uint64, 'get_kernel_workspace_size': ctypes.c_uint64, 'uses_default_epilogue': ctypes.c_bool}
...
class EmitGemmUniversalInstance3x:
    ...
    def __init__(self, operation_suffix=''):
        ...
        self.includes = ['cutlass/cutlass.h', 'cute/tensor.hpp', 'cute/atom/mma_atom.hpp', 'cutlass/numeric_types.h', 'cutlass/gemm/collective/collective_builder.hpp', 'cutlass/gemm/kernel/sm90_tile_scheduler.hpp', 'cutlass/gemm/kernel/gemm_universal.hpp', 'cutlass/epilogue/collective/collective_builder.hpp', 'cutlass/epilogue/collective/default_epilogue.hpp', 'cutlass/epilogue/thread/linear_combination.h']
...
class GemmRTGrouped(GemmRTbase):
    ...
    def __init__(self, operation: 'GemmOperation'):
        ...
        self.extra_funcs = {'precompute': None, 'get_tiled_shape': GemmCoord_, 'get_grid_shape': dim3_}
    ...
    def plan(self, arguments):
        return LaunchConfiguration([arguments.total_tiles, 1, 1], [self.threads, 1, 1], self.shared_memory_capacity)
    ...
    def get_workspace_size(self, arguments):
        if self.operation.precompute_mode == SchedulerMode.Device:
            ...
            entries_per_block = 1
            return 8 * entries_per_block * total_tiles
...
class GemmOperationBase:
    ...
    def __init__(self, gemm_kind, arch, tile_description: TileDescription, A: TensorDescription, B: TensorDescription, C: TensorDescription, epilogue_functor, swizzling_functor=SwizzlingFunctor.Identity1, api=ApiVersion.v2x, emission_type=EmissionType.Kernel, **kwargs):
        ...
        self.switched = self.api != ApiVersion.v3x and self.emission_type == EmissionType.Kernel and (C.layout == LayoutType.ColumnMajor)
    ...
    def run(self, arguments: GemmArguments) -> cuda.CUresult:
        ...
        err = self.rt_module.run(arguments.host_workspace, arguments.device_workspace, arguments.launch_config, arguments.stream)
        if err != cuda.CUresult.CUDA_SUCCESS:
            raise RuntimeError('CUDA Error %s' % str(err))
        return err
    ...
    def procedural_name(self):
        ...
        if self.api == ApiVersion.v3x and self.arch >= 90:
            ...
            return kernel_name_template.format(p=self.prefix, ar=self.arch, op=opcode_class_name, ex=self.extended_name_3x(), tbm=self.tile_description.threadblock_shape[0], tbn=self.tile_description.threadblock_shape[1], tbk=self.tile_description.threadblock_shape[2], cm=self.tile_description.cluster_shape[0], cn=self.tile_description.cluster_shape[1], ck=self.tile_description.cluster_shape[2], l=self.tile_description.stages, s=self.layout_name_3x(), al=str(self.A.alignment), k=self.kernel_schedule_name_3x(), e=self.epilogue_schedule_name_3x())
        else:
            threadblock = self.tile_description.procedural_name_2x()
            return 'cutlass{p}_{op}_{ex}_{tb}_{l}_align{a}'.format(p=self.prefix, op=opcode_class_name, ex=self.extended_name(), tb=threadblock, l=self.layout_name(), a=str(self.A.alignment))
...
class GemmOperationUniversal(GemmOperationBase):
    ...
    def __init__(self, arch, tile_description: TileDescription, A: TensorDescription, B, C, epilogue_functor, swizzling_functor=SwizzlingFunctor.Identity1, **kwargs):
        api = api_version(arch, tile_description.math_instruction.opcode_class, A.element)
        super(GemmOperationUniversal, self).__init__(GemmKind.Universal, arch, tile_description, A, B, C, epilogue_functor, swizzling_functor, api=api, **kwargs)
        ...
        elif swizzling_functor == SwizzlingFunctor.StreamK:
    ...
    def device_op(self):
        ...
        return GemmOperationUniversal(self.arch, self.tile_description, A, B, C, self.epilogue_functor, self.swizzling_functor, emission_type=EmissionType.Device, direct_store=self.direct_store)

class GemmOperationGrouped(GemmOperationBase):

    def __init__(self, arch, tile_description: TileDescription, A: TensorDescription, B, C, epilogue_functor, swizzling_functor=SwizzlingFunctor.Identity1, **kwargs):
        super(GemmOperationGrouped, self).__init__(GemmKind.Grouped, arch, tile_description, A, B, C, epilogue_functor, swizzling_functor, **kwargs)
    ...
    def device_op(self):
        ...
        """
        A, B, C = GemmOperationBase.get_operands(self.A, self.B, self.C, self.switched)
        return GemmOperationGrouped(self.arch, self.tile_description, A, B, C, self.epilogue_functor, self.swizzling_functor, emission_type=EmissionType.Device, direct_store=self.direct_store, precompute_mode=self.precompute_mode)
...
class EmitGemmUniversalInstance:
    ...
    def __init__(self, operation_suffix='', direct_store=False):
        ...
        self.includes = ['cutlass/cutlass.h', 'cutlass/gemm_coord.h', 'cutlass/numeric_types.h', 'cutlass/arch/arch.h', 'cutlass/arch/mma.h', 'cutlass/layout/matrix.h', 'cutlass/gemm/device/gemm.h', 'cutlass/gemm/device/gemm_universal_adapter.h', 'cutlass/gemm/kernel/default_gemm_universal.h']
    ...
    def emit(self, operation):
        ...
        if hasattr(operation.epilogue_functor, 'visitor'):
            self.includes += ['cutlass/epilogue/threadblock/fusion/visitors.hpp', 'cutlass/gemm/kernel/default_gemm_universal_with_visitor.h']
            callback_name, callback_decl = operation.epilogue_functor.emit(operation)
            values['callback_name'] = callback_name
            values['callback_decl'] = callback_decl
