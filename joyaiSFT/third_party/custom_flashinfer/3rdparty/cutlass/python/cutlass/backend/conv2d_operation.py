...
import numpy as np
from cutlass_library import ConvKindNames, ConvKindTag, DataTypeNames, DataTypeSize, DataTypeTag, IteratorAlgorithmNames, IteratorAlgorithmTag, LayoutTag, LayoutType, MathOperation, MathOperationTag, OpcodeClass, OpcodeClassNames, OpcodeClassTag, OperationKind, ShortDataTypeNames, ShortLayoutTypeNames, SplitKMode, StrideSupport, StrideSupportTag, SwizzlingFunctor, SwizzlingFunctorTag, get_complex_from_real
from cutlass.backend.arguments import ArgumentBase
from cutlass.backend.c_types import dim3_, get_conv2d_arguments
from cutlass.backend.library import EmissionType, TensorDescription, TileDescription
...
class Conv2dArguments(ArgumentBase):
    ...
    def __init__(self, operation, problem_size, A, B, C, D, split_k_mode=SplitKMode.Serial, **kwargs) -> None:
    ...
    def get_arguments(self):
        ...
        self.c_arguments = self.operation.argument_type(int(self.conv_kind), self.problem_size.ctype, int(to_device_ptr(self.ptr_A)), int(to_device_ptr(self.ptr_B)), int(to_device_ptr(self.ptr_C)), int(to_device_ptr(self.ptr_D)), tc_numel, self.output_op, int(self.split_k_mode))
...
class Conv2dRT(ExecutableOperation):
    ...
    def plan(self, arguments: Conv2dArguments):
        tile_size = GemmCoord(self.operation.tile_description.threadblock_shape[0], self.operation.tile_description.threadblock_shape[1], self.operation.tile_description.threadblock_shape[2])
        grid = self.get_grid_shape(int(self.conv_kind), arguments.problem_size.ctype, tile_size.ctype, arguments.split_k_slices)
...
class Conv2dOperation:
    ...
    :param element_epilogue: element type for computation in epilogue     :type element_epilogue: cutlass_library.library.DataType

    :param stride_support: distinguish among partial specializations that     accelerate certain problems where convolution stride is unit     :type stride_support: :class:`cutlass_library.library.StrideSupport`
    ...
    def __init__(self, conv_kind, iterator_algorithm, arch: int, tile_description: TileDescription, A: TensorDescription, B: TensorDescription, C: TensorDescription, stride_support, epilogue_functor, swizzling_functor=SwizzlingFunctor.Identity1, emission_type=EmissionType.Kernel, **kwargs):
    ...
    def configuration_name(self):
        ...
        threadblock = '%dx%d_%dx%d' % (self.tile_description.threadblock_shape[0], self.tile_description.threadblock_shape[1], self.tile_description.threadblock_shape[2], self.tile_description.stages)
        ...
        return SubstituteTemplate(configuration_name, {'arch': str(self.arch), 'opcode_class': opcode_class_name, 'extended_name': self.extended_name(), 'threadblock': threadblock, 'layout': self.layout_name(), 'alignment': '%d' % self.A.alignment})

    def extended_name(self):
        """Append data types if they differ from compute type."""
        if self.C.element != self.tile_description.math_instruction.element_accumulator and self.A.element != self.tile_description.math_instruction.element_accumulator:
    ...
    def core_name(self):
        ...
        if self.tile_description.math_instruction.opcode_class == OpcodeClass.TensorOp:
            ...
            if self.tile_description.math_instruction.element_a != self.A.element and self.tile_description.math_instruction.element_a != self.accumulator_type():
                intermediate_type = DataTypeNames[self.tile_description.math_instruction.element_a]
        else:
            inst_shape = ''
        return '%s%s%s%s_%s' % (ShortDataTypeNames[self.accumulator_type()], inst_shape, intermediate_type, ConvKindNames[self.conv_kind], IteratorAlgorithmNames[self.iterator_algorithm])
    ...
    def device_op(self):
        ...
        """
        return Conv2dOperation(self.conv_kind, self.iterator_algorithm, self.arch, self.tile_description, self.A, self.B, self.C, self.stride_support, self.epilogue_functor, self.swizzling_functor, emission_type=EmissionType.Device)

class EmitConv2dInstance:

    def __init__(self, operation_suffix=''):
        self.operation_suffix = operation_suffix
        self.includes = ['cutlass/cutlass.h', 'cutlass/conv/kernel/default_conv2d_fprop.h', 'cutlass/conv/kernel/default_conv2d_dgrad.h', 'cutlass/conv/kernel/default_conv2d_wgrad.h', 'cutlass/conv/device/implicit_gemm_convolution.h']
