...
class Gemm(OperationBase):
    ...
    def __init__(self, A=None, B=None, C=None, D=None, alpha=1.0, beta=0.0, element_accumulator=None, element=None, layout=None, element_A=None, element_B=None, element_C=None, element_D=None, layout_A=None, layout_B=None, layout_C=None, cc: int=None, kernel_cc: int=None):
        super().__init__(cc=cc, kernel_cc=kernel_cc)
        self.name = 'gemm'
        self.compiled = False
        elements = []
        layouts = []
        for elt, lay, tens, name in zip([element_A, element_B, element_C, element_D], [layout_A, layout_B, layout_C, layout_C], [A, B, C, D], ['A', 'B', 'C', 'D']):
    ...
    def construct(self, tile_description: TileDescription=None, alignment_A: int=None, alignment_B: int=None, alignment_C: int=None) -> GemmOperationUniversal:
        ...
        operation = GemmOperationUniversal(arch=self.current_cc, tile_description=tile_description, A=tensor_A, B=tensor_B, C=tensor_C, epilogue_functor=self.epilogue_functor, swizzling_functor=self._swizzling_functor)
        return operation

    def compile(self, tile_description: TileDescription=None, alignment_A: int=None, alignment_B: int=None, alignment_C: int=None, print_module: bool=False) -> cutlass.backend.GemmOperationUniversal:
        """
    ...
    def _verify_type_and_layout(self, tensor, ref_type, ref_layout, name):
        ...
        if dtype != ref_type or layout != ref_layout:
            try:
                ...
                raise Exception(f'Tensor {name} with type and layout ({dtype}, {layout}) does not match the expected type and layout of ({ref_type}, {ref_layout}) and transpose failed.')

    def run(self, A=None, B=None, C=None, D=None, alpha=None, beta=None, sync: bool=True, print_module: bool=False, visitor_args: dict=None, stream: cuda.CUstream=cuda.CUstream(0)) -> GemmArguments:
        """
        ...
        self.compile(self._tile_description, alignment_A=alignment_a, alignment_B=alignment_b, alignment_C=alignment_c, print_module=print_module)
        ...
        if mode == GemmUniversalMode.Gemm or batch_count == 1:
            ...
            kwargs = {'batch': batch_count, 'batch_strides': {'A': self._get_batch_stride(A), 'B': self._get_batch_stride(B), 'C': self._get_batch_stride(C), 'D': self._get_batch_stride(D)}}
        ...
        arguments = GemmArguments(operation=self.operation, problem_size=problem_size, A=A, B=B, C=C, D=D, output_op=output_op, gemm_mode=mode, **kwargs)
