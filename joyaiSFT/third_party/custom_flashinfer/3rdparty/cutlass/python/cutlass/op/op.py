...
class OperationBase:
    ...
    def opclass(self, oc: cutlass.OpcodeClass):
        ...
        if oc in self.possible_op_classes:
            ...
            raise Exception(f'Unsupported operation class {oc} for CC {self.cc} and data type combination ({self._element_a}, {self._element_b}, {self._element_accumulator}) and layout combination ({self._layout_a}, {self._layout_b}).')
        self.possible_operations = self.options.operations(self.op_class, self._element_a, self._element_b, self._element_accumulator, self._layout_a, self._layout_b, self._math_operation)
    ...
    def math_operation(self, mo: cutlass.MathOperation):
        ...
        if not self.specified_kernel_cc:
            ...
            raise Exception('CUTLASS 3.0 kernels do not use different math operations. To use 2.x kernels with a specific math operation, do not set the `kernel_cc`parameter when constructing the plan.')
    ...
    def _create_epilogue_functor_activation(self, activation):
        ...
        if not self.specified_kernel_cc:
            if self.current_cc == 90 and activation != identity:
                ...
                self._reset_operations(reset_epilogue=False)
        elif self.current_cc == 90 and activation != identity:
            raise Exception('Epilogues with elementwise fusion are not currently supported in the Python interface for 3.x kernels. To use 2.x kernels with fused elementwise epilogues, do not set the `kernel_cc` parameter when constructing the plan.')
        return get_activation_epilogue(activation, self._element_d, elements_per_access, self._element_accumulator, self._element_accumulator)
    ...
    def _reset_epilogue_functor_alignment(self, alignment, epilogue_functor):
        ...
        epilogue_functor = get_activation_epilogue(activation, self._element_d, alignment, self._element_accumulator, self._element_accumulator)
        return epilogue_functor
    ...
    def epilogue_visitor(self, visitor):
        ...
        for operation in self.possible_operations.all_operations:
            ...
            if td.epilogue_schedule not in [cutlass.EpilogueScheduleType.TmaWarpSpecialized, cutlass.EpilogueScheduleType.TmaWarpSpecializedCooperative]:
                continue
        ...
        if len(new_possible_operations.all_operations) == 0:
            raise RuntimeError('The epilogue consumes too much shared memory. No valid tile description is found in the generator.')
