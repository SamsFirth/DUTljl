...
from cutlass_library import ConvKind, ConvMode, DataTypeSize, IteratorAlgorithm, OperationKind, SplitKMode, StrideSupport
import cutlass
...
class Conv2d(OperationBase):
    ...
    def __init__(self, kind='fprop', A=None, B=None, C=None, D=None, alpha=1.0, beta=0.0, element=None, element_A=None, element_B=None, element_C=None, element_D=None, element_accumulator=None, cc: int=None, kernel_cc: int=None):
    ...
    def _reset_operations(self, reset_epilogue: bool=True):
        ...
        self.possible_op_classes = self.options.supporting_opclasses(self._element_a, self._element_b, self._element_accumulator, self._layout_a, self._layout_b, self._math_operation)
        if cutlass.OpcodeClass.TensorOp in self.possible_op_classes:
            ...
            if self._math_operation is not None:
                ...
                math_op_str = ''
            raise Exception(f'No kernel configuration found for supported data type and layout combination {datatype_comb}x{layout_comb}{math_op_str}')
        if reset_epilogue:
    ...
    def _propose_iterator_algorithm(self, problem_size, alignment_a, alignment_b) -> IteratorAlgorithm:
        ...
        if self.conv_kind == ConvKind.Fprop:
            ...
            if problem_size.K % alignment_a == 0 and problem_size.R <= 32 and (problem_size.S <= 32) and (problem_size.C % alignment_b == 0):
                return IteratorAlgorithm.Optimized
            else:
                return IteratorAlgorithm.Analytic
    ...
    def _validate_iterator_algorithm(self, iterator_algorithm, problem_size, alignment_a, alignment_b) -> bool:
        ...
        if self.conv_kind == ConvKind.Fprop:
            ...
            if iterator_algorithm == IteratorAlgorithm.Optimized:
                return problem_size.K % alignment_a == 0 and problem_size.R <= 32 and (problem_size.S <= 32) and (problem_size.C % alignment_b == 0)
        elif self.conv_kind == ConvKind.Wgrad:
            if iterator_algorithm == IteratorAlgorithm.Optimized:
    ...
    def construct(self, tile_description: TileDescription=None, alignment_A: int=None, alignment_B: int=None, alignment_C: int=None, iterator_algorithm: IteratorAlgorithm=None, stride_support=None, swizzling_functor: cutlass.swizzle=None, epilogue_functor=None) -> cutlass.backend.Conv2dOperation:
        """
        ...
        operation = Conv2dOperation(conv_kind=self.conv_kind, iterator_algorithm=iterator_algorithm, arch=self.current_cc, tile_description=tile_description, A=tensor_A, B=tensor_B, C=tensor_C, stride_support=stride_support, epilogue_functor=epilogue_functor, swizzling_functor=swizzling_functor)
        return operation

    def compile(self, tile_description: TileDescription=None, alignment_A: int=None, alignment_B: int=None, alignment_C: int=None, iterator_algorithm: IteratorAlgorithm=None, stride_support=None, swizzling_functor: cutlass.swizzle=None, epilogue_functor=None, print_module: bool=False) -> cutlass.backend.Conv2dOperation:
        """
        ...
        :rtype: cutlass.backend.Conv2dOperation
        """
        self.operation = self.construct(tile_description, alignment_A, alignment_B, alignment_C, iterator_algorithm, stride_support, swizzling_functor, epilogue_functor)
        if print_module:
    ...
    def _verify_type_and_layout(self, tensor, ref_type, ref_layout, name):
        ...
        if dtype != ref_type:
            raise Exception(f'Tensor {name} with type and layout {dtype} does not match the expected type of {ref_type}.')
    ...
    def _get_and_verify_conv_problem_size(self, A, B, C, stride, padding, dilation):
        ...
        problem_size = Conv2DProblemSize(N_, H_, W_, C_, K_, R_, S_, C_, padding[0], padding[1], stride[0], stride[1], dilation[0], dilation[1], ConvMode.CrossCorrelation, 1, 1)
        if P_ != problem_size.P or Q_ != problem_size.Q:
            raise Exception(f'Tensor {output_tensor} size should be ({N_}, {problem_size.P}, {problem_size.Q}, {K_}), got ({N_}, {P_}, {Q_}, {K_})')
        return problem_size

    def run(self, A=None, B=None, C=None, D=None, stride=(1, 1), padding=(0, 0), dilation=(1, 1), alpha=None, beta=None, split_k=('serial', 1), sync: bool=True, print_module: bool=False, stream: cuda.CUstream=cuda.CUstream(0)) -> Conv2dArguments:
        """
        ...
        elif self._validate_iterator_algorithm(self._iterator_algorithm, problem_size, alignment_a, alignment_b):
        ...
        if split_k[0] == 'parallel' and split_k[1] > 1:
            ...
            epilogue_functor = self.epilogue_functor
        self.compile(tile_description=self.tile_description, alignment_A=alignment_a, alignment_B=alignment_b, alignment_C=alignment_c, iterator_algorithm=iterator_algorithm, stride_support=stride_support, swizzling_functor=swizzling_functor, epilogue_functor=epilogue_functor, print_module=print_module)
        if split_k[0] == 'parallel' and split_k[1] > 1:
            epilogue_functor_reduction = self._reset_epilogue_functor_alignment(alignment_c, self.epilogue_functor)
            self.reduction_operation = ReductionOperation(shape=MatrixCoord(4, 32 * alignment_c), C=self.operation.C, element_accumulator=self._element_accumulator, element_compute=self._element_accumulator, epilogue_functor=epilogue_functor_reduction, count=alignment_c)
            if print_module:
                print(self.reduction_operation.rt_module.emit())
            compiler.add_module([self.reduction_operation])
        arguments = Conv2dArguments(operation=self.operation, problem_size=problem_size, A=A, B=B, C=C, D=D, output_op=self.operation.epilogue_type(*epilogue_args), split_k_mode=datatypes.getattr_enum(SplitKMode, split_k[0]), split_k_slices=split_k[1], stream=stream)
        self.operation.run(arguments)
        if split_k[0] == 'parallel' and split_k[1] > 1:
            implicit_gemm_size = arguments.problem_size.implicit_gemm_size(self.conv_kind)
            reduction_arguments = ReductionArguments(self.reduction_operation, problem_size=[implicit_gemm_size.m, implicit_gemm_size.n], partitions=split_k[1], workspace=arguments.ptr_D, destination=D, source=C, output_op=self.reduction_operation.epilogue_type(*epilogue_args), stream=stream)
    ...
    def output_size(input_size, weight_size, padding, stride, dilation):
        problem_size = Conv2DProblemSize(*input_size, *weight_size, padding[0], padding[1], stride[0], stride[1], dilation[0], dilation[1], ConvMode.CrossCorrelation, 1, 1)
        return (problem_size.N, problem_size.P, problem_size.Q, problem_size.K)

class Conv2dFprop(Conv2d):

    def __init__(self, input=None, weight=None, C=None, output=None, alpha=1, beta=0, element=None, element_input=None, element_weight=None, element_C=None, element_output=None, element_accumulator=None, cc: int=None, kernel_cc: int=None):
        A, B, D = (input, weight, output)
        element_A, element_B, element_D = (element_input, element_weight, element_output)
        super().__init__('fprop', A, B, C, D, alpha, beta, element, element_A, element_B, element_C, element_D, element_accumulator, cc, kernel_cc)

    def run(self, input=None, weight=None, C=None, output=None, alpha=None, beta=None, stride=(1, 1), padding=(0, 0), dilation=(1, 1), split_k=('serial', 1), sync: bool=True, print_module: bool=False, stream: cuda.CUstream=cuda.CUstream(0)) -> Conv2dArguments:
        A, B, D = (input, weight, output)
        return super().run(A, B, C, D, alpha, beta, stride, padding, dilation, split_k, sync, print_module, stream)

class Conv2dDgrad(Conv2d):

    def __init__(self, grad_output=None, weight=None, C=None, grad_input=None, alpha=1, beta=0, element=None, element_grad_output=None, element_weight=None, element_C=None, element_grad_input=None, element_accumulator=None, cc: int=None, kernel_cc: int=None):
        A, B, D = (grad_output, weight, grad_input)
        element_A, element_B, element_D = (element_grad_output, element_weight, element_grad_input)
        super().__init__('dgrad', A, B, C, D, alpha, beta, element, element_A, element_B, element_C, element_D, element_accumulator, cc, kernel_cc)

    def run(self, grad_output=None, weight=None, C=None, grad_input=None, alpha=None, beta=None, stride=(1, 1), padding=(0, 0), dilation=(1, 1), split_k=('serial', 1), sync: bool=True, print_module: bool=False, stream: cuda.CUstream=cuda.CUstream(0)) -> Conv2dArguments:
        A, B, D = (grad_output, weight, grad_input)
        return super().run(A, B, C, D, alpha, beta, stride, padding, dilation, split_k, sync, print_module, stream)

class Conv2dWgrad(Conv2d):

    def __init__(self, grad_output=None, input=None, C=None, grad_weight=None, alpha=1, beta=0, element=None, element_grad_output=None, element_input=None, element_C=None, element_grad_weight=None, element_accumulator=None, cc: int=None, kernel_cc: int=None):
        A, B, D = (grad_output, input, grad_weight)
        element_A, element_B, element_D = (element_grad_output, element_input, element_grad_weight)
        super().__init__('wgrad', A, B, C, D, alpha, beta, element, element_A, element_B, element_C, element_D, element_accumulator, cc, kernel_cc)

    def run(self, grad_output=None, input=None, C=None, grad_weight=None, alpha=None, beta=None, stride=(1, 1), padding=(0, 0), dilation=(1, 1), split_k=('serial', 1), sync: bool=True, print_module: bool=False, stream: cuda.CUstream=cuda.CUstream(0)) -> Conv2dArguments:
        A, B, D = (grad_output, input, grad_weight)
        return super().run(A, B, C, D, alpha, beta, stride, padding, dilation, split_k, sync, print_module, stream)