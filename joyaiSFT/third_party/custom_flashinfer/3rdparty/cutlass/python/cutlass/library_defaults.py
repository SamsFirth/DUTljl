...
class KernelsForDataType:
            ...
            if key not in self.kernels_by_alignment:
                ...
                for align_A, align_B, align_C in alignments:
                    if alignment_A % align_A == 0 and alignment_B % align_B == 0 and (alignment_C % align_C == 0):
                        ...
                        break
                if key is None:
                    raise Exception(f'No operations of alignment {og_key} found for data type and layout combination {self.datatype_comb} {self.layout_comb}. Compatible alignments are {self.kernels_by_alignment.keys()}')
    ...
    def sort(self):
        ...
        """
        key = lambda op: op.tile_description.threadblock_shape[0] * op.tile_description.threadblock_shape[1] * op.tile_description.threadblock_shape[2]
...
class ArchOptions:
    ...
    def __init__(self, target_cc: int, kernel_cc: int, operation_kind: cutlass_library.OperationKind, gemm_kinds: list, allowed_math_operations: list=[cutlass_library.MathOperation.multiply_add, cutlass_library.MathOperation.multiply_add_saturate, cutlass_library.MathOperation.multiply_add_mixed_input_upcast, cutlass_library.MathOperation.multiply_add_fast_f32]):
        ...
        if len(manifest.operations[operation_kind].keys()) != 1 or kernel_cc not in manifest.operations[operation_kind]:
            raise Exception(f'Error finding kernels for SM{kernel_cc}. Check that your CUDA toolkit version is sufficient for the architecture in question.')
            ...
            for op in op_list:
                ...
                if datatype_comb == (cutlass_library.DataType.tf32, cutlass_library.DataType.tf32, cutlass_library.DataType.f32):
                    if self.cc < 80:
                        ...
                        if op.A.element != cutlass_library.DataType.f32 or op.B.element != cutlass_library.DataType.f32 or op.C.element != cutlass_library.DataType.f32:
                            continue
        ...
        if operation_kind == cutlass_library.OperationKind.Gemm:
            types = [(cutlass_library.DataType.s8, cutlass_library.DataType.s8, cutlass_library.DataType.s8), (cutlass_library.DataType.s8, cutlass_library.DataType.s8, cutlass_library.DataType.s32), (cutlass_library.DataType.f16, cutlass_library.DataType.f16, cutlass_library.DataType.f16), (cutlass_library.DataType.f16, cutlass_library.DataType.f16, cutlass_library.DataType.f32), (cutlass_library.DataType.f32, cutlass_library.DataType.f32, cutlass_library.DataType.f32), (cutlass_library.DataType.f64, cutlass_library.DataType.f64, cutlass_library.DataType.f64)]
            ...
            layouts = [(cutlass_library.LayoutType.RowMajor, cutlass_library.LayoutType.RowMajor), (cutlass_library.LayoutType.RowMajor, cutlass_library.LayoutType.ColumnMajor), (cutlass_library.LayoutType.ColumnMajor, cutlass_library.LayoutType.RowMajor), (cutlass_library.LayoutType.ColumnMajor, cutlass_library.LayoutType.ColumnMajor)]
        elif operation_kind == cutlass_library.OperationKind.Conv2d:
            types = [(cutlass_library.DataType.f16, cutlass_library.DataType.f16, cutlass_library.DataType.f16), (cutlass_library.DataType.f16, cutlass_library.DataType.f16, cutlass_library.DataType.f32), (cutlass_library.DataType.f32, cutlass_library.DataType.f32, cutlass_library.DataType.f32), (cutlass_library.DataType.f64, cutlass_library.DataType.f64, cutlass_library.DataType.f64)]
            layouts = [(cutlass_library.LayoutType.TensorNHWC, cutlass_library.LayoutType.TensorNHWC)]
        else:
        ...
        for type_comb in types:
            for layout_comb in layouts:
                ...
                math_inst = cutlass_library.MathInstruction([1, 1, 1], type_comb[0], type_comb[1], type_comb[2], cutlass_library.OpcodeClass.Simt, cutlass_library.MathOperation.multiply_add)
                ...
                if operation_kind == cutlass_library.OperationKind.Gemm:
                    new_operation = cutlass_library.manifest.GemmOperation(cutlass_library.GemmKind.Universal, td.minimum_compute_capability, td, A, B, C, type_comb[2], epilogue_functor, swizzling_functor)
                    new_kernels.add(new_operation)
                elif operation_kind == cutlass_library.OperationKind.Conv2d:
                    for conv_kind in [ConvKind.Fprop, ConvKind.Dgrad, ConvKind.Wgrad]:
                        new_operation = cutlass_library.manifest.Conv2dOperation(conv_kind, IteratorAlgorithm.Analytic, td.minimum_compute_capability, td, A, B, C, type_comb[2], StrideSupport.Strided, epilogue_functor, swizzling_functor, group_mode=GroupMode.SingleGroup)
                        new_kernels.add(new_operation)
    ...
    def opclass_supports_combination(self, op_class: cutlass_library.OpcodeClass, datatype_comb: tuple, layout_comb: tuple, math_operation: cutlass_library.MathOperation) -> bool:
        """
    ...
    def supporting_opclasses(self, element_a: cutlass_library.DataType, element_b: cutlass_library.DataType, element_accumulator: cutlass_library.DataType, layout_a: cutlass_library.LayoutType, layout_b: cutlass_library.LayoutType, math_operation: cutlass_library.MathOperation) -> set:
        """
    ...
    def operations(self, op_class: cutlass_library.OpcodeClass, element_a: cutlass_library.DataType, element_b: cutlass_library.DataType, element_accumulator: cutlass_library.DataType, layout_a: cutlass_library.LayoutType, layout_b: cutlass_library.LayoutType, math_operation: cutlass_library.MathOperation) -> KernelsForDataType:
        ...
        if not self.opclass_supports_combination(op_class, datatype_comb, layout_comb, math_operation):
            raise Exception(f'Data type layout combination {datatype_comb}, {layout_comb} is not supported by opcode class {op_class} on CC {self.cc}.')
