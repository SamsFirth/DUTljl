...
import pytest
from mscclpp import DataType, EndpointConfig, ExecutionPlan, Executor, Fifo, Host2DeviceSemaphore, Host2HostSemaphore, ProxyService, SmDevice2DeviceSemaphore, TcpBootstrap, Transport, is_nvls_supported
...
def test_connection_write(mpi_group: MpiGroup, transport: Transport, nelem: int):
    ...
    for rank in connections:
        connections[rank].write(all_reg_memories[rank], sizePerRank * group.my_rank, all_reg_memories[group.my_rank], sizePerRank * group.my_rank, sizePerRank)
    poll_for = 100
...
def test_connection_write_and_signal(mpi_group: MpiGroup, transport: Transport, nelem: int, device: str):
    ...
    if group.my_rank == 0:
        ...
        if device == 'cuda':
            cp.cuda.runtime.deviceSynchronize()
    connections[next_rank].update_and_sync(all_signal_memories[next_rank], 0, dummy_memory_on_cpu.ctypes.data, signal_val)
    all_correct = False
    if group.my_rank == 0:
        while signal_memory[0] != signal_val:
            time.sleep(0.1)
...
class MscclppKernel:
    ...
    def __init__(self, test_name, my_rank=None, nranks=None, semaphore_or_channels=None, tensor=None, use_packet=False, scratch=None, fifo=None, nvls_mem_handle=None, nvls_buffer_size=None):
        file_dir = os.path.dirname(os.path.abspath(__file__))
        if test_name == 'h2d_semaphore':
            self._kernel = KernelBuilder(file='h2d_semaphore_test.cu', kernel_name='h2d_semaphore', file_dir=file_dir).get_compiled_kernel()
            self.nblocks = 1
            self.nthreads = nranks
        elif test_name == 'd2d_semaphore':
            self._kernel = KernelBuilder(file='d2d_semaphore_test.cu', kernel_name='d2d_semaphore', file_dir=file_dir).get_compiled_kernel()
            self.nblocks = 1
            self.nthreads = nranks
        elif test_name == 'sm_channel':
            self._kernel = KernelBuilder(file='sm_channel_test.cu', kernel_name='sm_channel', file_dir=file_dir).get_compiled_kernel()
            self.nblocks = nranks
            self.nthreads = 1024
        elif test_name == 'fifo':
            self._kernel = KernelBuilder(file='fifo_test.cu', kernel_name='fifo', file_dir=file_dir).get_compiled_kernel()
            self.nblocks = 1
            self.nthreads = 1
        elif test_name == 'proxy':
            self._kernel = KernelBuilder(file='proxy_test.cu', kernel_name='proxy', file_dir=file_dir).get_compiled_kernel()
            self.nblocks = 1
            self.nthreads = nranks
        elif test_name == 'simple_proxy_channel':
            self._kernel = KernelBuilder(file='simple_proxy_channel_test.cu', kernel_name='simple_proxy_channel', file_dir=file_dir).get_compiled_kernel()
            self.nblocks = 1
            self.nthreads = 1024
        elif test_name == 'nvls':
            self._kernel = KernelBuilder(file='nvls_test.cu', kernel_name='nvls_test', file_dir=file_dir).get_compiled_kernel()
            self.nblocks = 64
            self.nthreads = 1024
        else:
            assert False
        ...
        if semaphore_or_channels != None:
            ...
            for rank in range(nranks):
                if rank == my_rank:
                    device_handles.append(bytes(size_of_semaphore_or_channels))
                else:
...
def test_proxy(mpi_group: MpiGroup, nelem: int, transport: str):
    ...
    kernel = MscclppKernel('proxy', my_rank=group.my_rank, nranks=group.nranks, semaphore_or_channels=semaphores, fifo=fifo_device_handle)
    proxy.start()
    group.barrier()
    kernel()
    cp.cuda.runtime.deviceSynchronize()
...
def test_simple_proxy_channel(mpi_group: MpiGroup, nelem: int, transport: str, use_packet: bool):
    ...
    if use_packet:
        ...
        scratch = cp.zeros(1, dtype=cp.int32)
    nelemPerRank = nelem // group.nranks
    nelemPerRank * memory.itemsize
    memory[nelemPerRank * group.my_rank:nelemPerRank * (group.my_rank + 1)] = group.my_rank + 1
    memory_expected = cp.zeros_like(memory)
    ...
    kernel = MscclppKernel('simple_proxy_channel', my_rank=group.my_rank, nranks=group.nranks, semaphore_or_channels=simple_channels, tensor=memory, use_packet=use_packet, scratch=scratch)
...
def test_nvls(mpi_group: MpiGroup):
    ...
    kernel = MscclppKernel('nvls', my_rank=group.my_rank, nranks=group.nranks, nvls_mem_handle=mem_handle, nvls_buffer_size=nbytes, semaphore_or_channels=semaphores)
    kernel()
    cp.cuda.runtime.deviceSynchronize()
    group.barrier()
...
def test_executor(mpi_group: MpiGroup, filename: str):
    ...
    executor.execute(mpi_group.comm.rank, sendbuf.data.ptr, sendbuf.data.ptr, sendbuf.nbytes, sendbuf.nbytes, DataType.float16, 512, execution_plan, stream.ptr)
    stream.synchronize()
    assert cp.allclose(sendbuf, expected, atol=0.001 * mpi_group.comm.size)