...
class Kernel:
    ...
    CU_LAUNCH_PARAM_END = 0 if not cp.cuda.runtime.is_hip else 3
    ...
    def launch_kernel(self, params: bytes, nblocks: int, nthreads: int, shared: int, stream: Type[cp.cuda.Stream] or Type[None]):
        buffer = (ctypes.c_byte * len(params)).from_buffer_copy(params)
        buffer_size = ctypes.c_size_t(len(params))
        config = np.array([Kernel.CU_LAUNCH_PARAM_BUFFER_POINTER, ctypes.addressof(buffer), Kernel.CU_LAUNCH_PARAM_BUFFER_SIZE, ctypes.addressof(buffer_size), Kernel.CU_LAUNCH_PARAM_END], dtype=np.uint64)
        cuda_stream = 0
        if stream:
            cuda_stream = stream.ptr if isinstance(stream, cp.cuda.Stream) else stream.cuda_stream
        cp.cuda.driver.launchKernel(self._kernel, nblocks, 1, 1, nthreads, 1, 1, shared, cuda_stream, 0, config.ctypes.data)
...
class KernelBuilder:
    ...
    def _compile_cuda(self, source_file, output_file, std_version='c++17'):
        ...
        if not cp.cuda.runtime.is_hip:
            ...
            command = [nvcc, f'-std={std_version}', '-ptx', '-Xcompiler', '-Wall,-Wextra', f'-I{include_dir}', f'{source_file}', f'--gpu-architecture=compute_{compute_capability}', f'--gpu-code=sm_{compute_capability},compute_{compute_capability}', '-o', f'{self._tempdir.name}/{output_file}']
        else:
            gcn_arch = cp.cuda.runtime.getDeviceProperties(cp.cuda.Device().id)['gcnArchName'].decode('utf-8').split(':')[0]
            rocm_home = os.environ.get('ROCM_HOME')
            hipcc = os.path.join(rocm_home, 'bin/hipcc') if rocm_home else 'hipcc'
            command = [hipcc, f'-std={std_version}', '--genco', '-D__HIP_PLATFORM_AMD__', f'--offload-arch={gcn_arch}', f'-I{include_dir}', f'{source_file}', '-o', f'{self._tempdir.name}/{output_file}']
        if self.macros:
            command += self.macros
        try:
