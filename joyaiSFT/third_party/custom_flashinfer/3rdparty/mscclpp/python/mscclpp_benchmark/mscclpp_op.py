...
IB_TRANSPORTS = [Transport.IB0, Transport.IB1, Transport.IB2, Transport.IB3, Transport.IB4, Transport.IB5, Transport.IB6, Transport.IB7]
...
class MscclppAllReduce1:
    ...
    def __init__(self, group: mscclpp_comm.CommGroup, memory: cp.ndarray, read_only: int=1, block_size: int=1024, nblocks: int=24):
        self.group = group
        self.memory = memory
        ...
        self.kernel = KernelBuilder(file='allreduce.cu', kernel_name='allreduce1', file_dir=file_dir, macro_dict={'TYPE': type_str}).get_compiled_kernel()
        self.device_handles = []
    ...
    def set_params(self, nblocks, block_size, read_only):
        ...
        self.params = b''
        self.params += pack(self.device_handles_cp, self.memory, self.group.my_rank, self.group.nranks, ctypes.c_size_t(self.memory.size), self.read_only)
...
class MscclppAllReduce2:
    ...
    def __init__(self, group: mscclpp_comm.CommGroup, memory: cp.ndarray, memory_out: cp.ndarray, block_size: int=512, nblocks: int=21):
        ...
        file_dir = os.path.dirname(os.path.abspath(__file__))
        self.kernel = KernelBuilder(file='allreduce.cu', kernel_name='allreduce2', file_dir=file_dir, macro_dict={'TYPE': type_str}).get_compiled_kernel()
        self.device_handles = []
        for rank in range(self.group.nranks):
            if rank != self.group.my_rank:
                self.device_handles.append(self.sm_channels[rank].device_handle().raw)
    ...
    def set_params(self, nblocks, block_size):
        ...
        self.block_size = block_size
        self.params = b''
        self.params += pack(self.device_handles_cp, self.memory, self.scratch, self.memory_out, self.group.my_rank, self.group.nranks, ctypes.c_size_t(self.memory.size))
...
class MscclppAllReduce3:
    ...
    def __init__(self, group: mscclpp_comm.CommGroup, memory: cp.ndarray, proxy_service: ProxyService, block_size: int=1024, nblocks: int=24):
        ...
        file_dir = os.path.dirname(os.path.abspath(__file__))
        self.kernel = KernelBuilder(file='allreduce.cu', kernel_name='allreduce3', file_dir=file_dir, macro_dict={'TYPE': type_str}).get_compiled_kernel()
    ...
    def set_params(self, nblocks, block_size):
        ...
        self.block_size = block_size
        self.params = b''
        self.params += pack(self.fst_device_handles_cp, self.snd_device_handles_cp, self.memory, self.scratch, self.group.my_rank, self.group.nranks, ctypes.c_size_t(self.memory.size))
...
class MscclppAllReduce4:
    ...
    def __init__(self, group: mscclpp_comm.CommGroup, memory: cp.ndarray, nranks_per_node: int, proxy_service: ProxyService, nblocks: int=45, block_size: int=512, pipeline_depth: int=3):
        self.group = group
        self.memory = memory
        ...
        self.reduce_scatter_proxy_channels = self.group.make_proxy_channels_with_scratch(self.proxy_service, self.memory, self.scratch, self.connections)
        self.all_gather_proxy_channels = self.group.make_proxy_channels(self.proxy_service, self.memory, self.connections)
        file_dir = os.path.dirname(os.path.abspath(__file__))
        self.kernel = KernelBuilder(file='allreduce.cu', kernel_name='allreduce4', file_dir=file_dir, macro_dict={'TYPE': type_str}).get_compiled_kernel()
        ...
        for rank in range(self.group.nranks):
            ...
            if rank != self.group.my_rank:
                self.reduce_sactter_proxy_device_handles.append(self.reduce_scatter_proxy_channels[rank].device_handle().raw)
        ...
        self.reduce_sactter_proxy_device_handles_cp = cp.asarray(memoryview(b''.join(self.reduce_sactter_proxy_device_handles)), dtype=cp.uint8)
        self.all_gather_proxy_device_handles_cp = cp.asarray(memoryview(b''.join(self.all_gather_proxy_device_handles)), dtype=cp.uint8)
    ...
    def set_params(self, nblocks, block_size, pipeline_depth):
        ...
        self.params = b''
        self.params += pack(self.sm_device_handles_cp, self.reduce_sactter_proxy_device_handles_cp, self.all_gather_proxy_device_handles_cp, self.memory, self.scratch, self.group.my_rank, self.nranks_per_node, self.group.nranks, bytes(4), ctypes.c_size_t(self.memory.size), self.pipeline_depth)
...
class MscclppAllReduce5:
    ...
    def __init__(self, group: mscclpp_comm.CommGroup, memory: cp.ndarray, memory_out: cp.ndarray, nranks_per_node: int, proxy_service: ProxyService, nblocks: int=21, block_size: int=512):
        self.group = group
        self.memory = memory
        self.memory_out = memory_out
        self.nranks_per_node = nranks_per_node
        ...
        self.proxy_channels = self.group.make_proxy_channels_with_scratch(self.proxy_service, self.put_buff, self.scratch, across_node_connections)
        file_dir = os.path.dirname(os.path.abspath(__file__))
        self.kernel = KernelBuilder(file='allreduce.cu', kernel_name='allreduce5', file_dir=file_dir, macro_dict={'TYPE': type_str}).get_compiled_kernel()
        self.sm_device_handles = []
    ...
    def set_params(self, nblocks, block_size):
        ...
        self.block_size = block_size
        self.params = b''
        self.params += pack(self.sm_device_handles_cp, self.proxy_device_handles_cp, self.memory, self.scratch, self.put_buff, self.memory_out, self.group.my_rank, self.nranks_per_node, self.group.nranks, bytes(4), ctypes.c_size_t(self.memory.size))
...
class MscclppAllReduce6:
    ...
    def __init__(self, group: mscclpp_comm.CommGroup, nelem: int, memory_dtype: cp.dtype, block_size: int=1024, nblocks: int=32):
        self.group = group
        ...
        self.nvls_mem_handle = self.nvls_connection.allocate_bind_memory(aligned_buffer_size)
        ...
        file_dir = os.path.dirname(os.path.abspath(__file__))
        self.kernel = KernelBuilder(file='allreduce.cu', kernel_name='allreduce6', file_dir=file_dir, macro_dict={'TYPE': type_str}).get_compiled_kernel()
        self.device_handles = []
        for rank in range(self.group.nranks):
            if rank != self.group.my_rank:
    ...
    def set_params(self, nblocks, block_size):
        ...
        self.block_size = block_size
        self.params = b''
        self.params += pack(self.device_handles_cp, self.nvls_handle, self.memory, self.group.my_rank, self.group.nranks, ctypes.c_size_t(self.memory.size))
