...
def check_correctness(memory, func, niter=100):
    ...
    for p in range(niter):
        ...
        is_close = cp.isclose(output_memory, expected, rtol=0.01, atol=2)
        icf = is_close == 0
...
def run_benchmark(mscclpp_group: mscclpp_comm.CommGroup, nccl_op: nccl.NcclCommunicator, table: PrettyTable, niter: int, nelem: int):
    ...
    if MPI.COMM_WORLD.size // N_GPUS_PER_NODE == 1:
        if memory.nbytes < 2 ** 20:
            ...
            mscclpp_algos = [MscclppAllReduce1(mscclpp_group, memory), MscclppAllReduce3(mscclpp_group, memory, proxy_service)]
            if is_nvls_supported():
                mscclpp_algos.append(MscclppAllReduce6(mscclpp_group, nelem, data_type))
    elif memory.nbytes < 2 ** 22:
    ...
    mscclpp_algBw = memory_nbytes / mscclpp_time / 1000.0
    mscclpp_check = 'PASS' if check_correctness(memory, mscclpp_call) else 'FAIL'
    nccl_time = bench_time(niter, nccl_call)
    nccl_algBw = memory_nbytes / nccl_time / 1000.0
    nccl_check = 'PASS' if check_correctness(memory, nccl_call) else 'FAIL'
    MPI.COMM_WORLD.barrier()
    proxy_service.stop_proxy()
    speed_up = nccl_time / mscclpp_time
    if MPI.COMM_WORLD.rank == 0:
        table.add_row([human_readable_size(memory_nbytes), '{:.2f}'.format(mscclpp_time), '{:.2f}'.format(mscclpp_algBw), mscclpp_check, '{:.2f}'.format(nccl_time), '{:.2f}'.format(nccl_algBw), nccl_check, '{:.2f}'.format(speed_up)])
    if MPI.COMM_WORLD.rank == 0:
...
if __name__ == '__main__':
    ...
    ifIpPortTrio = network_interface + ':' + root_ip + ':50000'
    mscclpp_group = mscclpp_comm.CommGroup(interfaceIpPortTrio=ifIpPortTrio, rank=MPI.COMM_WORLD.rank, size=MPI.COMM_WORLD.size)
    if MPI.COMM_WORLD.rank == 0:
