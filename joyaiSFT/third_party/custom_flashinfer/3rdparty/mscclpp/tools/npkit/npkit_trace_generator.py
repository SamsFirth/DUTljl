...
def parse_gpu_clock_scale(gpu_clock_file_path):
    with open(gpu_clock_file_path, 'r') as f:
        freq_in_khz = f.read()
        return float(freq_in_khz) * 1000.0 / 1000000.0
...
def parse_cpu_clock_scale(cpu_clock_den_file_path, cpu_clock_num_file_path):
    ...
    with open(cpu_clock_den_file_path, 'r') as f:
        den = float(f.read())
    return den / num / 1000000.0

def parse_gpu_event(event_bytes):
    return {'id': int.from_bytes(event_bytes[0:1], byteorder='little', signed=False), 'size': int.from_bytes(event_bytes[1:5], byteorder='little', signed=False), 'rsvd': int.from_bytes(event_bytes[5:8], byteorder='little', signed=False), 'timestamp': int.from_bytes(event_bytes[8:16], byteorder='little', signed=False)}

def parse_cpu_event(event_bytes):
    return {'id': int.from_bytes(event_bytes[0:1], byteorder='little', signed=False), 'size': int.from_bytes(event_bytes[1:5], byteorder='little', signed=False), 'slot': int.from_bytes(event_bytes[5:8], byteorder='little', signed=False), 'timestamp': int.from_bytes(event_bytes[8:16], byteorder='little', signed=False)}
    ...
    with open(gpu_event_file_path, 'rb') as f:
        ...
        while raw_content_idx < raw_content_size:
            ...
            if npkit_event_def['id_to_type'][parsed_gpu_event['id']] == 'NPKIT_EVENT_TIME_SYNC_CPU':
                ...
                gpu_events.append({'ph': phase, 'ts': curr_cpu_base_time + parsed_gpu_event['timestamp'] / gpu_clock_scale - curr_gpu_base_time, 'pid': rank, 'tid': buf_idx + 1})
                if phase == 'B':
                    if event_type not in event_type_to_seq:
                        event_type_to_seq[event_type] = 0
                    gpu_events[-1].update({'name': trim_event_name(event_type), 'cat': 'GPU', 'args': {'rank': rank, 'buf_idx': buf_idx, 'seq': event_type_to_seq[event_type], 'rsvd_0': parsed_gpu_event['rsvd'], 'size_0': parsed_gpu_event['size']}})
                    ...
                    gpu_events[-1]['args']['bw (GB/s)'] = gpu_events[-1]['args']['size'] / delta_time / 1000.0
    ...
    with open(cpu_event_file_path, 'rb') as f:
        ...
        while raw_content_idx < raw_content_size:
            ...
            if phase == 'B':
                ...
                if event_type not in event_type_to_seq:
                    event_type_to_seq[event_type] = 0
                cpu_events[-1].update({'name': trim_event_name(event_type), 'cat': 'CPU', 'args': {'rank': rank, 'channel': channel, 'slot': parsed_cpu_event['slot'], 'seq': event_type_to_seq[event_type], 'size_0': parsed_cpu_event['size']}})
                event_type_to_seq[event_type] += 1
            else:
                ...
                cpu_events[-1]['args']['bw (GB/s)'] = cpu_events[-1]['args']['size'] / delta_time / 1000.0
