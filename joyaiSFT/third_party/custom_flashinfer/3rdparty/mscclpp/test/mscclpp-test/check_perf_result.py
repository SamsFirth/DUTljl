...
def load_perf_file(perf_fine: str) -> dict:
    ...
    with open(perf_fine, 'r') as f:
        for line in f:
            ...
            res[data['name'], data['kernel'], data['ranks'], data['ranksPerNode'], data['size']] = {'algBw': data['algBw'], 'busBw': data['busBw'], 'time': data['time']}
            if 'target' in data:
                res[data['name'], data['kernel'], data['ranks'], data['ranksPerNode'], data['size']]['target'] = data['target']
    return res
...
def check_perf_result(perf_result: dict, baseline: dict, time_threshold: float, bandwidth_threshold: float) -> bool:
    ...
    for key, value in perf_result.items():
        ...
        if abs(value['time'] - baseline[key]['time']) / baseline[key]['time'] > threshold:
            logging.error('%s: time %f not match baseline %f with threshold %f', str(key), value['time'], baseline[key]['time'], threshold)
            res = False
    return res
