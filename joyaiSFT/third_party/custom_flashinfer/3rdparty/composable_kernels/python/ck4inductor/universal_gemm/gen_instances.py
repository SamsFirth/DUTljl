...
def _ck_library_dir():
    gemm_instances_path = os.path.join(library_path(), 'src', 'tensor_operation_instance', 'gpu', 'gemm_universal')
    ...
    for line in str_instances:
        ...
        while i_current < len(s_template_args):
            if s_template_args[i_current] == ' ':
                ...
                template_args.append(tuple(map(int, s_template_args[i_current + 2:i_next].split(','))))
                i_current = i_next + 2
            else:
                i_next = s_template_args.find(',', i_current)
                template_args.append(maybe_int(s_template_args[i_current:i_next if i_next != -1 else None]))
                if i_next != -1:
                    i_current = i_next + 1
            if i_next == -1:
                break
        new_instance = CKGemmOperation(*template_args, *(None,) * (len(fields(CKGemmOperation)) - len(template_args)))
...
def gen_ops_library() -> List[CKGemmOperation]:
    ...
    if not ck_library_dir:
        return []
    grep_result = subprocess.run(['grep', '-inR', 'DeviceGemm_Xdl_CShuffleV3', _ck_library_dir()], capture_output=True, text=True)
    op_instances = parse_instances(grep_result.stdout.strip().split('\n'))
    log.debug('ck instances from library: %d', len(op_instances))
    schedulers = ['BlockGemmPipelineScheduler::Intrawave', 'BlockGemmPipelineScheduler::Interwave']
    gemm_specs = ['GemmSpecialization::Default', 'GemmSpecialization::MPadding', 'GemmSpecialization::NPadding', 'GemmSpecialization::KPadding', 'GemmSpecialization::MNPadding', 'GemmSpecialization::MKPadding', 'GemmSpecialization::NKPadding', 'GemmSpecialization::MNKPadding']
    ...
    for instance in op_instances:
        ...
        for scheduler in schedulers_range:
            for spec in spec_range:
                substitute_instances.append(replace(instance, block_gemm_pipeline_scheduler=scheduler, gemm_specialization=spec))
