...
class CKGemmOperation:
    ...
    def key_name(self):
        return '_'.join(['K' + field_name.replace('_', '').lower() + 'V' + ('x'.join(map(str, iter(field_value))) if isinstance(field_value, tuple) else str(field_value).replace(':', '')) for field_name, field_value in self.dict_items()])
