    ...
    if 'perf_gemm.log' in logfile:
        for line in open(logfile):
            if 'Best Perf' in line:
                ...
                elif len(lst) < 37 and len(lst) >= 33:
                    tests.append(glue.join(lst[5:30]))
                    kernels.append('N/A')
                    tflops.append(lst[33])
                    dtype.append(lst[5])
    ...
    if not baseline.empty:
        ...
        for i in range(len(base_list)):
            if base_list[i] > 1.01 * float(test[i]):
                print('test # ', i, 'shows regression by {:.3f}%'.format((float(test[i]) - base_list[i]) / base_list[i] * 100))
                regression = 1
            if base_list[i] > 0:
...
def main():
    ...
    with SSHTunnelForwarder((ssh_host, ssh_port), ssh_username=ssh_user, ssh_password=ssh_pass, remote_bind_address=(sql_hostname, sql_port)) as tunnel:
        sqlEngine = sqlalchemy.create_engine('mysql+pymysql://{0}:{1}@{2}:{3}/{4}'.format(sql_username, sql_password, sql_hostname, tunnel.local_bind_port, sql_main_database))
