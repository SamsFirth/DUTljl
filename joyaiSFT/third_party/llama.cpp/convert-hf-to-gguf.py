    ...
    def write_tensors(self):
        ...
        for name, data_torch in self.get_tensors():
            ...
            for new_name, data in ((n, d.squeeze().numpy()) for n, d in self.modify_tensors(data_torch, name, bid)):
                ...
                extra_f32 = any((cond for cond in (extra_f32, n_dims == 1, new_name.endswith('_norm.weight'))))
                extra_f32 = extra_f32 or any((self.match_model_tensor_name(new_name, key, bid) for key in (gguf.MODEL_TENSOR.FFN_GATE_INP, gguf.MODEL_TENSOR.POS_EMBD, gguf.MODEL_TENSOR.TOKEN_TYPES)))
                extra_f16 = any((cond for cond in (extra_f16, name.endswith('.weight') and n_dims >= 2)))
...
class RefactModel(Model):
    ...
    def set_vocab(self):
        ...
        special_vocab._set_special_token('fsep', 4)
        special_vocab.add_to_gguf(self.gguf_writer)
...
class InternLM2Model(Model):
    ...
    def set_vocab(self):
        ...
        if 'chat' in os.path.basename(self.dir_model.absolute()):
            ...
            logger.warning(f"Replace eos:{old_eos} with a special token:{special_vocab.special_token_ids['eos']} in chat mode so that the conversation can end normally.")
        special_vocab.add_to_gguf(self.gguf_writer)
    ...
    def modify_tensors(self, data_torch: Tensor, name: str, bid: int | None) -> Iterable[tuple[str, Tensor]]:
        ...
        qkv_pattern = 'model\\.layers\\.(\\d+)\\.attention\\.wqkv'
...
class MambaModel(Model):
    ...
    def extra_f32_tensors(self, name: str, new_name: str, bid: int | None, n_dims: int) -> bool:
        del n_dims
        return bid is not None and new_name in (self.format_tensor_name(n, bid, '.weight' if name.endswith('.weight') else '') for n in [gguf.MODEL_TENSOR.SSM_CONV1D, gguf.MODEL_TENSOR.SSM_X, gguf.MODEL_TENSOR.SSM_DT, gguf.MODEL_TENSOR.SSM_A, gguf.MODEL_TENSOR.SSM_D])
...
@Model.register('ChatrhinoV0ForCausalLM')
class ChatrhinoV0Model(Model):
...
class LazyTorchTensor(gguf.LazyBase):
    ...
    def numpy(self) -> gguf.LazyNumpyTensor:
        ...
        return gguf.LazyNumpyTensor(meta=gguf.LazyNumpyTensor.meta_with_dtype_and_shape(dtype, self.shape), lazy=self._lazy, args=(self,), func=lambda s: s[0].numpy())
