...
def get_members_structure(cls, rule_name):
    ...
    if cls.__annotations__ and cls.__annotations__ != {}:
        result = f'{rule_name} ::= "{{"'
        members = [f'  "\\"{name}\\"" ":"  {map_pydantic_type_to_gbnf(param_type)}' for name, param_type in cls.__annotations__.items() if name != 'self']
        result += '"," '.join(members)
        result += '  "}"'
        return result
...
def generate_gbnf_rule_for_type(model_name, field_name, field_type, is_optional, processed_models, created_rules, field_info=None) -> tuple[str, list[str]]:
    ...
    if isclass(field_type) and issubclass(field_type, BaseModel):
        ...
        gbnf_type, rules = (model_name + '-' + field_name, rules)
    elif get_origin(field_type) == set or field_type == set:
        element_type = get_args(field_type)[0]
        element_rule_name, additional_rules = generate_gbnf_rule_for_type(model_name, f'{field_name}-element', element_type, is_optional, processed_models, created_rules)
        rules.extend(additional_rules)
        array_rule = f'{model_name}-{field_name} ::= "[" ws {element_rule_name} ("," ws {element_rule_name})*  "]" '
    ...
    elif isclass(field_type) and issubclass(field_type, float) and field_info and hasattr(field_info, 'json_schema_extra') and (field_info.json_schema_extra is not None):
    ...
    elif isclass(field_type) and issubclass(field_type, int) and field_info and hasattr(field_info, 'json_schema_extra') and (field_info.json_schema_extra is not None):
        max_digits = field_info.json_schema_extra.get('max_digit') if field_info and hasattr(field_info, 'json_schema_extra') else None
        min_digits = field_info.json_schema_extra.get('min_digit') if field_info and hasattr(field_info, 'json_schema_extra') else None
