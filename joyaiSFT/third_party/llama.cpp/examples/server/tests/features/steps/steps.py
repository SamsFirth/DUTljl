...
async def step_wait_for_the_server_to_be_started(context, expecting_status):
    match expecting_status:
        case 'healthy':
            await wait_for_health_status(context, context.base_url, 200, 'ok', timeout=30)
        case 'ready' | 'idle':
            await wait_for_health_status(context, context.base_url, 200, 'ok', timeout=30, params={'fail_on_no_slot': 0, 'include_slots': 0}, slots_idle=context.n_slots, slots_processing=0, expected_slots=[{'id': slot_id, 'state': 0} for slot_id in range(context.n_slots if context.n_slots else 1)])
        case 'busy':
            await wait_for_health_status(context, context.base_url, 503, 'no slot available', params={'fail_on_no_slot': 0, 'include_slots': 0}, slots_idle=0, slots_processing=context.n_slots, expected_slots=[{'id': slot_id, 'state': 1} for slot_id in range(context.n_slots if context.n_slots else 1)])
        case _:
...
async def step_all_slots_status(context, expected_slot_status_string):
    match expected_slot_status_string:
        ...
        case _:
            assert False, 'unknown status'
    expected_slots = [{'id': slot_id, 'state': expected_slot_status} for slot_id in range(context.n_slots)]
...
async def step_request_completion(context, api_error):
    ...
    completion = await request_completion(context.prompts.pop(), seeds[0] if seeds is not None else seeds, context.base_url, debug=context.debug, n_predict=context.n_predict, cache_prompt=context.cache_prompt, id_slot=context.id_slot, expect_api_error=expect_api_error, user_api_key=context.user_api_key, temperature=context.temperature)
...
async def step_oai_chat_completions(context, api_error):
    ...
    expect_api_error = api_error == 'raised'
    seeds = (await completions_seed(context, num_seeds=1),)
    completion = await oai_chat_completions(context.prompts.pop(), seeds[0] if seeds is not None else seeds, context.system_prompt, context.base_url, '/v1/chat', False, model=context.model if hasattr(context, 'model') else None, n_predict=context.n_predict if hasattr(context, 'n_predict') else None, enable_streaming=context.enable_streaming if hasattr(context, 'enable_streaming') else None, response_format=context.response_format if hasattr(context, 'response_format') else None, user_api_key=context.user_api_key if hasattr(context, 'user_api_key') else None, expect_api_error=expect_api_error)
    context.tasks_result.append(completion)
    if context.debug:
        print(f'Completion response: {completion}')
    if expect_api_error:
...
async def step_concurrent_completion_requests(context):
    await concurrent_requests(context, request_completion, context.base_url, debug=context.debug, prompt_prefix=context.prompt_prefix, prompt_suffix=context.prompt_suffix, n_predict=context.n_predict if hasattr(context, 'n_predict') else None, user_api_key=context.user_api_key if hasattr(context, 'user_api_key') else None, temperature=context.temperature)

@step('concurrent OAI completions requests')
@async_run_until_complete
async def step_oai_chat_completions(context):
    await concurrent_requests(context, oai_chat_completions, context.system_prompt, context.base_url, '/v1/chat/completions', True, model=context.model if hasattr(context, 'model') else None, n_predict=context.n_predict if hasattr(context, 'n_predict') else None, enable_streaming=context.enable_streaming if hasattr(context, 'enable_streaming') else None, response_format=context.response_format if hasattr(context, 'response_format') else None, user_api_key=context.user_api_key if hasattr(context, 'user_api_key') else None)

@step('concurrent OAI completions requests no v1')
@async_run_until_complete
async def step_oai_chat_completions(context):
    await concurrent_requests(context, oai_chat_completions, context.system_prompt, context.base_url, '/chat/completions', True, model=context.model if hasattr(context, 'model') else None, n_predict=context.n_predict if hasattr(context, 'n_predict') else None, enable_streaming=context.enable_streaming if hasattr(context, 'enable_streaming') else None, response_format=context.response_format if hasattr(context, 'response_format') else None, user_api_key=context.user_api_key if hasattr(context, 'user_api_key') else None)
...
def step_assert_embeddings(context):
    assert context.n_prompts == len(context.embeddings), f'unexpected response:\ncontext.n_prompts={context.n_prompts}\ncontext.embeddings={context.embeddings}'
...
async def step_oai_compute_embeddings(context):
    context.n_prompts = 1
    context.embeddings = await request_oai_embeddings(context_text(context), None, base_url=context.base_url, user_api_key=context.user_api_key, model=context.model)

@step('an OAI compatible embeddings computation request for multiple inputs')
@async_run_until_complete
async def step_oai_compute_embeddings_multiple_inputs(context):
    context.embeddings = await request_oai_embeddings(context.prompts, None, base_url=context.base_url, user_api_key=context.user_api_key, model=context.model)
    context.prompts.clear()

@step('concurrent embedding requests')
@async_run_until_complete()
async def step_concurrent_embedding_requests(context):
    await concurrent_requests(context, request_embedding, base_url=context.base_url)

@step('concurrent OAI embedding requests')
@async_run_until_complete()
async def step_concurrent_oai_embedding_requests(context):
    await concurrent_requests(context, request_oai_embeddings, base_url=context.base_url, async_client=True, model=context.model)
...
async def step_tokenize(context):
    ...
    async with aiohttp.ClientSession() as session:
        tokenize_args = {'content': context.tokenized_text}
        if getattr(context, 'tokenize_add_special', None) is not None:
            tokenize_args['add_special'] = context.tokenize_add_special
        async with session.post(f'{context.base_url}/tokenize', json=tokenize_args) as response:
...
async def step_detokenize(context):
    ...
    async with aiohttp.ClientSession() as session:
        async with session.post(f'{context.base_url}/detokenize', json={'tokens': context.tokens}) as response:
            assert response.status == 200
...
async def step_options_request(context, origin):
    async with aiohttp.ClientSession() as session:
        ...
        async with session.options(f'{context.base_url}/v1/chat/completions', headers=headers) as response:
            assert response.status == 200
...
async def step_save_slot(context, slot_id, filename):
    async with aiohttp.ClientSession() as session:
        async with session.post(f'{context.base_url}/slots/{slot_id}?action=save', json={'filename': filename}, headers={'Content-Type': 'application/json'}) as response:
...
async def step_restore_slot(context, slot_id, filename):
    async with aiohttp.ClientSession() as session:
        async with session.post(f'{context.base_url}/slots/{slot_id}?action=restore', json={'filename': filename}, headers={'Content-Type': 'application/json'}) as response:
            context.response = response
...
async def step_erase_slot(context, slot_id):
    async with aiohttp.ClientSession() as session:
        async with session.post(f'{context.base_url}/slots/{slot_id}?action=erase', headers={'Content-Type': 'application/json'}) as response:
            context.response = response
...
async def request_completion(prompt, seed, base_url, debug=False, prompt_prefix=None, prompt_suffix=None, n_predict=None, cache_prompt=False, id_slot=None, expect_api_error=None, user_api_key=None, temperature=None):
    if debug:
    ...
    async with aiohttp.ClientSession() as session:
        async with session.post(f'{base_url}/completion', json={'input_prefix': prompt_prefix, 'prompt': prompt, 'input_suffix': prompt_suffix, 'n_predict': n_predict if n_predict is not None else -1, 'cache_prompt': cache_prompt, 'id_slot': id_slot, 'seed': seed if seed is not None else 42, 'temperature': temperature if temperature is not None else 0.8, 'n_probs': 2}, headers=headers, timeout=3600) as response:
...
async def oai_chat_completions(user_prompt, seed, system_prompt, base_url, base_path, async_client, debug=False, temperature=None, model=None, n_predict=None, enable_streaming=None, response_format=None, user_api_key=None, expect_api_error=None):
    if debug:
        print(f'Sending OAI Chat completions request: {user_prompt}')
    user_api_key = user_api_key if user_api_key is not None else 'nope'
    seed = seed if seed is not None else 42
    enable_streaming = enable_streaming if enable_streaming is not None else False
    payload = {'messages': [{'role': 'system', 'content': system_prompt}, {'role': 'user', 'content': user_prompt}], 'model': model, 'max_tokens': n_predict, 'stream': enable_streaming, 'temperature': temperature if temperature is not None else 0.0, 'seed': seed}
    ...
    if async_client:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(f'{base_url}{base_path}', json=payload, headers=headers) as response:
                if enable_streaming:
                    assert response.status == 200
                    assert response.headers['Access-Control-Allow-Origin'] == origin
                ...
                elif expect_api_error is None or not expect_api_error:
                    assert response.status == 200
                    assert response.headers['Access-Control-Allow-Origin'] == origin
                    assert response.headers['Content-Type'] == 'application/json; charset=utf-8'
                    chat_completion_raw = await response.json()
                    completion_response = {'content': chat_completion_raw['choices'][0]['message'], 'timings': {'predicted_n': chat_completion_raw['usage']['completion_tokens'], 'prompt_n': chat_completion_raw['usage']['prompt_tokens']}}
                else:
                    return response.status
    else:
        try:
            ...
            chat_completion = openai.Completion.create(messages=payload['messages'], model=model, max_tokens=n_predict, stream=enable_streaming, response_format=payload.get('response_format'), seed=seed, temperature=payload['temperature'])
        ...
        if enable_streaming:
            ...
            completion_response = {'content': chat_completion.choices[0].message.content, 'timings': {'predicted_n': chat_completion.usage.completion_tokens, 'prompt_n': chat_completion.usage.prompt_tokens}, 'truncated': chat_completion.choices[0].finish_reason != 'stop'}
    if debug:
...
async def request_embedding(content, seed, base_url=None):
    async with aiohttp.ClientSession() as session:
        async with session.post(f'{base_url}/embedding', json={'content': content}) as response:
            assert response.status == 200
            response_json = await response.json()
            return [response_json['embedding']]

async def request_oai_embeddings(input, seed, base_url=None, user_api_key=None, model=None, async_client=False):
    user_api_key = user_api_key if user_api_key is not None else 'nope'
    if async_client:
        origin = 'llama.cpp'
        headers = []
        if user_api_key is not None:
            headers = {'Authorization': f'Bearer {user_api_key}', 'Origin': origin}
        async with aiohttp.ClientSession() as session:
            async with session.post(f'{base_url}/v1/embeddings', json={'input': input, 'model': model}, headers=headers, timeout=3600) as response:
        ...
        oai_embeddings = openai.Embedding.create(model=model, input=input)
        if isinstance(input, collections.abc.Sequence):
            embeddings = []
...
def assert_n_tokens_predicted(completion_response, expected_predicted_n=None, re_content=None):
    ...
    if expected_predicted_n and expected_predicted_n > 0:
        assert n_predicted == expected_predicted_n, f'invalid number of tokens predicted: {n_predicted} <> {expected_predicted_n}'
...
async def wait_for_health_status(context, base_url, expected_http_status_code, expected_health_status, timeout=3, params=None, slots_idle=None, slots_processing=None, expected_slots=None):
    if context.debug:
    ...
    async with aiohttp.ClientSession() as session:
        while True:
            async with await session.get(f'{base_url}/health', params=params) as health_response:
                ...
                if context.debug:
                    print(f"HEALTH - response for expected health status='{expected_health_status}' on '{base_url}/health'?{params} is {health}\n")
                if status_code == expected_http_status_code and health['status'] == expected_health_status and (slots_idle is None or health['slots_idle'] == slots_idle) and (slots_processing is None or health['slots_processing'] == slots_processing):
            ...
            if counter >= timeout:
                if expected_http_status_code == 503:
                    if len(context.tasks_result) == 0:
                        print('\x1b[5;37;43mWARNING: forcing concurrent tasks, busy health check missed, probably too fast inference\x1b[0m\n')
                        n_completions = await gather_tasks_results(context)
                        if n_completions > 0:
                            return
...
def assert_slots_status(slots, expected_slots):
    ...
    for slot_id, (expected, slot) in enumerate(zip(expected_slots, slots)):
        for key in expected:
            assert expected[key] == slot[key], f'invalid slot {slot_id} expected[{key}] != slot[{key}] = {expected[key]} != {slot[key]}'
...
def start_server_background(context):
    ...
    server_args = ['--host', server_listen_addr, '--port', context.server_port]
    ...
    pkwargs = {'creationflags': flags, 'stdout': subprocess.PIPE, 'stderr': subprocess.PIPE}
    context.server_process = subprocess.Popen([str(arg) for arg in [context.server_path, *server_args]], **pkwargs)
