...
class Tensor:
    ...
    def save_gguf(self, gguf_writer, name):
        gguf_writer.add_tensor(name=name, tensor=self.data, raw_shape=np.array(list(reversed(self.ne))), raw_dtype=gguf.GGMLQuantizationType.F32)
...
class OptimizationParamsV0:
    ...
    def load(self, data, offset):
        self.type = struct.unpack('<I', bytes(data[offset:offset + 4]))[0]
        offset += 4
        self.n_threads = struct.unpack('<i', bytes(data[offset:offset + 4]))[0]
        offset += 4
        self.past = struct.unpack('<i', bytes(data[offset:offset + 4]))[0]
        offset += 4
        self.delta = struct.unpack('<f', bytes(data[offset:offset + 4]))[0]
        offset += 4
        self.print_forward_graph = struct.unpack('<?', bytes(data[offset:offset + 1]))[0]
        offset += 4
        self.print_backward_graph = struct.unpack('<?', bytes(data[offset:offset + 1]))[0]
        offset += 4
        self.adam_n_iter = struct.unpack('<i', bytes(data[offset:offset + 4]))[0]
        offset += 4
        self.adam_sched = struct.unpack('<f', bytes(data[offset:offset + 4]))[0]
        offset += 4
        self.adam_decay = struct.unpack('<f', bytes(data[offset:offset + 4]))[0]
        offset += 4
        self.adam_alpha = struct.unpack('<f', bytes(data[offset:offset + 4]))[0]
        offset += 4
        self.adam_beta1 = struct.unpack('<f', bytes(data[offset:offset + 4]))[0]
        offset += 4
        self.adam_beta2 = struct.unpack('<f', bytes(data[offset:offset + 4]))[0]
        offset += 4
        self.adam_eps = struct.unpack('<f', bytes(data[offset:offset + 4]))[0]
        offset += 4
        self.adam_eps_f = struct.unpack('<f', bytes(data[offset:offset + 4]))[0]
        offset += 4
        self.adam_eps_g = struct.unpack('<f', bytes(data[offset:offset + 4]))[0]
        offset += 4
        self.lbfgs_m = struct.unpack('<i', bytes(data[offset:offset + 4]))[0]
        offset += 4
        self.lbfgs_n_iter = struct.unpack('<i', bytes(data[offset:offset + 4]))[0]
        offset += 4
        self.lbfgs_max_linesearch = struct.unpack('<i', bytes(data[offset:offset + 4]))[0]
        offset += 4
        self.lbfgs_eps = struct.unpack('<f', bytes(data[offset:offset + 4]))[0]
        offset += 4
        self.lbfgs_ftol = struct.unpack('<f', bytes(data[offset:offset + 4]))[0]
        offset += 4
        self.lbfgs_wolfe = struct.unpack('<f', bytes(data[offset:offset + 4]))[0]
        offset += 4
        self.lbfgs_min_step = struct.unpack('<f', bytes(data[offset:offset + 4]))[0]
        offset += 4
        self.lbfgs_max_step = struct.unpack('<f', bytes(data[offset:offset + 4]))[0]
        offset += 4
        self.lbfgs_linesearch = struct.unpack('<I', bytes(data[offset:offset + 4]))[0]
        offset += 4
        return offset
    ...
    def load(self, data, offset):
        ...
        if self.version == 0:
            ...
            if self.type == 0:
                ...
                self.adam_fx_best = struct.unpack('<f', bytes(data[offset:offset + 4]))[0]
                offset += 4
                self.adam_fx_prev = struct.unpack('<f', bytes(data[offset:offset + 4]))[0]
                offset += 4
                self.adam_n_no_improvement = struct.unpack('<i', bytes(data[offset:offset + 4]))[0]
                offset += 4
            elif self.type == 1:
                ...
                self.lbfgs_fx_best = struct.unpack('<f', bytes(data[offset:offset + 4]))[0]
                offset += 4
                self.lbfgs_step = struct.unpack('<f', bytes(data[offset:offset + 4]))[0]
                offset += 4
                self.lbfgs_j = struct.unpack('<i', bytes(data[offset:offset + 4]))[0]
                offset += 4
                self.lbfgs_k = struct.unpack('<i', bytes(data[offset:offset + 4]))[0]
                offset += 4
                self.lbfgs_end = struct.unpack('<i', bytes(data[offset:offset + 4]))[0]
                offset += 4
                self.lbfgs_n_no_improvement = struct.unpack('<i', bytes(data[offset:offset + 4]))[0]
                offset += 4
            else:
            ...
            self.lbfgs_lms = Tensor('f', [self.nx, self.lbfgs_m])
            self.lbfgs_lmy = Tensor('f', [self.nx, self.lbfgs_m])
            size_type_0 = 12 + sum([t.max_storage_size() for t in [self.adam_m, self.adam_v] + ([self.adam_pf] if self.past > 0 else [])])
            size_type_1 = 24 + sum([t.max_storage_size() for t in [self.lbfgs_x, self.lbfgs_xp, self.lbfgs_g, self.lbfgs_gp, self.lbfgs_d, self.lbfgs_pf, self.lbfgs_lmal, self.lbfgs_lmys, self.lbfgs_lms, self.lbfgs_lmy] + ([self.lbfgs_pf] if self.past > 0 else [])])
...
class ModelParams:
    ...
    def load(self, data, offset):
        self.n_vocab = struct.unpack('<I', bytes(data[offset:offset + 4]))[0]
        offset += 4
        self.n_embd = struct.unpack('<I', bytes(data[offset:offset + 4]))[0]
        offset += 4
        self.n_mult = struct.unpack('<I', bytes(data[offset:offset + 4]))[0]
        offset += 4
        self.n_head = struct.unpack('<I', bytes(data[offset:offset + 4]))[0]
        offset += 4
        self.n_layer = struct.unpack('<I', bytes(data[offset:offset + 4]))[0]
        offset += 4
        self.n_rot = struct.unpack('<I', bytes(data[offset:offset + 4]))[0]
        offset += 4
        return offset
