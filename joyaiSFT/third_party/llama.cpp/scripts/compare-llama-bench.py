...
KEY_PROPERTIES = ['cpu_info', 'gpu_info', 'n_gpu_layers', 'cuda', 'vulkan', 'kompute', 'metal', 'sycl', 'rpc', 'gpu_blas', 'blas', 'model_filename', 'model_type', 'model_size', 'model_n_params', 'n_batch', 'n_ubatch', 'embeddings', 'n_threads', 'type_k', 'type_v', 'use_mmap', 'no_kv_offload', 'split_mode', 'main_gpu', 'tensor_split', 'flash_attn', 'n_prompt', 'n_gen']
BOOL_PROPERTIES = ['cuda', 'vulkan', 'kompute', 'metal', 'sycl', 'gpu_blas', 'blas', 'embeddings', 'use_mmap', 'no_kv_offload', 'flash_attn']
PRETTY_NAMES = {'cuda': 'CUDA', 'vulkan': 'Vulkan', 'kompute': 'Kompute', 'metal': 'Metal', 'sycl': 'SYCL', 'rpc': 'RPC', 'gpu_blas': 'GPU BLAS', 'blas': 'BLAS', 'cpu_info': 'CPU', 'gpu_info': 'GPU', 'model_filename': 'File', 'model_type': 'Model', 'model_size': 'Model Size [GiB]', 'model_n_params': 'Num. of Par.', 'n_batch': 'Batch size', 'n_ubatch': 'Microbatch size', 'n_threads': 'Threads', 'type_k': 'K type', 'type_v': 'V type', 'n_gpu_layers': 'GPU layers', 'split_mode': 'Split mode', 'main_gpu': 'Main GPU', 'no_kv_offload': 'NKVO', 'flash_attn': 'FlashAttention', 'tensor_split': 'Tensor split', 'use_mmap': 'Use mmap', 'embeddings': 'Embeddings'}
...
help_i = "Input SQLite file for comparing commits. Defaults to 'llama-bench.sqlite' in the current working directory. If no such file is found and there is exactly one .sqlite file in the current directory, that file is instead used as input."
...
help_s = f"Columns to add to the table. Accepts a comma-separated list of values. Legal values: {', '.join(KEY_PROPERTIES[:-2])}. Defaults to model name (model_type) and CPU and/or GPU name (cpu_info, gpu_info) plus any column where not all data points are the same. If the columns are manually specified, then the results for each unique combination of the specified values are averaged WITHOUT weighing by the --repetitions parameter of llama-bench."
...
if known_args.compare is not None:
    ...
    builds_timestamp = cursor.execute('SELECT build_commit, test_time FROM test ORDER BY test_time;').fetchall()
...
def get_rows(properties):
    ...
    """
    select_string = ', '.join([f'tb.{p}' for p in properties] + ['tb.n_prompt', 'tb.n_gen', 'AVG(tb.avg_ts)', 'AVG(tc.avg_ts)'])
    equal_string = ' AND '.join([f'tb.{p} = tc.{p}' for p in KEY_PROPERTIES] + [f"tb.build_commit = '{hexsha8_baseline}'", f"tc.build_commit = '{hexsha8_compare}'"])
    group_order_string = ', '.join([f'tb.{p}' for p in properties] + ['tb.n_gen', 'tb.n_prompt'])
    query = f'SELECT {select_string} FROM test tb JOIN test tc ON {equal_string} GROUP BY {group_order_string} ORDER BY {group_order_string};'
