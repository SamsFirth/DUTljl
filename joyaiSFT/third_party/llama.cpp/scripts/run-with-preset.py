...
epilog = '  --<ARG_NAME> specify additional CLI ars to be passed to the binary (override all preset files). Unknown args will be ignored.'
parser = argparse.ArgumentParser(description=description, usage=usage, epilog=epilog, formatter_class=argparse.RawTextHelpFormatter)
parser.add_argument('-bin', '--binary', help='The binary to run.')
parser.add_argument('yaml_files', nargs='*', help='Arbitrary number of YAML files from which to read preset values. If two files specify the same values the later one will be used.')
