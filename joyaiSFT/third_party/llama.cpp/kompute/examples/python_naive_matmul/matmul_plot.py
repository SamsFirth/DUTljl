    ...
    for y in range(tensor.shape[1]):
        for x in range(tensor.shape[0]):
            if coord_highlight and x == coord_highlight[1] and (y == coord_highlight[0]):
                cv2.putText(image, str(int(tensor[y, x])), (x * font_size, int((y + 0.8) * font_size)), cv2.FONT_HERSHEY_TRIPLEX, 1.0, (127, 127, 255))
            else:
                cv2.putText(image, str(int(tensor[y, x])), (x * font_size, int((y + 0.8) * font_size)), cv2.FONT_HERSHEY_TRIPLEX, 1.0, (255, 255, 255))
...
def main():
    ...
    cv2.waitKey(-1)
    print(f'workgroup={workgroup!r} local_size={local_size!r}')
            ...
            for invocation_x in range(workgroup_x * local_size[0], (workgroup_x + 1) * local_size[0]):
                for invocation_y in range(workgroup_y * local_size[1], (workgroup_y + 1) * local_size[1]):
                    ...
                    try:
                        ...
                        print(f'workgroup_x={workgroup_x!r} workgroup_y={workgroup_y!r} row={row!r} col={col!r}')
                        raise error
    plot_tensor('tensor_1', tensor_1)
    plot_tensor('tensor_2', tensor_2)
    plot_tensor('tensor_out', tensor_out)
