...
def test_end_to_end():
    ...
    mgr.sequence().record(kp.OpTensorSyncDevice(params)).record(kp.OpAlgoDispatch(algo)).record(kp.OpAlgoDispatch(algo, push_consts_b)).eval()
    sq = mgr.sequence()
...
def test_pushconsts():
    ...
    mgr.sequence().record(kp.OpTensorSyncDevice([tensor])).record(kp.OpAlgoDispatch(algo)).record(kp.OpAlgoDispatch(algo, [0.3, 0.2, 0.1])).record(kp.OpAlgoDispatch(algo, [0.3, 0.2, 0.1])).record(kp.OpTensorSyncLocal([tensor])).eval()
...
def test_pushconsts_int():
    ...
    mgr.sequence().record(kp.OpTensorSyncDevice([tensor])).record(kp.OpAlgoDispatch(algo)).record(kp.OpAlgoDispatch(algo, np.array([-1, -1, -1], dtype=np.int32))).record(kp.OpAlgoDispatch(algo, np.array([-1, -1, -1], dtype=np.int32))).record(kp.OpTensorSyncLocal([tensor])).eval()
...
def test_workgroup():
    ...
    @ps.python2shader
    def compute_shader_wg(gl_idx=('input', 'GlobalInvocationId', ps.ivec3), gl_wg_id=('input', 'WorkgroupId', ps.ivec3), gl_wg_num=('input', 'NumWorkgroups', ps.ivec3), data1=('buffer', 0, ps.Array(ps.f32)), data2=('buffer', 1, ps.Array(ps.f32))):
    ...
    mgr.sequence().record(kp.OpTensorSyncDevice([tensor_a, tensor_b])).record(kp.OpAlgoDispatch(algo)).record(kp.OpTensorSyncLocal([tensor_a, tensor_b])).eval()
    print(tensor_a.data())
    print(tensor_b.data())
