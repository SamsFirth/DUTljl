...
def tests(session: nox.Session) -> None:
    ...
    session.run('cmake', '-S.', f'-B{tmpdir}', '-DPYBIND11_WERROR=ON', '-DDOWNLOAD_CATCH=ON', '-DDOWNLOAD_EIGEN=ON', *session.posargs)
...
def docs(session: nox.Session) -> None:
    ...
    session.chdir('docs')
    shared_args = ('-n', '-T', f'-b={args.builder}', '.', f'_build/{args.builder}', *posargs)
    if serve:
        session.run('sphinx-autobuild', '--open-browser', '--ignore=.build', *shared_args)
    else:
