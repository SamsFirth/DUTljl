...
def test_tuple_variable_length_annotations(doc):
    assert doc(m.annotate_tuple_variable_length) == 'annotate_tuple_variable_length(arg0: tuple[float, ...]) -> None'
...
def test_fn_return_only(doc):
    assert doc(m.annotate_fn_only_return) == 'annotate_fn_only_return(arg0: Callable[..., int]) -> None'
...
def test_union_typing_only(doc):
    assert doc(m.union_typing_only) == 'union_typing_only(arg0: list[Union[str]]) -> list[Union[int]]'

def test_union_object_annotations(doc):
    assert doc(m.annotate_union_to_object) == 'annotate_union_to_object(arg0: Union[int, str]) -> object'
...
def test_type_guard_annotations(doc):
    assert doc(m.annotate_type_guard) == 'annotate_type_guard(arg0: object) -> TypeGuard[str]'
...
def test_optional_object_annotations(doc):
    assert doc(m.annotate_optional_to_object) == 'annotate_optional_to_object(arg0: Optional[int]) -> object'

@pytest.mark.skipif(not m.defined_PYBIND11_TYPING_H_HAS_STRING_LITERAL, reason='C++20 feature not available.')
def test_literal(doc):
    assert doc(m.annotate_literal) == 'annotate_literal(arg0: Literal[26, 0x1A, "hello world", b"hello world", u"hello world", True, Color.RED, None]) -> object'
