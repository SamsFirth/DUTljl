...
def test_dim_check_fail(arr):
    for func in (m.index_at, m.index_at_t, m.offset_at, m.offset_at_t, m.data, m.data_t, m.mutate_data, m.mutate_data_t):
        with pytest.raises(IndexError) as excinfo:
            func(arr, 1, 2, 3)
...
def test_overload_resolution(msg):
    ...
    assert msg(excinfo.value) == expected_exc + repr(np.array([1.0 + 0j]))
...
def test_array_view():
    ...
    assert a_float_view.shape == (100 * 1,)
    a_int16_view = m.array_view(a, 'int16')
...
@pytest.mark.parametrize('test_func', [m.test_fmt_desc_float, m.test_fmt_desc_double, m.test_fmt_desc_const_float, m.test_fmt_desc_const_double])
...
@pytest.mark.filterwarnings('ignore:Casting complex values to real discards the imaginary part:' + ('numpy.exceptions.ComplexWarning' if hasattr(np, 'exceptions') else 'numpy.ComplexWarning'))
        ...
        for order in ['C', 'F']:
            for shape in [(2, 2), (1, 3, 1, 1), (1, 1, 1), (0,)]:
                if not noconvert:
                    ...
                    should_raise = dtype.name != 'float64' or (contiguity is not None and contiguity != order and (not trivially_contiguous))
...
def test_pass_array_pyobject_ptr_return_sum_str_values_ndarray():
    assert m.pass_array_pyobject_ptr_return_sum_str_values(np.array(WrapWithPyValueHolder(-3, 'four', 5.0), dtype=object)) == '-3four5.0'
...
def test_pass_array_pyobject_ptr_return_as_list():
    assert UnwrapPyValueHolder(m.pass_array_pyobject_ptr_return_as_list(np.array(WrapWithPyValueHolder(-1, 'two', 3.0), dtype=object))) == [-1, 'two', 3.0]

@pytest.mark.parametrize(('return_array_pyobject_ptr', 'unwrap'), [(m.return_array_pyobject_ptr_cpp_loop, list), (m.return_array_pyobject_ptr_from_list, UnwrapPyValueHolder)])
