...
def dt_fmt():
    ...
    return "{{'names':['bool_','uint_','float_','ldbl_'],'formats':['?','" + e + "u4','" + e + "f4','" + e + "f{}'],'offsets':[0,4,8,{}],'itemsize':{}}}"
...
def test_format_descriptors():
    ...
    partial_fmt = '^T{?:bool_:3xI:uint_:f:float_:' + str(4 * (dbl.alignment > 4) + dbl.itemsize + 8 * (ld.alignment > 8)) + 'xg:ldbl_:' + (str(end_padding) + 'x}' if end_padding > 0 else '}')
...
def test_dtype(simple_dtype):
    from sys import byteorder
    e = '<' if byteorder == 'little' else '>'
    assert [x.replace(' ', '') for x in m.print_dtypes()] == [simple_dtype_fmt(), packed_dtype_fmt(), f"[('a',{simple_dtype_fmt()}),('b',{packed_dtype_fmt()})]", partial_dtype_fmt(), partial_nested_fmt(), "[('a','S3'),('b','S3')]", f"{{'names':['a','b','c','d'],'formats':[('S4',(3,)),('{e}i4',(2,)),('u1',(3,)),('{e}f4',(4,2))],'offsets':[0,12,20,24],'itemsize':56}}", "[('e1','" + e + "i8'),('e2','u1')]", "[('x','i1'),('y','" + e + "u8')]", "[('cflt','" + e + "c8'),('cdbl','" + e + "c16')]"]
...
def test_array_array():
    ...
    assert str(arr.dtype).replace(' ', '') == f"{{'names':['a','b','c','d'],'formats':[('S4',(3,)),('{e}i4',(2,)),('u1',(3,)),('{e}f4',(4,2))],'offsets':[0,12,20,24],'itemsize':56}}"
