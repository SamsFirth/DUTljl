...
```
7. 执行如下命令，获取joyaisft包，注意将第一个参数 **`<api-token>`** 替换为自己的api-token（[API Token 生成入口](http://xingyun.jd.com/codingRoot/profile/settings/apiToken)）：
```bash
wget \
...
```
9. 执行如下命令，获取jdea包，注意替换 **`<api-token>`**：
```bash
wget \
  --header="PRIVATE-TOKEN: <api-token>" \
...
```
11. （flash-attn包已经在该仓库的发行版网页中，可以直接下载，不必按照下面的命令下载）执行如下命令，获取flash-attention包，注意替换 **`<api-token>`**：
```bash
wget \
  --header="PRIVATE-TOKEN: <api-token>" \
