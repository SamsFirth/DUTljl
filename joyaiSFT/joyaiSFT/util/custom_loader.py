...
from joyaiSFT.util.custom_gguf import *
...
class SafeTensorLoader(ModelLoader):
    ...
    def load_experts(self, key: str, device: str='cpu'):
        ...
        if self.has_tensor(translate_name_to_gguf(key) + '.ffn_gate_exps.weight'):
            ...
            return {'up': up_tensor, 'gate': gate_tensor, 'down': down_tensor, 'up_type': up_type, 'gate_type': gate_type, 'down_type': down_type}
        else:
            base_key = key
            experts_count = 0
            ...
            return {'up': up_numpy, 'gate': gate_numpy, 'down': down_numpy, 'up_type': get_ggml_type(orig_up_dtype), 'gate_type': get_ggml_type(orig_gate_dtype), 'down_type': get_ggml_type(orig_down_dtype)}
...
class GGUFLoader(ModelLoader):
    ...
    def load_gguf(self, f):
        ...
        for _ in range(n_tensors):
            ...
            tensor_info[name] = {'ggml_type': ggml_type, 'shape': shape, 'bad_offset': bad_offset, 'item_type': item_type, 'item_count': item_count, 'np_dims': np_dims}
        start = f.tell()
    ...
    def load_gguf_tensor(self, name: str, device: str='cpu', target_dtype=None) -> torch.Tensor:
        ...
        if 'attn_q' in name and self.gguf_file_meta['general.architecture'] in ['llama']:
            ...
            values = values.reshape(n_head, values.shape[0] // n_head // 2, 2, *values.shape[1:]).swapaxes(1, 2).reshape(values.shape)
