...
GGML_TYPES = {'F32': 0, 'F16': 1, 'Q4_0': 2, 'Q5_0': 6, 'Q8_0': 8, 'Q2_K': 10, 'Q3_K': 11, 'Q4_K': 12, 'Q5_K': 13, 'Q6_K': 14, 'IQ4_XS': 23, 'BF16': 30}
GGML_NAMES = {ggml_type: name for name, ggml_type in GGML_TYPES.items()}
GGML_BLOCK_SIZES = {'F32': 4, 'F16': 2, 'BF16': 2, 'Q4_0': 2 + 16, 'Q5_0': 2 + 4 + 16, 'Q8_0': 2 + 32, 'Q2_K': 256 // 16 + 256 // 4 + 2 + 2, 'Q3_K': 256 // 8 + 256 // 4 + 12 + 2, 'Q4_K': 2 + 2 + 12 + 256 // 2, 'Q5_K': 2 + 2 + 12 + 256 // 8 + 256 // 2, 'Q6_K': 256 // 2 + 256 // 4 + 256 // 16 + 2, 'IQ4_XS': 2 + 2 + 256 // 2 + 256 // 64, 'FP8': 1}
GGML_ELEMENTS_PER_BLOCK = {'F32': 1, 'F16': 1, 'BF16': 1, 'Q4_0': 32, 'Q5_0': 32, 'Q8_0': 32, 'Q2_K': 256, 'Q3_K': 256, 'Q4_K': 256, 'Q5_K': 256, 'Q6_K': 256, 'IQ4_XS': 256, 'FP8': 1}
DATA_TYPES = {'uint8': 0, 'int8': 1, 'uint16': 2, 'int16': 3, 'uint32': 4, 'int32': 5, 'float32': 6, 'bool': 7, 'string': 8, 'array': 9, 'uint64': 10, 'int64': 11, 'float64': 12, 'FP8': 13}
...
def dequantize_q2_k(data):
    ...
    tmp = np.stack([qs[:, 0:16] >> 0, qs[:, 16:32] >> 0, qs[:, 0:16] >> 2, qs[:, 16:32] >> 2, qs[:, 0:16] >> 4, qs[:, 16:32] >> 4, qs[:, 0:16] >> 6, qs[:, 16:32] >> 6, qs[:, 32:48] >> 0, qs[:, 48:64] >> 0, qs[:, 32:48] >> 2, qs[:, 48:64] >> 2, qs[:, 32:48] >> 4, qs[:, 48:64] >> 4, qs[:, 32:48] >> 6, qs[:, 48:64] >> 6], axis=1)
...
def dequantize_q2_k_gpu(data, device: str='cuda', target_dtype=torch.get_default_dtype()):
    ...
    return JOYSFTOps.dequantize_q2_k(c_pointer, data.size, block_size, ele_per_blk, device, target_dtype)
...
def dequantize_q3_k(data):
    ...
    return d * (scales - 32) * np.stack([(qs[:, 0:16] >> 0 & 3) - bits[:, :16, 0], (qs[:, 16:32] >> 0 & 3) - bits[:, 16:, 0], (qs[:, 0:16] >> 2 & 3) - bits[:, :16, 1], (qs[:, 16:32] >> 2 & 3) - bits[:, 16:, 1], (qs[:, 0:16] >> 4 & 3) - bits[:, :16, 2], (qs[:, 16:32] >> 4 & 3) - bits[:, 16:, 2], (qs[:, 0:16] >> 6 & 3) - bits[:, :16, 3], (qs[:, 16:32] >> 6 & 3) - bits[:, 16:, 3], (qs[:, 32:48] >> 0 & 3) - bits[:, :16, 4], (qs[:, 48:64] >> 0 & 3) - bits[:, 16:, 4], (qs[:, 32:48] >> 2 & 3) - bits[:, :16, 5], (qs[:, 48:64] >> 2 & 3) - bits[:, 16:, 5], (qs[:, 32:48] >> 4 & 3) - bits[:, :16, 6], (qs[:, 48:64] >> 4 & 3) - bits[:, 16:, 6], (qs[:, 32:48] >> 6 & 3) - bits[:, :16, 7], (qs[:, 48:64] >> 6 & 3) - bits[:, 16:, 7]], axis=1)
...
def dequantize_q3_k_gpu(data, device: str='cuda', target_dtype=torch.get_default_dtype()):
    ...
    device = torch.device(device)
    c_pointer = ctypes.addressof(ctypes.cast(data.ctypes.data, ctypes.POINTER(ctypes.c_int8)).contents)
    return JOYSFTOps.dequantize_q3_k(c_pointer, data.size, block_size, ele_per_blk, device, target_dtype)
...
def dequantize_q4_k(data):
    ...
    factors = scale_factors * np.concatenate([qs1[:, 0:4] & 63, qs1[:, 8:] & 15 | qs1[:, 0:4] >> 6 << 4], axis=1)
    offsets = scale_offsets * np.concatenate([qs1[:, 4:8] & 63, qs1[:, 8:] >> 4 | qs1[:, 4:8] >> 6 << 4], axis=1)
    qs2 = np.stack([qs2 & 15, qs2 >> 4], axis=2).reshape(num_blocks, 8, 32)
...
def dequantize_q4_k_gpu(data, device: str='cuda', target_dtype=torch.get_default_dtype()):
    ...
    device = torch.device(device)
    c_pointer = ctypes.addressof(ctypes.cast(data.ctypes.data, ctypes.POINTER(ctypes.c_int8)).contents)
    return JOYSFTOps.dequantize_q4_k(c_pointer, data.size, block_size, ele_per_blk, device, target_dtype)
...
def dequantize_q5_k(data):
    ...
    return np.concatenate([d1 * (qs_lo_4[:, 0] + (bits[:, :, 0] << 4)) - m1, d2 * (qs_hi_4[:, 0] + (bits[:, :, 1] << 4)) - m2, d3 * (qs_lo_4[:, 1] + (bits[:, :, 2] << 4)) - m3, d4 * (qs_hi_4[:, 1] + (bits[:, :, 3] << 4)) - m4, d5 * (qs_lo_4[:, 2] + (bits[:, :, 4] << 4)) - m5, d6 * (qs_hi_4[:, 2] + (bits[:, :, 5] << 4)) - m6, d7 * (qs_lo_4[:, 3] + (bits[:, :, 6] << 4)) - m7, d8 * (qs_hi_4[:, 3] + (bits[:, :, 7] << 4)) - m8], axis=1)
...
def dequantize_q5_k_gpu(data, device: str='cuda', target_dtype=torch.get_default_dtype()):
    ...
    device = torch.device(device)
    c_pointer = ctypes.addressof(ctypes.cast(data.ctypes.data, ctypes.POINTER(ctypes.c_int8)).contents)
    return JOYSFTOps.dequantize_q5_k(c_pointer, data.size, block_size, ele_per_blk, device, target_dtype)
...
def dequantize_q6_k(data):
    ...
    q1 = ql[:, :32] & 15 | ((qh[:, :32] >> 0 & 3) << 4) - 32
    q2 = ql[:, 32:64] & 15 | ((qh[:, :32] >> 2 & 3) << 4) - 32
    q3 = ql[:, :32] >> 4 | ((qh[:, :32] >> 4 & 3) << 4) - 32
    q4 = ql[:, 32:64] >> 4 | ((qh[:, :32] >> 6 & 3) << 4) - 32
    q5 = ql[:, 64:96] & 15 | ((qh[:, 32:] >> 0 & 3) << 4) - 32
    q6 = ql[:, 96:128] & 15 | ((qh[:, 32:] >> 2 & 3) << 4) - 32
    q7 = ql[:, 64:96] >> 4 | ((qh[:, 32:] >> 4 & 3) << 4) - 32
    q8 = ql[:, 96:128] >> 4 | ((qh[:, 32:] >> 6 & 3) << 4) - 32
    return scales * np.concatenate([sc[:, 0] * q1[:, :16], sc[:, 1] * q1[:, 16:], sc[:, 2] * q2[:, :16], sc[:, 3] * q2[:, 16:], sc[:, 4] * q3[:, :16], sc[:, 5] * q3[:, 16:], sc[:, 6] * q4[:, :16], sc[:, 7] * q4[:, 16:], sc[:, 8] * q5[:, :16], sc[:, 9] * q5[:, 16:], sc[:, 10] * q6[:, :16], sc[:, 11] * q6[:, 16:], sc[:, 12] * q7[:, :16], sc[:, 13] * q7[:, 16:], sc[:, 14] * q8[:, :16], sc[:, 15] * q8[:, 16:]], axis=1)
...
def dequantize_q6_k_gpu(data: np.ndarray, device: str='cuda', target_dtype=torch.get_default_dtype()):
    ...
    data = np.frombuffer(data, dtype=data.dtype)
    c_pointer = ctypes.addressof(ctypes.cast(data.ctypes.data, ctypes.POINTER(ctypes.c_int8)).contents)
    return JOYSFTOps.dequantize_q6_k(c_pointer, data.size, block_size, ele_per_blk, device, target_dtype)
...
def dequantize_iq4_xs(data):
    ...
    for ib in range(QK_K // 32):
        ls[:, ib] = scales_l[:, ib // 2] >> 4 * (ib % 2) & 15 | (scales_h[:, 0] >> 2 * ib & 3) << 4
    dl = (d * (ls - 32)).reshape(num_blocks, -1, 1)
    qs_lo_4 = qs[:, :QK_K // 2].reshape(num_blocks, -1, 16) & 15
...
def dequantize_iq4_xs_gpu(data: np.ndarray, device: str='cuda', target_dtype=torch.get_default_dtype()):
    ...
    data = np.frombuffer(data, dtype=data.dtype)
    c_pointer = ctypes.addressof(ctypes.cast(data.ctypes.data, ctypes.POINTER(ctypes.c_int8)).contents)
    return JOYSFTOps.dequantize_iq4_xs(c_pointer, data.size, block_size, ele_per_blk, device, target_dtype)
...
def dequantize_q4_0(data):
    ...
    return np.concatenate([scales * ((qs & 15).astype(np.int8) - 8), scales * ((qs >> 4).astype(np.int8) - 8)], axis=1)
...
def dequantize_q5_0(data):
    ...
    bits = np.unpackbits(qh, axis=-1, bitorder='little')
    x0 = ((qs & 15).astype(np.int8) | bits[:, :16] << 4) - 16
    x1 = ((qs >> 4).astype(np.int8) | bits[:, 16:] << 4) - 16
    return np.concatenate([scales * x0, scales * x1], axis=1)
...
def dequantize_q8_0_gpu(data, device: str='cuda', target_dtype=torch.get_default_dtype()):
    ...
    data = np.frombuffer(data, dtype=data.dtype)
    c_pointer = ctypes.addressof(ctypes.cast(data.ctypes.data, ctypes.POINTER(ctypes.c_int8)).contents)
    return JOYSFTOps.dequantize_q8_0(c_pointer, data.size, block_size, ele_per_blk, device, target_dtype)
...
def dequantize_bf16_gpu(data, device, target_dtype=torch.get_default_dtype()):
    ...
    res_gpu.copy_(res)
    return res_gpu
GGML_DEQUANTIZE = {'F32': dequantize_f32, 'F16': dequantize_f16, 'BF16': dequantize_f16, 'Q4_0': dequantize_q4_0, 'Q5_0': dequantize_q5_0, 'Q8_0': dequantize_q8_0, 'Q2_K': dequantize_q2_k, 'Q3_K': dequantize_q3_k, 'Q4_K': dequantize_q4_k, 'Q5_K': dequantize_q5_k, 'Q6_K': dequantize_q6_k, 'IQ4_XS': dequantize_iq4_xs}
GGML_DEQUANTIZE_GPU = {'F32': dequantize_f32_gpu, 'F16': dequantize_f16_gpu, 'BF16': dequantize_bf16_gpu, 'Q4_0': dequantize_q4_0_gpu, 'Q5_0': dequantize_q5_0_gpu, 'Q8_0': dequantize_q8_0_gpu, 'Q2_K': dequantize_q2_k_gpu, 'Q3_K': dequantize_q3_k_gpu, 'Q4_K': dequantize_q4_k_gpu, 'Q5_K': dequantize_q5_k_gpu, 'Q6_K': dequantize_q6_k_gpu, 'IQ4_XS': dequantize_iq4_xs_gpu}

def translate_name_to_gguf_mixtral(name):
    replacement_template = {'w1.weight': 'ffn_gate', 'w2.weight': 'ffn_down', 'w3.weight': 'ffn_up'}
    pattern = re.compile('model.layers\\.(\\d+)\\.block_sparse_moe\\.experts\\.(\\d+)\\.(w\\d\\.weight)')
