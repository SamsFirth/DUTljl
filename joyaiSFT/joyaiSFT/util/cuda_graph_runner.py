...
class CUDAGraphRunner:
    ...
    def capture(self, model, cur_token, position_ids, cache_position, past_key_values, main_device, **kwargs) -> None:
        ...
        with torch.cuda.graph(self.graph, stream=capture_stream):
            logits = model(inputs_embeds=inputs_embeds, position_ids=position_ids, cache_position=cache_position, past_key_values=past_key_values, **kwargs)[0]
        ...
        self.input_buffers = {'inputs_embeds': inputs_embeds, 'position_ids': position_ids, 'cache_position': cache_position}
        self.output_buffers = {'logits': logits}
        return

    def forward(self, cur_token, position_ids, cache_position) -> torch.Tensor:
