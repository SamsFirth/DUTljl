...
def attention_ref_torch(batch_size, q: torch.Tensor, k: torch.Tensor, v: torch.Tensor, causal: bool, sm_scale: float) -> torch.Tensor:
    ...
    if causal:
        mask = (torch.arange(kv_len - qo_len, kv_len).unsqueeze(1) >= torch.arange(0, kv_len).unsqueeze(0)).to(q.device)
    else:
...
class MLAWrapper:
    ...
    def __init__(self, max_batch_size, max_pages, use_cuda_graph=True, device='cuda'):
        ...
        self.wrapper = flashinfer.mla.BatchMLAPagedAttentionWrapper(self.float_workspace_buffer, use_cuda_graph=use_cuda_graph, qo_indptr=self.qo_indptr_buf, kv_indptr=self.kv_indptr_buf, kv_indices=self.kv_indices_buf, kv_len_arr=self.kv_len_arr_buf, bsz_tensor=self.batch_size_tensor_buf, backend='fa2')
        self.need_plan = True

    def plan(self, qo_indptr, kv_indptr, kv_indices, kv_len_arr, bsz_tensor, num_heads, head_dim_ckv, head_dim_kpe, page_size, sm_scale, q_data_type, kv_data_type):
        ...
        self.wrapper.plan(qo_indptr, kv_indptr, kv_indices, kv_len_arr, num_heads, head_dim_ckv, head_dim_kpe, page_size, True, sm_scale, q_data_type, kv_data_type, bsz_tensor)
...
class MLAWrapperSingleton:
    ...
    @classmethod
    def plan_all(cls, qo_indptr, kv_indptr, kv_indices, kv_len_arr, bsz_tensor, num_heads, head_dim_ckv, head_dim_kpe, page_size, sm_scale, q_data_type, kv_data_type):
        for device, wrapper in cls.wrappers.items():
            kv_len_arr_cur_device = kv_len_arr.to(device)
            wrapper.plan(qo_indptr, kv_indptr, kv_indices, kv_len_arr_cur_device, bsz_tensor, num_heads, head_dim_ckv, head_dim_kpe, page_size, sm_scale, q_data_type, kv_data_type)
    ...
    def update_buffer(cls, max_pages):
        for device, wrapper in cls.wrappers.items():
            wrapper.kv_indptr_buf[1] = max_pages
...
def checksame():
    ...
    for forward_id in range(0, 19):
        ...
        for layer_id in range(max_layer_id):
            ...
            triton_tensor = torch.load(triton_path)[1:2, :62]
            try:
                torch.testing.assert_close(flashinfer_tensor, triton_tensor, rtol=1e-09, atol=1e-09)
...
if __name__ == '__main__':
    ...
    wrapper = MLAWrapperSingleton.get_instance('cuda', max_batch_size, max_pages)
    ...
    wrapper.plan(qo_indptr, kv_indptr, kv_indices, kv_len_arr, bsz_tensor, 128, 512, 64, page_size, 192 ** (-0.5), torch.bfloat16, torch.bfloat16)
    ...
    k = torch.cat([ckv, k_pe], dim=-1).view(-1, 1, 512 + 64).repeat_interleave(num_heads, dim=1)
    v = ckv.view(-1, 1, 512).repeat_interleave(num_heads, dim=1)
    attn_ref, lse_ref = attention_ref_torch(1, q[:q_len], k[:kv_len], v[:kv_len], True, 192 ** (-0.5))
    torch.testing.assert_close(attn_output[:q_len], attn_ref, rtol=0.005, atol=0.005)
    kv_len = 512
    q_len = 128
    pages = max_pages
    used_pages = (kv_len + page_size - 1) // page_size
    ...
    k = torch.cat([ckv, k_pe], dim=-1).view(-1, 1, 512 + 64).repeat_interleave(num_heads, dim=1)
    v = ckv.view(-1, 1, 512).repeat_interleave(num_heads, dim=1)
    attn_ref, lse_ref = attention_ref_torch(max_batch_size, q, k[:2 * kv_len], v[:2 * kv_len], True, 192 ** (-0.5))
    torch.testing.assert_close(attn_ref[:q_len], attn_ref[q_len:q_len * 2], rtol=1e-09, atol=1e-09)
    torch.testing.assert_close(attn_output[:q_len], attn_output[q_len:q_len * 2], rtol=1e-09, atol=1e-09)
    torch.testing.assert_close(attn_output[:q_len], attn_ref[:q_len], rtol=0.005, atol=0.005)
    torch.testing.assert_close(attn_output[q_len:q_len * 2], attn_ref[q_len:q_len * 2], rtol=0.005, atol=0.005)
    exit(0)
    for forward_id in range(0, 1):
        ...
        for layer_id in range(1):
            ...
            kv_len_arr = torch.tensor([kv_len], dtype=torch.int32, device='cuda')
            qo_indptr = torch.tensor([0, q_len], dtype=torch.int32, device='cuda')
            wrapper.plan(None, None, None, kv_len_arr, 128, 512, 64, page_size, 192 ** (-0.5), torch.bfloat16, torch.bfloat16)
            q_nope_buf.copy_(q_nope)
            q_pe_buf.copy_(q_pe)
            kv_buf[:pages].copy_(kv_cache)
            torch.cuda.synchronize()
            ...
            k = torch.cat([ckv, k_pe], dim=-1).view(-1, 1, 512 + 64).repeat_interleave(num_heads, dim=1)
            v = ckv.view(-1, 1, 512).repeat_interleave(num_heads, dim=1)
            attn_ref, lse_ref = attention_ref_torch(max_batch_size, q, k[:kv_len], v[:kv_len], False, 192 ** (-0.5))
            torch.testing.assert_close(attn_output, attn_ref, rtol=0.001, atol=0.001)
            attn_logits = torch.empty((max_batch_size, num_heads, 4, 512 + 1), dtype=torch.float32, device='cuda')
            ...
            decode_attention_fwd_grouped(q, ckv_with_pe, ckv, triton_ref, page_table, kv_len_arr, attn_logits, 4, 192 ** (-0.5), page_size)
            torch.testing.assert_close(attn_output, triton_ref, rtol=0.001, atol=0.001)
            print('test past')