...
def profile_origin(model, inputs, custom_ops=None, verbose=True, report_missing=False):
    ...
    def add_hooks(m):
        ...
        if hasattr(m, 'total_ops') or hasattr(m, 'total_params'):
            logging.warning("Either .total_ops or .total_params is already defined in %s. Be careful, it might change your code's behavior." % str(m))
...
def custom_profile(model: nn.Module, inputs, content, tokenizer, custom_ops=None, verbose=True, ret_layer_info=False, report_missing=False):
    handler_collection = {}
    types_collection = set()
    if custom_ops is None:
        custom_ops = {}
    ...
    with torch.no_grad():
        simple_prefill_and_generate_for_test(model.model, tokenizer, input_tensor.cuda(), max_new_tokens=1000, use_cuda_graph=False, mode='normal', force_think=False, chunk_prefill_size=8192)
