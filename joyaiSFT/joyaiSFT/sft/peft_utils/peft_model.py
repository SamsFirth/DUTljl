...
from peft.utils import SAFETENSORS_WEIGHTS_NAME, TRANSFORMERS_MODELS_TO_PREFIX_TUNING_POSTPROCESS_MAPPING, WEIGHTS_NAME, PeftType, TaskType, _get_batch_size, _prepare_prompt_learning_config, _set_adapter, _set_trainable, get_peft_model_state_dict, id_tensor_storage, infer_device, load_peft_weights, map_cache_to_layer_device_map, set_peft_model_state_dict, shift_tokens_right
from joyaiSFT.sft.peft_utils.lora_model import LoraModel
...
class PeftModel(PushToHubMixin, torch.nn.Module):
    ...
    **Attributes**:
    ...
    def __init__(self, model: PreTrainedModel, peft_config: PeftConfig, adapter_name: str='default', autocast_adapter_dtype: bool=True, low_cpu_mem_usage: bool=False) -> None:
        super().__init__()
    ...
    def save_pretrained(self, save_directory: str, safe_serialization: bool=True, selected_adapters: Optional[list[str]]=None, save_embedding_layers: Union[str, bool]='auto', is_main_process: bool=True, path_initial_model_for_weight_conversion: Optional[str]=None, **kwargs: Any) -> None:
        """
    ...
    @classmethod
    def from_pretrained(cls, model: torch.nn.Module, model_id: Union[str, os.PathLike], adapter_name: str='default', is_trainable: bool=False, config: Optional[PeftConfig]=None, autocast_adapter_dtype: bool=True, ephemeral_gpu_offload: bool=False, low_cpu_mem_usage: bool=False, **kwargs: Any) -> PeftModel:
        """
        ...
        if config.task_type not in MODEL_TYPE_TO_PEFT_MODEL_MAPPING.keys():
            model = cls(model, config, adapter_name, autocast_adapter_dtype=autocast_adapter_dtype, low_cpu_mem_usage=low_cpu_mem_usage)
        else:
            model = MODEL_TYPE_TO_PEFT_MODEL_MAPPING[config.task_type](model, config, adapter_name, autocast_adapter_dtype=autocast_adapter_dtype, low_cpu_mem_usage=low_cpu_mem_usage)
        load_result = model.load_adapter(model_id, adapter_name, is_trainable=is_trainable, autocast_adapter_dtype=autocast_adapter_dtype, low_cpu_mem_usage=low_cpu_mem_usage, **kwargs)
        missing_keys = [k for k in load_result.missing_keys if 'vblora_vector_bank' not in k and 'prompt_encoder' not in k]
        if missing_keys:
    ...
    def _check_new_adapter_config(self, peft_config: PeftConfig, is_trainable: bool) -> None:
        ...
        if len(all_configs) > 1:
            if any((getattr(config, 'init_lora_weights', None) == 'pissa' for config in all_configs)):
                msg = 'PiSSA changes the base weights of the model and should thus not be used with other adapters. Consider converting the PiSSA adapter into a normal LoRA adapter: https://github.com/huggingface/peft/tree/main/examples/pissa_finetuning#convert-pissa-to-lora'
                warnings.warn(msg)
            elif any((getattr(config, 'init_lora_weights', None) == 'olora' for config in all_configs)):
                msg = 'OLoRA changes the base weights of the model and should thus not be used with other adapters. Consider converting the OLoRA adapter into a normal LoRA adapter: https://github.com/huggingface/peft/tree/main/examples/olora_finetuning#olora-and-lora'
                warnings.warn(msg)

    def load_adapter(self, model_id: Union[str, os.PathLike], adapter_name: str, is_trainable: bool=False, torch_device: Optional[str]=None, autocast_adapter_dtype: bool=True, ephemeral_gpu_offload: bool=False, low_cpu_mem_usage: bool=False, **kwargs: Any):
        """
        ...
        load_result = set_peft_model_state_dict(self, adapters_weights, adapter_name=adapter_name, ignore_mismatched_sizes=ignore_mismatched_sizes, low_cpu_mem_usage=low_cpu_mem_usage)
...
class PeftModelForCausalLM(PeftModel):
        ...
        if not peft_config.is_prompt_learning:
            ...
            with self._enable_peft_forward_hooks(**kwargs):
                ...
                if isinstance(self.base_model, LoraModel):
                    return self.base_model.model(input_ids=input_ids, attention_mask=attention_mask, inputs_embeds=inputs_embeds, labels=labels, output_attentions=output_attentions, output_hidden_states=output_hidden_states, return_dict=return_dict, **kwargs)
    ...
    def _cpt_forward(self, input_ids=None, inputs_embeds=None, peft_config=None, task_ids=None, batch_size=None, **kwargs):
        ...
        if labels is None:
            ...
            base_model_output = CPTEmbedding.calculate_loss(base_model_output, cpt_labels, cpt_type_mask, self.peft_config['default'])
            return base_model_output
    ...
    def prepare_inputs_for_generation(self, *args, task_ids: Optional[torch.Tensor]=None, **kwargs):
        ...
        if peft_config.is_prompt_learning:
            ...
            if kwargs.get('token_type_ids', None) is not None:
                warnings.warn('Token type ids are not supported for parameter efficient tuning. Ignoring token type ids')
                kwargs['token_type_ids'] = None
            requires_prompt_injection = model_kwargs.get('past_key_values', None) is None or (isinstance(model_kwargs['past_key_values'], transformers.Cache) and (not model_kwargs['past_key_values'].get_seq_length()))
