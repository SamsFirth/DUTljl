...
from joyaiSFT.operators.linear import JOYSFTLinear, JOYLinearTorch, JOYLinearBase
...
def dispatch_default(target: torch.nn.Module, adapter_name: str, lora_config: LoraConfig, **kwargs) -> Optional[torch.nn.Module]:
    ...
    elif isinstance(target_orig_module, JOYSFTLinear):
        kwargs.update(lora_config.loftq_config)
        new_module = JOYSFTLinearLora(target, adapter_name, **kwargs)
    return new_module
...
class LoraLayer(BaseTunerLayer):
    ...
    def update_layer(self, adapter_name, r, lora_alpha, lora_dropout, init_lora_weights, use_rslora: bool=False, use_dora: bool=False, lora_bias: bool=False):
        if r <= 0:
...
class Linear(nn.Module, LoraLayer):
    ...
    def __init__(self, orig_module, adapter_name: str, r: int=0, lora_alpha: int=1, lora_dropout: float=0.0, fan_in_fan_out: bool=False, is_target_conv_1d_layer: bool=False, init_lora_weights: Union[bool, str]=True, use_rslora: bool=False, use_dora: bool=False, lora_bias: bool=False, **kwargs) -> None:
        super().__init__()
            ...
            if active_adapter in self.lora_A.keys():
                ...
                if safe_merge:
                    ...
                    if not self.use_dora[active_adapter]:
                        ...
                        weight_norm = self.lora_magnitude_vector[active_adapter].get_weight_norm(orig_module.weight, transpose(delta_weight, self.fan_in_fan_out), scaling=1).detach()
                        self._cache_store(f'{active_adapter}-weight_norm', weight_norm)
                        dora_factor = self.lora_magnitude_vector[active_adapter].weight / weight_norm
                        dora_factor = transpose(dora_factor.view(-1, 1), self.fan_in_fan_out)
...
class Embedding(nn.Module, LoraLayer):
    ...
    def __init__(self, orig_module: nn.Module, adapter_name: str, r: int=0, lora_alpha: int=1, lora_dropout: float=0.0, init_lora_weights: Union[bool, str]=True, use_rslora: bool=False, use_dora: bool=False, lora_bias: bool=False, **kwargs) -> None:
        if lora_bias:
    ...
    def _embed(self, input: torch.Tensor, weight: torch.Tensor) -> torch.Tensor:
        orig_module = self.get_orig_module()
        return F.embedding(input, weight, padding_idx=orig_module.padding_idx, max_norm=orig_module.max_norm, norm_type=orig_module.norm_type, scale_grad_by_freq=orig_module.scale_grad_by_freq, sparse=orig_module.sparse)
        ...
        if self.disable_adapters:
            ...
            for active_adapter in self.active_adapters:
                ...
                if not self.use_dora[active_adapter]:
                    ...
                    mag_norm_scale, dora_result = self.lora_magnitude_vector[active_adapter](x, lora_A=embedding_A, lora_B=embedding_B, scaling=scaling, orig_module=self.get_orig_module(), embed_fn=self._embed)
...
class JOYSFTLinearLora(JOYSFTLinear, LoraLayer):

    def __init__(self, orig_module: JOYSFTLinear, adapter_name: str, r: int=0, lora_alpha: int=1, lora_dropout: float=0.0, fan_in_fan_out: bool=False, is_target_conv_1d_layer: bool=False, init_lora_weights: Union[bool, str]=True, use_rslora: bool=False, use_dora: bool=False, lora_bias: bool=False, **kwargs):
        JOYSFTLinear.__init__(self, key=orig_module.key, gguf_loader=orig_module.gguf_loader, config=orig_module.config, orig_module=orig_module.orig_module, generate_device=orig_module.generate_device, prefill_device=orig_module.prefill_device, prefill_op='JOYLinearTorch', generate_op='JOYLinearTorch', **kwargs)
            ...
            if active_adapter in self.lora_A:
                ...
                if safe_merge:
                    ...
                    if not self.use_dora.get(active_adapter, False):
                        ...
                        weight_norm = self.lora_magnitude_vector[active_adapter].get_weight_norm(orig_module.weight.data, transpose(delta_weight, self.fan_in_fan_out), scaling=1).detach()
                        self._cache_store(f'{active_adapter}-weight_norm', weight_norm)
                        dora_factor = self.lora_magnitude_vector[active_adapter].weight / weight_norm
                        dora_factor = transpose(dora_factor.view(-1, 1), self.fan_in_fan_out)
