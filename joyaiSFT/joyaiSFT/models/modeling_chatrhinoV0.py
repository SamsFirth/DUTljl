...
from .configuration_chatrhinoV0 import ChatrhinoV0Config
...
_CONFIG_FOR_DOC = 'ChatrhinoV0Config'
...
class ChatrhinoV0RMSNorm(nn.Module):

    def __init__(self, hidden_size, eps=1e-06):
        """
        ChatrhinoV0RMSNorm is equivalent to T5LayerNorm
        """
        super().__init__()
...
ALL_LAYERNORM_LAYERS.append(ChatrhinoV0RMSNorm)

class ChatrhinoV0RotaryEmbedding(nn.Module):
...
class ChatrhinoV0LinearScalingRotaryEmbedding(ChatrhinoV0RotaryEmbedding):
    """ChatrhinoV0RotaryEmbedding extended with linear scaling. Credits to the Reddit user /u/kaiokendev"""
...
class ChatrhinoV0DynamicNTKScalingRotaryEmbedding(ChatrhinoV0RotaryEmbedding):
    """ChatrhinoV0RotaryEmbedding extended with Dynamic NTK scaling. Credits to the Reddit users /u/bloc97 and /u/emozilla"""
...
class ChatrhinoV0YarnRotaryEmbedding(ChatrhinoV0RotaryEmbedding):
...
class ChatrhinoV0MLP(nn.Module):
...
class ChatrhinoV0MoE(nn.Module):
    """
    ...
    def __init__(self, config):
        ...
        if hasattr(config, 'ep_size') and config.ep_size > 1:
            ...
            self.experts = nn.ModuleList([ChatrhinoV0MLP(config, intermediate_size=config.moe_intermediate_size) if i >= self.ep_rank * self.experts_per_rank and i < (self.ep_rank + 1) * self.experts_per_rank else None for i in range(config.n_routed_experts)])
        else:
            self.ep_size = 1
            self.experts_per_rank = config.n_routed_experts
            self.ep_rank = 0
            self.experts = nn.ModuleList([ChatrhinoV0MLP(config, intermediate_size=config.moe_intermediate_size) for i in range(config.n_routed_experts)])
        self.gate = MoEGate(config)
        if config.n_shared_experts is not None:
            intermediate_size = config.moe_intermediate_size * config.n_shared_experts
            self.shared_experts = ChatrhinoV0MLP(config=config, intermediate_size=intermediate_size)
...
class ChatrhinoV0Attention(nn.Module):
    """Multi-headed attention from 'Attention Is All You Need' paper"""

    def __init__(self, config: ChatrhinoV0Config, layer_idx: Optional[int]=None):
        super().__init__()
        self.config = config
        ...
        if self.q_lora_rank is None:
            ...
            self.q_a_layernorm = ChatrhinoV0RMSNorm(config.q_lora_rank)
        ...
        self.kv_a_layernorm = ChatrhinoV0RMSNorm(config.kv_lora_rank)
    ...
    def _init_rope(self):
        if self.config.rope_scaling is None:
            self.rotary_emb = ChatrhinoV0RotaryEmbedding(self.qk_rope_head_dim, max_position_embeddings=self.max_position_embeddings, base=self.rope_theta)
        else:
            scaling_type = self.config.rope_scaling['type']
            scaling_factor = self.config.rope_scaling['factor']
            if scaling_type == 'linear':
                self.rotary_emb = ChatrhinoV0LinearScalingRotaryEmbedding(self.qk_rope_head_dim, max_position_embeddings=self.max_position_embeddings, scaling_factor=scaling_factor, base=self.rope_theta)
            elif scaling_type == 'dynamic':
                self.rotary_emb = ChatrhinoV0DynamicNTKScalingRotaryEmbedding(self.qk_rope_head_dim, max_position_embeddings=self.max_position_embeddings, scaling_factor=scaling_factor, base=self.rope_theta)
                ...
                self.rotary_emb = ChatrhinoV0YarnRotaryEmbedding(self.qk_rope_head_dim, max_position_embeddings=self.max_position_embeddings, scaling_factor=scaling_factor, base=self.rope_theta, **kwargs)
            else:
...
class ChatrhinoV0FlashAttention2(ChatrhinoV0Attention):
    """
    ChatrhinoV0 flash attention module. This module inherits from `ChatrhinoV0Attention` as the weights of the module stays
    ...
    def forward(self, hidden_states: torch.Tensor, attention_mask: Optional[torch.LongTensor]=None, position_ids: Optional[torch.LongTensor]=None, past_key_value: Optional[Cache]=None, output_attentions: bool=False, use_cache: bool=False, cache_position: Optional[torch.LongTensor]=None, **kwargs) -> Tuple[torch.Tensor, Optional[torch.Tensor], Optional[Tuple[torch.Tensor]]]:
        ...
        attn_output = self._flash_attention_forward(query_states, key_states, value_states, attention_mask, q_len, position_ids=position_ids, dropout=dropout_rate, softmax_scale=self.softmax_scale)
        if self.q_head_dim != self.v_head_dim:
    ...
    def _flash_attention_forward(self, query_states, key_states, value_states, attention_mask, query_length, position_ids, dropout=0.0, softmax_scale=None):
        """
        ...
        if attention_mask is not None:
            ...
            attn_output = flash_attn_with_kvcache(query_states, key_states, value_states, cache_seqlens=position_ids, softmax_scale=softmax_scale, causal=causal)
        else:
...
ATTENTION_CLASSES = {'eager': ChatrhinoV0Attention, 'flash_attention_2': ChatrhinoV0FlashAttention2}

class ChatrhinoV0DecoderLayer(nn.Module):

    def __init__(self, config: ChatrhinoV0Config, layer_idx: int):
        super().__init__()
        self.hidden_size = config.hidden_size
        self.self_attn = ATTENTION_CLASSES[config._attn_implementation](config=config, layer_idx=layer_idx)
        self.mlp = ChatrhinoV0MoE(config) if config.n_routed_experts is not None and layer_idx >= config.first_k_dense_replace and (layer_idx % config.moe_layer_freq == 0) else ChatrhinoV0MLP(config)
        self.input_layernorm = ChatrhinoV0RMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.post_attention_layernorm = ChatrhinoV0RMSNorm(config.hidden_size, eps=config.rms_norm_eps)
    ...
    def forward(self, hidden_states: torch.Tensor, attention_mask: Optional[torch.Tensor]=None, position_ids: Optional[torch.LongTensor]=None, past_key_value: Optional[Tuple[torch.Tensor]]=None, output_attentions: Optional[bool]=False, use_cache: Optional[bool]=False, cache_position: Optional[torch.LongTensor]=None, **kwargs) -> Tuple[torch.FloatTensor, Optional[Tuple[torch.FloatTensor, torch.FloatTensor]]]:
        ...
        return outputs
ChatrhinoV0_START_DOCSTRING = '\n    This model inherits from [`PreTrainedModel`]. Check the superclass documentation for the generic methods the\n    library implements for all its model (such as downloading or saving, resizing the input embeddings, pruning heads\n    etc.)\n\n    This model is also a PyTorch [torch.nn.Module](https://pytorch.org/docs/stable/nn.html#torch.nn.Module) subclass.\n    Use it as a regular PyTorch Module and refer to the PyTorch documentation for all matter related to general usage\n    and behavior.\n\n    Parameters:\n        config ([`ChatrhinoV0Config`]):\n            Model configuration class with all the parameters of the model. Initializing with a config file does not\n            load the weights associated with the model, only the configuration. Check out the\n            [`~PreTrainedModel.from_pretrained`] method to load the model weights.\n'

@add_start_docstrings('The bare ChatrhinoV0 Model outputting raw hidden-states without any specific head on top.', ChatrhinoV0_START_DOCSTRING)
class ChatrhinoV0PreTrainedModel(PreTrainedModel):
    config_class = ChatrhinoV0Config
    base_model_prefix = 'model'
    supports_gradient_checkpointing = True
    _no_split_modules = ['ChatrhinoV0DecoderLayer']
...
@add_start_docstrings('The bare ChatrhinoV0 Model outputting raw hidden-states without any specific head on top.', ChatrhinoV0_START_DOCSTRING)
class ChatrhinoV0Model(ChatrhinoV0PreTrainedModel):
    """
    ...
    def __init__(self, config: ChatrhinoV0Config):
        super().__init__(config)
        self.padding_idx = config.pad_token_id
        self.vocab_size = config.vocab_size
        self.embed_tokens = nn.Embedding(config.vocab_size, config.hidden_size, self.padding_idx)
        self.layers = nn.ModuleList([ChatrhinoV0DecoderLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)])
        self._use_flash_attention_2 = config._attn_implementation == 'flash_attention_2'
        self.norm = ChatrhinoV0RMSNorm(config.hidden_size, eps=config.rms_norm_eps)
...
class ChatrhinoV0ForCausalLM(ChatrhinoV0PreTrainedModel):
    _tied_weights_keys = ['lm_head.weight']

    def __init__(self, config):
        super().__init__(config)
        self.model = ChatrhinoV0Model(config)
        self.vocab_size = config.vocab_size
    ...
    def forward(self, input_ids: torch.LongTensor=None, attention_mask: Optional[torch.Tensor]=None, position_ids: Optional[torch.LongTensor]=None, past_key_values: Optional[List[torch.FloatTensor]]=None, inputs_embeds: Optional[torch.FloatTensor]=None, labels: Optional[torch.LongTensor]=None, use_cache: Optional[bool]=None, output_attentions: Optional[bool]=None, output_hidden_states: Optional[bool]=None, return_dict: Optional[bool]=None, cache_position: Optional[torch.LongTensor]=None, **kwargs) -> Union[Tuple, CausalLMOutputWithPast]:
        ...
        ```python
        >>> from transformers import AutoTokenizer, ChatrhinoV0ForCausalLM

        >>> model = ChatrhinoV0ForCausalLM.from_pretrained(PATH_TO_CONVERTED_WEIGHTS)
    ...
    def prepare_inputs_for_generation(self, input_ids, past_key_values=None, attention_mask=None, inputs_embeds=None, cache_position=None, use_cache=True, **kwargs):
        ...
        model_inputs.update({'position_ids': position_ids, 'past_key_values': past_key_values, 'use_cache': use_cache, 'attention_mask': attention_mask, 'cache_position': cache_position})
        return model_inputs
...
@add_start_docstrings('\n    The ChatrhinoV0 Model transformer with a sequence classification head on top (linear layer).\n\n    [`ChatrhinoV0ForSequenceClassification`] uses the last token in order to do the classification, as other causal models\n    (e.g. GPT-2) do.\n\n    Since it does classification on the last token, it requires to know the position of the last token. If a\n    `pad_token_id` is defined in the configuration, it finds the last token that is not a padding token in each row. If\n    no `pad_token_id` is defined, it simply takes the last value in each row of the batch. Since it cannot guess the\n    padding tokens when `inputs_embeds` are passed instead of `input_ids`, it does the same (take the last value in\n    each row of the batch).\n    ', ChatrhinoV0_START_DOCSTRING)
class ChatrhinoV0ForSequenceClassification(ChatrhinoV0PreTrainedModel):
