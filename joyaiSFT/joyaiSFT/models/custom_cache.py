...
class JOYChatrhinoCache(nn.Module):

    def __init__(self, config: PretrainedConfig, page_size: int=256, dtype=torch.bfloat16, device=torch.device('cuda:0')):
        super().__init__()
        self.config = config
        self.dtype = dtype
        self.device = device
    ...
    def load(self, inference_context: 'sched_ext.InferenceContext'):
        for i in range(self.config.num_hidden_layers):
            self.k_caches.append(inference_context.k_cache[0][i])
        self.max_cache_len = self.k_caches[0].shape[0] * self.k_caches[0].shape[1]

    def update(self, key_states: torch.Tensor, value_states: torch.Tensor, layer_idx: int, page_idx: torch.Tensor, page_offset: torch.Tensor, cache_kwargs: Optional[Dict[str, Any]]=None) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        Updates the cache with the new `key_states` and `value_states` for the layer `layer_idx`.
        It is VERY important to index using a tensor, otherwise you introduce a copy to the device.
...
class JOYGQACache(nn.Module):
    ...
    def load(self, inference_context: 'sched_ext.InferenceContext'):
        ...
        for i in range(self.config.num_hidden_layers):
            self.k_caches.append(inference_context.k_cache[0][i])
            self.v_caches.append(inference_context.v_cache[0][i])
        self.max_cache_len = self.k_caches[0].shape[0] * self.k_caches[0].shape[1]
