...
from joyaiSFT.models.modeling_chatrhinoV0 import ChatrhinoV0Model, ChatrhinoV0PreTrainedModel
from joyaiSFT.models.configuration_chatrhinoV0 import ChatrhinoV0Config
torch.set_grad_enabled(False)
torch.set_default_dtype(torch.bfloat16)
import flashinfer

class JOYChatrhinoV0ForCausalLM(ChatrhinoV0PreTrainedModel):
    kv_cache: JOYChatrhinoCache
    ...
    def init_wrapper(self, use_cuda_graph, device, max_batch_size, max_pages):
        ...
        self.wrapper = flashinfer.mla.BatchMLAPagedAttentionWrapper(self.workspace_buffer, use_cuda_graph=use_cuda_graph, qo_indptr=self.qo_indptr_buf, kv_indptr=self.paged_kv_indptr_buf, kv_indices=self.paged_kv_indices_buf, kv_len_arr=self.paged_kv_len_buf, backend='fa2')
    ...
    def forward(self, batch: ForwardBatchInput | None=None, features: List[torch.Tensor] | None=None, bsz_tensors: torch.Tensor | None=None, num_tokens_tensors: torch.Tensor | None=None, page_idx: torch.Tensor | None=None, page_offset: torch.Tensor | None=None) -> ForwardBatchOutput:
    ...
    def flash_infer_attn_plan(self, batch: ForwardBatchInput, bsz_tensors, num_tokens_tensors, num_heads: int, head_dim_ckv: int, head_dim_kpe: int, page_size: int, causal: bool, sm_scale: float, q_data_type: torch.dtype, kv_data_type: torch.dtype):
        ...
        self.wrapper.plan(minibatch.q_indptr, minibatch.kv_indptr, minibatch.kv_indices, minibatch.kv_len, num_heads, head_dim_ckv, head_dim_kpe, page_size, causal, sm_scale, q_data_type, kv_data_type)