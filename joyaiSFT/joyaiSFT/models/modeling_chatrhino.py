...
from .configuration_chatrhino import ChatrhinoConfig
...
_CONFIG_FOR_DOC = 'ChatrhinoConfig'
...
class ChatrhinoRMSNorm(nn.Module):

    def __init__(self, hidden_size, eps=1e-06):
        """
        ChatrhinoRMSNorm is equivalent to T5LayerNorm
        """
        super().__init__()
...
ALL_LAYERNORM_LAYERS.append(ChatrhinoRMSNorm)

class ChatrhinoRotaryEmbedding(nn.Module):
...
class ChatrhinoLinearScalingRotaryEmbedding(ChatrhinoRotaryEmbedding):
    """ChatrhinoRotaryEmbedding extended with linear scaling. Credits to the Reddit user /u/kaiokendev"""
...
class ChatrhinoDynamicNTKScalingRotaryEmbedding(ChatrhinoRotaryEmbedding):
    """ChatrhinoRotaryEmbedding extended with Dynamic NTK scaling. Credits to the Reddit users /u/bloc97 and /u/emozilla"""
...
class ChatrhinoYarnRotaryEmbedding(ChatrhinoRotaryEmbedding):
...
class ChatrhinoMoE(nn.Module):
    ...
    def __init__(self, config):
        ...
        if hasattr(config, 'ep_size') and config.ep_size > 1:
            ...
            self.experts = nn.ModuleList([ChatrhinoMLP(config, intermediate_size=config.moe_intermediate_size) if i >= self.ep_rank * self.experts_per_rank and i < (self.ep_rank + 1) * self.experts_per_rank else None for i in range(config.n_routed_experts)])
        else:
            self.ep_size = 1
            self.experts_per_rank = config.n_routed_experts
            self.ep_rank = 0
            self.experts = nn.ModuleList([ChatrhinoMLP(config, intermediate_size=config.moe_intermediate_size) for i in range(config.n_routed_experts)])
        self.gate = MoEGate(config)
        if config.n_shared_experts is not None:
            intermediate_size = config.moe_intermediate_size * config.n_shared_experts
            self.shared_experts = ChatrhinoMLP(config=config, intermediate_size=intermediate_size)
...
class ChatrhinoAttention(nn.Module):
    """Multi-headed attention from 'Attention Is All You Need' paper"""

    def __init__(self, config: ChatrhinoConfig, layer_idx: Optional[int]=None):
        super().__init__()
        self.config = config
        ...
        if self.q_lora_rank is None:
            ...
            self.q_a_layernorm = ChatrhinoRMSNorm(config.q_lora_rank)
        ...
        self.kv_a_layernorm = ChatrhinoRMSNorm(config.kv_lora_rank)
    ...
    def _init_rope(self):
        if self.config.rope_scaling is None:
            self.rotary_emb = ChatrhinoRotaryEmbedding(self.qk_rope_head_dim, max_position_embeddings=self.max_position_embeddings, base=self.rope_theta)
        else:
            scaling_type = self.config.rope_scaling['type']
            scaling_factor = self.config.rope_scaling['factor']
            if scaling_type == 'linear':
                self.rotary_emb = ChatrhinoLinearScalingRotaryEmbedding(self.qk_rope_head_dim, max_position_embeddings=self.max_position_embeddings, scaling_factor=scaling_factor, base=self.rope_theta)
            elif scaling_type == 'dynamic':
                self.rotary_emb = ChatrhinoDynamicNTKScalingRotaryEmbedding(self.qk_rope_head_dim, max_position_embeddings=self.max_position_embeddings, scaling_factor=scaling_factor, base=self.rope_theta)
                ...
                self.rotary_emb = ChatrhinoYarnRotaryEmbedding(self.qk_rope_head_dim, max_position_embeddings=self.max_position_embeddings, scaling_factor=scaling_factor, base=self.rope_theta, **kwargs)
            else:
...
class ChatrhinoFlashAttention2(ChatrhinoAttention):
    """
    Chatrhino flash attention module. This module inherits from `ChatrhinoAttention` as the weights of the module stays
...
ATTENTION_CLASSES = {'eager': ChatrhinoAttention, 'flash_attention_2': ChatrhinoFlashAttention2}

class ChatrhinoDecoderLayer(nn.Module):

    def __init__(self, config: ChatrhinoConfig, layer_idx: int):
        super().__init__()
        self.hidden_size = config.hidden_size
        self.self_attn = ATTENTION_CLASSES[config._attn_implementation](config=config, layer_idx=layer_idx)
        self.mlp = ChatrhinoMoE(config) if config.n_routed_experts is not None and layer_idx >= config.first_k_dense_replace and (layer_idx % config.moe_layer_freq == 0) else ChatrhinoMLP(config)
        self.input_layernorm = ChatrhinoRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.post_attention_layernorm = ChatrhinoRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
    ...
    def forward(self, hidden_states: torch.Tensor, attention_mask: Optional[torch.Tensor]=None, position_ids: Optional[torch.LongTensor]=None, past_key_value: Optional[Tuple[torch.Tensor]]=None, output_attentions: Optional[bool]=False, use_cache: Optional[bool]=False, cache_position: Optional[torch.LongTensor]=None, **kwargs) -> Tuple[torch.FloatTensor, Optional[Tuple[torch.FloatTensor, torch.FloatTensor]]]:
        ...
        return outputs
Chatrhino_START_DOCSTRING = '\n    This model inherits from [`PreTrainedModel`]. Check the superclass documentation for the generic methods the\n    library implements for all its model (such as downloading or saving, resizing the input embeddings, pruning heads\n    etc.)\n\n    This model is also a PyTorch [torch.nn.Module](https://pytorch.org/docs/stable/nn.html#torch.nn.Module) subclass.\n    Use it as a regular PyTorch Module and refer to the PyTorch documentation for all matter related to general usage\n    and behavior.\n\n    Parameters:\n        config ([`ChatrhinoConfig`]):\n            Model configuration class with all the parameters of the model. Initializing with a config file does not\n            load the weights associated with the model, only the configuration. Check out the\n            [`~PreTrainedModel.from_pretrained`] method to load the model weights.\n'

@add_start_docstrings('The bare Chatrhino Model outputting raw hidden-states without any specific head on top.', Chatrhino_START_DOCSTRING)
class ChatrhinoPreTrainedModel(PreTrainedModel):
    config_class = ChatrhinoConfig
    base_model_prefix = 'model'
    supports_gradient_checkpointing = True
    _no_split_modules = ['ChatrhinoDecoderLayer']
...
class ChatrhinoModel(ChatrhinoPreTrainedModel):
    ...
    @add_start_docstrings_to_model_forward(Chatrhino_INPUTS_DOCSTRING)
    def forward(self, input_ids: torch.LongTensor=None, attention_mask: Optional[torch.Tensor]=None, position_ids: Optional[torch.LongTensor]=None, past_key_values: Optional[List[torch.FloatTensor]]=None, inputs_embeds: Optional[torch.FloatTensor]=None, use_cache: Optional[bool]=None, output_attentions: Optional[bool]=None, output_hidden_states: Optional[bool]=None, return_dict: Optional[bool]=None, cache_position: Optional[torch.LongTensor]=None) -> Union[Tuple, BaseModelOutputWithPast]:
        ...
        for decoder_layer in self.layers:
            ...
            layer_outputs = decoder_layer(hidden_states, attention_mask=attention_mask, position_ids=position_ids, past_key_value=past_key_values, output_attentions=output_attentions, use_cache=use_cache, cache_position=cache_position)
...
class ChatrhinoForCausalLM(ChatrhinoPreTrainedModel, GenerationMixin):
    _tied_weights_keys = ['lm_head.weight']

    def __init__(self, config):
        super().__init__(config)
        self.model = ChatrhinoModel(config)
        self.vocab_size = config.vocab_size
    ...
    def forward(self, input_ids: torch.LongTensor=None, attention_mask: Optional[torch.Tensor]=None, position_ids: Optional[torch.LongTensor]=None, past_key_values: Optional[List[torch.FloatTensor]]=None, inputs_embeds: Optional[torch.FloatTensor]=None, labels: Optional[torch.LongTensor]=None, use_cache: Optional[bool]=None, output_attentions: Optional[bool]=None, output_hidden_states: Optional[bool]=None, return_dict: Optional[bool]=None, cache_position: Optional[torch.LongTensor]=None) -> Union[Tuple, CausalLMOutputWithPast]:
        ...
        ```python
        >>> from transformers import AutoTokenizer, ChatrhinoForCausalLM

        >>> model = ChatrhinoForCausalLM.from_pretrained(PATH_TO_CONVERTED_WEIGHTS)
...
@add_start_docstrings('\n    The Chatrhino Model transformer with a sequence classification head on top (linear layer).\n\n    [`ChatrhinoForSequenceClassification`] uses the last token in order to do the classification, as other causal models\n    (e.g. GPT-2) do.\n\n    Since it does classification on the last token, it requires to know the position of the last token. If a\n    `pad_token_id` is defined in the configuration, it finds the last token that is not a padding token in each row. If\n    no `pad_token_id` is defined, it simply takes the last value in each row of the batch. Since it cannot guess the\n    padding tokens when `inputs_embeds` are passed instead of `input_ids`, it does the same (take the last value in\n    each row of the batch).\n    ', Chatrhino_START_DOCSTRING)
class ChatrhinoForSequenceClassification(ChatrhinoPreTrainedModel):
