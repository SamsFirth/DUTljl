...
from joyaiSFT.models.modeling_chatrhino import ChatrhinoModel, ChatrhinoPreTrainedModel
from joyaiSFT.models.configuration_chatrhino import ChatrhinoConfig
torch.set_grad_enabled(False)
torch.set_default_dtype(torch.bfloat16)
import flashinfer

class JOYChatrhinoForCausalLM(ChatrhinoPreTrainedModel):
    cache: JOYChatrhinoCache
    use_cuda_graph = False

    def __init__(self, config: ChatrhinoConfig, cache):
    ...
    def init_wrapper(self, use_cuda_graph, device, max_batch_size, max_pages):
        ...
        self.wrapper = flashinfer.mla.BatchMLAPagedAttentionWrapper(self.workspace_buffer, use_cuda_graph=use_cuda_graph, qo_indptr=self.qo_indptr_buf, kv_indptr=self.paged_kv_indptr_buf, kv_indices=self.paged_kv_indices_buf, kv_len_arr=self.paged_kv_len_buf, bsz_tensor=self.bsz_tensor_buf, backend='fa2')

    def batch_embeddings(self, batch: ForwardBatchInput, device='cuda:0'):
        features = []
        for i in range(batch.batch_size):
            tokens = batch.minibatch.tokens.contiguous()
            feature = self.model.embed_tokens(tokens.to(torch.device('cpu'))).to(torch.bfloat16).to(device=device)
            features.append(feature)
        return features

    def forward(self, batch: ForwardBatchInput | None=None, features: List[torch.Tensor] | None=None, bsz_tensors: torch.Tensor | None=None, num_tokens_tensors: torch.Tensor | None=None, page_idx: torch.Tensor | None=None, page_offset: torch.Tensor | None=None, cuda_graph_idx: int | None=-1) -> ForwardBatchOutput:
        ...
        with torch.cuda.stream(current_stream):
            ...
            for i, decode_layer in enumerate(self.model.layers):
                if self.model.transfer_map is not None and i in self.model.transfer_map:
                    ...
                    batch.minibatch.position_ids = batch.minibatch.position_ids.to(self.model.transfer_map[i], non_blocking=True) if batch.minibatch.position_ids is not None else None
                hidden_states, residual = decode_layer.input_layernorm(hidden_states, num_tokens_tensors, residual)
                hidden_states = decode_layer.self_attn(hidden_states, self.cache, position_ids=batch.minibatch.position_ids, wrapper=self.wrapper, num_tokens_tensors=num_tokens_tensors, page_idx=page_idx, page_offset=page_offset)
    ...
    def flash_infer_attn_plan(self, batch: ForwardBatchInput, bsz_tensors, num_tokens_tensors, num_heads: int, head_dim_ckv: int, head_dim_kpe: int, page_size: int, causal: bool, sm_scale: float, q_data_type: torch.dtype, kv_data_type: torch.dtype):
        minibatch = batch.minibatch
        self.wrapper.plan(minibatch.q_indptr, minibatch.kv_indptr, minibatch.kv_indices, minibatch.kv_len, num_heads, head_dim_ckv, head_dim_kpe, page_size, causal, sm_scale, q_data_type, kv_data_type, bsz_tensors)