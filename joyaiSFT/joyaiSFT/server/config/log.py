    ...
    def delete_expired_files(self):
        ...
        for file_name in file_names:
            if file_name[:plen] == prefix:
                ...
                if re.match('^\\d{4}-\\d{2}-\\d{2}(\\.\\w+)?$', suffix):
...
class Logger(object):
    ...
    logger class
    """
    level_relations = {'debug': logging.DEBUG, 'info': logging.INFO, 'warn': logging.WARNING, 'error': logging.ERROR, 'crit': logging.CRITICAL}
    ...
    def __init__(self, level: str='info'):
        ...
        color_formatter = colorlog.ColoredFormatter(color_fmt, log_colors={'DEBUG': 'cyan', 'INFO': 'green', 'WARNING': 'yellow', 'ERROR': 'red', 'CRITICAL': 'bold_red'})
