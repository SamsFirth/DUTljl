...
class ConfigArgs(BaseModel):
    ...
    max_batch_size: int = Field(None, description='Max number of batches to run at once, assuming the sequences will fit within total_context')
    chunk_size: int = Field(None, description='Max chunk size. Determines the size of prefill operations. Can be reduced to reduce pauses whenever a new job is started, but at the expense of overall prompt ingestion speed')
    max_new_tokens: int = Field(None, description='Max new tokens per completion. For this example applies to all jobs')
    json_mode: bool = Field(None, description='Use LMFE to constrain the output to JSON format. See schema and details below')
    ...
    experts_per_token: Optional[int] = Field(None, description="Override MoE model's default number of experts per token")
    ...
    no_draft_scale: bool = Field(None, description="If draft model has smaller context size than model, don't apply alpha (NTK) scaling to extend it")
    ...
    dynamic_temperature: Optional[str] = Field(None, description='Dynamic temperature min,max,exponent, e.g. -dyntemp 0.2,1.5,1')
