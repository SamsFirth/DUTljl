...
from joyaiSFT.server.crud.assistants.threads import ThreadsDatabaseManager
from joyaiSFT.server.exceptions import request_error
from joyaiSFT.server.schemas.assistants.assistants import AssistantObject
from joyaiSFT.server.schemas.assistants.messages import MessageCreate, MessageObject, Role
...
class ThreadContext:
    ...
    def put_user_message(self, message: MessageObject):
        assert message.role.is_user() and message.thread_id == self.thread.id and (message.status == MessageObject.Status.in_progress)
    ...
    async def work(self) -> AsyncIterator:
        ...
        local_messages = self.get_local_messages()
        response_str_count = 0
        reply_message = self.message_manager.create_message_object(self.thread.id, self.run.id, MessageCreate(role=Role.assistant, content=''))
