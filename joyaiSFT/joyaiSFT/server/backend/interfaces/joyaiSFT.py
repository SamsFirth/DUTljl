...
from joyaiSFT.server.backend.interfaces.transformers import TransformersInterface, ConfigArgs, TransformersThreadContext, default_args, TextStreamer
...
from joyaiSFT.local_chat import custom_models, default_optimize_rules
from joyaiSFT.util.utils import get_device
from typing import Optional
from joyaiSFT.operators.flashinfer_wrapper import flashinfer_enabled, MLAWrapperSingleton
from joyaiSFT.server.schemas.endpoints.chat import RawUsage
from joyaiSFT.util.grad_wrapper import maybe_no_grad
warm_uped = False

class JOYSFTThreadContext(TransformersThreadContext):
    pass

class JOYSFTInterface(TransformersInterface):
    ...
    def __init__(self, args: ConfigArgs=default_args):
        ...
        self.cache = StaticCache(config=self.model.config, max_batch_size=args.batch_size, max_cache_len=args.cache_lens, device=self.device_map, dtype=self.model.dtype)
    ...
    def decode_one_tokens(self):
        ...
        if self.use_static_cache:
            logits = self.model(self.current_ids.to(torch_device), cache_position=self.active_cache_position, past_key_values=self.cache, return_dict=False, use_cache=True)[0]
        else:
    ...
    def prefill(self, input_ids: torch.Tensor, is_new: bool, temperature: Optional[float]=None, top_p: Optional[float]=None, max_tokens: Optional[float]=None, max_completion_tokens: Optional[float]=None):
        ...
        if is_new:
            ...
            if getattr(self, 'generated_ids', None) is None:
                self.generated_ids = torch.zeros(self.args.batch_size, input_ids.shape[-1] + max_new_tokens + 1, dtype=torch.int, device=self.args.device)
                self.seq_length = 1
        ...
        def chunk_prefill(input_ids, cache_position):
            ...
            if self.use_static_cache:
                logits = self.model(inputs_embeds=inputs_embeds, cache_position=cache_position, past_key_values=self.cache, return_dict=False, use_cache=True)[0]
            else:
    ...
    async def inference(self, local_messages, thread_id: str, temperature: Optional[float]=None, top_p: Optional[float]=None, max_tokens: Optional[float]=None, max_completion_tokens: Optional[float]=None):
        async with self._infer_lock:
            async for v in super().inference(local_messages, thread_id, temperature, top_p, max_tokens, max_completion_tokens):
                yield v
            yield RawUsage(tokenize_time=self.profiler.get_timer_sec('tokenize'), prefill_time=self.profiler.get_timer_sec('prefill'), decode_time=self.profiler.get_timer_sec('decode'), prefill_count=self.profiler.get_counter('prefill'), decode_count=self.profiler.get_counter('decode'))