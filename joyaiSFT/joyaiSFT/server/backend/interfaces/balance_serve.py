...
from transformers import AutoTokenizer, AutoConfig, GenerationConfig, StaticCache, AutoModelForCausalLM, BitsAndBytesConfig
from joyaiSFT.server.config.config import Config
from ..base import ThreadContext, BackendInterfaceBase
import torch
from joyaiSFT.server.backend.interfaces.transformers import ConfigArgs, default_args, TextStreamer
...
from joyaiSFT.models.custom_modeling_chatrhino import JOYChatrhinoForCausalLM
from joyaiSFT.models.custom_modeling_chatrhinoV0 import JOYChatrhinoV0ForCausalLM
from joyaiSFT.server.balance_serve.inference.model_runner import ModelRunner
from joyaiSFT.server.balance_serve.inference.sampling.sampler import Sampler, SamplingOptions
from joyaiSFT.server.balance_serve.inference.query_manager import QueryManager
from joyaiSFT.server.balance_serve.inference.forward_batch import ForwardBatchInput, ForwardBatchOutput
from joyaiSFT.server.balance_serve.sched_rpc import SchedulerClient
from joyaiSFT.server.balance_serve.settings import sched_ext
from torch.multiprocessing import Queue
import torch.multiprocessing as mp
from multiprocessing.synchronize import Event
from joyaiSFT.server.schemas.endpoints.chat import RawUsage
from joyaiSFT.server.utils.multi_timer import Profiler
import zmq
import time
import queue
import tempfile
...
import atexit
import signal
joy_rules_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', '..', '..', './optimize/optimize_rules/')
default_optimize_rules = {'ChatrhinoForCausalLM': joy_rules_dir}
...
class Engine:
    ...
    cache: JOYChatrhinoCache | JOYGQACache
    ...
    def __init__(self, args: ConfigArgs=default_args, generated_token_queue: Queue=None, broadcast_endpoint: str=None, kvcache_event: Event=None):
        ...
        try:
            ...
            raise RuntimeError(f'Failed to load config from {args.model_dir}') from e
        self.gen_queue = generated_token_queue
        with torch.device('meta'):
            if config.architectures[0] == 'ChatrhinoForCausalLM':
                self.cache = JOYChatrhinoCache(config, self.args.page_size)
                self.model = JOYChatrhinoForCausalLM(config, self.cache)
            elif config.architectures[0] == 'ChatrhinoV0ForCausalLM':
                self.cache = JOYChatrhinoCache(config, self.args.page_size)
                self.model = JOYChatrhinoV0ForCausalLM(config, self.cache)
        context = zmq.Context()
        ...
        try:
            ...
            generation_config = GenerationConfig(max_length=args.max_new_tokens, temperature=args.temperature, top_p=args.top_p, do_sample=True)
...
class BalanceServeInterface(BackendInterfaceBase):
    ...
    def get_params(self, temperature: Optional[float]=None, top_p: Optional[float]=None, max_tokens: Optional[float]=None, max_completion_tokens: Optional[float]=None) -> tuple[float, float]:
    ...
    async def inference(self, local_messages, thread_id: str, temperature: Optional[float]=None, top_p: Optional[float]=None, max_tokens: Optional[float]=None, max_completion_tokens: Optional[float]=None):
        ...
        if profiler.get_counter('decode') >= max_new_tokens - 1:
            ...
            yield ('', 'stop')
        yield RawUsage(tokenize_time=profiler.get_timer_sec('tokenize'), prefill_time=profiler.get_timer_sec('prefill'), decode_time=profiler.get_timer_sec('decode'), prefill_count=profiler.get_counter('prefill'), decode_count=profiler.get_counter('decode'))