...
import json
import uuid
from transformers import AutoTokenizer, AutoConfig, GenerationConfig, StaticCache, AutoModelForCausalLM, BitsAndBytesConfig, LogitsProcessorList, TemperatureLogitsWarper, TopKLogitsWarper, TopPLogitsWarper, MinPLogitsWarper, TypicalLogitsWarper, EpsilonLogitsWarper, EtaLogitsWarper
...
class TransformersInterface(BackendInterfaceBase):
    ...
    def prepare_logits_wrapper(self, inputs, device, temperature: Optional[float]=None, top_p: Optional[float]=None):
        ...
        if top_p == 0:
            top_p = 0.0001
        generation_config, model_kwargs = self.model._prepare_generation_config(None, max_length=self.args.max_new_tokens, do_sample=True, top_k=self.args.top_k, top_p=top_p, temperature=temperature, repetition_penalty=self.args.repetition_penalty)
        self.inputs = inputs
    ...
    def decode_one_tokens(self):
        if self.use_static_cache:
            logits = self.model(self.current_ids, cache_position=self.active_cache_position, past_key_values=self.cache, return_dict=False, use_cache=True)[0]
        else:
    ...
    def check_is_new(self, thread_id: str):
        ...
        if self.last_request_id is None:
            ...
            return True
        elif self.last_request_id == thread_id:
            return False
        else:
            self.last_request_id = thread_id
            return True
