...
from joyaiSFT.server.args import ArgumentParser
from joyaiSFT.server.config.config import Config
from joyaiSFT.server.utils.create_interface import create_interface, GlobalInterface
from fastapi.openapi.utils import get_openapi
from fastapi import FastAPI
from fastapi.middleware.cors import CORSMiddleware
from joyaiSFT.server.api import router, post_db_creation_operations
from joyaiSFT.server.utils.sql_utils import Base, SQLUtil
...
def update_web_port(config_file: str):
    ip_port_pattern = '(localhost|((25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\\.){3}(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)):[0-9]{1,5}'
...
def custom_openapi(app):
    ...
    openapi_schema = get_openapi(title='joyaiSFT server', version='1.0.0', summary='This is a server that provides a RESTful API for joyaiSFT.', description='We provided chat completion and openai assistant interfaces.', routes=app.routes)
...
def main():
    ...
    app = create_app()
    custom_openapi(app)
    run_api(app=app, host=args.host, port=args.port, ssl_keyfile=args.ssl_keyfile, ssl_certfile=args.ssl_certfile)
