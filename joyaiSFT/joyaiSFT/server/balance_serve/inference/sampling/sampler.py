...
from flashinfer.sampling import min_p_sampling_from_probs, top_k_renorm_probs, top_k_top_p_sampling_from_logits, top_p_renorm_probs
...
class Sampler(nn.Module):
    ...
    def forward(self, logits: torch.Tensor, sampling_config: SamplingOptions=None):
        ...
        if sampling_config.is_all_greedy:
            ...
            if sampling_config.need_min_p_sampling:
                ...
                batch_next_token_ids = min_p_sampling_from_probs(probs, sampling_config.min_ps)
                ...
                probs = logits
                batch_next_token_ids = top_k_top_p_sampling_from_logits(logits, sampling_config.top_ks, sampling_config.top_ps, filter_apply_order='joint')
                temperature_0_idx = torch.where(sampling_config.temperatures == 0)[0]
                batch_next_token_ids[temperature_0_idx] = torch.argmax(origin_logits[temperature_0_idx], -1).to(torch.int32)
