...
class BatchedPenalizerOrchestrator:
    ...
    def __init__(self, vocab_size: int, batch: _BatchLike, device: str, Penalizers: typing.Set[typing.Type['_BatchedPenalizer']]):
        ...
        if self.is_required:
            self.cumulate_input_tokens(input_ids=[req.origin_input_ids for req in self.reqs()])
    ...
    def cumulate_input_tokens(self, input_ids: typing.Union[typing.List[torch.Tensor], typing.List[typing.List[int]]]):
        """
    ...
    def cumulate_output_tokens(self, output_ids: typing.Union[typing.List[torch.Tensor], typing.List[typing.List[int]]]):
        """
    ...
    def filter(self, indices_to_keep: typing.List[int], indices_tensor_to_keep: torch.Tensor=None):
        """
        ...
        for penalizer in self.penalizers.values():
            ...
            if not tmp_is_required or empty_indices:
                ...
                if indices_tensor_to_keep is None:
                    indices_tensor_to_keep = torch.tensor(indices_to_keep, dtype=torch.int32, device=self.device)
                penalizer.filter(indices_to_keep=indices_to_keep, indices_tensor_to_keep=indices_tensor_to_keep)
        self.is_required = is_required
...
class _TokenIDs:
    ...
    def __init__(self, orchestrator: BatchedPenalizerOrchestrator, token_ids: typing.Union[typing.List[torch.Tensor], typing.List[typing.List[int]]]):
        self.orchestrator = orchestrator
        if not isinstance(token_ids[0], torch.Tensor):
            token_ids = [torch.tensor(data=ids, dtype=torch.int64, device=self.orchestrator.device) for ids in token_ids]
    ...
    def occurrence_count(self) -> torch.Tensor:
        ...
        padded_token_ids = torch.nn.utils.rnn.pad_sequence(sequences=token_ids, batch_first=True, padding_value=self.orchestrator.vocab_size)
        self.cached_counts = torch.zeros(size=(self.orchestrator.batch_size(), self.orchestrator.vocab_size + 1), dtype=torch.int64, device=self.orchestrator.device).scatter_add_(dim=1, index=padded_token_ids, src=torch.ones_like(padded_token_ids))[:, :self.orchestrator.vocab_size]
        return self.cached_counts
...
class _BatchedPenalizer(abc.ABC):
    ...
    def filter(self, indices_to_keep: typing.List[int], indices_tensor_to_keep: torch.Tensor):
        if not self.is_prepared():
            return
        self._filter(indices_to_keep=indices_to_keep, indices_tensor_to_keep=indices_tensor_to_keep)
    ...
    @abc.abstractmethod
    def _filter(self, indices_to_keep: typing.List[int], indices_tensor_to_keep: torch.Tensor):
        """
