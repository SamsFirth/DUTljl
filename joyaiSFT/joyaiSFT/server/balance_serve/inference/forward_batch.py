...
from joyaiSFT.server.balance_serve.inference.query_manager import QueryManager, QueryInfo
import time
...
class ForwardBatchInput:
    ...
    @classmethod
    def gen_max_forward_batch(cls, device=None, tokens: torch.Tensor=None, num_mini_batches: int=1, max_seq_length: int=4096, prefill_query_length: int=(Config().chunk_size - Config().max_decode_batch_size) // Config().max_prefill_batch_size, prefill_active_length: int=(Config().chunk_size - Config().max_decode_batch_size) // Config().max_prefill_batch_size, gen_prefill: bool=True, decode_batch_size: int=Config().max_decode_batch_size, decode_active_position: torch.Tensor=None, page_size=256, cuda_lens=1):
        instance = cls()
