...
if supports_custom_op():
    ...
    def inplace_all_reduce_fake(tensor: torch.Tensor, group_name: str) -> None:
        return
    direct_register_custom_op(op_name='inplace_all_reduce', op_func=inplace_all_reduce, mutates_args=['tensor'], fake_impl=inplace_all_reduce_fake)
    ...
    direct_register_custom_op(op_name='outplace_all_reduce', op_func=outplace_all_reduce, mutates_args=[], fake_impl=outplace_all_reduce_fake)
...
class GroupCoordinator:
    ...
    pynccl_comm: Optional[Any]
    ca_comm: Optional[Any]
    mq_broadcaster: Optional[Any]

    def __init__(self, group_ranks: List[List[int]], local_rank: int, torch_distributed_backend: Union[str, Backend], use_pynccl: bool, use_custom_allreduce: bool, use_tpu_communicator: bool, use_hpu_communicator: bool, use_xpu_communicator: bool, use_message_queue_broadcaster: bool=False, group_name: Optional[str]=None):
    ...
    def all_reduce(self, input_: torch.Tensor, bsz_tensor: torch.Tensor, is_compute_bound=False, overlap=False) -> torch.Tensor:
        ...
        if self.ca_comm is not None and (not self.ca_comm.disabled) and self.ca_comm.should_custom_ar(input_):
            return torch.ops.vllm.outplace_all_reduce(input_, group_name=self.unique_name, bsz_tensor=bsz_tensor, is_compute_bound=is_compute_bound, overlap=overlap)
        else:
...
def init_world_group(ranks: List[int], local_rank: int, backend: str) -> GroupCoordinator:
    return GroupCoordinator(group_ranks=[ranks], local_rank=local_rank, torch_distributed_backend=backend, use_pynccl=False, use_custom_allreduce=False, use_tpu_communicator=False, use_hpu_communicator=False, use_xpu_communicator=False, group_name='world')

def init_model_parallel_group(group_ranks: List[List[int]], local_rank: int, backend: str, use_custom_allreduce: Optional[bool]=None, use_message_queue_broadcaster: bool=False, group_name: Optional[str]=None) -> GroupCoordinator:
...
def init_distributed_environment(world_size: int=-1, rank: int=-1, distributed_init_method: str='env://', local_rank: int=-1, backend: str='nccl'):
    print('world_size=%d rank=%d local_rank=%d distributed_init_method=%s backend=%s', world_size, rank, local_rank, distributed_init_method, backend)
...
def initialize_model_parallel(tensor_model_parallel_size: int=1, pipeline_model_parallel_size: int=1, backend: Optional[str]=None) -> None:
    ...
    for i in range(num_pipeline_model_parallel_groups):
        ...
        group_ranks.append(ranks)
    _PP = init_model_parallel_group(group_ranks, get_world_group().local_rank, backend, use_custom_allreduce=False, group_name='pp')
