...
from joyaiSFT.server.utils.create_interface import get_interface
from joyaiSFT.server.schemas.assistants.streaming import chat_stream_response
from joyaiSFT.server.schemas.endpoints.chat import ChatCompletionCreate
from joyaiSFT.server.schemas.endpoints.chat import RawUsage, Role
from joyaiSFT.server.backend.base import BackendInterfaceBase
from joyaiSFT.server.config.config import Config
from joyaiSFT.server.config.log import logger
from fastapi.responses import JSONResponse
from joyaiSFT.server.schemas.endpoints.chat import ChatCompletionChunk, CompletionUsage
...
def getTools(buffer):
    ...
    while tool_call_begin_marker in working_buffer and tool_call_end_marker in working_buffer:
        ...
        if json_match:
            ...
            extracted_tools.append({'id': tool_call_id, 'type': 'function', 'function': {'name': function_name, 'arguments': arguments_str}})
...
async def chat_completion(request: Request, create: ChatCompletionCreate):
    ...
    if create.max_tokens is not None and create.max_tokens < 0:
        return JSONResponse(status_code=400, content={'object': 'error', 'message': f'max_tokens must be at least 0, got {create.max_tokens}.', 'type': 'BadRequestError', 'param': None, 'code': 400})
    if create.max_completion_tokens is not None and create.max_completion_tokens < 0:
        return JSONResponse(status_code=400, content={'object': 'error', 'message': f'max_completion_tokens must be at least 0, got {create.max_completion_tokens}.', 'type': 'BadRequestError', 'param': None, 'code': 400})
    if create.temperature < 0 or create.temperature > 2:
        return JSONResponse(status_code=400, content={'object': 'error', 'message': f'temperature must be in [0, 2], got {create.temperature}.', 'type': 'BadRequestError', 'param': None, 'code': 400})
    if create.top_p <= 0 or create.top_p > 1:
        return JSONResponse(status_code=400, content={'object': 'error', 'message': f'top_p must be in (0, 1], got {create.top_p}.', 'type': 'BadRequestError', 'param': None, 'code': 400})
    if create.frequency_penalty < -2 or create.frequency_penalty > 2:
        return JSONResponse(status_code=400, content={'object': 'error', 'message': f'frequency_penalty must be in [-2, 2], got {create.frequency_penalty}.', 'type': 'BadRequestError', 'param': None, 'code': 400})
    if create.presence_penalty < -2 or create.presence_penalty > 2:
        return JSONResponse(status_code=400, content={'object': 'error', 'message': f'presence_penalty must be in [-2, 2], got {create.presence_penalty}.', 'type': 'BadRequestError', 'param': None, 'code': 400})
    ...
    if create.stream:
        ...
        async def inner():
            chunk = ChatCompletionChunk(id=id, choices=[], object='chat.completion.chunk', created=int(time()), model=Config().model_name, system_fingerprint=f'fp_{uuid4().hex[:12]}')
            full_content = ''
            buffer = ''
            ...
            tool_call_end_marker = '<｜tool▁call▁end｜>'
            tool_calls_end_marker = '<｜tool▁calls▁end｜>'
            too_calls_dict = {'<tools▁begin>': '<｜tool▁calls▁begin｜>', '<tool▁begin>': '<｜tool▁call▁begin｜>', '<tool▁sep>': '<｜tool▁sep｜>', '<tool▁end>': '<｜tool▁call▁end｜>', '<tools▁end>': '<｜tool▁calls▁end｜>'}
            async for res in interface.inference(input_message, id, create.temperature, create.top_p, create.max_tokens, create.max_completion_tokens):
                if isinstance(res, RawUsage):
                    raw_usage = res
                    chunk.choices = []
                    chunk.usage = CompletionUsage(prompt_tokens=raw_usage.prefill_count, completion_tokens=raw_usage.decode_count, total_tokens=raw_usage.prefill_count + raw_usage.decode_count)
                    ...
                    if not tool_call_mode and tool_calls_begin_marker in buffer + token:
                        ...
                        buffer = ''
                        if full_content:
                            chunk.choices = [{'index': 0, 'delta': {'content': full_content}, 'finish_reason': None}]
                            yield chunk
                            full_content = ''
                            ...
                            try:
                                ...
                                if len(tool_calls):
                                    ...
                                    for idx, tool_call in enumerate(tool_calls):
                                        chunk.choices = [{'index': 0, 'delta': {'role': 'assistant', 'content': None, 'tool_calls': [{'index': idx, 'id': tool_call['id'], 'type': 'function', 'function': {'name': tool_call['function']['name'], 'arguments': ''}}]}, 'finish_reason': None}]
                                        yield chunk
                                        chunk.choices = [{'index': 0, 'delta': {'tool_calls': [{'index': idx, 'function': {'arguments': tool_call['function']['arguments']}}]}, 'finish_reason': None}]
                                        yield chunk
                    ...
                    if not tool_call_mode and token:
                        if finish_reason is not None:
                            ...
                            yield chunk
                        elif any((marker in token for marker in [tool_calls_begin_marker, tool_call_begin_marker])):
                            pass
                        else:
        ...
        async for res in interface.inference(input_message, id, create.temperature, create.top_p, create.max_tokens, create.max_completion_tokens):
            if isinstance(res, RawUsage):
                raw_usage = res
                usage = CompletionUsage(prompt_tokens=raw_usage.prefill_count, completion_tokens=raw_usage.decode_count, total_tokens=raw_usage.prefill_count + raw_usage.decode_count)
                if create.return_speed:
        ...
        response = {'id': id, 'object': 'chat.completion', 'created': int(time()), 'model': Config().model_name, 'choices': [{'index': 0, 'message': message, 'finish_reason': finish_reason or 'stop'}], 'usage': usage.__dict__ if 'usage' in locals() else None, 'system_fingerprint': f'fp_{uuid4().hex[:12]}'}
        return response