...
from joyaiSFT.server.crud.assistants.assistants import AssistantDatabaseManager
from joyaiSFT.server.crud.assistants.runs import RunsDatabaseManager
from joyaiSFT.server.schemas.assistants.assistants import AssistantCreate, AssistantModify, ObjectID, AssistantBuildStatus, AssistantObject
from joyaiSFT.server.schemas.base import DeleteResponse, Order
...
async def list_assistants(limit: Optional[int]=20, order: Order=Order.DESC, after: Optional[str]=None, before: Optional[str]=None):
...
async def list_assistants_with_status(limit: Optional[int]=20, order: Order=Order.DESC, after: Optional[str]=None, before: Optional[str]=None):
...
async def modify_assistant(assistant_id: str, assistant: AssistantModify):
...
def create_default_assistant():
    ...
    if assistant_manager.db_count_assistants() == 0:
        default_assistant = assistant_manager.db_create_assistant(AssistantCreate(name='KT Assistant', model='default model', instructions='You are a helpful, respectful and honest assistant. Always answer as helpfully as possible, while being safe.  ' + 'Your answers should not include any harmful, unethical, racist, sexist, toxic, dangerous, or illegal content. ' + 'Please ensure that your responses are socially unbiased and positive in nature.'))
