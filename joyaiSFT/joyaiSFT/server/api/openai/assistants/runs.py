...
from joyaiSFT.server.backend.base import ThreadContext
from joyaiSFT.server.schemas.assistants.runs import RunCreate, RunObject, RunThreadCreate, RunModify, RunSubmit
from joyaiSFT.server.schemas.assistants.streaming import api_stream_response
from joyaiSFT.server.utils.create_interface import get_thread_context_manager
from joyaiSFT.server.schemas.base import Order
from joyaiSFT.server.config.log import logger
from joyaiSFT.server.exceptions import internal_server_error
router = APIRouter()
...
async def list_runs(thread_id: str, limit: Optional[int]=20, order: Optional[Order]=Order.DESC, after: Optional[str]=None, before: Optional[str]=None):
    raise NotImplementedError
...
async def modify_run(thread_id: str, run_id: str, run: RunModify):
    raise NotImplementedError
