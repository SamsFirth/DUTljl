...
from joyaiSFT.server.schemas.assistants.streaming import check_link_response
...
async def generate(request: Request, input: OllamaGenerateCompletionRequest):
    ...
    if input.stream:
        ...
        response = OllamaGenerationResponse(model=config.model_name, created_at=str(datetime.now()), response=complete_response, done=True)
        return response
...
class OllamaChatCompletionRequest(BaseModel):
    model: str = Field(..., description='The model name, which is required.')
    messages: List[OllamaChatCompletionMessage] = Field(..., description='A list of messages to generate a response for.')
        ...
        async def inner():
            ...
            async for res in interface.inference(input_message, id):
                if isinstance(res, RawUsage):
                    ...
                    token, finish_reason = res
                    d = OllamaChatCompletionStreamResponse(model=config.model_name, created_at=str(datetime.now()), message={'role': 'assistant', 'content': token}, done=False)
                    yield (d.model_dump_json() + '\n')
            end_time = time()
            total_duration = int((end_time - start_time) * 1000000000)
            prompt_eval_count = raw_usage.prefill_count
            eval_count = raw_usage.decode_count
            eval_duration = int(raw_usage.decode_time * 1000000000)
            prompt_eval_duration = int(raw_usage.prefill_time * 1000000000)
            load_duration = int(raw_usage.tokenize_time * 1000000000)
            done_reason = finish_reason
            d = OllamaChatCompletionStreamResponse(model=config.model_name, created_at=str(datetime.now()), message={}, done=True, total_duration=total_duration, load_duration=load_duration, prompt_eval_count=prompt_eval_count, prompt_eval_duration=prompt_eval_duration, eval_count=eval_count, eval_duration=eval_duration, done_reason=done_reason)
            yield (d.model_dump_json() + '\n')
        return check_link_response(request, inner())
    else:
        start_time = time()
...
async def show(request: Request, input: OllamaShowRequest):
    config = Config()
    return OllamaShowResponse(modelfile='# Modelfile generated by ...', parameters=' ', template=' ', details=OllamaShowDetial(parent_model=' ', format='gguf', family=' ', families=[' '], parameter_size=' ', quantization_level=' '), model_info=OllamaModelInfo())