...
from joyaiSFT.server.models.assistants.runs import Run
from joyaiSFT.server.schemas.assistants.runs import RunCreate, RunObject
...
class RunsDatabaseManager:
    ...
    def create_run_object(self, thread_id: ObjectID, run: RunCreate) -> RunObject:
        run_obj = RunObject(**run.model_dump(mode='json', exclude={'stream'}), id=str(uuid4()), object='run', created_at=int(time()), thread_id=thread_id, status=RunObject.Status.queued)
        run_obj.set_compute_save(0)
        return run_obj

    def db_create_run(self, thread_id: str, run: RunCreate):
        db_run = Run(**run.model_dump(mode='json', exclude={'stream'}), id=str(uuid4()), created_at=int(time()), status='queued', thread_id=thread_id)
    ...
    def db_sync_run(self, run: RunObject) -> None:
        db_run = Run(**run.model_dump(mode='json'))
        with self.sql_util.get_db() as db:
