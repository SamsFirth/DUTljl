...
from joyaiSFT.server.schemas.assistants.messages import MessageCore, MessageCreate, MessageObject
from joyaiSFT.server.schemas.base import Order, ObjectID
...
class MessageDatabaseManager:
    ...
    def create_message_object(thread_id: ObjectID, run_id: ObjectID, message: MessageCreate):
        ...
        return MessageObject(**core.model_dump(mode='json'), id=str(uuid4()), object='thread.message', created_at=int(time()), thread_id=thread_id, run_id=run_id, status=MessageObject.Status.in_progress)

    def db_sync_message(self, message: MessageObject):
        db_message = Message(**message.model_dump(mode='json'))
        with self.sql_util.get_db() as db:
    ...
    def db_list_messages_of_thread(self, thread_id: str, limit: Optional[int]=None, order: Order=Order.DESC):
        with self.sql_util.get_db() as db:
            query = db.query(Message).filter(Message.thread_id == thread_id).order_by(order.to_sqlalchemy_order()(Message.created_at))
