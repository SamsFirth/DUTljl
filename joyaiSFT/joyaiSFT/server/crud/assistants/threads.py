...
from joyaiSFT.server.models.assistants.messages import Message
from joyaiSFT.server.models.assistants.threads import Thread
from joyaiSFT.server.schemas.assistants.threads import ThreadCreate, ThreadObject
from joyaiSFT.server.schemas.base import ObjectID, Order
from joyaiSFT.server.schemas.conversation import ThreadPreview
from joyaiSFT.server.utils.sql_utils import SQLUtil
from joyaiSFT.server.crud.assistants.messages import MessageDatabaseManager
...
class ThreadsDatabaseManager:
    ...
    def db_create_thread(self, thread: ThreadCreate):
        ...
        with self.sql_util.get_db() as db:
            ...
            db_thread = Thread(**thread.model_dump(exclude='messages'), id=str(uuid4()), created_at=int(time()), messages=db_messages)
    ...
    def db_list_threads(self, limit: Optional[int], order: Order) -> List[ThreadObject]:
        with self.sql_util.get_db() as db:
            query = db.query(Thread).order_by(order.to_sqlalchemy_order()(Thread.created_at)).filter(~Thread.meta_data.contains('assistant_id'))
