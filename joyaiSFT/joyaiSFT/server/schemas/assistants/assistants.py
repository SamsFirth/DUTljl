...
from joyaiSFT.server.models.assistants.assistants import Assistant
from joyaiSFT.server.models.assistants.threads import Thread
from joyaiSFT.server.schemas.assistants.messages import Role
from joyaiSFT.server.schemas.assistants.runs import RunObject, RunStreamResponse, ObjectWithCreatedTime
from joyaiSFT.server.schemas.assistants.threads import ThreadObject
from joyaiSFT.server.schemas.base import Metadata, MetadataField, ObjectID
from joyaiSFT.server.schemas.assistants.tool import Tool, CodeInterpreter, FileSearch, RelatedThreads, FuntionTool, ToolResource, CodeInterpreterResource, FileSearchResource, RelatedThreadsResource, ToolType
from joyaiSFT.server.utils.sql_utils import SQLUtil
...
class AssistantObject(AssistantBase, ObjectWithCreatedTime):
    ...
    def get_related_threads_objects(self) -> List:
        ...
        if self.related_threads_objects is None:
            ...
            self.related_threads_objects = [tool for tool in [ThreadObject.model_validate(tool.__dict__) for tool in db_threads] if tool.is_related_threads and tool.meta_data['assistant_id'] == self.id]
    ...
    def sync_db(self) -> None:
        sql_utils = SQLUtil()
        db_assistant = Assistant(**self.model_dump(mode='json'))
        with sql_utils.get_db() as db:
