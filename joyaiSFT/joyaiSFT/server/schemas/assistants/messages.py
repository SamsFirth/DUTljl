...
from joyaiSFT.server.schemas.assistants.tool import Field, CodeInterpreter, FileSearch
...
class MessageObject(MessageBase, ObjectWithCreatedTime):
    ...
    def sync_db(self):
        sql_utils = SQLUtil()
        db_message = Message(**self.model_dump(mode='json'))
...
class MessageCreate(BaseModel):
    ...
    def to_core(self) -> MessageCore:
        core = MessageCore(role=self.role, content=[], attachments=self.attachments, meta_data=self.meta_data)
