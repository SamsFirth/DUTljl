...
def permute_rows(q_w: torch.Tensor, group_size: int):
    ...
    return (q_w.to(device=orig_device), g_idx.to(device=orig_device), rand_perm.to(device=orig_device))
...
def quantize_weights(w: torch.Tensor, num_bits: int, group_size: int, act_order: bool):
    ...
    return (q_w.to(device=orig_device), s.to(device=orig_device), g_idx.to(device=orig_device), rand_perm.to(device=orig_device))
