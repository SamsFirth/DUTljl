...
from joyaiSFT.util.custom_loader import GGUFLoader, ModelLoaderFactory
from joyaiSFT.util.utils import set_module, load_weights
import itertools
import copy
    ...
    for rule in rule_list:
        ...
        if 'replace' in rule:
            ...
            if module_name not in out_data:
                out_data[module_name] = {'key': module_name, 'class': replace_meta['class'] if 'class' in replace_meta else 'default', 'kwargs': copy.deepcopy(replace_meta['kwargs']) if 'kwargs' in replace_meta else dict()}
            else:
        ...
        break
    if module_name not in out_data:
        out_data[module_name] = {'class': 'default', 'key': module_name, 'kwargs': {'generate_device': default_device, 'prefill_device': default_device}}
    if recursive:
        for name, child in module._modules.items():
            if child is not None:
                child_prefix = prefix + name + '.'
...
def optimize_and_load_gguf(module: nn.Module, rule_yaml_text: str, gguf_path: str, model_config: PretrainedConfig, default_device: str='cuda:0'):
    rule_list = yaml.load(rule_yaml_text, Loader=yaml.FullLoader)  # 或 yaml.safe_load
    if rule_list is None:
        rule_list = []
    if not isinstance(rule_list, list):
        raise ValueError("JOY optimize rule YAML must be a list at top-level.")
