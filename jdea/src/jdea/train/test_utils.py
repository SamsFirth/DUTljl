...
def compare_model(model_a: 'torch.nn.Module', model_b: 'torch.nn.Module', diff_keys: list[str]=[]) -> None:
    ...
    for name in state_dict_a.keys():
        if any((key in name for key in diff_keys)):
            assert torch.allclose(state_dict_a[name], state_dict_b[name], rtol=0.0001, atol=1e-05) is False
        else:
            assert torch.allclose(state_dict_a[name], state_dict_b[name], rtol=0.0001, atol=1e-05) is True
...
def load_reference_model(model_path: str, lora_path: Optional[str]=None, use_lora: bool=False, use_pissa: bool=False, is_trainable: bool=False, add_valuehead: bool=False) -> Union['PreTrainedModel', 'LoraModel']:
    current_device = get_current_device()
    if add_valuehead:
        model: AutoModelForCausalLMWithValueHead = AutoModelForCausalLMWithValueHead.from_pretrained(model_path, torch_dtype=torch.float16, device_map=current_device)
        if not is_trainable:
    ...
    if use_lora or use_pissa:
        model = PeftModel.from_pretrained(model, lora_path, subfolder='pissa_init' if use_pissa else None, is_trainable=is_trainable)
