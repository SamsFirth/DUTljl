...
class DummyOptimizer(torch.optim.Optimizer):
    ...
    def __init__(self, lr: float=0.001, optimizer_dict: Optional[dict['torch.nn.Parameter', 'torch.optim.Optimizer']]=None) -> None:
...
def create_modelcard_and_push(trainer: 'Trainer', model_args: 'ModelArguments', data_args: 'DataArguments', training_args: 'TrainingArguments', finetuning_args: 'FinetuningArguments') -> None:
    kwargs = {'tasks': 'text-generation', 'finetuned_from': model_args.model_name_or_path, 'tags': ['llama-factory', finetuning_args.finetuning_type]}
    if data_args.dataset is not None:
        kwargs['dataset'] = data_args.dataset
    if model_args.use_unsloth:
        kwargs['tags'] = kwargs['tags'] + ['unsloth']
    if model_args.use_joysft:
        kwargs['tags'] = kwargs['tags'] + ['joyaiSFT']
...
def create_ref_model(model_args: 'ModelArguments', finetuning_args: 'FinetuningArguments', add_valuehead: bool=False) -> Optional[Union['PreTrainedModel', 'AutoModelForCausalLMWithValueHead']]:
    """Create reference model for PPO/DPO training. Evaluation mode is not supported.

    The valuehead parameter is randomly initialized since it is useless for PPO training.
    """
    if finetuning_args.ref_model is not None:
        ref_model_args = ModelArguments.copyfrom(model_args, model_name_or_path=finetuning_args.ref_model, adapter_name_or_path=finetuning_args.ref_model_adapters, quantization_bit=finetuning_args.ref_model_quantization_bit)
        ref_finetuning_args = FinetuningArguments()
        tokenizer = load_tokenizer(ref_model_args)['tokenizer']
        ref_model = load_model(tokenizer, ref_model_args, ref_finetuning_args, is_trainable=False, add_valuehead=add_valuehead)
...
def create_reward_model(model: 'AutoModelForCausalLMWithValueHead', model_args: 'ModelArguments', finetuning_args: 'FinetuningArguments') -> Optional['AutoModelForCausalLMWithValueHead']:
    ...
    if finetuning_args.reward_model_type == 'api':
        ...
        reward_model_args = ModelArguments.copyfrom(model_args, model_name_or_path=finetuning_args.reward_model, adapter_name_or_path=finetuning_args.reward_model_adapters, quantization_bit=finetuning_args.reward_model_quantization_bit)
        reward_finetuning_args = FinetuningArguments()
        tokenizer = load_tokenizer(reward_model_args)['tokenizer']
        reward_model = load_model(tokenizer, reward_model_args, reward_finetuning_args, is_trainable=False, add_valuehead=True)
...
def _create_galore_optimizer(model: 'PreTrainedModel', training_args: 'TrainingArguments', finetuning_args: 'FinetuningArguments') -> 'torch.optim.Optimizer':
    ...
    galore_kwargs = {'rank': finetuning_args.galore_rank, 'update_proj_gap': finetuning_args.galore_update_interval, 'scale': finetuning_args.galore_scale, 'proj_type': finetuning_args.galore_proj_type}
    ...
    if finetuning_args.galore_layerwise:
        ...
        param_groups = [dict(params=nodecay_params, weight_decay=0.0), dict(params=decay_params, weight_decay=training_args.weight_decay), dict(params=galore_params, weight_decay=training_args.weight_decay, **galore_kwargs)]
        optimizer = optim_class(param_groups, **optim_kwargs)
    logger.info_rank0(f'Using GaLore optimizer with args: {galore_kwargs}. It may cause hanging at the start of training, wait patiently.')
    return optimizer

def _create_apollo_optimizer(model: 'PreTrainedModel', training_args: 'TrainingArguments', finetuning_args: 'FinetuningArguments') -> 'torch.optim.Optimizer':
    ...
    if finetuning_args.apollo_layerwise:
        ...
        param_groups = [dict(params=nodecay_params, weight_decay=0.0), dict(params=decay_params, weight_decay=training_args.weight_decay), dict(params=apollo_params, weight_decay=training_args.weight_decay, **apollo_kwargs)]
        optimizer = optim_class(param_groups, **optim_kwargs)
    logger.info_rank0(f'Using APOLLO optimizer with args: {apollo_kwargs}.')
    return optimizer

def _create_loraplus_optimizer(model: 'PreTrainedModel', training_args: 'TrainingArguments', finetuning_args: 'FinetuningArguments') -> 'torch.optim.Optimizer':
    ...
    param_groups = [dict(params=param_dict['lora_a'], lr=default_lr, weight_decay=training_args.weight_decay), dict(params=param_dict['lora_b'], lr=loraplus_lr, weight_decay=training_args.weight_decay), dict(params=param_dict['lora_b_nodecay'], lr=loraplus_lr, weight_decay=0.0), dict(params=param_dict['embedding'], lr=embedding_lr, weight_decay=training_args.weight_decay)]
    optimizer = optim_class(param_groups, **optim_kwargs)
    logger.info_rank0(f'Using LoRA+ optimizer with loraplus lr ratio {finetuning_args.loraplus_lr_ratio:.2f}.')
    return optimizer

def _create_badam_optimizer(model: 'PreTrainedModel', training_args: 'TrainingArguments', finetuning_args: 'FinetuningArguments') -> 'torch.optim.Optimizer':
    decay_params, nodecay_params = ([], [])
    decay_param_names = _get_decay_parameter_names(model)
    for name, param in model.named_parameters():
        if param.requires_grad:
            if name in decay_param_names:
                ...
                nodecay_params.append(param)
    optim_class, optim_kwargs = Trainer.get_optimizer_cls_and_kwargs(training_args)
    param_groups = [dict(params=nodecay_params, weight_decay=0.0), dict(params=decay_params, weight_decay=training_args.weight_decay)]
    if finetuning_args.badam_mode == 'layer':
        from badam import BlockOptimizer
        base_optimizer = optim_class(param_groups, **optim_kwargs)
        optimizer = BlockOptimizer(base_optimizer=base_optimizer, named_parameters_list=list(model.named_parameters()), block_prefix_list=None, switch_block_every=finetuning_args.badam_switch_interval, start_block=finetuning_args.badam_start_block, switch_mode=finetuning_args.badam_switch_mode, verbose=finetuning_args.badam_verbose, ds_zero3_enabled=is_deepspeed_zero3_enabled())
        logger.info_rank0(f'Using BAdam optimizer with layer-wise update, switch mode is {finetuning_args.badam_switch_mode}, switch block every {finetuning_args.badam_switch_interval} steps, default start block is {finetuning_args.badam_start_block}')
    elif finetuning_args.badam_mode == 'ratio':
        from badam import BlockOptimizerRatio
        assert finetuning_args.badam_update_ratio > 1e-06
        optimizer = BlockOptimizerRatio(param_groups=param_groups, named_parameters_list=list(model.named_parameters()), update_ratio=finetuning_args.badam_update_ratio, mask_mode=finetuning_args.badam_mask_mode, verbose=finetuning_args.badam_verbose, include_embedding=False, **optim_kwargs)
        logger.info_rank0(f'Using BAdam optimizer with ratio-based update, update ratio is {finetuning_args.badam_update_ratio}, mask mode is {finetuning_args.badam_mask_mode}')
    return optimizer

def _create_adam_mini_optimizer(model: 'PreTrainedModel', training_args: 'TrainingArguments') -> 'torch.optim.Optimizer':
    ...
    optimizer = Adam_mini(named_parameters=model.named_parameters(), lr=training_args.learning_rate, betas=(training_args.adam_beta1, training_args.adam_beta2), eps=training_args.adam_epsilon, weight_decay=training_args.weight_decay, model_sharding=is_fsdp_enabled() or is_deepspeed_zero3_enabled(), dim=hidden_size, n_heads=num_q_head, n_kv_heads=num_kv_head)
    logger.info_rank0('Using Adam-mini optimizer.')
    return optimizer

def _create_muon_optimizer(model: 'PreTrainedModel', training_args: 'TrainingArguments') -> 'torch.optim.Optimizer':
    ...
    optimizer = Muon(lr=training_args.learning_rate, wd=training_args.weight_decay, muon_params=muon_params, adamw_params=adamw_params, adamw_betas=(training_args.adam_beta1, training_args.adam_beta2), adamw_eps=training_args.adam_epsilon)
    logger.info_rank0(f'Using Muon optimizer with {len(muon_params)} Muon params and {len(adamw_params)} AdamW params.')
    return optimizer

def create_custom_optimizer(model: 'PreTrainedModel', training_args: 'TrainingArguments', finetuning_args: 'FinetuningArguments') -> Optional['torch.optim.Optimizer']:
...
def create_custom_scheduler(training_args: 'TrainingArguments', num_training_steps: int, optimizer: Optional['torch.optim.Optimizer']=None) -> None:
    if training_args.lr_scheduler_type == 'warmup_stable_decay':
        ...
        default_kwargs = {'num_stable_steps': num_stable_steps, 'num_decay_steps': num_decay_steps}
    ...
    if optimizer is not None and isinstance(optimizer, DummyOptimizer):
        ...
        for param in optimizer_dict.keys():
            scheduler_dict[param] = get_scheduler(training_args.lr_scheduler_type, optimizer=optimizer_dict[param], num_warmup_steps=training_args.get_warmup_steps(num_training_steps), num_training_steps=num_training_steps, scheduler_specific_kwargs=training_args.lr_scheduler_kwargs)
...
def get_batch_logps(logits: 'torch.Tensor', labels: 'torch.Tensor', label_pad_token_id: int=IGNORE_INDEX, ld_alpha: Optional[float]=None) -> tuple['torch.Tensor', 'torch.Tensor']:
...
def _dft_cross_entropy(source: torch.Tensor, target: torch.Tensor, num_items_in_batch: Optional[torch.Tensor]=None, ignore_index: int=-100) -> torch.Tensor:
...
def nested_detach(tensors: Union['torch.Tensor', list['torch.Tensor'], tuple['torch.Tensor'], dict[str, 'torch.Tensor']], clone: bool=False):
...
def get_swanlab_callback(finetuning_args: 'FinetuningArguments') -> 'TrainerCallback':
    ...
    if finetuning_args.swanlab_lark_webhook_url is not None:
        ...
        lark_callback = LarkCallback(webhook_url=finetuning_args.swanlab_lark_webhook_url, secret=finetuning_args.swanlab_lark_secret)
    ...
    swanlab_callback = SwanLabCallbackExtension(project=finetuning_args.swanlab_project, workspace=finetuning_args.swanlab_workspace, experiment_name=finetuning_args.swanlab_run_name, mode=finetuning_args.swanlab_mode, config={'Framework': '🦙LlamaFactory'}, logdir=finetuning_args.swanlab_logdir, tags=['🦙LlamaFactory'])
    return swanlab_callback

def get_ray_trainer(training_function: Callable, train_loop_config: dict[str, Any], ray_args: 'RayArguments') -> 'TorchTrainer':
