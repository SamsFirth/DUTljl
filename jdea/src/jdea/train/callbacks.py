    ...
    def on_train_end(self, args: 'TrainingArguments', state: 'TrainerState', control: 'TrainerControl', **kwargs):
        if args.should_save:
            ...
            if isinstance(model, PeftModel):
                ...
                setattr(model.peft_config['default'], 'init_lora_weights', init_lora_weights)
                model.save_pretrained(pissa_convert_dir, safe_serialization=args.save_safetensors, path_initial_model_for_weight_conversion=pissa_init_dir)
...
class LogCallback(TrainerCallback):
    ...
    def on_log(self, args: 'TrainingArguments', state: 'TrainerState', control: 'TrainerControl', **kwargs):
        ...
        logs = dict(current_steps=self.cur_steps, total_steps=self.max_steps, loss=state.log_history[-1].get('loss'), eval_loss=state.log_history[-1].get('eval_loss'), predict_loss=state.log_history[-1].get('predict_loss'), reward=state.log_history[-1].get('reward'), accuracy=state.log_history[-1].get('rewards/accuracies'), lr=state.log_history[-1].get('learning_rate'), epoch=state.log_history[-1].get('epoch'), percentage=round(self.cur_steps / self.max_steps * 100, 2) if self.max_steps != 0 else 100, elapsed_time=self.elapsed_time, remaining_time=self.remaining_time)
    ...
    def on_prediction_step(self, args: 'TrainingArguments', state: 'TrainerState', control: 'TrainerControl', **kwargs):
        ...
        if has_length(eval_dataloader):
            ...
            if self.cur_steps % 5 == 0 and self.thread_pool is not None:
                logs = dict(current_steps=self.cur_steps, total_steps=self.max_steps, percentage=round(self.cur_steps / self.max_steps * 100, 2) if self.max_steps != 0 else 100, elapsed_time=self.elapsed_time, remaining_time=self.remaining_time)
                self.thread_pool.submit(self._write_log, args.output_dir, logs)
...
class ReporterCallback(TrainerCallback):
    ...
    def __init__(self, model_args: 'ModelArguments', data_args: 'DataArguments', finetuning_args: 'FinetuningArguments', generating_args: 'GeneratingArguments') -> None:
        ...
        os.environ['WANDB_PROJECT'] = os.getenv('WANDB_PROJECT', 'jdea')
    ...
    def on_train_begin(self, args: 'TrainingArguments', state: 'TrainerState', control: 'TrainerControl', **kwargs):
        ...
        if 'wandb' in args.report_to:
            import wandb
            wandb.config.update({'model_args': self.model_args.to_dict(), 'data_args': self.data_args.to_dict(), 'finetuning_args': self.finetuning_args.to_dict(), 'generating_args': self.generating_args.to_dict()})
        if self.finetuning_args.use_swanlab:
            import swanlab
            swanlab.config.update({'model_args': self.model_args.to_dict(), 'data_args': self.data_args.to_dict(), 'finetuning_args': self.finetuning_args.to_dict(), 'generating_args': self.generating_args.to_dict()})