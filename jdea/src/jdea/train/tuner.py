...
def _training_function(config: dict[str, Any]) -> None:
    ...
    if finetuning_args.stage == 'sft':
        if model_args.use_joysft:
            from .joysft.workflow import run_sft as run_sft_joy
            run_sft_joy(model_args, data_args, training_args, finetuning_args, generating_args, callbacks)
        else:
...
def run_exp(args: Optional[dict[str, Any]]=None, callbacks: Optional[list['TrainerCallback']]=None) -> None:
    ...
    if ray_args.use_ray:
        ...
        trainer = get_ray_trainer(training_function=_training_function, train_loop_config={'args': args, 'callbacks': callbacks}, ray_args=ray_args)
        trainer.fit()
    else:
...
def export_model(args: Optional[dict[str, Any]]=None) -> None:
    ...
    model.save_pretrained(save_directory=model_args.export_dir, max_shard_size=f'{model_args.export_size}GB', safe_serialization=not model_args.export_legacy_format)
    if model_args.export_hub_model_id is not None:
        model.push_to_hub(model_args.export_hub_model_id, token=model_args.hf_hub_token, max_shard_size=f'{model_args.export_size}GB', safe_serialization=not model_args.export_legacy_format)
    if finetuning_args.stage == 'rm':
        ...
        if os.path.exists(os.path.join(vhead_path, V_HEAD_SAFE_WEIGHTS_NAME)):
            shutil.copy(os.path.join(vhead_path, V_HEAD_SAFE_WEIGHTS_NAME), os.path.join(model_args.export_dir, V_HEAD_SAFE_WEIGHTS_NAME))
            logger.info_rank0(f'Copied valuehead to {model_args.export_dir}.')
        elif os.path.exists(os.path.join(vhead_path, V_HEAD_WEIGHTS_NAME)):
            shutil.copy(os.path.join(vhead_path, V_HEAD_WEIGHTS_NAME), os.path.join(model_args.export_dir, V_HEAD_WEIGHTS_NAME))
            logger.info_rank0(f'Copied valuehead to {model_args.export_dir}.')
    try:
