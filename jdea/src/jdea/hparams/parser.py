...
def _parse_args(parser: 'HfArgumentParser', args: Optional[Union[dict[str, Any], list[str]]]=None, allow_extra_keys: bool=False) -> tuple[Any]:
    args = read_args(args)
...
def _set_transformers_logging() -> None:
    if os.getenv('LLAMAFACTORY_VERBOSITY', 'ERROR') in ['DEBUG', 'INFO']:
...
def _verify_model_args(model_args: 'ModelArguments', data_args: 'DataArguments', finetuning_args: 'FinetuningArguments') -> None:
...
def _check_extra_dependencies(model_args: 'ModelArguments', finetuning_args: 'FinetuningArguments', training_args: Optional['TrainingArguments']=None) -> None:
    if model_args.use_joysft:
        check_version('joyaiSFT', mandatory=True)
...
def _parse_train_mca_args(args: Optional[Union[dict[str, Any], list[str]]]=None) -> _TRAIN_MCA_CLS:
    ...
    allow_extra_keys = is_env_enabled('ALLOW_EXTRA_ARGS')
    model_args, data_args, training_args, finetuning_args, generating_args = _parse_args(parser, args, allow_extra_keys=allow_extra_keys)
...
def get_train_args(args: Optional[Union[dict[str, Any], list[str]]]=None) -> _TRAIN_CLS:
    ...
    if finetuning_args.stage == 'ppo':
        ...
        if finetuning_args.reward_model_type == 'lora' and model_args.use_joysft:
            raise ValueError('JOYSFT does not support lora reward model.')
        if finetuning_args.reward_model_type == 'lora' and model_args.use_unsloth:
            raise ValueError('Unsloth does not support lora reward model.')
        if training_args.report_to and training_args.report_to[0] not in ['wandb', 'tensorboard']:
            raise ValueError('PPO only accepts wandb or tensorboard logger.')
    if not model_args.use_joysft and training_args.parallel_mode == ParallelMode.NOT_DISTRIBUTED:
        raise ValueError('Please launch distributed training with `jdea-cli` or `torchrun`.')
    ...
    if (training_args.do_eval or training_args.do_predict) and (data_args.eval_dataset is None and data_args.val_size < 1e-06):
    ...
    if model_args.infer_backend in [EngineName.VLLM, EngineName.SGLANG]:
        raise ValueError(
            'vLLM/SGLang backend is only available for API, CLI and Web.'
        )
    if model_args.use_unsloth and is_deepspeed_zero3_enabled():
        raise ValueError('Unsloth is incompatible with DeepSpeed ZeRO-3.')
    if model_args.use_joysft and is_deepspeed_zero3_enabled():
        raise ValueError('JOYSFT is incompatible with DeepSpeed ZeRO-3.')
    ...
    if training_args.do_train and finetuning_args.finetuning_type == 'lora' and (model_args.quantization_bit is None) and model_args.resize_vocab and (finetuning_args.additional_target is None):
        logger.warning_rank0('Remember to add embedding layers to `additional_target` to make the added tokens trainable.')
    ...
    if training_args.do_train and (finetuning_args.use_galore or finetuning_args.use_apollo) and (not finetuning_args.pure_bf16):
        logger.warning_rank0('Using GaLore or APOLLO with mixed precision training may significantly increases GPU memory usage.')
    ...
    if training_args.parallel_mode == ParallelMode.DISTRIBUTED and training_args.ddp_find_unused_parameters is None and (finetuning_args.finetuning_type == 'lora'):
    ...
    if training_args.resume_from_checkpoint is None and training_args.do_train and os.path.isdir(training_args.output_dir) and (not training_args.overwrite_output_dir) and can_resume_from_checkpoint:
    ...
    if finetuning_args.stage in ['rm', 'ppo'] and finetuning_args.finetuning_type == 'lora' and (training_args.resume_from_checkpoint is not None):
        logger.warning_rank0(f'Add {training_args.resume_from_checkpoint} to `adapter_name_or_path` to resume training from checkpoint.')
    ...
    logger.info(f'Process rank: {training_args.process_index}, world size: {training_args.world_size}, device: {training_args.device}, distributed training: {training_args.parallel_mode == ParallelMode.DISTRIBUTED}, compute dtype: {str(model_args.compute_dtype)}')
