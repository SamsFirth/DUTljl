...
class SGLangEngine(BaseEngine):
    ...
    def __init__(self, model_args: 'ModelArguments', data_args: 'DataArguments', finetuning_args: 'FinetuningArguments', generating_args: 'GeneratingArguments') -> None:
        ...
        if self.lora_request:
            launch_cmd.extend(['--max-loras-per-batch 1', f'--lora-backend {model_args.sglang_lora_backend}', f'--lora-paths lora0={model_args.adapter_name_or_path[0]}', '--disable-radix-cache'])
    ...
    async def _generate(self, messages: list[dict[str, str]], system: Optional[str]=None, tools: Optional[str]=None, images: Optional[list['ImageInput']]=None, videos: Optional[list['VideoInput']]=None, audios: Optional[list['AudioInput']]=None, **input_kwargs) -> AsyncIterator[dict[str, Any]]:
        ...
        sampling_params = {'temperature': temperature if temperature is not None else self.generating_args['temperature'], 'top_p': (top_p if top_p is not None else self.generating_args['top_p']) or 1.0, 'top_k': (top_k if top_k is not None else self.generating_args['top_k']) or -1, 'stop': stop, 'stop_token_ids': self.template.get_stop_token_ids(self.tokenizer), 'max_new_tokens': max_tokens, 'repetition_penalty': (repetition_penalty if repetition_penalty is not None else self.generating_args['repetition_penalty']) or 1.0, 'skip_special_tokens': skip_special_tokens if skip_special_tokens is not None else self.generating_args['skip_special_tokens']}
    ...
    @override
    async def chat(self, messages: Sequence[dict[str, str]], system: Optional[str]=None, tools: Optional[str]=None, images: Optional[Sequence['ImageInput']]=None, videos: Optional[Sequence['VideoInput']]=None, audios: Optional[Sequence['AudioInput']]=None, **input_kwargs) -> list['Response']:
        final_output = None
        generator = await self._generate(messages, system, tools, images, videos, audios, **input_kwargs)
        for request_output in generator:
            final_output = request_output
        results = [Response(response_text=final_output['text'], response_length=final_output['meta_info']['completion_tokens'], prompt_length=final_output['meta_info']['prompt_tokens'], finish_reason='stop' if final_output['meta_info']['finish_reason'] == 'stop' else 'length')]
        return results
