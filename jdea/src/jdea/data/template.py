...
class Template:
    ...
    def encode_oneturn(self, tokenizer: 'PreTrainedTokenizer', messages: list[dict[str, str]], system: Optional[str]=None, tools: Optional[str]=None) -> tuple[list[int], list[int]]:
    ...
    def encode_multiturn(self, tokenizer: 'PreTrainedTokenizer', messages: list[dict[str, str]], system: Optional[str]=None, tools: Optional[str]=None) -> list[tuple[list[int], list[int]]]:
    ...
    def _encode(self, tokenizer: 'PreTrainedTokenizer', messages: list[dict[str, str]], system: Optional[str], tools: Optional[str]) -> list[list[int]]:
    ...
    def _get_jinja_template(self, tokenizer: 'PreTrainedTokenizer') -> str:
        ...
        jinja_template += "{% if messages[0]['role'] == 'system' %}{% set loop_messages = messages[1:] %}{% set system_message = messages[0]['content'] %}{% else %}{% set loop_messages = messages %}{% endif %}{% if system_message is defined %}{{ " + system + " }}{% endif %}{% for message in loop_messages %}{% set content = message['content'] %}{% if message['role'] == 'user' %}{{ " + user + " }}{% elif message['role'] == 'assistant' %}{{ " + assistant + ' }}{% endif %}{% endfor %}'
    ...
    def _get_ollama_template(self, tokenizer: 'PreTrainedTokenizer') -> str:
        ...
        return f'{prefix}{{{{ if .System }}}}{system}{{{{ end }}}}{{{{ range .Messages }}}}{{{{ if eq .Role "user" }}}}{user}{{{{ else if eq .Role "assistant" }}}}{assistant}{{{{ end }}}}{{{{ end }}}}'
    ...
    def get_ollama_modelfile(self, tokenizer: 'PreTrainedTokenizer') -> str:
        ...
        """
        modelfile = '# ollama modelfile auto-generated by jdea\n\n'
...
class Llama2Template(Template):
    ...
    @override
    def _encode(self, tokenizer: 'PreTrainedTokenizer', messages: list[dict[str, str]], system: str, tools: str) -> list[list[int]]:
        system = system or self.default_system
        encoded_messages = []
        for i, message in enumerate(messages):
            elements = []
    ...
    def _get_jinja_template(self, tokenizer: 'PreTrainedTokenizer') -> str:
        prefix = self._convert_slots_to_jinja(self.format_prefix.apply(), tokenizer)
        system_message = self._convert_slots_to_jinja(self.format_system.apply(), tokenizer, placeholder='system_message')
        ...
        if self.default_system:
            jinja_template += "{% set system_message = '" + self._jinja_escape(self.default_system) + "' %}"
        jinja_template += "{% if messages[0]['role'] == 'system' %}{% set loop_messages = messages[1:] %}{% set system_message = messages[0]['content'] %}{% else %}{% set loop_messages = messages %}{% endif %}{% for message in loop_messages %}{% if loop.index0 == 0 and system_message is defined %}{% set content = " + system_message + " + message['content'] %}{% else %}{% set content = message['content'] %}{% endif %}{% if message['role'] == 'user' %}{{ " + user_message + " }}{% elif message['role'] == 'assistant' %}{{ " + assistant_message + ' }}{% endif %}{% endfor %}'
        return jinja_template
...
class ReasoningTemplate(Template):
    ...
    def encode_oneturn(self, tokenizer: 'PreTrainedTokenizer', messages: list[dict[str, str]], system: Optional[str]=None, tools: Optional[str]=None) -> tuple[list[int], list[int]]:
        ...
        if self.thought_words[0].strip() not in messages[-1]['content'] and self.thought_words[1].strip() not in messages[-1]['content']:
    ...
    def encode_multiturn(self, tokenizer: 'PreTrainedTokenizer', messages: list[dict[str, str]], system: Optional[str]=None, tools: Optional[str]=None) -> list[tuple[list[int], list[int]]]:
        ...
        for i in range(0, len(messages), 2):
            if self.thought_words[0].strip() not in messages[i + 1]['content'] and self.thought_words[1].strip() not in messages[i + 1]['content']:
                if not self.enable_thinking:
...
def get_template_and_fix_tokenizer(tokenizer: 'PreTrainedTokenizer', data_args: 'DataArguments') -> 'Template':
    ...
    if isinstance(template, ReasoningTemplate):
        logger.warning_rank0('You are using reasoning template, please add `_nothink` suffix if the model is not a reasoning model. e.g., qwen3_vl_nothink')
...
register_template(name='bailing', format_user=StringFormatter(slots=['<role>HUMAN</role>{{content}}<role>ASSISTANT</role>']), format_system=StringFormatter(slots=['<role>SYSTEM</role>{{content}}']), format_observation=StringFormatter(slots=['<role>OBSERVATION</role>{{content}}<role>ASSISTANT</role>']), stop_words=['<|endoftext|>'], efficient_eos=True)
register_template(name='bailing_v2', format_user=StringFormatter(slots=['<role>HUMAN</role>{{content}}<|role_end|><role>ASSISTANT</role>']), format_system=StringFormatter(slots=['<role>SYSTEM</role>{{content}}<|role_end|>']), format_assistant=StringFormatter(slots=['{{content}}<|role_end|>']), format_observation=StringFormatter(slots=['<role>OBSERVATION</role>\n<tool_response>\n{{content}}\n</tool_response><|role_end|><role>ASSISTANT</role>']), format_function=FunctionFormatter(slots=['{{content}}<|role_end|>'], tool_format='ling'), format_tools=ToolFormatter(tool_format='ling'), stop_words=['<|endoftext|>'], efficient_eos=True)
...
register_template(name='deepseekr1', format_user=StringFormatter(slots=['<｜User｜>{{content}}<｜Assistant｜>']), format_prefix=EmptyFormatter(slots=[{'bos_token'}]), template_class=ReasoningTemplate)
...
register_template(name='dots_ocr', format_user=StringFormatter(slots=['<|user|>{{content}}<|endofuser|><|assistant|>']), format_assistant=StringFormatter(slots=['{{content}}<|endofassistant|>']), format_system=StringFormatter(slots=['<|system|>{{content}}<|endofsystem|>\n']), stop_words=['<|endofassistant|>'], efficient_eos=True, mm_plugin=get_mm_plugin(name='qwen2_vl', image_token='<|imgpad|>', video_token='<|vidpad|>', vision_bos_token='<|img|>', vision_eos_token='<|endofimg|>'))
register_template(name='empty', format_assistant=StringFormatter(slots=['{{content}}']))
register_template(name='ernie', format_user=StringFormatter(slots=['<|im_start|>user\n{{content}}<|im_end|>\n\n<|im_start|>assistant\n']), format_assistant=StringFormatter(slots=['{{content}}<|im_end|>\n\n']), format_system=StringFormatter(slots=['<|im_start|>system\n{{content}}<|im_end|>\n\n']), format_observation=StringFormatter(slots=['<|im_start|>tool\n{{content}}<|im_end|>\n\n<|im_start|>assistant\n']), default_system='<global_setting>\nthink_mode=True\n</global_setting>', stop_words=['<|im_end|>'])
register_template(name='ernie_nothink', format_user=StringFormatter(slots=['User: {{content}}\nAssistant: ']), format_assistant=StringFormatter(slots=['{{content}}<|end_of_sentence|>']), format_system=StringFormatter(slots=['{{content}}\n']), format_prefix=EmptyFormatter(slots=['<|begin_of_sentence|>']), stop_words=['<|end_of_sentence|>'])
register_template(name='ernie_vl', format_user=StringFormatter(slots=['User: {{content}}']), format_assistant=StringFormatter(slots=['\nAssistant: {{content}}<|end_of_sentence|>']), format_system=StringFormatter(slots=['{{content}}\n']), stop_words=['<|end_of_sentence|>'], replace_eos=True, replace_jinja_template=True, template_class=ReasoningTemplate, mm_plugin=get_mm_plugin(name='ernie_vl', image_token='<|image@placeholder|>', video_token='<|video@placeholder|>'))
...
register_template(name='gemma', format_user=StringFormatter(slots=['<start_of_turn>user\n{{content}}<end_of_turn>\n<start_of_turn>model\n']), format_assistant=StringFormatter(slots=['{{content}}<end_of_turn>\n']), format_system=StringFormatter(slots=['{{content}}\n\n']), format_observation=StringFormatter(slots=['<start_of_turn>tool\n{{content}}<end_of_turn>\n<start_of_turn>model\n']), format_prefix=EmptyFormatter(slots=[{'bos_token'}]), stop_words=['<end_of_turn>'], replace_eos=True, template_class=Llama2Template)
register_template(name='gemma2', format_user=StringFormatter(slots=['<start_of_turn>user\n{{content}}<end_of_turn>\n<start_of_turn>model\n']), format_assistant=StringFormatter(slots=['{{content}}<end_of_turn>\n']), format_system=StringFormatter(slots=['{{content}}\n\n']), format_observation=StringFormatter(slots=['<start_of_turn>tool\n{{content}}<end_of_turn>\n<start_of_turn>model\n']), format_prefix=EmptyFormatter(slots=[{'bos_token'}]), stop_words=['<eos>', '<end_of_turn>'], efficient_eos=True, template_class=Llama2Template)
register_template(name='gemma3', format_user=StringFormatter(slots=['<start_of_turn>user\n{{content}}<end_of_turn>\n<start_of_turn>model\n']), format_assistant=StringFormatter(slots=['{{content}}<end_of_turn>\n']), format_system=StringFormatter(slots=['{{content}}\n\n']), format_observation=StringFormatter(slots=['<start_of_turn>tool\n{{content}}<end_of_turn>\n<start_of_turn>model\n']), format_prefix=EmptyFormatter(slots=[{'bos_token'}]), stop_words=['<end_of_turn>'], replace_eos=True, mm_plugin=get_mm_plugin('gemma3', image_token='<image_soft_token>'), template_class=Llama2Template)
register_template(name='gemma3n', format_user=StringFormatter(slots=['<start_of_turn>user\n{{content}}<end_of_turn>\n<start_of_turn>model\n']), format_assistant=StringFormatter(slots=['{{content}}<end_of_turn>\n']), format_system=StringFormatter(slots=['{{content}}\n\n']), format_observation=StringFormatter(slots=['<start_of_turn>tool\n{{content}}<end_of_turn>\n<start_of_turn>model\n']), format_prefix=EmptyFormatter(slots=[{'bos_token'}]), stop_words=['<end_of_turn>'], replace_eos=True, mm_plugin=get_mm_plugin('gemma3n', image_token='<image_soft_token>', audio_token='<audio_soft_token>'), template_class=Llama2Template)
...
register_template(name='glm4_moe', format_user=StringFormatter(slots=['<|user|>\n{{content}}<|assistant|>']), format_assistant=StringFormatter(slots=['\n{{content}}']), format_system=StringFormatter(slots=['<|system|>\n{{content}}']), format_function=FunctionFormatter(slots=['{{content}}'], tool_format='glm4_moe'), format_observation=StringFormatter(slots=['<|observation|>\n{{content}}<|assistant|>']), format_tools=ToolFormatter(tool_format='glm4_moe'), format_prefix=EmptyFormatter(slots=['[gMASK]<sop>']), stop_words=['<|user|>', '<|observation|>'], efficient_eos=True, template_class=ReasoningTemplate)
register_template(name='glm4v', format_user=StringFormatter(slots=['<|user|>\n{{content}}<|assistant|>']), format_assistant=StringFormatter(slots=['\n{{content}}']), format_system=StringFormatter(slots=['<|system|>\n{{content}}']), format_function=FunctionFormatter(slots=['{{content}}'], tool_format='glm4'), format_observation=StringFormatter(slots=['<|observation|>\n{{content}}<|assistant|>']), format_tools=ToolFormatter(tool_format='glm4'), format_prefix=EmptyFormatter(slots=['[gMASK]<sop>']), stop_words=['<|user|>', '<|observation|>', '</answer>'], efficient_eos=True, mm_plugin=get_mm_plugin(name='glm4v', image_token='<|image|>', video_token='<|video|>'), template_class=ReasoningTemplate)
register_template(name='glm4v_moe', format_user=StringFormatter(slots=['<|user|>\n{{content}}<|assistant|>']), format_assistant=StringFormatter(slots=['\n{{content}}']), format_system=StringFormatter(slots=['<|system|>\n{{content}}']), format_function=FunctionFormatter(slots=['{{content}}'], tool_format='glm4_moe'), format_observation=StringFormatter(slots=['<|observation|>\n{{content}}<|assistant|>']), format_tools=ToolFormatter(tool_format='glm4_moe'), format_prefix=EmptyFormatter(slots=['[gMASK]<sop>']), stop_words=['<|user|>', '<|observation|>', '</answer>'], efficient_eos=True, mm_plugin=get_mm_plugin(name='glm4v', image_token='<|image|>', video_token='<|video|>'), template_class=ReasoningTemplate)
register_template(name='glmz1', format_user=StringFormatter(slots=['<|user|>\n{{content}}<|assistant|>']), format_assistant=StringFormatter(slots=['\n{{content}}']), format_system=StringFormatter(slots=['<|system|>\n{{content}}']), format_function=FunctionFormatter(slots=['{{content}}'], tool_format='glm4'), format_observation=StringFormatter(slots=['<|observation|>\n{{content}}<|assistant|>']), format_tools=ToolFormatter(tool_format='glm4'), format_prefix=EmptyFormatter(slots=['[gMASK]<sop>']), stop_words=['<|user|>', '<|observation|>'], efficient_eos=True, template_class=ReasoningTemplate)
register_template(name='gpt', format_user=StringFormatter(slots=['<|start|>user<|message|>{{content}}<|end|><|start|>assistant']), format_assistant=StringFormatter(slots=['{{content}}<|end|>']), format_system=StringFormatter(slots=['<|start|>system<|message|>{{content}}<|end|>']), default_system='You are ChatGPT, a large language model trained by OpenAI.', thought_words=('<|channel|>analysis<|message|>', '<|end|><|start|>assistant<|channel|>final<|message|>'), efficient_eos=True, template_class=ReasoningTemplate)
...
register_template(name='granite3_vision', format_user=StringFormatter(slots=['<|user|>\n{{content}}\n<|assistant|>\n']), format_system=StringFormatter(slots=['<|system|>\n{{content}}\n']), default_system="A chat between a curious user and an artificial intelligence assistant. The assistant gives helpful, detailed, and polite answers to the user's questions.", mm_plugin=get_mm_plugin(name='llava_next', image_token='<image>'))
register_template(name='granite4', format_user=StringFormatter(slots=['<|start_of_role|>user<|end_of_role|>{{content}}<|end_of_text|>\n<|start_of_role|>assistant<|end_of_role|>']), format_assistant=StringFormatter(slots=['{{content}}<|end_of_text|>\n']), format_system=StringFormatter(slots=['<|start_of_role|>system<|end_of_role|>{{content}}<|end_of_text|>\n']), format_function=FunctionFormatter(slots=['{{content}}<|end_of_text|>\n'], tool_format='default'), format_observation=StringFormatter(slots=['<|start_of_role|>tool<|end_of_role|>{{content}}<|end_of_text|>\n<|start_of_role|>assistant\n']), format_tools=ToolFormatter(tool_format='default'), stop_words=['<|end_of_text|>'], default_system='You are Granite, developed by IBM. You are a helpful AI assistant.')
...
register_template(name='hunyuan', format_user=StringFormatter(slots=['{{content}}<|extra_0|>']), format_assistant=StringFormatter(slots=['{{content}}<|eos|>']), format_system=StringFormatter(slots=['{{content}}<|extra_4|>']), format_prefix=EmptyFormatter(slots=['<|startoftext|>']), stop_words=['<|eos|>'])
...
register_template(name='intern_vl', format_user=StringFormatter(slots=['<|im_start|>user\n{{content}}<|im_end|>\n<|im_start|>assistant\n']), format_assistant=StringFormatter(slots=['{{content}}<|im_end|>\n']), format_system=StringFormatter(slots=['<|im_start|>system\n{{content}}<|im_end|>\n']), format_prefix=EmptyFormatter(slots=[{'bos_token'}]), default_system='你是书生·万象，英文名是InternVL，是由上海人工智能实验室、清华大学及多家合作单位联合开发的多模态大语言模型。', stop_words=['<|im_end|>'], mm_plugin=get_mm_plugin(name='intern_vl', image_token='<image>', video_token='<video>'))
register_template(name='intern_s1', format_user=StringFormatter(slots=['<|im_start|>user\n{{content}}<|im_end|>\n<|im_start|>assistant\n']), format_assistant=StringFormatter(slots=['{{content}}<|im_end|>\n']), format_system=StringFormatter(slots=['<|im_start|>system\n{{content}}<|im_end|>\n']), format_prefix=EmptyFormatter(slots=[{'bos_token'}]), stop_words=['<|im_end|>'], mm_plugin=get_mm_plugin(name='intern_vl', image_token='<image>', video_token='<video>'))
register_template(name='keye_vl', format_user=StringFormatter(slots=['<|im_start|>user\n{{content}}<|im_end|>\n<|im_start|>assistant\n']), format_assistant=StringFormatter(slots=['{{content}}<|im_end|>\n']), format_system=StringFormatter(slots=['<|im_start|>system\n{{content}}<|im_end|>\n']), format_function=FunctionFormatter(slots=['{{content}}<|im_end|>\n'], tool_format='qwen'), format_observation=StringFormatter(slots=['<|im_start|>user\n<tool_response>\n{{content}}\n</tool_response><|im_end|>\n<|im_start|>assistant\n']), format_tools=ToolFormatter(tool_format='qwen'), stop_words=['<|im_end|>'], replace_eos=True, mm_plugin=get_mm_plugin(name='qwen2_vl', image_token='<|image_pad|>', video_token='<|video_pad|>'), template_class=ReasoningTemplate)
register_template(name='kimi_vl', format_user=StringFormatter(slots=['<|im_user|>user<|im_middle|>{{content}}<|im_end|><|im_assistant|>assistant<|im_middle|>']), format_assistant=StringFormatter(slots=['{{content}}<|im_end|>']), format_system=StringFormatter(slots=['<|im_system|>system<|im_middle|>{{content}}<|im_end|>']), default_system='You are a helpful assistant', stop_words=['<|im_end|>'], thought_words=('◁think▷', '◁/think▷'), mm_plugin=get_mm_plugin('kimi_vl', image_token='<|media_pad|>'), template_class=ReasoningTemplate)
...
register_template(name='llava_next_llama3', format_user=StringFormatter(slots=['<|start_header_id|>user<|end_header_id|>\n\n{{content}}<|eot_id|><|start_header_id|>assistant<|end_header_id|>\n\n']), format_assistant=StringFormatter(slots=['{{content}}<|eot_id|>']), format_system=StringFormatter(slots=['<|start_header_id|>system<|end_header_id|>\n\n{{content}}<|eot_id|>']), format_function=FunctionFormatter(slots=['{{content}}<|eot_id|>'], tool_format='llama3'), format_observation=StringFormatter(slots=['<|start_header_id|>ipython<|end_header_id|>\n\n{{content}}<|eot_id|><|start_header_id|>assistant<|end_header_id|>\n\n']), format_tools=ToolFormatter(tool_format='llama3'), format_prefix=EmptyFormatter(slots=[{'bos_token'}]), stop_words=['<|eot_id|>', '<|eom_id|>'], replace_eos=True, mm_plugin=get_mm_plugin(name='llava_next', image_token='<image>'))
...
register_template(name='llava_next_qwen', format_user=StringFormatter(slots=['<|im_start|>user\n{{content}}<|im_end|>\n<|im_start|>assistant\n']), format_assistant=StringFormatter(slots=['{{content}}<|im_end|>\n']), format_system=StringFormatter(slots=['<|im_start|>system\n{{content}}<|im_end|>\n']), format_function=FunctionFormatter(slots=['{{content}}<|im_end|>\n'], tool_format='qwen'), format_observation=StringFormatter(slots=['<|im_start|>user\n<tool_response>\n{{content}}\n</tool_response><|im_end|>\n<|im_start|>assistant\n']), format_tools=ToolFormatter(tool_format='qwen'), default_system='You are a helpful assistant.', stop_words=['<|im_end|>'], replace_eos=True, mm_plugin=get_mm_plugin(name='llava_next', image_token='<image>'))
...
register_template(name='mimo', format_user=StringFormatter(slots=['<|im_start|>user\n{{content}}<|im_end|>\n<|im_start|>assistant\n']), format_assistant=StringFormatter(slots=['{{content}}<|im_end|>\n']), format_system=StringFormatter(slots=['<|im_start|>system\n{{content}}<|im_end|>\n']), format_function=FunctionFormatter(slots=['{{content}}<|im_end|>\n'], tool_format='qwen'), format_observation=StringFormatter(slots=['<|im_start|>user\n<tool_response>\n{{content}}\n</tool_response><|im_end|>\n<|im_start|>assistant\n']), format_tools=ToolFormatter(tool_format='qwen'), default_system='You are a helpful assistant.', stop_words=['<|im_end|>'], replace_eos=True, template_class=ReasoningTemplate)
register_template(name='mimo_vl', format_user=StringFormatter(slots=['<|im_start|>user\n{{content}}<|im_end|>\n<|im_start|>assistant\n']), format_assistant=StringFormatter(slots=['{{content}}<|im_end|>\n']), format_system=StringFormatter(slots=['<|im_start|>system\n{{content}}<|im_end|>\n']), format_function=FunctionFormatter(slots=['{{content}}<|im_end|>\n'], tool_format='qwen'), format_observation=StringFormatter(slots=['<|im_start|>user\n<tool_response>\n{{content}}\n</tool_response><|im_end|>\n<|im_start|>assistant\n']), format_tools=ToolFormatter(tool_format='qwen'), default_system='You are MiMo, an AI assistant developed by Xiaomi.', stop_words=['<|im_end|>'], replace_eos=True, mm_plugin=get_mm_plugin(name='qwen2_vl', image_token='<|image_pad|>', video_token='<|video_pad|>'), template_class=ReasoningTemplate)
...
register_template(name='minicpm_o', format_user=StringFormatter(slots=['<|im_start|>user\n{{content}}<|im_end|>\n<|im_start|>assistant\n']), format_assistant=StringFormatter(slots=['{{content}}<|im_end|>\n']), format_system=StringFormatter(slots=['<|im_start|>system\n{{content}}<|im_end|>\n']), stop_words=['<|im_end|>'], default_system='You are a helpful assistant. You can accept audio and text input and output voice and text.', mm_plugin=get_mm_plugin(name='minicpm_v', image_token='<image>', video_token='<video>', audio_token='<audio>'))
...
register_template(name='paligemma', format_user=StringFormatter(slots=['{{content}}\n']), format_prefix=EmptyFormatter(slots=[{'bos_token'}]), mm_plugin=get_mm_plugin(name='paligemma', image_token='<image>'), template_class=Llama2Template)
register_template(name='paligemma_chat', format_user=StringFormatter(slots=['<start_of_turn>user\n{{content}}<end_of_turn>\n<start_of_turn>model\n']), format_assistant=StringFormatter(slots=['{{content}}<end_of_turn>\n']), format_observation=StringFormatter(slots=['<start_of_turn>tool\n{{content}}<end_of_turn>\n<start_of_turn>model\n']), format_prefix=EmptyFormatter(slots=[{'bos_token'}]), stop_words=['<end_of_turn>'], replace_eos=True, mm_plugin=get_mm_plugin(name='paligemma', image_token='<image>'), template_class=Llama2Template)
register_template(name='phi', format_user=StringFormatter(slots=['<|user|>\n{{content}}<|end|>\n<|assistant|>\n']), format_assistant=StringFormatter(slots=['{{content}}<|end|>\n']), format_system=StringFormatter(slots=['<|system|>\n{{content}}<|end|>\n']), stop_words=['<|end|>'], replace_eos=True)
register_template(name='phi_small', format_user=StringFormatter(slots=['<|user|>\n{{content}}<|end|>\n<|assistant|>\n']), format_assistant=StringFormatter(slots=['{{content}}<|end|>\n']), format_system=StringFormatter(slots=['<|system|>\n{{content}}<|end|>\n']), format_prefix=EmptyFormatter(slots=[{'<|endoftext|>'}]), stop_words=['<|end|>'], replace_eos=True)
register_template(name='phi4', format_user=StringFormatter(slots=['<|im_start|>user<|im_sep|>{{content}}<|im_end|><|im_start|>assistant<|im_sep|>']), format_assistant=StringFormatter(slots=['{{content}}<|im_end|>']), format_system=StringFormatter(slots=['<|im_start|>system<|im_sep|>{{content}}<|im_end|>']), stop_words=['<|im_end|>'], replace_eos=True)
...
register_template(name='qwen3', format_user=StringFormatter(slots=['<|im_start|>user\n{{content}}<|im_end|>\n<|im_start|>assistant\n']), format_assistant=StringFormatter(slots=['{{content}}<|im_end|>\n']), format_system=StringFormatter(slots=['<|im_start|>system\n{{content}}<|im_end|>\n']), format_function=FunctionFormatter(slots=['{{content}}<|im_end|>\n'], tool_format='qwen'), format_observation=StringFormatter(slots=['<|im_start|>user\n<tool_response>\n{{content}}\n</tool_response><|im_end|>\n<|im_start|>assistant\n']), format_tools=ToolFormatter(tool_format='qwen'), stop_words=['<|im_end|>'], replace_eos=True, template_class=ReasoningTemplate)
register_template(name='qwen3_nothink', format_user=StringFormatter(slots=['<|im_start|>user\n{{content}}<|im_end|>\n<|im_start|>assistant\n']), format_assistant=StringFormatter(slots=['{{content}}<|im_end|>\n']), format_system=StringFormatter(slots=['<|im_start|>system\n{{content}}<|im_end|>\n']), format_function=FunctionFormatter(slots=['{{content}}<|im_end|>\n'], tool_format='qwen'), format_observation=StringFormatter(slots=['<|im_start|>user\n<tool_response>\n{{content}}\n</tool_response><|im_end|>\n<|im_start|>assistant\n']), format_tools=ToolFormatter(tool_format='qwen'), stop_words=['<|im_end|>'], replace_eos=True)
register_template(name='qwen2_audio', format_user=StringFormatter(slots=['<|im_start|>user\n{{content}}<|im_end|>\n<|im_start|>assistant\n']), format_assistant=StringFormatter(slots=['{{content}}<|im_end|>\n']), format_system=StringFormatter(slots=['<|im_start|>system\n{{content}}<|im_end|>\n']), default_system='You are a helpful assistant.', stop_words=['<|im_end|>'], replace_eos=True, mm_plugin=get_mm_plugin(name='qwen2_audio', audio_token='<|AUDIO|>'))
register_template(name='qwen2_omni', format_user=StringFormatter(slots=['<|im_start|>user\n{{content}}<|im_end|>\n<|im_start|>assistant\n']), format_assistant=StringFormatter(slots=['{{content}}<|im_end|>\n']), format_system=StringFormatter(slots=['<|im_start|>system\n{{content}}<|im_end|>\n']), format_function=FunctionFormatter(slots=['{{content}}<|im_end|>\n'], tool_format='qwen'), format_observation=StringFormatter(slots=['<|im_start|>user\n<tool_response>\n{{content}}\n</tool_response><|im_end|>\n<|im_start|>assistant\n']), format_tools=ToolFormatter(tool_format='qwen'), default_system='You are a helpful assistant.', stop_words=['<|im_end|>'], replace_eos=True, mm_plugin=get_mm_plugin(name='qwen2_omni', image_token='<|IMAGE|>', video_token='<|VIDEO|>', audio_token='<|AUDIO|>', vision_bos_token='<|vision_bos|>', vision_eos_token='<|vision_eos|>', audio_bos_token='<|audio_bos|>', audio_eos_token='<|audio_eos|>'))
register_template(name='qwen3_omni', format_user=StringFormatter(slots=['<|im_start|>user\n{{content}}<|im_end|>\n<|im_start|>assistant\n']), format_assistant=StringFormatter(slots=['{{content}}<|im_end|>\n']), format_system=StringFormatter(slots=['<|im_start|>system\n{{content}}<|im_end|>\n']), format_function=FunctionFormatter(slots=['{{content}}<|im_end|>\n'], tool_format='qwen'), format_observation=StringFormatter(slots=['<|im_start|>user\n<tool_response>\n{{content}}\n</tool_response><|im_end|>\n<|im_start|>assistant\n']), format_tools=ToolFormatter(tool_format='qwen'), stop_words=['<|im_end|>'], replace_eos=True, mm_plugin=get_mm_plugin(name='qwen2_omni', image_token='<|image_pad|>', video_token='<|video_pad|>', audio_token='<|audio_pad|>'), template_class=ReasoningTemplate)
register_template(name='qwen3_omni_nothink', format_user=StringFormatter(slots=['<|im_start|>user\n{{content}}<|im_end|>\n<|im_start|>assistant\n']), format_assistant=StringFormatter(slots=['{{content}}<|im_end|>\n']), format_system=StringFormatter(slots=['<|im_start|>system\n{{content}}<|im_end|>\n']), format_function=FunctionFormatter(slots=['{{content}}<|im_end|>\n'], tool_format='qwen'), format_observation=StringFormatter(slots=['<|im_start|>user\n<tool_response>\n{{content}}\n</tool_response><|im_end|>\n<|im_start|>assistant\n']), format_tools=ToolFormatter(tool_format='qwen'), stop_words=['<|im_end|>'], replace_eos=True, mm_plugin=get_mm_plugin(name='qwen2_omni', image_token='<|image_pad|>', video_token='<|video_pad|>', audio_token='<|audio_pad|>'))
register_template(name='qwen2_vl', format_user=StringFormatter(slots=['<|im_start|>user\n{{content}}<|im_end|>\n<|im_start|>assistant\n']), format_assistant=StringFormatter(slots=['{{content}}<|im_end|>\n']), format_system=StringFormatter(slots=['<|im_start|>system\n{{content}}<|im_end|>\n']), format_function=FunctionFormatter(slots=['{{content}}<|im_end|>\n'], tool_format='qwen'), format_observation=StringFormatter(slots=['<|im_start|>user\n<tool_response>\n{{content}}\n</tool_response><|im_end|>\n<|im_start|>assistant\n']), format_tools=ToolFormatter(tool_format='qwen'), default_system='You are a helpful assistant.', stop_words=['<|im_end|>'], replace_eos=True, mm_plugin=get_mm_plugin(name='qwen2_vl', image_token='<|image_pad|>', video_token='<|video_pad|>'))
register_template(name='qwen3_vl', format_user=StringFormatter(slots=['<|im_start|>user\n{{content}}<|im_end|>\n<|im_start|>assistant\n']), format_assistant=StringFormatter(slots=['{{content}}<|im_end|>\n']), format_system=StringFormatter(slots=['<|im_start|>system\n{{content}}<|im_end|>\n']), format_function=FunctionFormatter(slots=['{{content}}<|im_end|>\n'], tool_format='qwen'), format_observation=StringFormatter(slots=['<|im_start|>user\n<tool_response>\n{{content}}\n</tool_response><|im_end|>\n<|im_start|>assistant\n']), format_tools=ToolFormatter(tool_format='qwen'), stop_words=['<|im_end|>'], replace_eos=True, mm_plugin=get_mm_plugin(name='qwen3_vl', image_token='<|image_pad|>', video_token='<|video_pad|>'), template_class=ReasoningTemplate)
register_template(name='qwen3_vl_nothink', format_user=StringFormatter(slots=['<|im_start|>user\n{{content}}<|im_end|>\n<|im_start|>assistant\n']), format_assistant=StringFormatter(slots=['{{content}}<|im_end|>\n']), format_system=StringFormatter(slots=['<|im_start|>system\n{{content}}<|im_end|>\n']), format_function=FunctionFormatter(slots=['{{content}}<|im_end|>\n'], tool_format='qwen'), format_observation=StringFormatter(slots=['<|im_start|>user\n<tool_response>\n{{content}}\n</tool_response><|im_end|>\n<|im_start|>assistant\n']), format_tools=ToolFormatter(tool_format='qwen'), stop_words=['<|im_end|>'], replace_eos=True, mm_plugin=get_mm_plugin(name='qwen3_vl', image_token='<|image_pad|>', video_token='<|video_pad|>'))
...
register_template(name='smollm', format_user=StringFormatter(slots=['<|im_start|>user\n{{content}}<|im_end|>\n<|im_start|>assistant\n']), format_assistant=StringFormatter(slots=['{{content}}<|im_end|>\n']), format_system=StringFormatter(slots=['<|im_start|>system\n{{content}}<|im_end|>\n']), stop_words=['<|im_end|>'])
register_template(name='smollm2', format_user=StringFormatter(slots=['<|im_start|>user\n{{content}}<|im_end|>\n<|im_start|>assistant\n']), format_assistant=StringFormatter(slots=['{{content}}<|im_end|>\n']), format_system=StringFormatter(slots=['<|im_start|>system\n{{content}}<|im_end|>\n']), stop_words=['<|im_end|>'], default_system='You are a helpful AI assistant named SmolLM, trained by Hugging Face.')
