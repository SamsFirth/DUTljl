...
class FeedbackDatasetProcessor(DatasetProcessor):
    ...
    def _encode_data_example(self, prompt: list[dict[str, str]], response: list[dict[str, str]], kl_response: list[dict[str, str]], system: Optional[str], tools: Optional[str], images: list['ImageInput'], videos: list['VideoInput'], audios: list['AudioInput']) -> tuple[list[int], list[int], list[int], list[int], bool]:
        ...
        kl_prompt_ids, _ = self.template.mm_plugin.process_token_ids(kl_prompt_ids, None, images, videos, audios, self.tokenizer, self.processor)
    ...
    def preprocess_dataset(self, examples: dict[str, list[Any]]) -> dict[str, list[Any]]:
        ...
        for i in range(len(examples['_prompt'])):
            if len(examples['_prompt'][i]) % 2 != 1 or len(examples['_response'][i]) < 2:
                ...
                continue
            input_ids, labels, kl_input_ids, kl_labels, kto_tag = self._encode_data_example(prompt=examples['_prompt'][i], response=examples['_response'][i], kl_response=kl_response[i], system=examples['_system'][i], tools=examples['_tools'][i], images=examples['_images'][i] or [], videos=examples['_videos'][i] or [], audios=examples['_audios'][i] or [])
