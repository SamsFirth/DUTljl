...
class DefaultToolUtils(ToolUtils):
    ...
    def tool_formatter(tools: list[dict[str, Any]]) -> str:
        ...
        for tool in tools:
            ...
            for name, param in tool['parameters']['properties'].items():
                ...
                param_text += '  - {name} ({type}{required}): {desc}{enum}{items}\n'.format(name=name, type=param.get('type', ''), required=required, desc=param.get('description', ''), enum=enum, items=items)
            tool_text += '> Tool Name: {name}\nTool Description: {desc}\nTool Args:\n{args}\n'.format(name=tool['name'], desc=tool.get('description', ''), args=param_text)
    ...
    @staticmethod
    def tool_extractor(content: str) -> Union[str, list['FunctionCall']]:
        regex = re.compile('Action:\\s*([a-zA-Z0-9_]+)\\s*Action Input:\\s*(.+?)(?=\\s*Action:|\\s*$)', re.DOTALL)
...
class GLM4ToolUtils(ToolUtils):
    ...
    def tool_formatter(tools: list[dict[str, Any]]) -> str:
        ...
        for tool in tools:
            tool = tool.get('function', '') if tool.get('type') == 'function' else tool
            tool_text += '\n\n## {name}\n\n{body}\n在调用上述函数时，请使用 Json 格式表示调用的参数。'.format(name=tool['name'], body=json.dumps(tool, indent=4, ensure_ascii=False))
...
class MistralToolUtils(ToolUtils):
    ...
    @staticmethod
    def function_formatter(functions: list['FunctionCall']) -> str:
        return json.dumps([{'name': name, 'arguments': json.loads(arguments)} for name, arguments in functions], ensure_ascii=False)
...
class QwenToolUtils(ToolUtils):
    ...
    @staticmethod
    def function_formatter(functions: list['FunctionCall']) -> str:
        function_texts = [json.dumps({'name': name, 'arguments': json.loads(arguments)}, ensure_ascii=False) for name, arguments in functions]
        return '\n'.join([f'<tool_call>\n{text}\n</tool_call>' for text in function_texts])

    @override
    @staticmethod
    def tool_extractor(content: str) -> Union[str, list['FunctionCall']]:
        regex = re.compile('<tool_call>(.+?)</tool_call>(?=\\s*<tool_call>|\\s*$)', re.DOTALL)
...
class GLM4MOEToolUtils(QwenToolUtils):
    ...
    @staticmethod
    def function_formatter(functions: list['FunctionCall']) -> str:
        function_json = [{'func_name': name, 'func_key_values': json.loads(arguments)} for name, arguments in functions]
        function_texts = []
...
class SeedToolUtils(ToolUtils):
    ...
    def tool_extractor(content: str) -> Union[str, list['FunctionCall']]:
        results = []
        regex = re.compile('<seed:tool_call>\\s*<function=\\s*([^\\s<]+)\\s*(.*?)\\s*</function>\\s*</seed:tool_call>', re.DOTALL)
        for func_name, params_block in re.findall(regex, content):
            args_dict = {}
            param_pattern = re.compile('<parameter=(.*?)>(.*?)</parameter>', re.DOTALL)
...
TOOLS = {'default': DefaultToolUtils(), 'glm4': GLM4ToolUtils(), 'llama3': Llama3ToolUtils(), 'mistral': MistralToolUtils(), 'qwen': QwenToolUtils(), 'glm4_moe': GLM4MOEToolUtils(), 'seed_oss': SeedToolUtils(), 'ling': LingToolUtils()}
