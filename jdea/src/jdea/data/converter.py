                ...
                for i in range(len(medias)):
                    for j in range(len(medias[i])):
                        ...
                        if os.path.isfile(media_path):
                            ...
                            logger.warning_rank0_once(f'Media {medias[i][j]} does not exist in `media_dir`. Use original path.')
        return medias
...
class AlpacaDatasetConverter(DatasetConverter):
    ...
    def __call__(self, example: dict[str, Any]) -> dict[str, Any]:
        ...
        elif self.dataset_attr.ranking and isinstance(example[self.dataset_attr.chosen], str) and isinstance(example[self.dataset_attr.rejected], str):
            response = [{'role': Role.ASSISTANT.value, 'content': example[self.dataset_attr.chosen]}, {'role': Role.ASSISTANT.value, 'content': example[self.dataset_attr.rejected]}]
...
class SharegptDatasetConverter(DatasetConverter):
    ...
    def __call__(self, example: dict[str, Any]) -> dict[str, Any]:
        tag_mapping = {self.dataset_attr.user_tag: Role.USER.value, self.dataset_attr.assistant_tag: Role.ASSISTANT.value, self.dataset_attr.observation_tag: Role.OBSERVATION.value, self.dataset_attr.function_tag: Role.FUNCTION.value, self.dataset_attr.system_tag: Role.SYSTEM.value}
        odd_tags = (self.dataset_attr.user_tag, self.dataset_attr.observation_tag)
        even_tags = (self.dataset_attr.assistant_tag, self.dataset_attr.function_tag)
        accept_tags = (odd_tags, even_tags)
        messages = example[self.dataset_attr.messages]
        if self.dataset_attr.system_tag and len(messages) != 0 and (messages[0][self.dataset_attr.role_tag] == self.dataset_attr.system_tag):
        ...
        for turn_idx, message in enumerate(messages):
            if message[self.dataset_attr.role_tag] not in accept_tags[turn_idx % 2]:
                ...
                broken_data = True
                break
            aligned_messages.append({'role': tag_mapping[message[self.dataset_attr.role_tag]], 'content': message[self.dataset_attr.content_tag]})
        if not self.dataset_attr.ranking and len(aligned_messages) % 2 != 0 or (self.dataset_attr.ranking and len(aligned_messages) % 2 == 0):
        ...
        if broken_data:
            ...
            if example[self.dataset_attr.kto_tag]:
                ...
                response = [{'role': Role.ASSISTANT.value, 'content': ''}] + response
        elif self.dataset_attr.ranking and isinstance(example[self.dataset_attr.chosen], dict) and isinstance(example[self.dataset_attr.rejected], dict):
            chosen = example[self.dataset_attr.chosen]
            rejected = example[self.dataset_attr.rejected]
            if chosen[self.dataset_attr.role_tag] not in accept_tags[-1] or rejected[self.dataset_attr.role_tag] not in accept_tags[-1]:
                logger.warning_rank0(f'Invalid role tag in {[chosen, rejected]}.')
                broken_data = True
            prompt = aligned_messages
            response = [{'role': tag_mapping[chosen[self.dataset_attr.role_tag]], 'content': chosen[self.dataset_attr.content_tag]}, {'role': tag_mapping[rejected[self.dataset_attr.role_tag]], 'content': rejected[self.dataset_attr.content_tag]}]
        else:
            prompt = aligned_messages[:-1]
            response = aligned_messages[-1:]
    ...
    def __call__(self, example: dict[str, Any]) -> dict[str, Any]:
        ...
        for turn_idx, message in enumerate(messages):
            ...
            if role == self.dataset_attr.observation_tag:
                ...
                _content = '\n</tool_response>\n<tool_response>\n'.join(tool_responses)
                aligned_messages.append({'role': Role.OBSERVATION.value, 'content': _content})
                tool_responses = []
            aligned_messages.append({'role': tag_mapping[role], 'content': content})
        ...
        return output
DATASET_CONVERTERS = {'alpaca': AlpacaDatasetConverter, 'sharegpt': SharegptDatasetConverter, 'openai': OpenAIDatasetConverter}
...
def align_dataset(dataset: Union['Dataset', 'IterableDataset'], dataset_attr: 'DatasetAttr', data_args: 'DataArguments', training_args: 'Seq2SeqTrainingArguments') -> Union['Dataset', 'IterableDataset']:
    ...
    if not data_args.streaming:
        kwargs = dict(num_proc=data_args.preprocessing_num_workers, load_from_cache_file=not data_args.overwrite_cache or training_args.local_process_index != 0, desc='Converting format of dataset')
    dataset_converter = get_dataset_converter(dataset_attr.formatting, dataset_attr, data_args)
    return dataset.map(dataset_converter, batched=False, remove_columns=column_names, **kwargs)