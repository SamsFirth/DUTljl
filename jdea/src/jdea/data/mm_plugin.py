...
from transformers.models.mllama.processing_mllama import convert_sparse_cross_attention_mask_to_dense, get_cross_attention_token_mask
from typing_extensions import NotRequired, override
from ..extras.constants import AUDIO_PLACEHOLDER, IGNORE_INDEX, IMAGE_PLACEHOLDER, VIDEO_PLACEHOLDER
from ..extras.packages import is_librosa_available, is_pillow_available, is_pyav_available, is_transformers_version_greater_than
...
class MMPluginMixin:
    ...
    def _validate_input(self, processor: Optional['MMProcessor'], images: list['ImageInput'], videos: list['VideoInput'], audios: list['AudioInput']) -> None:
        """Validate if this model accepts the input modalities."""
        image_processor: BaseImageProcessor = getattr(processor, 'image_processor', None)
        video_processor: BaseImageProcessor = getattr(processor, 'video_processor', getattr(processor, 'image_processor', None))
        ...
        if len(videos) != 0 and self.video_token is None:
            raise ValueError('This model does not support video input. Please check whether the correct `template` is used.')
        if len(audios) != 0 and self.audio_token is None:
            raise ValueError('This model does not support audio input. Please check whether the correct `template` is used.')
    ...
    def _validate_messages(self, messages: list[dict[str, str]], images: list['ImageInput'], videos: list['VideoInput'], audios: list['AudioInput']):
        ...
        if len(images) != num_image_tokens:
            raise ValueError(f'The number of images does not match the number of {IMAGE_PLACEHOLDER} tokens in {messages}.')
        if len(videos) != num_video_tokens:
            raise ValueError(f'The number of videos does not match the number of {VIDEO_PLACEHOLDER} tokens in {messages}.')
        if len(audios) != num_audio_tokens:
            raise ValueError(f'The number of audios does not match the number of {AUDIO_PLACEHOLDER} tokens in {messages}.')
    ...
    def _get_video_sample_indices(self, video_stream: 'Stream', video_fps: float, video_maxlen: int, **kwargs) -> list[int]:
    ...
    def _regularize_audios(self, audios: list['AudioInput'], sampling_rate: float, **kwargs) -> 'RegularizedAudioOutput':
    ...
    def _get_mm_inputs(self, images: list['ImageInput'], videos: list['VideoInput'], audios: list['AudioInput'], processor: 'MMProcessor', imglens: Optional[list[int]]=None) -> dict[str, 'torch.Tensor']:
        ...
        if len(images) != 0:
            image_processor: BaseImageProcessor = getattr(processor, 'image_processor', None)
            images = self._regularize_images(images, image_max_pixels=getattr(processor, 'image_max_pixels', 768 * 768), image_min_pixels=getattr(processor, 'image_min_pixels', 32 * 32))['images']
            if imglens is not None:
                images = _make_batched_images(images, imglens)
            image_processor_kwargs = {}
            if getattr(processor, 'image_do_pan_and_scan', False):
                image_processor_kwargs.update({'do_pan_and_scan': True, 'pan_and_scan_min_crop_size': 256, 'pan_and_scan_max_num_crops': 4, 'pan_and_scan_min_ratio_to_activate': 1.2})
            mm_inputs.update(image_processor(images, return_tensors='pt', **image_processor_kwargs))
        if len(videos) != 0:
            video_processor: BaseImageProcessor = getattr(processor, 'video_processor', getattr(processor, 'image_processor', None))
            videos = self._regularize_videos(videos, image_max_pixels=getattr(processor, 'video_max_pixels', 256 * 256), image_min_pixels=getattr(processor, 'video_min_pixels', 16 * 16), video_fps=getattr(processor, 'video_fps', 2.0), video_maxlen=getattr(processor, 'video_maxlen', 128))['videos']
        ...
        if len(audios) != 0:
            feature_extractor: SequenceFeatureExtractor = getattr(processor, 'feature_extractor', None)
            audios = self._regularize_audios(audios, sampling_rate=getattr(processor, 'audio_sampling_rate', 16000))['audios']
            mm_inputs.update(feature_extractor(audios, sampling_rate=getattr(processor, 'audio_sampling_rate', 16000), return_attention_mask=True, padding='max_length', return_tensors='pt'))
...
class BasePlugin(MMPluginMixin):
    ...
    def process_messages(self, messages: list[dict[str, str]], images: list['ImageInput'], videos: list['VideoInput'], audios: list['AudioInput'], processor: Optional['MMProcessor']) -> list[dict[str, str]]:
        """Pre-process input messages before tokenization for VLMs."""
        self._validate_input(processor, images, videos, audios)
        return messages

    def process_token_ids(self, input_ids: list[int], labels: Optional[list[int]], images: list['ImageInput'], videos: list['VideoInput'], audios: list['AudioInput'], tokenizer: 'PreTrainedTokenizer', processor: Optional['MMProcessor']) -> tuple[list[int], Optional[list[int]]]:
        """Pre-process token ids after tokenization for VLMs."""
        self._validate_input(processor, images, videos, audios)
        return (input_ids, labels)

    def get_mm_inputs(self, images: list['ImageInput'], videos: list['VideoInput'], audios: list['AudioInput'], imglens: list[int], vidlens: list[int], audlens: list[int], batch_ids: list[list[int]], processor: Optional['MMProcessor']) -> dict[str, Union[list[int], 'torch.Tensor']]:
    ...
    def process_messages(self, messages: list[dict[str, str]], images: list['ImageInput'], videos: list['VideoInput'], audios: list['AudioInput'], processor: Optional['MMProcessor']) -> list[dict[str, str]]:
        ...
        for message in messages:
            ...
            while IMAGE_PLACEHOLDER in content:
                image_idx += 1
                content = content.replace(IMAGE_PLACEHOLDER, f'Picture {image_idx}:<|IMAGE_START|>{image_token}<|IMAGE_END|>', 1)
            while VIDEO_PLACEHOLDER in content:
                video_idx += 1
                content = content.replace(VIDEO_PLACEHOLDER, f'Video {video_idx}:<|VIDEO_START|>{video_token}<|VIDEO_END|>', 1)
...
class Gemma3Plugin(BasePlugin):
    ...
    def process_messages(self, messages: list[dict[str, str]], images: list['ImageInput'], videos: list['VideoInput'], audios: list['AudioInput'], processor: Optional['MMProcessor']) -> list[dict[str, str]]:
        ...
        for message in messages:
            ...
            while IMAGE_PLACEHOLDER in content:
                if do_pan_and_scan:
                    image_placeholder_str = 'Here is the original image {{image}} and here are some crops to help you see better ' + ' '.join(['{{image}}'] * mm_inputs['num_crops'][0][num_image_tokens])
                else:
...
class InternVLPlugin(BasePlugin):
    ...
    @override
    def _get_mm_inputs(self, images: list['ImageInput'], videos: list['VideoInput'], audios: list['AudioInput'], processor: 'ProcessorMixin', **kwargs) -> dict[str, 'torch.Tensor']:
        ...
        image_video_patches = []
        if len(images) != 0:
            images = self._regularize_images(images, image_max_pixels=getattr(processor, 'image_max_pixels', 1024 * 1024), image_min_pixels=getattr(processor, 'image_min_pixels', 32 * 32))['images']
        if len(videos) != 0:
            videos = self._regularize_videos(videos, image_max_pixels=getattr(processor, 'video_max_pixels', 256 * 256), image_min_pixels=getattr(processor, 'video_min_pixels', 16 * 16), video_fps=getattr(processor, 'video_fps', 2.0), video_maxlen=getattr(processor, 'video_maxlen', 128))['videos']
        if len(images) != 0:
    ...
    @override
    def process_messages(self, messages: list[dict[str, str]], images: list['ImageInput'], videos: list['VideoInput'], audios: list['AudioInput'], processor: Optional['ProcessorMixin']) -> list[dict[str, str]]:
        self._validate_input(processor, images, videos, audios)
        self._validate_messages(messages, images, videos, audios)
        ...
        for message in messages:
            content = message['content']
            while IMAGE_PLACEHOLDER in content:
                content = content.replace(IMAGE_PLACEHOLDER, f"<img>{'<IMG_CONTEXT>' * image_seqlen * image_pixel_patch_list[num_image_tokens]}</img>", 1)
                num_image_tokens += 1
            while VIDEO_PLACEHOLDER in content:
                ...
                video_replaced_prompt = '\n'.join((f"Frame{i + 1}: <img>{'<IMG_CONTEXT>' * image_seqlen * num_patches[i]}</img>" for i in range(len(num_patches))))
    ...
    @override
    def get_mm_inputs(self, images: list['ImageInput'], videos: list['VideoInput'], audios: list['AudioInput'], imglens: list[int], vidlens: list[int], audlens: list[int], batch_ids: list[list[int]], processor: Optional['ProcessorMixin']) -> dict[str, Union[list[int], 'torch.Tensor']]:
        self._validate_input(processor, images, videos, audios)
        mm_inputs = self._get_mm_inputs(images, videos, audios, processor)
    ...
    def process_messages(self, messages, images, videos, audios, processor):
        ...
        for message in messages:
            ...
            while IMAGE_PLACEHOLDER in content:
                ...
                content = content.replace(IMAGE_PLACEHOLDER, f'<|media_start|>image<|media_content|>{self.image_token * image_seqlen}<|media_end|>', 1)
                num_image_tokens += 1
            message['content'] = content
        return messages
    ...
    def process_messages(self, messages: list[dict[str, str]], images: list['ImageInput'], videos: list['VideoInput'], audios: list['AudioInput'], processor: Optional['MMProcessor']) -> list[dict[str, str]]:
        ...
        if self.expand_mm_tokens:
            ...
            if 'pixel_values' in mm_inputs:
                ...
                num_patches_per_chunk = int(image_height // processor.patch_size * (image_width // processor.patch_size) // processor.downsample_ratio)
            ...
            if self.expand_mm_tokens:
                ...
                for local_image_index, split_part in enumerate(prompt_splits):
                    ...
                    if local_image_index < placeholder_count:
                        tokens_for_this_image = processor._prompt_split_image(aspect_ratios[num_image_tokens], num_patches_per_chunk)
                        num_image_tokens += 1
    ...
    def process_messages(self, messages: list[dict[str, str]], images: list['ImageInput'], videos: list['VideoInput'], audios: list['AudioInput'], processor: Optional['MMProcessor']) -> list[dict[str, str]]:
        ...
        if self.expand_mm_tokens:
            ...
            if 'pixel_values' in mm_inputs:
                ...
                image_seqlen = height // processor.patch_size * (width // processor.patch_size) + processor.num_additional_image_tokens
...
class MiniCPMVPlugin(BasePlugin):
    ...
    @override
    def _get_mm_inputs(self, images: list['ImageInput'], videos: list['VideoInput'], audios: list['AudioInput'], processor: 'MMProcessor', **kwargs) -> dict[str, 'torch.Tensor']:
        image_processor: BaseImageProcessor = getattr(processor, 'image_processor')
        mm_inputs = {}
        if len(images) != 0:
            images = self._regularize_images(images, image_max_pixels=getattr(processor, 'image_max_pixels', 768 * 768), image_min_pixels=getattr(processor, 'image_min_pixels', 32 * 32))['images']
        ...
        if len(audios) != 0:
            ...
            if 'valid_audio_nums_ls' in kwargs:
                ...
                audios_ls = [audios]
            audio_features, audio_feature_lens, audio_phs = processor.audio_feature_extract(audios_ls, chunk_input=True, sampling_rate=getattr(processor, 'audio_sampling_rate', 16000))
    ...
    def process_messages(self, messages: list[dict[str, str]], images: list['ImageInput'], videos: list['VideoInput'], audios: list['AudioInput'], processor: Optional['MMProcessor']) -> list[dict[str, str]]:
        ...
        for i, message in enumerate(messages):
            ...
            while AUDIO_PLACEHOLDER in content:
                ...
                num_audio_tokens += 1
            message['content'] = content.replace('{{image}}', '(<image>./</image>)').replace('{{audio}}', '(<audio>./</audio>)')
        if len(images):
        ...
        if self.expand_mm_tokens and mm_inputs:
            ...
            for index, message in enumerate(messages):
                ...
                for i in range(len(image_tags)):
                    final_text = final_text + text_chunks[i] + image_processor.get_slice_image_placeholder(image_sizes[0][idx], idx, max_slice_nums, use_image_id)
                    idx += 1
    ...
    def get_mm_inputs(self, images: list['ImageInput'], videos: list['VideoInput'], audios: list['AudioInput'], imglens: list[int], vidlens: list[int], audlens: list[int], batch_ids: list[list[int]], processor: Optional['MMProcessor']) -> dict[str, Union[list[int], 'torch.Tensor']]:
        ...
        for i, input_ids in enumerate(batch_ids):
            ...
            image_bounds = torch.hstack([image_start_tokens.unsqueeze(-1), image_end_tokens.unsqueeze(-1)])
...
class MllamaPlugin(BasePlugin):
    ...
    def get_mm_inputs(self, images: list['ImageInput'], videos: list['VideoInput'], audios: list['AudioInput'], imglens: list[int], vidlens: list[int], audlens: list[int], batch_ids: list[list[int]], processor: Optional['MMProcessor']) -> dict[str, Union[list[int], 'torch.Tensor']]:
        ...
        if mm_inputs:
            ...
            cross_attention_token_mask = [get_cross_attention_token_mask(input_ids, image_token_id) for input_ids in batch_ids]
            mm_inputs['cross_attention_mask'] = torch.from_numpy(convert_sparse_cross_attention_mask_to_dense(cross_attention_token_mask, num_tiles=num_tiles, max_num_tiles=max_image_tiles, length=max((len(input_ids) for input_ids in batch_ids))))
        return mm_inputs
        ...
        for message in messages:
            ...
            while AUDIO_PLACEHOLDER in content:
                if self.expand_mm_tokens:
                    ...
                    audio_seqlen = 1
                content = content.replace(AUDIO_PLACEHOLDER, f'{bos_token}{self.audio_token * audio_seqlen}{eos_token}', 1)
            message['content'] = content
        return messages
...
class Qwen2VLPlugin(BasePlugin):
    ...
    @override
    def _get_mm_inputs(self, images: list['ImageInput'], videos: list['VideoInput'], audios: list['AudioInput'], processor: 'MMProcessor') -> dict[str, 'torch.Tensor']:
        image_processor: BaseImageProcessor = getattr(processor, 'image_processor', None)
        ...
        if len(videos) != 0:
            video_data = self._regularize_videos(videos, image_max_pixels=getattr(processor, 'video_max_pixels', 256 * 256), image_min_pixels=getattr(processor, 'video_min_pixels', 16 * 16), video_fps=getattr(processor, 'video_fps', 2.0), video_maxlen=getattr(processor, 'video_maxlen', 128))
    ...
    def process_messages(self, messages: list[dict[str, str]], images: list['ImageInput'], videos: list['VideoInput'], audios: list['AudioInput'], processor: Optional['MMProcessor']) -> list[dict[str, str]]:
        ...
        for message in messages:
            ...
            while IMAGE_PLACEHOLDER in content:
                ...
                content = content.replace(IMAGE_PLACEHOLDER, f'{self.vision_bos_token}{self.image_token * image_seqlen}{self.vision_eos_token}', 1)
                num_image_tokens += 1
            while VIDEO_PLACEHOLDER in content:
                video_seqlen = video_grid_thw[num_video_tokens].prod() // merge_length if self.expand_mm_tokens else 1
                content = content.replace(VIDEO_PLACEHOLDER, f'{self.vision_bos_token}{self.video_token * video_seqlen}{self.vision_eos_token}', 1)
                num_video_tokens += 1
            message['content'] = content
        return messages
...
class Qwen3VLPlugin(Qwen2VLPlugin):
    ...
    def _get_mm_inputs(self, images: list['ImageInput'], videos: list['VideoInput'], audios: list['AudioInput'], processor: 'MMProcessor') -> dict[str, 'torch.Tensor']:
        ...
        if len(images) != 0:
            ...
            mm_inputs.update(image_processor(images, return_tensors='pt'))
        if len(videos) != 0:
            videos = self._regularize_videos(videos, image_max_pixels=getattr(processor, 'video_max_pixels', 256 * 256), image_min_pixels=getattr(processor, 'video_min_pixels', 16 * 16), video_fps=getattr(processor, 'video_fps', 2.0), video_maxlen=getattr(processor, 'video_maxlen', 128))
            video_metadata = [{'fps': getattr(processor, 'video_fps', 24.0), 'duration': duration, 'total_num_frames': len(video)} for video, duration in zip(videos['videos'], videos['durations'])]
        ...
        for idx, message in enumerate(messages):
            ...
            while VIDEO_PLACEHOLDER in content:
                if self.expand_mm_tokens:
                    ...
                    timestamps = processor._calculate_timestamps(metadata.frames_indices, metadata.fps, video_processor.merge_size)
                    video_structure = ''
                    for frame_index in range(num_frames):
                        video_seqlen = video_grid_thw[num_video_tokens][1:].prod() // video_merge_length if self.expand_mm_tokens else 1
                        timestamp_sec = timestamps[frame_index]
                        frame_structure = f'<{timestamp_sec:.1f} seconds>{self.vision_bos_token}{self.video_token * video_seqlen}{self.vision_eos_token}'
...
class GLM4VPlugin(Qwen2VLPlugin):
    ...
    def _get_mm_inputs(self, images: list['ImageInput'], videos: list['VideoInput'], audios: list['AudioInput'], processor: 'MMProcessor') -> dict[str, 'torch.Tensor']:
        ...
        if len(videos) != 0:
            video_data = self._regularize_videos(videos, image_max_pixels=getattr(processor, 'video_max_pixels', 256 * 256), image_min_pixels=getattr(processor, 'video_min_pixels', 16 * 16), video_fps=getattr(processor, 'video_fps', 2.0), video_maxlen=getattr(processor, 'video_maxlen', 128))
            video_metadata = [{'fps': 2, 'duration': duration, 'total_frames': len(video)} for video, duration in zip(video_data['videos'], video_data['durations'])]
    ...
    def process_messages(self, messages: list[dict[str, str]], images: list['ImageInput'], videos: list['VideoInput'], audios: list['AudioInput'], processor: Optional['MMProcessor']) -> list[dict[str, str]]:
        ...
        for message in messages:
            ...
            while IMAGE_PLACEHOLDER in content:
                image_seqlen = image_grid_thw[num_image_tokens].prod() // merge_length if self.expand_mm_tokens else 1
                content = content.replace(IMAGE_PLACEHOLDER, f'<|begin_of_image|>{self.image_token * image_seqlen}<|end_of_image|>', 1)
                num_image_tokens += 1
            while VIDEO_PLACEHOLDER in content:
                video_structure = ''
                for frame_index in range(num_frames):
...
class Qwen2OmniPlugin(Qwen2VLPlugin):
    ...
    def _get_mm_inputs(self, images: list['ImageInput'], videos: list['VideoInput'], audios: list['AudioInput'], processor: 'MMProcessor') -> dict[str, 'torch.Tensor']:
        ...
        if len(images) != 0:
            ...
            mm_inputs.update(image_processor(images, return_tensors='pt'))
        if len(videos) != 0:
            video_dict = self._regularize_videos(videos, image_max_pixels=getattr(processor, 'video_max_pixels', 256 * 256), image_min_pixels=getattr(processor, 'video_min_pixels', 16 * 16), video_fps=getattr(processor, 'video_fps', 2.0), video_maxlen=getattr(processor, 'video_maxlen', 128))
        ...
        for message in messages:
            ...
            if use_audio_in_video and len(audios) and len(videos):
                if len(videos) != len(audios):
                    raise ValueError(f'Number of videos ({len(videos)}) must match number of audios ({len(audios)}) when using audio in video.')
                while VIDEO_PLACEHOLDER in content:
                    ...
                    if audio_pos == -1 or audio_pos < video_pos:
                        raise ValueError(f'Each {VIDEO_PLACEHOLDER} must be followed by an {AUDIO_PLACEHOLDER} when using audio in video.')
                    audio_t_index = torch.arange(audio_lengths[num_audio_tokens])
                    video_t_index = (torch.arange(video_grid_thw[num_video_tokens][0]).view(-1, 1, 1).expand(-1, video_grid_thw[num_video_tokens][1] // image_processor.merge_size, video_grid_thw[num_video_tokens][2] // image_processor.merge_size).flatten() * mm_inputs['video_second_per_grid'][num_video_tokens] * 25).long()
                ...
                while AUDIO_PLACEHOLDER in content:
                    ...
                    content = content.replace(AUDIO_PLACEHOLDER, f'{self.audio_bos_token}{self.audio_token * audio_seqlen}{self.audio_eos_token}', 1)
                    num_audio_tokens += 1
                while VIDEO_PLACEHOLDER in content:
                    video_seqlen = video_grid_thw[num_video_tokens].prod() // merge_length if self.expand_mm_tokens else 1
                    content = content.replace(VIDEO_PLACEHOLDER, f'{self.vision_bos_token}{self.video_token * video_seqlen}{self.vision_eos_token}', 1)
...
def get_mm_plugin(name: str, image_token: Optional[str]=None, video_token: Optional[str]=None, audio_token: Optional[str]=None, **kwargs) -> 'BasePlugin':
