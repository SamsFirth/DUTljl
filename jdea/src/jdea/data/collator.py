...
class MultiModalDataCollatorForSeq2Seq(DataCollatorForSeq2Seq):
    ...
    def __call__(self, features: list[dict[str, Any]]) -> dict[str, 'torch.Tensor']:
        ...
        fake_input_ids = []
        if self.template.mm_plugin.image_token is not None and sum(batch_imglens) == 0 and (sum(batch_vidlens) == 0):
            ...
            _fake_input_ids, _ = self.template.mm_plugin.process_token_ids(_fake_input_ids, None, fake_images, [], [], self.tokenizer, self.processor)
        ...
        if self.template.mm_plugin.audio_token is not None and sum(batch_audlens) == 0:
            ...
            _fake_input_ids = self.tokenizer.encode(fake_messages[0]['content'], add_special_tokens=False)
            _fake_input_ids, _ = self.template.mm_plugin.process_token_ids(_fake_input_ids, None, [], [], fake_audios, self.tokenizer, self.processor)
            fake_input_ids.extend(_fake_input_ids)
        ...
        mm_inputs = self.template.mm_plugin.get_mm_inputs(batch_images, batch_videos, batch_audios, batch_imglens, batch_vidlens, batch_audlens, batch_input_ids, self.processor)
        ...
        if self.get_rope_func is not None:
            rope_index_kwargs = {'input_ids': features['input_ids'], 'image_grid_thw': mm_inputs.get('image_grid_thw'), 'video_grid_thw': mm_inputs.get('video_grid_thw'), 'attention_mask': (features['attention_mask'] >= 1).float()}
        ...
        if self.model is not None and getattr(self.model.config, 'model_type', None) in ['glm4v', 'Keye', 'qwen2_vl', 'qwen2_5_vl', 'qwen2_5_omni_thinker', 'qwen3_omni_moe_thinker', 'qwen3_vl', 'qwen3_vl_moe'] and ('position_ids' not in features or features['position_ids'].dim() != 3):
...
class PairwiseDataCollatorWithPadding(MultiModalDataCollatorForSeq2Seq):
    ...
    def __call__(self, features: list[dict[str, Any]]) -> dict[str, 'torch.Tensor']:
        ...
        for key in ('chosen', 'rejected'):
            for feature in features:
                target_feature = {'input_ids': feature[f'{key}_input_ids'], 'attention_mask': feature[f'{key}_attention_mask'], 'labels': feature[f'{key}_labels'], 'images': feature['images'], 'videos': feature['videos'], 'audios': feature['audios']}
...
class KTODataCollatorWithPadding(MultiModalDataCollatorForSeq2Seq):
    ...
    def __call__(self, features: list[dict[str, Any]]) -> dict[str, 'torch.Tensor']:
        ...
        kto_tags = []
        for feature in features:
            target_feature = {'input_ids': feature['input_ids'], 'attention_mask': feature['attention_mask'], 'labels': feature['labels'], 'images': feature['images'], 'videos': feature['videos'], 'audios': feature['audios']}
            kl_feature = {'input_ids': feature['kl_input_ids'], 'attention_mask': feature['kl_attention_mask'], 'labels': feature['kl_labels'], 'images': feature['images'], 'videos': feature['videos'], 'audios': feature['audios']}
