...
def merge_dataset(all_datasets: list[Union['Dataset', 'IterableDataset']], data_args: 'DataArguments', seed: int) -> Union['Dataset', 'IterableDataset']:
    ...
    if len(all_datasets) == 1:
        ...
        return interleave_datasets(datasets=all_datasets, probabilities=data_args.interleave_probs, seed=seed, stopping_strategy='first_exhausted' if data_args.mix_strategy.endswith('under') else 'all_exhausted')
    else:
        raise ValueError(f'Unknown mixing strategy: {data_args.mix_strategy}.')

def split_dataset(dataset: Optional[Union['Dataset', 'IterableDataset']], eval_dataset: Optional[Union['Dataset', 'IterableDataset', dict[str, 'Dataset']]], data_args: 'DataArguments', seed: int) -> 'DatasetDict':
    ...
    """
    if eval_dataset is not None and data_args.val_size > 1e-06:
        raise ValueError('Cannot specify `val_size` if `eval_dataset` is not None.')
    dataset_dict = {}
    if dataset is not None:
        if data_args.streaming:
            dataset = dataset.shuffle(buffer_size=data_args.buffer_size, seed=seed)
        if data_args.val_size > 1e-06:
            if data_args.streaming:
