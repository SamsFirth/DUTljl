...
_default_log_level: 'logging._Level' = logging.ERROR
...
class LoggerHandler(logging.Handler):
    ...
    def __init__(self, output_dir: str) -> None:
        super().__init__()
        self._formatter = logging.Formatter(fmt='[%(levelname)s|%(asctime)s] %(filename)s:%(lineno)s >> %(message)s', datefmt='%Y-%m-%d %H:%M:%S')
        self.setLevel(_get_default_logging_level())
...
def _configure_library_root_logger() -> None:
    ...
    with _thread_lock:
        if _default_handler:
            return
        formatter = logging.Formatter(fmt='[%(levelname)s|%(asctime)s] %(name)s:%(lineno)s >> %(message)s', datefmt='%Y-%m-%d %H:%M:%S')
        _default_handler = logging.StreamHandler(sys.stdout)
        _default_handler.setFormatter(formatter)
        _default_handler.setLevel(_get_default_logging_level())
