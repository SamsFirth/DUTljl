...
from transformers.utils import is_torch_bf16_gpu_available, is_torch_cuda_available, is_torch_mps_available, is_torch_npu_available, is_torch_xpu_available
...
def get_peak_memory() -> tuple[int, int]:
    """Get the peak memory usage (allocated, reserved) for the current device (in Bytes)."""
    if is_torch_xpu_available():
...
def try_download_model_from_other_hub(model_args: 'ModelArguments') -> str:
    ...
    if use_modelscope():
        ...
        with WeakFileLock(os.path.abspath(os.path.expanduser('~/.cache/jdea/modelscope.lock'))):
            model_path = snapshot_download(model_args.model_name_or_path, revision=revision, cache_dir=model_args.cache_dir)
        return model_path
    if use_openmind():
        check_version('openmind>=0.8.0', mandatory=True)
        from openmind.utils.hub import snapshot_download
        with WeakFileLock(os.path.abspath(os.path.expanduser('~/.cache/jdea/openmind.lock'))):
            model_path = snapshot_download(model_args.model_name_or_path, revision=model_args.model_revision, cache_dir=model_args.cache_dir)
        return model_path
