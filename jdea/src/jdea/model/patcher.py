...
def patch_tokenizer(tokenizer: 'PreTrainedTokenizer', model_args: 'ModelArguments') -> None:
    ...
    if model_args.add_special_tokens is not None:
        ...
        logger.info_rank0("Add special tokens {} to tokenizer's vocabulary.".format(','.join(model_args.add_special_tokens)))
...
def patch_config(config: 'PretrainedConfig', tokenizer: 'PreTrainedTokenizer', model_args: 'ModelArguments', init_kwargs: dict[str, Any], is_trainable: bool) -> None:
    ...
    if 'InternVLChatModel' in getattr(config, 'architectures', []):
        raise ValueError('Please download the internvl models in a Hugging Face–compatible format (for example, https://huggingface.co/OpenGVLab/InternVL3-8B-hf).')
...
def patch_model(model: 'PreTrainedModel', tokenizer: 'PreTrainedTokenizer', model_args: 'ModelArguments', is_trainable: bool, add_valuehead: bool) -> None:
    gen_config = model.generation_config
    if not gen_config.do_sample and (gen_config.temperature is not None and gen_config.temperature != 1.0 or (gen_config.top_p is not None and gen_config.top_p != 1.0) or (gen_config.typical_p is not None and gen_config.typical_p != 1.0)):
        gen_config.do_sample = True
    if getattr(model.config, 'model_type', None) not in ['minicpmv', 'minicpmo'] and 'GenerationMixin' not in str(model.generate.__func__):
        model.generate = MethodType(GenerationMixin.generate, model)
    if add_valuehead:
        prepare_valuehead_model(model)
    if model_args.resize_vocab:
        resize_embedding_layer(model, tokenizer, new_special_tokens_config=getattr(model_args, '_special_token_descriptions', None), init_special_tokens=model_args.init_special_tokens)
    if is_trainable:
