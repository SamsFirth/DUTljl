...
def _get_quantization_dataset(tokenizer: 'PreTrainedTokenizer', model_args: 'ModelArguments') -> list[dict[str, Any]]:
    ...
    if os.path.isfile(model_args.export_quantization_dataset):
        ...
        data_files = None
    dataset = load_dataset(path=data_path, data_files=data_files, split='train', cache_dir=model_args.cache_dir, token=model_args.hf_hub_token)
    samples = []
...
def configure_quantization(config: 'PretrainedConfig', tokenizer: 'PreTrainedTokenizer', model_args: 'ModelArguments', init_kwargs: dict[str, Any]) -> None:
    ...
    if getattr(config, 'quantization_config', None):
        ...
        init_kwargs['quantization_config'] = GPTQConfig(bits=model_args.export_quantization_bit, tokenizer=tokenizer, dataset=_get_quantization_dataset(tokenizer, model_args), block_name_to_quantize=block_name_to_quantize)
        ...
        if model_args.quantization_method == QuantizationMethod.BNB:
            if model_args.quantization_bit == 8:
                ...
                init_kwargs['quantization_config'] = BitsAndBytesConfig(load_in_4bit=True, bnb_4bit_compute_dtype=model_args.compute_dtype, bnb_4bit_use_double_quant=model_args.double_quantization, bnb_4bit_quant_type=model_args.quantization_type, bnb_4bit_quant_storage=model_args.compute_dtype)
            else:
            ...
            init_kwargs['quantization_config'] = HqqConfig(nbits=model_args.quantization_bit, quant_zero=False, quant_scale=False, axis=0)
