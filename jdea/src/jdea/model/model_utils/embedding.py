...
def _description_based_initialization(embed_weight: 'torch.Tensor', num_new_tokens: int, descriptions: dict[str, str], tokenizer: 'PreTrainedTokenizer', model: 'PreTrainedModel', add_noise: bool=False) -> None:
    ...
    for i, desc in enumerate(descriptions.values()):
        ...
        with torch.no_grad():
            ...
            if len(valid_token_ids) == 0:
                logger.warning_rank0(f'Description for token {i + 1}/{num_new_tokens} contains no valid tokens. Using mean of existing embeddings.')
...
def _initialize_embeddings(embed_weight: 'torch.Tensor', num_new_tokens: int, init_method: str, new_special_tokens_config: Optional[dict], tokenizer: 'PreTrainedTokenizer', model: 'PreTrainedModel') -> None:
    ...
    if init_method == 'desc_init' and new_special_tokens_config:
        ...
        _description_based_initialization(embed_weight, num_new_tokens, new_special_tokens_config, tokenizer, model, add_noise=False)
        ...
        _description_based_initialization(embed_weight, num_new_tokens, new_special_tokens_config, tokenizer, model, add_noise=True)
    else:
        if init_method != 'noise_init':
            logger.warning_rank0(f"init_method='{init_method}' requires descriptions config, falling back to 'noise_init'")
...
def resize_embedding_layer(model: 'PreTrainedModel', tokenizer: 'PreTrainedTokenizer', new_special_tokens_config: Optional[dict]=None, init_special_tokens: str='noise_init') -> None:
    ...
    if len(tokenizer) > current_embedding_size:
        ...
        with context_maybe_zero3:
            ...
            logger.info_rank0(f'Resizing embeddings: {current_embedding_size} -> {new_embedding_size} (+{num_new_tokens} tokens)')
            _initialize_embeddings(model.get_input_embeddings().weight.data, num_new_tokens, init_special_tokens, new_special_tokens_config, tokenizer, model)
            if model.get_output_embeddings() is not None and (not model.config.tie_word_embeddings):
                _initialize_embeddings(model.get_output_embeddings().weight.data, num_new_tokens, init_special_tokens, new_special_tokens_config, tokenizer, model)
