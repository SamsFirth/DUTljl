...
def _get_unsloth_kwargs(config: 'PretrainedConfig', model_name_or_path: str, model_args: 'ModelArguments', finetuning_args: 'FinetuningArguments') -> dict[str, Any]:
...
def load_unsloth_pretrained_model(config: 'PretrainedConfig', model_args: 'ModelArguments', finetuning_args: 'FinetuningArguments') -> Optional['PreTrainedModel']:
...
def get_unsloth_peft_model(model: 'PreTrainedModel', model_args: 'ModelArguments', peft_kwargs: dict[str, Any]) -> 'PreTrainedModel':
    """Get the peft model for the pretrained model with unsloth. Used in training."""
    from unsloth import FastLanguageModel
    unsloth_peft_kwargs = {'model': model, 'max_seq_length': model_args.model_max_length, 'use_gradient_checkpointing': 'unsloth'}
    return FastLanguageModel.get_peft_model(**peft_kwargs, **unsloth_peft_kwargs)

def load_unsloth_peft_model(config: 'PretrainedConfig', model_args: 'ModelArguments', finetuning_args: 'FinetuningArguments', is_trainable: bool) -> 'PreTrainedModel':
