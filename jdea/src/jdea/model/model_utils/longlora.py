...
def llama_attention_forward(self: 'LlamaAttention', hidden_states: 'torch.Tensor', attention_mask: Optional['torch.Tensor']=None, position_ids: Optional['torch.LongTensor']=None, past_key_value: Optional['Cache']=None, output_attentions: bool=False, cache_position: Optional['torch.LongTensor']=None, position_embeddings: Optional[tuple['torch.Tensor', 'torch.Tensor']]=None, **kwargs) -> tuple['torch.Tensor', Optional['torch.Tensor'], Optional[tuple['torch.Tensor']]]:
    ...
    if getattr(self.config, 'group_size_ratio', None) and self.training:
        ...
        def shift(state: 'torch.Tensor') -> 'torch.Tensor':
            ...
            state = torch.cat((state[:, :, :self.num_heads // 2], state[:, :, self.num_heads // 2:].roll(-groupsz // 2, dims=1)), dim=2)
...
def llama_flash_attention_2_forward(self: 'LlamaFlashAttention2', hidden_states: 'torch.Tensor', attention_mask: Optional['torch.Tensor']=None, position_ids: Optional['torch.LongTensor']=None, past_key_value: Optional['Cache']=None, output_attentions: bool=False, cache_position: Optional['torch.LongTensor']=None, position_embeddings: Optional[tuple['torch.Tensor', 'torch.Tensor']]=None, **kwargs) -> tuple['torch.Tensor', Optional['torch.Tensor'], Optional[tuple['torch.Tensor']]]:
    ...
    if getattr(self.config, 'group_size_ratio', None) and self.training:
        ...
        attn_output: torch.Tensor = _flash_attention_forward(query_states, key_states, value_states, attention_mask, query_states.size(1), dropout=dropout_rate, sliding_window=getattr(self, 'sliding_window', None), use_top_left_mask=self._flash_attn_uses_top_left_mask, is_causal=self.is_causal)
    if getattr(self.config, 'group_size_ratio', None) and self.training:
        attn_output.reshape(bsz, q_len, self.num_heads, self.head_dim)
        attn_output = torch.cat((attn_output[:, :, :self.num_heads // 2], attn_output[:, :, self.num_heads // 2:].roll(groupsz // 2, dims=1)), dim=2)
...
def llama_sdpa_attention_forward(self: 'LlamaSdpaAttention', hidden_states: 'torch.Tensor', attention_mask: Optional['torch.Tensor']=None, position_ids: Optional['torch.LongTensor']=None, past_key_value: Optional['Cache']=None, output_attentions: bool=False, cache_position: Optional['torch.LongTensor']=None, position_embeddings: Optional[tuple['torch.Tensor', 'torch.Tensor']]=None, **kwargs) -> tuple['torch.Tensor', Optional['torch.Tensor'], Optional[tuple['torch.Tensor']]]:
    if output_attentions:
        transformers_logger.warning_once('SDPA does not support `output_attentions=True`. Falling back to the vanilla attention')
        return llama_attention_forward(self, hidden_states=hidden_states, attention_mask=attention_mask, position_ids=position_ids, past_key_value=past_key_value, output_attentions=output_attentions, cache_position=cache_position, **kwargs)
    bsz, q_len, _ = hidden_states.size()
    query_states: torch.Tensor = self.q_proj(hidden_states)
    key_states: torch.Tensor = self.k_proj(hidden_states)
    value_states: torch.Tensor = self.v_proj(hidden_states)
