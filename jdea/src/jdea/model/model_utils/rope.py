...
def configure_rope(config: 'PretrainedConfig', model_args: 'ModelArguments') -> None:
    ...
    if model_args.model_max_length is not None:
        ...
        if model_args.rope_scaling == RopeScaling.DYNAMIC:
            logger.warning_rank0('Dynamic NTK scaling may not work well with fine-tuning. See: https://github.com/huggingface/transformers/pull/24653')
        rope_factor = float(math.ceil(model_args.model_max_length / old_max_length))
    else:
        rope_factor = 2.0
    rope_kwargs = {'rope_type': getattr(model_args.rope_scaling, 'value', model_args.rope_scaling), 'factor': rope_factor}
    ...
    logger.info_rank0(f"Using {rope_kwargs['rope_type']} scaling strategy and setting scaling factor to {rope_kwargs['factor']}.")