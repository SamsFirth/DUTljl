...
def check_lfi_path(path: str) -> None:
    ...
    try:
        ...
        if not real_path.startswith(safe_path):
            raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail='File access is restricted to the safe media directory.')
    except Exception:
...
def check_ssrf_url(url: str) -> None:
    ...
    try:
        ...
        if not ip.is_global:
            raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail='Access to private or reserved IP addresses is not allowed.')
    except socket.gaierror:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=f'Could not resolve hostname: {parsed_url.hostname}')
