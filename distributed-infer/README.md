...
**具体执行流程在第5.4与5.5节，README的其余部分为相关的说明**
...
api-test.py为调用sglang-infer.sh生成的api接口进行测试的代码，使用时修改代码开头的**API_URL**为sglang-infer.sh生成的日志文件中的ip即可。代码有每处理100条数据自动保存一次结果的功能

**注意**：代码对数据的处理方式对应金融催收数据集的格式，若更换数据集需要修改代码的数据处理逻辑与评判标准！！！
